...
- **Objective merge verification before any close chore** — a "merged it" statement is not proof. Verify **four dimensions** before moving `review/ → done/`: (a) `gh pr view <n> --json state,mergedAt,mergeCommit,statusCheckRollup` returns `state=MERGED`; (b) `git log origin/main` tip matches the squash commit; (c) the impl PR's pre-merge `gh pr checks <n>` snapshot had **0 failing required checks** (CI-RED-at-merge time creates a main regression — `statusCheckRollup` of the merged PR is the authoritative record); (d) the task's **`# Acceptance Criteria` section, opened and read** — not its `INDEX.md` row — shows every AC closed **against the verb that AC used** (a recommendation does not close an AC that says "decide"; conversely a ⚪ recording *"could not be measured, and why"* **is** the closure when the AC asked for that). 🔴 (a)(b)(c) all measure the **PR** («did it merge»); only (d) measures the **ticket** («did the work land»). If (d) alone fails, do **not** move the file — leave it in `review/` and append a `## CORRECTION`, because `done/` is frozen and a remainder filed there is never read again. No repo-side guard can judge this and none should pretend to (a ✅-grep would have passed `TASK-MONO-605`); it is on whoever runs the close chore. Evidence + how to read a ⚪: `platform/git-workflow-policy.md` § The Fourth Dimension. If any of the four fails: STOP. CI-RED-at-merge requires a separate fix-task that restores main GREEN before close chore. 🔴 **«required» now names a set, and that set is small** (`TASK-MONO-598`, 2026-08-28): `main` carries branch protection requiring exactly four contexts — `changes`, `INDEX queue drift (INDEX.md tables vs queue directories)`, `Task ID collision (duplicate IDs in active queues)`, `Walkthrough limitation ledger drift (§ 6 rows vs task queues)` — chosen because they are the only checks measured as SUCCESS in **24 of 24** sampled PRs, so requiring them can never deadlock on a permanent pending. Everything else, **including every build, integration and e2e job**, is still unenforced: dimension (c) passing does NOT mean the suites were green, only that no *required* check failed. Before 598 the set was **empty**, which made (c) true in every state including a fully red `ci.yml`. 🔴🔴 **Four are required, but on most PRs fewer than four actually run** (`TASK-MONO-601`, 2026-08-29): three of them (`INDEX queue drift …`, `Task ID collision …`, `Walkthrough limitation ledger drift …`) are gated on `tasks/**` paths, so a PR that touches no task file leaves all three `SKIPPED` — and GitHub does not treat `SKIPPED` as failing. On such a PR, (c) is satisfied with **only `changes` having actually executed**. Measured over 70 merged PRs: **2 were code-only** (`#3523`, `#3479`) and both showed exactly that; the other 68 touched a task file and ran all four. 🔵 The skipping is **correct** — those guards watch task-queue and doc drift, and a code-only PR is not on that arrival path; what is wrong is reading "four required" as "four ran". 🔴 It also explains 598's sample: `24 of 24` was measured on a population that is ~97% task-touching, where "always SUCCESS" and "often SKIPPED" look identical. `scripts/check-required-check-names.sh` cell (4) now asserts this gating so the sentence cannot drift. 🔴🔴 **Match those names by equality and keep the parentheses** — they are part of the registered string, not decoration (`TASK-MONO-599`, 2026-08-28: the first version of this very sentence dropped every parenthetical, so three of the four names matched nothing; re-registering from them would make `main` permanently `BLOCKED`). Only `changes` is short, because that job has no `name:` so its job id becomes the context verbatim. The pin is [`scripts/required-check-names.txt`](scripts/required-check-names.txt) — sourced from the API, never retyped — and `scripts/check-required-check-names.sh` keeps it, `ci.yml` and this rule's three homes from drifting apart. 🔴 Renaming a job in `ci.yml` breaks the match *silently*: that PR merges green and every PR after it is BLOCKED. (Worked incident behind this rule: [`platform/git-workflow-policy.md` § Merge-Verification Worked Incident](platform/git-workflow-policy.md#merge-verification-worked-incident).) When polling CI programmatically, never use the **exit code** of `gh pr checks <n>` to decide GREEN — a still-pending check makes it exit non-zero, so an exit-code gate misreads "pending" as "failed"; parse its text / `--json` output (or `gh pr view --json statusCheckRollup`) and check each check's state explicitly.
...
- **Stage before you run a repo guard locally** — 20 of the 56 `scripts/` guards build their population from `git ls-files`, which reads the **index**, so a file you have not `git add`ed yet does not exist as far as the guard is concerned. Running a guard before staging is **not a verdict**. The dangerous direction is a false GREEN: when both sides of a two-way check are empty they *agree*, and the guard says so out loud — measured on `TASK-MONO-646`, `check-index-queue-drift.sh` printed *"listing and directory agree in both directions"* with `rc=0` while `tasks/ready/` held an unlisted file (`ls`=1, `git ls-files`=0). The same run went `rc=0` for the right reason once both files were staged. It also fails the other way (an unstaged file plus an INDEX row → `rc=1` claiming *"no file with that ID exists anywhere under `tasks/`"*, which is false). Three of those 20 are `main`'s **required** checks — `check-index-queue-drift.sh`, `check-task-id-collision.sh`, `check-walkthrough-ledger-drift.sh` — so a pre-stage local green on the required set means nothing. Order: `git add` → eyeball the population (`git ls-files <dir>`) → run the guard. CI is unaffected (it reads a committed tree); this is a local-run rule. Worked incident + why the guards are right to use `git ls-files`: [`platform/git-workflow-policy.md` § Stage Before You Run a Guard](platform/git-workflow-policy.md).
- **Adding or deleting a file under `scripts/` is itself an input to other guards — sweep them all, not the ones that look related.** `TASK-MONO-650` added one guard, ran the three required checks plus its own new guard, saw four greens, and pushed; `main` went red on `Guard-count figure (scripts/ reading git ls-files vs its two prose homes)`. 🔴 The numerator never moved — the new guard does not read `git ls-files`. The **denominator** did, because `scripts/` gained a file, and `TASK-MONO-646` had put a gate on that count. The failure was not the guard's: **the chosen population was**. Picking "guards that look related to my change" can never surface this class, because the relationship runs through a count, not through the subject matter. So: `git status` shows an add or delete under `scripts/` → run every guard, then fix the prose homes the failing guard names (`check-ls-files-guard-count.sh` prints its own prescription: *"Fix the sentence in each home above, not this script: the counts are derived"*).
