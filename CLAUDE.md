...
- **`.claude/hooks/` + `.claude/settings.json` are classifier-blocked**: edits/commits there are hard-blocked **even with explicit approval** (intent-resistant — a direct user instruction does not clear them) → hand the patch to the user; never shell-write around it. **`commands/`, `agents/`, `config/` are NOT blocked** — measured, they pass edit+commit+push+merge (`agents/`: TASK-MONO-409 / #2616, `commands/`: TASK-MONO-396 / #2525, `config/`: TASK-MONO-167 / #1021). `skills/` passes with explicit per-action authorization. `platform/` is not subject. The classifier is external and can change silently, so **don't pre-emptively hand off on assumption — attempt once, and hand over only on an actual block** (a wasted attempt costs one round-trip; a wrong assumption costs a needless hand-off and files a false completion note).
