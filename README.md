...
🔴 **The rule it came from still binds everything else.** Volatile endpoints — API Gateway ids, `*.cloudfront.net` domains — are **state, not source**, and a guard in [`infra/demo/verify-demo-wrapper.sh`](infra/demo/verify-demo-wrapper.sh) fails CI if one is committed, including into this README. The control API's address is *obtained*, never pasted:

```bash
cd infra/demo/aws/terraform && terraform apply && terraform output api_base_url
# → set that as DEMO_API_BASE in the Vercel project. The launcher build reads it
#   and fails closed without it (infra/demo/aws/site/build.sh).
```
...
[projects/ecommerce-microservices-platform](projects/ecommerce-microservices-platform/) ships its own [MIT `LICENSE`](projects/ecommerce-microservices-platform/LICENSE) (badged in its own README). The rest of this monorepo — the shared library (`platform/`, `rules/`, `.claude/`, `libs/`) and the other project directories — has no license declared yet; treat those as portfolio-use-only pending a decision.
