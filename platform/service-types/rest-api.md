...
- **Which claims to validate is not decided here. Canonical: [`platform/contracts/jwt-standard-claims.md`](../contracts/jwt-standard-claims.md) § JWT Validation** — signature, `iss`, `exp`/`nbf` and `tenant_id` on every `rest-api` service, plus the `aud` client-allowlist rule **iff this service is an *edge*** (the first component inside the trust boundary to validate an externally-presented token). That contract defines "edge" positionally, so **a `rest-api` service can be one**: measured 2026-09-18 (`TASK-MONO-698`), two components that terminate externally-presented tokens declare `Service Type: rest-api` rather than a gateway type. A `rest-api` service that sits **behind** an edge declares **no** audience allowlist — the contract's *Behind the edge* bullet states what guards it instead (`iss` + `tenant_id` + the workload discriminator its chain carries: a required scope, a subject allowlist, or a role) and why a second allowlist there would judge the same axis twice.
- 🔵 **This bullet is a pointer, not a new constraint** (the § Versioning and § Idempotency pattern): the rule lives in the contract and is restated nowhere. It exists because the audience rule was written only into `service-types/identity-platform.md`, which a `rest-api` service never reads — so the file read by the services the rule binds said nothing about it at all (`TASK-MONO-698` § AC-2).
