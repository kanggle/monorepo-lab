...
**Two kinds of relying party, and rule 3b applies to only one of them.** A relying party is an **edge** if it is the first component inside the trust boundary to validate a token presented from outside it (every platform gateway; also a backend-for-frontend, or any other externally-reachable token-terminating component, whatever its declared `Service Type`). Every other relying party sits **behind** an edge. The distinction is positional and is defined once, normatively, in the JWT Standard Claims contract § JWT Validation rule 5 — this file does not restate it.
...
3. Validate every incoming bearer token: signature (via `kid`), `iss`, `exp`, `nbf` (if present), and `tenant_id` (a missing or blank claim is a rejection — "any tenant" is not "no tenant").
   - **`aud` — at an edge only.** An **edge** relying party additionally validates `aud` against its declared audience allowlist of client ids (a single string is a one-element set; admission requires a non-empty intersection). The allowlist is mandatory, an empty or absent one is a startup failure, and a mismatch is a `403`. A transitional shadow phase that logs and counts mismatches without rejecting is permitted; it is not required where the inbound population is already measured. **Canonical: the JWT Standard Claims contract § JWT Validation rule 5** — that rule owns the wording, including what a relying party *behind* an edge does instead (`iss` + `tenant_id` + the workload discriminator its chain already carries: a required scope, a subject allowlist, or a role) and why a second allowlist there would judge the same axis twice.
   - 🔴 **This requirement was previously unconditional, and the fleet did not obey it.** Measured 2026-09-18 (`TASK-MONO-698` § AC-0): 22 servlet resource servers validated signature, `iss`, expiry and `tenant_id`, carried **no** audience allowlist, and started up successfully. The MUST said their startup should have failed, so the pre-narrowing state of this file was not "an open question" but a written rule the tree violated. The correction is this narrowing rather than 22 code changes, because the owner's decision (`TASK-MONO-698` § AC-3) places the allowlist at the edge — and because a rule that 22 deployed services silently violate is a documentation defect before it is a code defect.
...
- Negative tests: HS256 token rejected, `aud` outside an **edge** relying party's audience allowlist rejected with 403 (and, while a shadow phase is active, admitted with the mismatch logged and counted), token with no `aud` rejected at an edge, an edge's empty audience allowlist fails startup, expired token rejected, revoked refresh token rejected, social-login token NOT accepted as platform token. (The three `aud` cells are about edges — a relying party behind an edge declares no allowlist, so they have nothing to assert there; § Integration Rules rule 3.)
