...
| DATA_INTEGRITY_VIOLATION | 409 | A **unique-constraint** violation surfaced by Spring `DataIntegrityViolationException` when no domain-specific `*Exception.java` mapping applies — the ecommerce services' fleet code for that case. **409 is for unique violations only.** FK / NOT NULL / CHECK violations carried by the same `DataIntegrityViolationException` are server defects, not client conflicts, and map to `INTERNAL_ERROR` (500) so they stay loud in logs and alerting rather than being masked as a 409 monitoring never sees. The split is decided at runtime by JDBC SQLSTATE `23505` via `com.example.common.persistence.DataIntegrityViolations.isUniqueViolation` — the discriminant is type- and message-independent because Spring maps every Hibernate `ConstraintViolationException` to a plain `DataIntegrityViolationException` (TASK-MONO-450; discriminant proven by TASK-BE-542). **Sibling fleet codes for the same unique-violation case are registered, not unified**: scm procurement and wms outbound emit `CONFLICT`; finance account emits `CONCURRENT_MODIFICATION`; fan-platform services emit `CONFLICT` — each a deliberate domain choice (scm distinguishes `CONFLICT` "change state first" from `CONCURRENT_MODIFICATION` "retry"), so the backstop code is not fleet-uniform and a `code`-keyed client must read the emitting service's own contract. Prefer a domain code when a known constraint is hit |
