...
5. **Validate audience (client allowlist) — at the edge:** Admit iff the token's `aud` values (a single string is a one-element set) **intersect** the edge's declared audience allowlist — i.e. at least one `aud` value is a client id that edge admits. Otherwise reject with `403 Forbidden` (§ Error Handling).
   ...
   - **Implementation status (2026-09-18).** This rule is *shipped* at the six platform gateways of this fleet, in **shadow** mode (`TASK-MONO-696`; the switch to rejection is `TASK-MONO-697`). The two non-gateway edges measured by `TASK-MONO-698` are **not yet covered** — `TASK-MONO-712` and `TASK-MONO-713` bring them under this rule, both going straight to rejection per the bullet above. Until those land, "every edge" states the **target** posture of this contract, not the implemented state of the tree. The *Behind the edge* bullet, by contrast, describes what is implemented today.
