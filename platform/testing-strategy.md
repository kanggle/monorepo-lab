...
- **The failure you are looking for may never have appeared as a red run.** A rerun-to-green hides it, and `gh run list` reports only the **latest attempt's** conclusion. Query the attempts (`.../runs/<id>/attempts/<n>/jobs`) — otherwise you measure zero and conclude, wrongly, that the lane is healthy. **One call does it: `.../runs/<id>/jobs?filter=all`** surfaces the non-latest attempts without walking `attempts/<n>` by hand. Measured on run `33151761319` (`TASK-MONO-597`): the default view reports that job `success`; `?filter=all` reports `failure`. Same run — and only the second is the fact a census needs.
- **Count job *executions*, never runs.** These lanes are reusable-workflow calls, and a call that is skipped still returns a job row — one whose name lacks the ` / <job-id>` suffix the executed form carries. Over 140 main `ci.yml` runs (2026-08-16 .. 08-28) that was **765 skipped rows against 672 executions**; putting the skips in the denominator understates the rate by more than half. `skip` is not `pass`.
- **Worked census (`TASK-MONO-597`) — what "record that you looked" actually produced.** Same window, every `Integration (…)` job, attempt-aware: **672 executions, 3 failures (0.45%)**. All three were *different shapes* on different days — a Kafka poll returning nothing in 30s (`AdjustmentTransferIntegrationTest:84`); a suite that produced **zero tests**, so the empty-suite guard fired (the build died before the tests did — by the rule above, not evidence of contention); and a Postgres SKIP LOCKED exclusivity assertion (`DeliverySkipLockedClaimIntegrationTest:214`). No shared population, no starvation signature, **no lane serialised**. The census earned its keep as a *negative* result: it is what stopped one observation from being generalised into a 22-file change.
