...
| 가동 중 (**r6i.2xlarge**) | **$0.608/시간** |

🔵 가동 단가는 **실측이다** — AWS 공개 요금표(`ap-northeast-2` · Linux · 온디맨드), 발행일 `2026-09-10`, `TASK-MONO-665`. 🔴 **이 표의 나머지 두 줄은 미측정**이고(AMI 스냅샷 ~$2 · EBS ~$11) 저장소가 예전부터 들고 있던 값이다 — 합계를 말할 때 그 구분을 지켜라.

🔴 **이 줄은 한 번 낡은 채로 방치됐다**: `TASK-MONO-552` 가 `m6i.2xlarge`(32GB)를 실측으로 기각하고 `r6i.2xlarge`(64GiB)로 올렸는데 이 표는 계속 m6i 를 말했고, 그 결과 `variables.tf` 의 예산 근거가 **28.8% 싼 인스턴스로 계산**돼 있었다.
...
3. **`monthly_budget_minutes`** (기본 **1800** = 30시간) — **월 누적 가동 분.** 소진되면 `/start` 가 429 를 반환하고 인스턴스를 켜지 않는다. 🔴 **이 값이 곧 그달 최악 청구다**(1800분 × $0.608 = **$18.24**, 컴퓨트 축만). 🔵 인상은 **두 번** 있었고 둘 다 사유가 같다 — «달을 못 넘긴다»: `600 → 1200`(`TASK-MONO-665`, 2026-09-11 · 600 이 **11일** 만에 543 소진), `1200 → 1800`(소유자 결정 2026-09-18 · 1200 이 **18일** 만에 **1170/1200** 까지 소진). 근거와 «무엇을 안 쟀는지» 는 `variables.tf` 의 주석에 있다.
...
`full` ≈ **96 컨테이너 / 메모리 ~26GB** (JVM 40+). `TASK-MONO-366` 이 실행 중인 스택을 **한 번 재서 얻은 값**이다.

🔴🔴 **여기 있던 *"m6i.2xlarge(32GB)에서 여유 ~5.5GB"* 는 삭제했다 — 수치가 낡은 것이 아니라 `TASK-MONO-552` 가 라이브에서 반증한 주장이다.** 552 가 32GB 호스트에서 실제로 잰 값은 **31.5 GiB 중 약 29 GiB 사용 / MemAvailable 2.4 GiB** 였고(pressure/memory full `0.00 → 41.16`, pgmajfault `+436k / 5분`), 그래서 인스턴스가 **r6i.2xlarge(64 GiB)** 로 올라갔다. ⇒ 위 `~26GB` 는 **합계 추정**이고, 552 가 정한 사이징 기준은 *"정상 상태 MemAvailable 6~8 GiB 확보"* 다(`variables.tf` 의 `instance_type` 주석).
