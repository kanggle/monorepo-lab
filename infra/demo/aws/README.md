...
3. **`monthly_budget_minutes`** (기본 **1200** = 20시간) — **월 누적 가동 분.** 소진되면 `/start` 가 429 를 반환하고 인스턴스를 켜지 않는다. 🔴 **이 값이 곧 그달 최악 청구다**(1200분 × $0.608 = **$12.16**). 🔵 `600 → 1200` 은 `TASK-MONO-665` 의 소유자 결정이고, 근거(창 평균 46.2분 실측 · 2026-09 에 600 이 11일 만에 소진)는 `variables.tf` 의 주석에 있다.
