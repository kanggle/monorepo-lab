...
#   1800분(30시간) × $0.6080 = **$18.24/월** ← 이 변수가 정하는 컴퓨트 상한
#   🔵 (직전 값 1200분 = 20시간 = $12.16 — 2026-09-18 에 1800 으로 올렸다, 아래)
#
...
#
# 🔵 왜 1800 인가 (소유자 결정 2026-09-18):
#   · **1200 도 같은 달에 거의 소진됐다** — 제어 API `/status` 실측으로 2026-09-18 에
#     **1170/1200(97.5%)**. 달의 **60% 시점**(18/30일)에 상한 직전이었다.
#     🔴 이 한 줄이 근거의 전부다 — 창 개수나 평균 길이는 이 달에 대해 **안 셌다**.
#   · 그래서 `TASK-MONO-665` 의 «창 평균 46.2분 ⇒ 월 26창» 추정은 **이 달에 대해
#     검증되지 않았다**(더 짧은 창을 더 자주 열었을 수도, 아닐 수도 있다 — 미측정).
#     🔴 그 추정을 이번 결정의 근거로 재사용하지 않는다.
#   · 1800 = **30시간**, 정시로 떨어진다 — 665 가 1200 을 고른 것과 같은 이유다
#     (이 주석이 분과 시간을 같이 쓰므로 "1800분(30시간)" 은 들어간다).
#
# 🔵 665 의 관측은 **역사로 남긴다**(지우지 않는다 — 다음 조정의 비교선이다):
#   · 예산 로직에 양자화가 없다 — handler.py 의 `seconds >= BUDGET_MINUTES*60` 한 줄이 전부다.
#     "600 단위" 는 근거가 아니었다.
#   · 창 평균 **46.2분**(CloudWatch 실측, 5일 4창 185분 — 부팅 9분 16초 포함).
#   · **600 분은 2026-09 에 11일 만에** 소진됐다(543/600) ⇒ 1200 으로 올렸다(2026-09-11).
#   · 🔴 그리고 **1200 은 18일 만에** 1170 까지 찼다 ⇒ 1800(2026-09-18). 두 번 다
#     «달을 못 넘긴다» 가 인상 사유였다.
