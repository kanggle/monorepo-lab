...
# json_objects <json> — JSON 배열 본문을 **객체 하나당 한 줄**로 쪼갠다.
#
# 왜 이게 필요한가: 목록 응답에서 조건 두 개(`"type":"TOPUP"` 과 그 금액)를 **같은
# 객체 안에서** 만족하는 행을 세려면, 본문 전체 grep 으로는 안 된다 — 서로 다른 두
# 객체의 필드가 우연히 함께 매치되는 **키메라 행**이 만들어진다.
#
# 🔵 중첩 객체(`"money":{…}`)에는 안전하다: 경계 패턴 `},{` 는 중첩이 닫힌 뒤 다음 키가
# 오는 `},"key"` 와 모양이 다르다. 중첩 **배열**의 객체에는 안전하지 않다 — 그 모양이
# 나오면 이 헬퍼로는 셀 수 없다.
#
# 🔵 `seed-erp.sh` 가 이걸 자기 안에 갖고 있었다. `wait_backend` 와 같은 이유로 여기로
# 승격했다(2026-08-18, TASK-MONO-556) — 한 사실이 두 집을 가지면 한쪽만 고쳐진다.
