...
if container_up fan-platform-postgres; then
  dbexec --why "아티스트 디렉터리에는 **API 호출자가 존재하지 않는다 — 결정에 의해서다.** POST /api/v1/artists|artist-groups|fandoms 는 hasAnyRole(ADMIN,OPERATOR,SUPER_ADMIN,FAN_OPERATOR) 인데, FAN_OPERATOR 는 tenant_domain_subscription(*, 'fan') 에서 파생되고 그 행은 전 테넌트 0/18 이며(fan-platform 은 operator_tenant_assignment 에도 없다), ADR-MONO-059 ACCEPTED-A 가 B(운영자 대리)를 배제하면서 'B2C_CONSUMER 테넌트를 운영자가 assume 하는 조합은 열지 않는다' 를 구속력 있게 확정했다 ⇒ 그 행들을 넣는 것은 미완의 작업이 아니라 결정에 반하는 작업이다. 🔴 이 사유를 TASK-MONO-512 로 적으면 안 된다: 그 티켓은 닫혔고, 닫히면서 이 블록을 **열지 않기로** 확정했다(같은 티켓이 ARTIST_POST 쪽 예외는 회수했다 — 아래 2번). 남은 질문 '아티스트 디렉터리를 운영자 평면 없이 어떻게 관리하나' 는 TASK-MONO-522 소관이다" \
