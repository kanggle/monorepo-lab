...
`seed-erp.sh` 는 **세 번째 분류(`⛔ 차단`)** 를 둔다. 실패 신호가 그 결함의 **정확한
지문**(응답 코드 + `"cause":"…"` 같은 본문 마커)과 일치할 때만 차단으로 세고 **티켓
번호를 찍는다**. 지문이 어긋나면 그대로 `seed_fail` 이다. 이 형태의 값은 두 가지다 —
결함이 고쳐지면 그 경로는 **저절로 다시 성립하고**, 그 사이 생긴 **새로운 고장은
여전히 빨개진다**. 요약 줄에 차단 건수를 함께 찍어 "0 실패" 가 "전부 잘 됐다" 로
읽히지 않게 하라.
