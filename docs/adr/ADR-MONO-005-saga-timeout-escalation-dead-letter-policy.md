...
| 2 | ecommerce order saga (`order.placed → payment.completed → order CONFIRMED`) | ecommerce / order-service, payment-service | distributed: `orders.status` + `payments.status` (no orchestrator row) | none — pure choreography; per-service outbox poll only | none | none (post-ADR-006 outbox is at-least-once but no stuck-saga detector for orders frozen in `PAYMENT_PENDING`) | **A — choreographed multi-step saga (gap)** |
...
| 7 | inventory reservation TTL (reserve → confirm or expire) | wms / inventory-service | `reservation` row with `expires_at` | `ReservationExpiryJob` 60 s poll, batch 200 | none (single-shot release; OL race → retry next tick) | none — released state is the terminal | **D — TTL expiry sweep** |
