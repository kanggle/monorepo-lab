...
**Status:** ACCEPTED
**Date:** 2026-05-11
**History:** PROPOSED 2026-05-11 (TASK-MONO-054 / PR #358) → ACCEPTED 2026-05-11 (TASK-MONO-055 § D7 spec surface bundle merged via PR #361, gate 1/2; TASK-BE-139 `TossPaymentsAdapter` Resilience4j wrap merged, gate 2/2 — payment-service joined the Category B reference column). 2026-05-11 TASK-BE-138 merged — order-service choreographed-saga stuck-detector landed; ecommerce order joined the Category A reference column. 2026-05-11 TASK-BE-140 merged — `inventory.reservation.expiry.swept.total` counter landed; § 5 outstanding follow-ups now cleared. **Amended 2026-06-26 (TASK-MONO-306):** § 2.3 D3 gains the R1–R4-gated **auto-resolving-terminal refinement** (a Category A saga MAY use a domain-meaningful terminal such as `CANCELLED` instead of `STUCK_RECOVERY_FAILED` when the stuck condition has a clean self-serve resolution + guaranteed compensation + retained escalation/metrics + fallback); § 2.6 D6 ecommerce-order row re-decided to the auto-resolving terminal (`CANCELLED(PAYMENT_TIMEOUT)`, adopter TASK-BE-435). Refinement only — the default operator-escalation terminal and D1–D8 decision bodies are unchanged; Status stays ACCEPTED.
...
- **Attempt cap** = **5** by default. Externalised as `${<service>.saga.sweeper.max-attempts}`. At cap the saga MUST transition to a terminal `STUCK_RECOVERY_FAILED`-shaped state (operator-escalation terminal — the system gave up, a human triages). **This is the default and applies unless the auto-resolving-terminal refinement below is satisfied.**
  - **Auto-resolving-terminal refinement (opt-in, per-saga).** A saga MAY instead transition at cap to a **domain-meaningful terminal that auto-resolves the stuck condition** (e.g. `CANCELLED`) — replacing the operator-escalation terminal — **iff ALL** of the following hold:
    - **(R1) clean self-serve resolution** — the terminal is a legitimate business outcome of the stuck condition (not "we gave up"), reachable through the aggregate's existing state machine.
    - **(R2) compensation guaranteed** — any external side-effect that may have occurred despite the stuck condition (e.g. a captured-but-unacknowledged payment) is reversed by a **defined, idempotent, at-least-once** compensation. No value is left stranded.
    - **(R3) escalation/observability preserved** — the `<service>.alert.saga.recovery.exhausted` escalation event (or an equivalent informational alert) still fires at cap, and the D3 metric set below is retained. The terminal's *name/semantics* change; the operator's monorepo-wide "a saga exhausted retries" **paging surface does not** (this preserves the § 1 Context invariant).
    - **(R4) defensive fallback** — the `STUCK_RECOVERY_FAILED`-shaped terminal is retained as the fallback for when the auto-resolving compensation cannot be co-committed (so a saga never silently stays in its non-terminal stuck state).
  - A saga that does not satisfy R1–R4 keeps the default operator-escalation terminal unchanged. Adoption is recorded per-saga in the § 2.6 D6 table.
...
| ecommerce order saga | **A — Compliant (post-TASK-BE-138); terminal refined to auto-resolving (TASK-BE-435).** `OrderStuckDetector` (60s `@Scheduled`) + `OrderStuckRecoveryHandler` (REQUIRES_NEW per-order, AOP-split mirror of wms `SagaSweeper` + `SagaRecoveryHandler`) sweep `PENDING + payment_id IS NULL + created_at < NOW() - 30m`. Adopts the § 2.3 D3 **auto-resolving-terminal refinement**: at cap the order transitions to **`CANCELLED(PAYMENT_TIMEOUT)`** (R1 — a customer who never paid simply gets a cancelled order) rather than `STUCK_RECOVERY_FAILED`. R2 — payment-service reverses any captured-but-unacknowledged payment (`OrderCancelled` → `COMPLETED→auto-refund` / `PENDING→void guard` + post-capture auto-refund); stuck orders hold no reserved stock (TASK-BE-428) so no inventory compensation is needed. R3 — `order.alert.saga.recovery.exhausted` outbox event (T3 co-commit) + the 3 metrics (`order_stuck_detector_run_total`, `…_recovery_fired_total{from_state}`, `…_exhausted_total{from_state}`) retained as informational. R4 — `STUCK_RECOVERY_FAILED` retained as the defensive fallback. Composite `idx_orders_status_created_at`. All knobs externalised under `order.saga.stuck-detector.*`. | See follow-up. | ✅ TASK-BE-138 MERGED · ⏳ TASK-BE-435 (terminal refinement) |
...
| payment stranded-refund | **A — Compliant (post-TASK-BE-438).** When `confirm()` captures funds for a concurrently-cancelled order and the post-capture auto-refund fails at the PG (TASK-BE-435/437), a `stranded_refund` row is persisted in the same `REQUIRES_NEW` boundary as the `PaymentRefundStranded` escalation. `StrandedRefundSweeper` (`@Scheduled`, default 60s) + `StrandedRefundReconciler` (REQUIRES_NEW per-record, AOP-split mirror of order-service's `OrderStuckDetector`+`OrderStuckRecoveryHandler`) poll `status='STRANDED' AND next_attempt_at<=now`. **PG-state-first double-refund guard** (the Category-A central hazard here, not present in the order saga): the reconciler reads `PaymentGatewayPort.fetchStatus` before re-issuing a cancel — an already-`CANCELED` payment resolves **without** a second cancel. Bounded retry `payment.stranded-refund.max-attempts` (default 8) + exponential backoff `1s→2s→4s…30s` (Clock-injected); at cap **or** a definitive 4xx PG rejection the record transitions to terminal **`UNRESOLVED`** and re-emits a `PaymentRefundUnresolved` escalation (`payment.alert.refund.unresolved`) co-committed with the status transition. Uses the **default operator-escalation terminal** (D3 base rule), NOT the auto-resolving-terminal refinement — captured funds the machine could not reverse genuinely require a human (R1 "clean self-serve resolution" does not hold; the money is stranded, not merely a never-paid order). Metrics: `payment_refund_stranded_total` (stranding) · `payment_refund_stranded_resolved_total` (auto-healed) · `payment_refund_stranded_unresolved_total` (terminal) · gauge `payment_refund_stranded_open`. All knobs under `payment.stranded-refund.*`. | No further change. | ✅ TASK-BE-438 |
...
| **TASK-MONO-306** — § 2.3 D3 auto-resolving-terminal refinement (R1–R4) + § 2.6 D6 ecommerce-order re-decision | Category A sagas with a clean self-serve stuck resolution should not be forced into the operator-escalation terminal | ✅ this amendment |
| **TASK-BE-435** — ecommerce order stuck-detector terminal → `CANCELLED(PAYMENT_TIMEOUT)` + payment-service late-payment compensation | Adopter of the § 2.3 D3 refinement (depends on TASK-MONO-306) | ✅ MERGED (#1954) |
| **TASK-BE-438** — payment stranded-refund auto-reconciliation sweeper (`stranded_refund` row + `StrandedRefundSweeper`/`Reconciler`, PG-state-first double-refund guard, bounded retry → terminal `UNRESOLVED` + `PaymentRefundUnresolved`) | New Category-A saga: auto-resolve the BE-437 stranded-refund escalation instead of leaving it for an operator | ✅ this task (§ 2.6 D6 row added) |
