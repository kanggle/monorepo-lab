...
> **PROPOSED (staged, sibling: ADR-019/020/023/024/030/042).** This ADR records the **decision direction** (D1–D7) + the **hard invariants the chosen direction inherits** (multi-tenant M1-M7 untouched; `SUPER_ADMIN` net-zero; self-elevation confined to the NEW tenant only; no privilege escalation; ADR-023 plane separation — new tenant born with zero entitlements) + a **zero-regression roadmap**. The ACCEPTED transition is a **separate, user-explicit-intent-gated** task; all execution steps remain **PAUSED** until ACCEPTED. **Self-ACCEPT prohibited.**
...
|---|---|---|
| **A. One atomic "create-organization" transaction: {new tenant} + {caller's operator account/identity, born-unified} + {`TENANT_ADMIN @ new-tenant` grant} + {self-assignment `operator_tenant_assignment`}** | An authenticated caller (a normal IAM user JWT — NOT an operator token) hits a new self-service endpoint. The orchestration creates a `tenants` row (status ACTIVE), resolves-or-creates the caller's central identity + a backing operator account (ADR-036 `resolveOrCreate`, D5), writes one `admin_operator_roles(role=TENANT_ADMIN, tenant_id=<new>)` grant row, and an `operator_tenant_assignment(operator, <new tenant>)` so they can assume-tenant into it. All-or-nothing (D3). | **CHOSEN** — mirrors AWS/GCP "one act creates the boundary AND makes you its admin"; reuses every existing primitive (tenant-management BE-250, born-unified mint ADR-036/BE-402, the ADR-024 `TENANT_ADMIN` role + grant-row scoping); nothing downstream is new. |
...
| Option | Mechanics | Verdict |
|---|---|---|
| **A. New tenant is born entitlement-empty; the `TENANT_ADMIN` (or a `TENANT_BILLING_ADMIN`) subscribes to domains as a SEPARATE act (ADR-023 plane separation)** | Onboarding creates the tenant + admin only. Which platform domains (ecommerce/wms/erp/…) the tenant may use is **not** decided at onboarding — the admin later self-serves subscriptions via the ADR-024 D5-C `TENANT_BILLING_ADMIN`/`subscription.manage` surface (or is granted `TENANT_BILLING_ADMIN` at onboarding as an optional step). IAM plane (who administers) and entitlement plane (what the tenant may use) stay separate (ADR-023). | **CHOSEN** — preserves ADR-023 plane separation; keeps the onboarding transaction small and the tenant's capability an explicit, auditable subsequent choice (GCP: creating a project doesn't enable any API until you enable it). Whether onboarding also grants `TENANT_BILLING_ADMIN` to the first admin is a D7-scope sub-choice (recommended: yes, so the owner can self-enable domains — the AWS/GCP "owner can turn on services" parity). |
...
2. **`TASK-BE-xxx`** (admin-service) — implement the orchestration reusing tenant-mgmt + born-unified mint + ADR-024 `TENANT_ADMIN` grant + self-assignment; D2 confinement enforcement + D3 compensation + D4 rate-limit; cross-tenant-origination-leak IT (assert the self-grant can only ever target the new tenant). Model = **Opus** (isolation/escalation).
...
(PROPOSED row appended 2026-07-04 per the ADR-019/023/024/030/042 staged-child format. **No ACCEPTED row — the ACCEPTED transition is a separate user-explicit-intent-gated task; self-ACCEPT prohibited.** D1–D7 are CHOSEN-PROPOSED; ACCEPTED will *finalise* (not re-decide) unless the user directs gate adjustments as ADR-024's ACCEPTED did.)
