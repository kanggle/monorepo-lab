...
The cross-domain ABAC data-scope attribute is the signed claim **`data_scope`**: a JSON array of **opaque scope tokens** (strings the producer copies verbatim from `operator_tenant_assignment.org_scope`; it does NOT interpret them). `org_scope` is kept as a **backward-compatible alias** — consumers MUST dual-read (`data_scope` then `org_scope`), exactly as erp already does. **Net-zero invariant (verified against erp's `RoleScopeAuthorizationAdapter`):** `["*"]` ⟺ unrestricted; the producer maps an UNSCOPED assignment (`org_scope` NULL) to `["*"]`, so an un-scoped operator carries `["*"]` and sees everything (net-zero). An **empty / absent** claim is **net-zero — no data-scope filter**; a domain filters only when the operator is *deliberately scoped*. **[CORRECTED 2026-07-20 · `TASK-MONO-448` — this is the *second* correction of this sentence; read the next paragraph before reverting it]** The sentence read *"is NOT unrestricted — it is **fail-closed (deny)**"*, which contradicted § 3.1 of this same ADR, the contract this ADR produced ([`platform/abac-data-scope.md`](../../platform/abac-data-scope.md) § 2 — whose heading is literally "Why empty = net-zero (not fail-closed)"), and every consumer implementation.

**⚠️ The fail-closed wording was not an authoring slip — a prior correction introduced it.** This ADR was created (§ 6 row 1, 2026-06-11) saying *"absent/null/`["*"]`=unrestricted net-zero"*. `TASK-MONO-214` then changed it to fail-closed the same day, citing *"confirmed against erp's `RoleScopeAuthorizationAdapter` lines 92-102"*. **That citation over-generalised a real but narrower behaviour.** erp distinguishes two cases the ADR text lumped together: an **absent** claim resolves to unrestricted (net-zero), while an **explicitly-present-but-empty** claim fail-closes — and even that is erp's domain-local hardening for *targeted writes*, not the default for the visibility filter. Generalising it to "empty/absent ⇒ deny" made the ADR forbid what its own contract and every consumer do. **Anyone tempted to flip this back on the strength of § 6's 2026-06-11 row should read that row's citation against the erp code first.** **Why net-zero is the only safe default:** a base `authorization_code` token and a `client_credentials` machine token carry no data-scope claim at all, and both legitimately reach a domain — fail-closing on absence would deny them. Fail-closed remains available as a **domain-local hardening for a single high-security *targeted* decision** (erp does exactly this for department-targeted writes), but it is not the default for the visibility filter.
...
- finance → **does not adopt** (single-tenant by design — no org-unit / accounting-unit partition axis; see `platform/abac-data-scope.md` § 3, TASK-MONO-229). The earlier "(future)" placeholder is removed as a false forward-promise.
...
2b. **Empty / absent (unscoped)**: no tokens → **no filter (net-zero)** — the operator was never scoped, so the domain behaves as it did before data-scoping existed. **[CORRECTED 2026-07-20 · `TASK-MONO-448`; originally "fail-closed deny (NOT unrestricted)" — see D1 for why that was wrong.]** A domain MAY additionally fail-closed for one high-security *targeted* decision where it can guarantee real operators always carry a scope; that is a domain-local hardening, not this rule.
...
| 2026-06-11 | accuracy correction (post-ACCEPTED, § 3.3 step 1) | ⚠️ **THIS ROW'S EMPTY/ABSENT CLAIM WAS REVERTED 2026-07-20 — see the last row.** D1/D2/D4 net-zero wording corrected to the verified semantics: `["*"]`=unrestricted (producer maps unscoped assignment NULL→`["*"]`); ~~**empty/absent=fail-closed deny, NOT unrestricted** (confirmed against erp's `RoleScopeAuthorizationAdapter` lines 92-102)~~ — *the erp citation was real but over-generalised: erp fail-closes only on an **explicitly-present-but-empty** claim, and only for targeted writes; an **absent** claim resolves net-zero there too.* D5/D7 erp re-point downgraded to OPTIONAL/deferred (regression-risk avoidance); the shared `AbacDataScope` reader is introduced for new consumers. No decision reversal — wording accuracy + the step-1 deliverable. | (execution of the ACCEPTED roadmap) | #<this> (TASK-MONO-214) |
| 2026-06-12 | descriptive accuracy (post-ACCEPTED follow-on) | **erp re-point executed** — the D5/D7 OPTIONAL/deferred erp re-point is now done: erp's three inline `org_scope`/`data_scope` parsing sites (masterdata + approval converters, read-model gate) consume the canonical `AbacDataScope` reader, net-zero (parsing-only; domain interpretation preserved). **finance non-adoption clarified** — a survey established finance has no org-unit / accounting-unit partition axis (single-tenant by design), so the D2 / contract § 3 `finance → accounting-unit ids (future)` placeholder was a false forward-promise and is removed; finance does not adopt data-scope. No decision reversal — descriptive accuracy only. | "erp re-point만 진행 (net-zero 통합 refactor)" (user selected the erp re-point + finance non-adoption clarification at the recalibration gate, after the survey showed finance has no scope axis and erp already enforced via inline readers) | #1332 (TASK-ERP-BE-019) · #<this> (TASK-MONO-229) |
| 2026-07-20 | accuracy correction (post-ACCEPTED) — **reverts the empty/absent half of the 2026-06-11 row** | **D1 / D2-2b / D4: empty or absent `data_scope` ⇒ net-zero (no filter), not fail-closed deny.** Restores what this ADR said when created and what its own contract `platform/abac-data-scope.md` § 2 has said all along ("Why empty = net-zero (not fail-closed)"), matching every consumer (`AbacDataScope`, wms `DataScopeSupport`, erp `ReadAuthorizationGate`). **Why the 2026-06-11 correction went wrong:** its erp citation was accurate but over-generalised — erp fail-closes on an *explicitly-present-but-empty* claim for *targeted writes* only; an *absent* claim resolves net-zero there too. Collapsing "empty/absent" into one deny rule made the ADR forbid the behaviour its contract mandates, and would deny base `authorization_code` and `client_credentials` tokens, which carry no data-scope claim at all yet legitimately reach a domain. Fail-closed survives as an explicitly-labelled domain-local hardening. **No decision reversal and no code change** — option (가) of the two the audit put to the owner; (나) (tighten the code to fail-closed) was declined as a breaking change. Found by the 70-ADR drift audit. | "025 는 (가)로, 007a·043 은 편차 노트" (user chose (가) at the gate, naming each ADR and its disposition) | #&lt;this&gt; (TASK-MONO-448) |
