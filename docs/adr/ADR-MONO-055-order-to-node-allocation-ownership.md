...
**Status:** PROPOSED
**Date:** 2026-07-24
**History:** PROPOSED 2026-07-24 (this record). ACCEPT is a human gate — an agent may not accept its own ADR ([`platform/architecture-decision-rule.md` § The ACCEPTED Gate](../../platform/architecture-decision-rule.md)); `PROPOSED → ACCEPTED` requires the owner's exact-form instruction naming this ADR ("ADR-MONO-055 ACCEPTED"). A bare affirmative does not clear it. This PROPOSED record authorises **no code**.
...
- **ACCEPT gate (human).** PROPOSED. `PROPOSED → ACCEPTED` requires exact-form human intent ("ADR-MONO-055 ACCEPTED") per [`platform/architecture-decision-rule.md`](../../platform/architecture-decision-rule.md); a bare affirmative does not clear it, and no self-ACCEPT. On ACCEPT: (a) author TASK-SCM-BE-048 (+049) into scm `backlog/`; (b) add a forward-pointer to **ADR-054's amendment/status record** noting §D5's "no owner" is refined to "ADR-022 §D3, two decisions" here.
...
**ACCEPT gate — held until exact-form intent.** Per [`platform/architecture-decision-rule.md` § The ACCEPTED Gate](../../platform/architecture-decision-rule.md), a bare affirmative ("진행"/"OK") does **not** accept this ADR even though it is the only one under discussion, and the authoring agent may **not** self-ACCEPT. Until the owner issues the exact-form instruction naming this ADR ("ADR-MONO-055 ACCEPTED"), it stays PROPOSED and BE-048/049 stay unauthored.

**What acceptance will bind.** Acceptance authorises the **inbound** half only: **D2** (demand-planning owns replenishment-target allocation, widened to node type), **D3** (the three-hop seam surfacing/sourcing node type), **D4** (the scm-internal 3PL inbound-expectation sink; external notification stays deferred), and the authoring of **TASK-SCM-BE-048 (+049)**. It binds **D6** (the 054 §D5 refinement + forward-pointer). Explicitly **not** bound: **D5's outbound build** — Surface B stays **deferred**, its owner named as ADR-022 §D3 and its build routed to a separate, owner-elected ADR-022 amendment; acceptance does **not** authorise building Surface B, does **not** decide the outbound routing policy, and does **not** reopen 054 §D7 (the external `ThirdPartyFulfillmentPort` stays deferred). **D7** enumerates what stays out.
