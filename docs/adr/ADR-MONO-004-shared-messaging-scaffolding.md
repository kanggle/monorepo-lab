...
| **ecommerce services to v2** — `promotion-service` **DONE (TASK-BE-444)**: V7 `promotion_outbox` (UUID PK), `SpringPromotionEventPublisher` write-path rewrite + `PromotionOutboxPublisher extends AbstractOutboxPublisher`, metrics swap, lib `OutboxAutoConfiguration` retained (v1 tables kept for EntityScan validate). Wire (topics `promotion.coupon.{used,expired}` + envelope payload + key) preserved exactly. **Correction:** the ecommerce producers use the lib **`OutboxWriter`** write path (NOT `BaseEventPublisher`) — a single-axis swap, simpler than master/account. Remaining ecommerce holdouts: `review`, `shipping`, `settlement`, `order`, `payment`. | Schema review per service | per-project backlog |
| Migrate GAP services + fan-platform services off `BaseEventPublisher` to v2 (each runs BOTH the v1 `OutboxPollingScheduler` relay and a `BaseEventPublisher` write path — same dual-axis shape as the migrated master/account services) | Schema review per service | per-project backlog |
...
| Cleanup pre-existing javadoc reference in `BaseEventPublisher.java` ("account and admin publishers") | Trivial; bundle with the next libs/java-messaging PR | shared backlog |
