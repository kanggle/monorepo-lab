...
| Migrate GAP services + ecommerce services + fan-platform services off `BaseEventPublisher` to v2 (note: each still runs BOTH the v1 `OutboxPollingScheduler` relay and a `BaseEventPublisher` write path — same dual-axis shape as the migrated master/account services) | Schema review per service | per-project backlog |
