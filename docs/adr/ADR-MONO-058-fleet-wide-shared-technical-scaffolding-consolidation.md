...
**Status:** ACCEPTED
**Date:** 2026-07-29
**History:** PROPOSED 2026-07-29 (this record). **ACCEPT is a human gate — this record authorises no code.** Per this repo's ADR convention, acceptance requires the owner's exact-form instruction (e.g. `"ADR-MONO-058 ACCEPTED"`); a general "proceed" does not accept it, and the authoring agent must not self-accept. · **ACCEPTED 2026-07-30** — owner reviewed this record alongside sibling `ADR-003` (finance-platform) and selected exact-form intent via an `AskUserQuestion` option explicitly naming `"ADR-MONO-058 ACCEPTED"` (a bare "진행"/"proceed" earlier in the same thread was correctly treated as insufficient per this ADR's own gate, and the session paused to re-ask in exact form). **§ 2 (D1–D8) is byte-unchanged** — ACCEPT finalises the decision, it does not re-decide it. **NOT self-accepted** — the authoring agent (this session) did not accept its own proposal; the owner did, in a separate explicit turn. Per § 5, ACCEPT authorises no single mega-PR — each per-project/per-service adoption in § 6 remains its own future task, filed and implemented separately following this repo's ordinary task-driven workflow. · **D8 (ecommerce) resolved as DEFER, 2026-08-01** (`TASK-BE-571`) — investigation found § 2 D8's "not a new decision, just closing a gap" framing does not hold for ecommerce: `TenantClaimEnforcer` requires a local Spring Security OAuth2 Resource Server JWT chain feeding `SecurityContextHolder`, which none of ecommerce's 10 services run today (only `gateway-service` does RS256 JWT verification; downstream services trust gateway-forwarded headers), and § 2 D4 was never confirmed applicable to ecommerce. § 2's D8 text above is left byte-unchanged per this ADR's own finalization rule; the resolution, reasoning, and re-evaluation trigger are recorded in `projects/ecommerce-microservices-platform/specs/features/multi-tenancy-and-marketplace.md` § 2.7 (not here, to avoid re-opening the bound § 2 decision text). Also note: § 2 D8's cited `TASK-BE-557` does not exist in `ecommerce-microservices-platform`'s tasks — the settlement `.trim()` divergence fix it refers to actually landed as `TASK-BE-564`; cite `TASK-BE-564` going forward.
