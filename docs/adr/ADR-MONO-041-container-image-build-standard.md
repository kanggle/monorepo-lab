...
- **D2 — A single versioned shared base image carries the common runtime preparation (ARCHITECTURE DECISION — separately ACCEPT-gated).**
  ...
  > **Correction (2026-06-17, D2 implementation — distro reversed Debian→Alpine).** The proposal above leaned Debian-slim "since the iam family already uses it." Implementation evidence reversed this: after the D1 sweep, **36 of 41 services were already Alpine** (only iam's 5 were Debian), so standardizing on **Alpine** is far lower churn (re-base 5, not 36) and avoids a musl→glibc move for the majority. The shared base is therefore `eclipse-temurin:21-jre-alpine` + `curl` + `tini` + non-root `app` user (`addgroup -S app && adduser -S` — the alpine system-user form the 36 already used, **not** a fixed uid 1000). iam's `getent` DNS-wait ENTRYPOINT works unchanged on Alpine. Tagged `monorepo/java-service-base:v1` (bump the version, never `:latest`); with no image registry, every CI job that builds an image must `docker build` the base first (added to `federation-hardening-e2e.yml`, `ci.yml` iam-smoke, `nightly-e2e.yml` iam-full). Measured live on iam: account **638 → 472 MB** (−26% vs the D1 Debian image; −42% vs the original 818 MB), base 288 MB shared. The §3 "console-bff moves Debian ← Alpine" consequence is **void** — nothing moves to Debian; iam moves *to* Alpine instead. Rolled out as a **pilot (iam only)**; the other 36 services keep their self-contained Alpine Dockerfiles until a follow-up re-bases them onto the shared image.
...
| 2026-06-17 | D1 COMPLETE | D1 layered-jar swept across all 8 backend projects + merged (#1784–#1793). 27 Dockerfiles converted (+14 already layered); 11 services live-verified (health 200, JarLauncher). |
| 2026-06-17 | D2 PILOT (iam) | Shared base `monorepo/java-service-base:v1` (Alpine — see D2 Correction) created; iam's 5 Dockerfiles re-based onto it; CI base-build step added to 3 workflows. iam account/admin/auth live-verified (health 200); 5/5 build GREEN. account 638→472 MB (−26%). Remaining 36 services re-based in a follow-up. |
| 2026-06-18 | D2 COMPLETE | The remaining 36 services re-based — **all 41 Spring Boot service Dockerfiles now `FROM monorepo/java-service-base:v1`** (TASK-MONO-294, merged + 3-dim verified). D2 PRs: #1796 (iam pilot) · #1800 (finance 2 + console-bff 1 + platform-console nightly base-step) · #1801 (erp 4) · #1802 (wms 7 + ci.yml/nightly base-steps) · #1803 (scm 4 + base-steps) · #1804 (fan 5 + base-steps) · #1806 (ecommerce 13, otel 3-stage). CI base-build step present in every image-building job (`ci.yml`, `nightly-e2e.yml`, `federation-hardening-e2e.yml`). |
| 2026-06-18 | D3 DESCOPED | Context narrowing (AC-4) deferred as optional — low reward (local context-transfer only; CI negligible) vs multi-surface regression risk (Dockerfile COPY + every compose `build.context` + CI + demo overlays in lockstep) and only partially applicable to ecommerce (web-store needs the project-root pnpm-workspace context). D1+D2 fully capture the standardization win. Re-open with a fresh task if local build context-transfer becomes a real bottleneck (iam-only narrowing = the clean subset). |
