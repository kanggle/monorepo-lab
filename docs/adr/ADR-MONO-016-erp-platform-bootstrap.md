...
> **[Amendment — TASK-ERP-BE-013, 2026-06-05] approval-service v2.1 — 대결/위임 (delegation/substitution) (THIRD and FINAL increment of the §D3 approval forward-declaration).** The TASK-ERP-BE-009 amendment deferred "대결/위임 (delegation/substitution) + the `erp.approval.delegated` event"; the TASK-ERP-BE-012 amendment executed the multi-stage/IN_REVIEW half and re-deferred delegation to v2.1. Delegation is now executed as an **additive, backward-compatible** increment, layered on the v2.0 multi-stage base. A new `DelegationGrant` aggregate (delegator A → delegate D, validity window, reason, ACTIVE|REVOKED) is created/revoked via REST (`POST /api/erp/approval/delegations` + `/{id}/revoke` + `GET`), each change written to an **immutable audit row** (rules/domains/erp.md L131 — 결재선/대결 위임은 인가-영향 변경, 불변 감사+운영 조회). At transition time, `approve`/`reject` accept **the current stage's approver OR an active delegate of that approver** (rules/domains/erp.md L116 — 결재자 또는 위임받은 대결자); the audit records `actor` (D) + `onBehalfOf` (A), and the transition event gains an additive `actingForApproverId`. **Separation of Duties is preserved** — a delegate who is the request's submitter is refused (no self-approval via delegation), and `withdraw` stays submitter-only. Grant create emits the new `erp.approval.delegated.v1` topic (rules/domains/erp.md L101 catalog-named) — a **producer-only forward interface** (the existing `read-model-service` BE-010 / `notification-service` BE-011 consumers do not subscribe to it, the four transition topics are unchanged → **consumers UNCHANGED**). New error codes `DELEGATION_INVALID` (422) / `DELEGATION_NOT_FOUND` (404) registered in `platform/error-handling.md` (a new `## Approval Workflow [domain: erp]` section that also consolidates the previously-domain-catalog-only approval codes). This is **additive** and does **not** reopen D3 — it executes the recorded forward-declaration; no new architecture decision (HARDSTOP-09 satisfied by the `approval-service/architecture.md` § v2.1 amendment + additive `approval-api.md`/`erp-approval-events.md` + error-handling.md registration). **This increment COMPLETES the §D3 approval forward-declaration (단계/대결/위임)**: BE-009 single-stage → BE-012 multi-stage + IN_REVIEW → BE-013 delegation. **v2.2-deferred (named)**: per-request/per-route delegation, automatic absence detection, transitive/chained delegation, a read-model/notification consumer of `erp.approval.delegated.v1`, and the console delegation UI. Spec: `approval-service/architecture.md` § v2.1 amendment + `approval-api.md` + `erp-approval-events.md` + `platform/error-handling.md`.

> **[Forward-pointer — ADR-MONO-043 PROPOSED, 2026-06-26]** The `notification-service` bootstrapped here (TASK-ERP-BE-011, the § D3 *"notification first increment"*) is the **erp-domain** instance of what is, across the monorepo, **four** independently-bootstrapped per-domain notification-services (erp / ecommerce / wms / fan — each with its own REST convention, auth/recipient model, schema, idempotency store, external-channel set, and UI consumer). The cross-domain unification that the *"first increment"* framing anticipated — a shared notification envelope + inbox contract, a lifted `libs/` consumer/delivery/channel-SPI library, and a `console-bff` aggregator that gives the shared-shell notification bell one per-domain-failure-isolated feed — is recorded (CHOSEN-PROPOSED) in **[ADR-MONO-043](ADR-MONO-043-notification-architecture-unification.md)** (TASK-MONO-308). This is an **additive** forward-pointer only — this ADR's § D3 decision + the BE-011 amendment stay byte-unchanged (HARDSTOP-04); ADR-043 retrofits no erp producer/consumer.
