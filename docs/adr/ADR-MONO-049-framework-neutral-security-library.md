...
| **D5-6** | fan — artist, community, membership, notification | 12 | **`TASK-MONO-387` (ready)** — **26 → 14**. **The first project that leaves a switch OFF**: fan holds no `isEntitled` anywhere (measured), so `.trustEntitledDomains()` is *not* called — which is where "every switch defaults closed" finally earns its keep, and where § 1.9's null-branch must degenerate to the unconditional 401 fan's copies wrote by hand. Also decides `membership`'s `/internal/**` exemption (§ 1.8) — **by mutation, not by prose**: the workload-identity chain puts a `tenant_id`-less JWT in the context, so removing the exemption should red membership's internal tests. If it does not, the exemption is dead code. |
