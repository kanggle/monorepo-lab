...
| **(deferred follow-ups)** | order-service `account.deleted(anonymized=true)` PII cascade (P3-B); synchronous IAM profile-fetch onboarding enrichment (P1-B); user-service `processed_event` store (P5-B). | Out of v1 scope; tracked. |
...
- **Optional follow-ups:** order-service PII cascade (P3-B); synchronous IAM profile-fetch onboarding enrichment (P1-B); `processed_event` dedup store (P5-B); `account.locked`/`status.changed` projection.

> **PAUSED until ACCEPTED.** TASK-BE-388 (ecommerce `tasks/ready/`) is the implement-ready child; it carries the HARDSTOP-09 ADR-PREREQUISITE banner and must NOT be implemented until this ADR is ACCEPTED. On ACCEPT, the banner is lifted and TASK-BE-388 proceeds dependency-correct (M1→M2→M3→M4); P1–P6 are finalised and not re-litigated at execution.
...
> **PROPOSED — awaiting the explicitly-required ACCEPT gate (NOT self-ACCEPT).** Per the staged-child pattern (ADR-019/020/021/023/024/032/033/034/035/036), ACCEPTED + implementation (TASK-BE-388) are separate user-explicit-intent-gated steps. On ACCEPT, P1–P6 are finalised byte-unchanged (ACCEPTED *finalises*, does not re-decide); the § 3.3 roadmap UNPAUSES and TASK-BE-388's HARDSTOP-09 banner lifts.
