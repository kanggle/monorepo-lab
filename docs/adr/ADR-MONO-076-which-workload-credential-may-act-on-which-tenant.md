...
2. **새 마이그레이션 `V0037` 의 헤더**에 정정을 싣는다 — 다음 사람이 `V0019` 를 읽을 때 옆 파일에서
   반증을 만난다.
3. `V0019` 자체의 바이트 정정은 **신선 볼륨을 얻는 날**로 미루고, 그때까지 `TASK-MONO-672` 가
   항목으로 들고 있는다(그 집이 정확히 그런 의무를 위해 있다).
...
- 🔴 **`V0019` 헤더의 바이트 정정** — 신선 볼륨을 얻는 날까지 보류(D5-3). `TASK-MONO-672` 가 항목으로 든다.
