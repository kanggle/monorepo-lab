...
**Status:** ACCEPTED
**Date:** 2026-07-24
**History:** PROPOSED 2026-07-24 (this record) · ACCEPTED 2026-07-24 (owner exact-form instruction **"ADR-MONO-054 ACCEPTED"**). ACCEPT is a human gate — an agent may not accept its own ADR; author (agent) and acceptor (owner) were separate parties. The PROPOSED record authorised no code; acceptance binds **Phase 2a** (see § What acceptance binds).
...
| 2026-07-24 | ACCEPTED | Flipped on the owner's exact-form instruction **"ADR-MONO-054 ACCEPTED"**. Author and acceptor were separate parties — no self-ACCEPT. |

**ACCEPT gate — cleared, not bypassed.** The owner's preceding bare **"진행"** did **not** accept this ADR — `platform/architecture-decision-rule.md` excludes bare affirmatives from the ACCEPT gate even when only one ADR is under discussion, and the agent held the gate rather than laundering the affirmative into acceptance (the same bite 052 and 053 record). Acceptance came as a **separate, exact-form instruction naming this ADR** ("ADR-MONO-054 ACCEPTED"), issued by the owner — a different party from the authoring agent. The gate's purpose is attributability; it held.

**What acceptance binds.** Acceptance authorises **Phase 2a — Surface A only**: **D2** (the `THIRD_PARTY_LOGISTICS` node factory in inventory-visibility, no Flyway), **D3** (honour a 3PL-destined inbound-expected by routing it away from the wms gate, which stays), **D4** (3PL stock observed **read-only**), and the authoring of **TASK-SCM-BE-046** into scm `ready/`. It also binds **D6** — the two 053 corrections — and adds a forward-pointer to 053's amendment section. Explicitly **not** bound: **D5 / D7** (Surface B — the outbound 3PL fulfillment path) remain **deferred on a named missing owner** (the order-to-node allocation decision upstream of wms); acceptance does **not** authorise building Surface B, and does not upgrade 052 §D2① — §5 retired that analogy. **D8** (no `tms-platform`) declines to reopen 052 §D6 rather than re-deciding it.

**Amending this ADR.** What was authorised is the text as read. Later changes — including any decision that gives Surface B an owner — go through an amendment section (the ADR-050 §7 pattern) or a new ADR, not an in-place rewrite of §1–§6.

---

## 9. Amendment — [ADR-MONO-055](ADR-MONO-055-order-to-node-allocation-ownership.md) (order→node allocation ownership), 2026-07-24

Per the "Amending this ADR" note above ("any decision that gives Surface B an owner [goes] through an amendment section … or a new ADR"), [ADR-MONO-055](ADR-MONO-055-order-to-node-allocation-ownership.md) — ACCEPTED 2026-07-24 — answers the question §D5/§7 deferred ("who owns the order-to-node allocation decision?"). It **refines** two framings here (it does not overturn the Surface-A decisions §D2–§D4/§D7, which stand and which 055 consumes):

- **§D5 / §D6 — "the outbound routing decision has no owner yet."** Refined: the outbound owner is **not missing**. Re-measurement (055 §5) finds it **pre-exists** — [ADR-MONO-022](ADR-MONO-022-ecommerce-wms-fulfillment-integration.md) §D3 already owns "warehouse routing" and deferred *multi-warehouse routing* to v2 (*"v1 = single default warehouse (config). Multi-warehouse routing = v2"*). Outbound self-vs-3PL **is** that deferred v2, generalised to node type, decided in the ecommerce ACL (022 §D6). Surface B stays **deferred** — but on a **named routing-policy prerequisite in ADR-022 territory** (an owner-elected ADR-022 amendment), not on a missing owner.
- **§D5's singular "the order-to-node allocation decision."** Refined: it is **two** decisions at two altitudes (055 §1.2). §D5's deferral correctly covered the **outbound** decision, but its "no owner yet" phrasing swept in the **inbound** replenishment-target decision — which is `demand-planning-service`'s existing reorder decision ([ADR-MONO-027](ADR-MONO-027-wms-scm-replenishment-loop.md)), **not** the retired 052 §D2① *outbound-fulfillment* claim — and which 055 **un-defers** (§D2–§D4, authoring TASK-SCM-BE-048/049). BE-047's read-only observation is exactly its input.

Unchanged and still binding: §D2 (node factory, done via BE-046), §D3 (route inbound away from wms), §D4 (read-only observation, done via BE-047), §D7 (`ThirdPartyFulfillmentPort` designed-not-built — stays deferred; 055 §D4's inbound sink is scm-internal and does not depend on it), §D8 (no `tms-platform`). Read those together with ADR-055 §D5/§D6.
