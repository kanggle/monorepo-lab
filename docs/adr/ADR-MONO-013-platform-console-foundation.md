...
- **service_types:** `frontend-app` (the console UI) at v1; `rest-api` added when the console-bff (Phase 7) lands (architectural scope for the BFF — D1-D8 — recorded in [ADR-MONO-017](ADR-MONO-017-platform-console-bff-architecture.md) PROPOSED 2026-05-20; the `service_types` mutation itself happens at the post-ACCEPTED `console-bff` skeleton task, not at this ADR) (see [ADR-MONO-018](ADR-MONO-018-platform-console-phase-8-federation-hardening.md) for Phase 8 federation-hardening architecture; sibling staged-child to ADR-MONO-017's Phase 7 BFF architecture; § D6 row 8 deferred axis decomposition into D1-D8 PROPOSED).
...
> **Additive note — Phase 7 PROPOSED authoring (ADR-MONO-017) — recorded by TASK-MONO-125 (PR (this PR)).** The § D6 Phase 7 deliverable (*"console-bff fan-out + cross-domain dashboards"*) gate (*"5 domains live"*) is **satisfied 2026-05-20** by Phase 6 COMPLETE (above; 5/5 backend domains: GAP + wms + scm + finance + erp). The architectural *what* of the BFF (aggregation pattern / federation shape / dashboards composition / credential reuse / resilience / multi-tenant / observability / phasing — 8 axes D1-D8) is recorded in [`ADR-MONO-017`](ADR-MONO-017-platform-console-bff-architecture.md) **PROPOSED 2026-05-20** (TASK-MONO-125, this PR) — staged-child ADR under this ADR (§ D5 + § D6 Phase 7), identical pattern to [`ADR-MONO-014`](ADR-MONO-014-platform-console-operator-auth-token-exchange.md) (Phase 2 operator-auth) + [`ADR-MONO-015`](ADR-MONO-015-platform-console-dashboards-model.md) (Phase 2 dashboards). PROPOSED CHOSEN-direction (finalised at ACCEPTED): **D1** = REST orchestrator (B; smallest blast radius, zero-retrofit affirming) · **D2** = server-side fan-out only (affirms § D5) · **D3** = reuse existing per-domain reads verbatim (§ 3.3 fifth confirmation) · **D4 (HARD INVARIANT)** = per-domain credential rule (§ 2.4.5/6/7/8) extended verbatim, never retroactively redefined · **D5** = per-domain circuit-breaker inherited from § 2.5 · **D6** = `tenant_id` claim pass-through (producer-side gate authority preserved) · **D7** = per-domain fan-out attribution (reuse ADR-MONO-006 observability stack) · **D8** = MVP = 1 "Operator Overview" cross-domain dashboard (ADR-MONO-015 Composed-overview pattern generalised across 5 domains). This note is **additive only** — no ADR-MONO-013 decision (D1–D8) is changed (ADR-013 ACCEPTED; HARDSTOP-04 discipline). ADR-MONO-017 PROPOSED is its own decision record; ACCEPTED transition + the `console-bff` skeleton + the first cross-domain dashboard MVP are **separate future tasks** (sibling: ADR-MONO-014 → TASK-BE-298; ADR-MONO-015 → TASK-PC-FE-005). § D5 line 76 is also additively amended in this PR with a parenthetical scope-pointer to ADR-MONO-017 PROPOSED (no D1-D8 change). The `service_types` mutation (`rest-api` added to `platform-console`'s `PROJECT.md`) does **not** happen on this PROPOSED — it happens on the post-ACCEPTED `console-bff` skeleton task per § D5.

> **Additive note (2026-05-25, ADR-MONO-018 PROPOSED)** — Phase 8 federation hardening 의 architecture-decision 차원이 ADR-MONO-018 으로 분리 (TASK-MONO-137, PR #<this>). § D6 row 8 의 location/trigger/harness/scope/observability federation/isolation regression cohort/phasing/gate 가 ADR-018 D1-D8 로 분해되어 PROPOSED 됨. 본 ADR (013) 의 D1-D8 / § D5 / § D6 / § D7 / § D8 본문은 byte-unchanged (HARDSTOP-04). ADR-018 ACCEPTED 후 execution task series (cross-product e2e + observability federation + per-domain isolation regression IT) 가 별도 분리됨.

> **Additive note (2026-05-26, ADR-MONO-018 ACCEPTED)** — Phase 8 federation hardening 의 architecture-decision 차원 [`ADR-MONO-018`](ADR-MONO-018-platform-console-phase-8-federation-hardening.md) 가 **ACCEPTED 2026-05-26** (TASK-MONO-138, this PR — sibling ADR-MONO-014/MONO-110 + ADR-MONO-015/MONO-112 + ADR-MONO-017/MONO-126 staged-child ACCEPTED 패턴 정확 답습). User-explicit intent = first-message direct request 2026-05-26 ("ADR-MONO-018 ACCEPTED 로 승격 진행해줘" + § D8.1 acceptable form *"ADR-018 ACCEPTED"* 첫 form 직접 인용 + sibling pattern explicit citation; ambiguous form 아닌 명시 confirm form). D1-D8 CHOSEN-PROPOSED finalised **byte-unchanged** from PROPOSED (ACCEPTED = *finalise* not re-decide; HARDSTOP-04 + sibling ADR-014/015/017 ACCEPTED-flip discipline; ADR-018 § 1 Context + § 2 Decision tables + § 3 Consequences + § 4 Alternatives + § 5 Relationship + § 7 Provenance byte-identical; flip = Status + History ACCEPTED 줄 append + § 6 ACCEPTED row append + § 1.3 minimal past-tense). 본 ACCEPTED 가 § D6 **row 8 (Phase 8 federation hardening) 의 architectural authorization base** 를 확립 — 후속 3 execution task series (cross-product e2e cohort + observability federation impl + multi-tenant isolation regression IT cohort, ADR-018 § 3.3 Future-self step 2/3/4) 가 dependency-correct base = 본 PR ACCEPTED main 위에서 진행. 본 ADR (013) 의 D1-D8 / § D5 / § D6 / § D7 / § D8 본문은 byte-unchanged (HARDSTOP-04). § D6 Phase roadmap 의 **final row (Phase 8)** 가 architecture-decision 차원에서 ACCEPTED → ADR-013 Phase roadmap 의 모든 row (Phase 0~8) 가 architecturally-authorised 상태에 도달 (실 execution 은 각 phase 별 별 task series 의 일).

> **Additive note (2026-05-30, ADR-MONO-019 PROPOSED)** — the § D5 / § 1.2 **data-driven catalog "tenant == domain slug" shape** is recorded as a **demo simplification** whose *production* form is decided in [`ADR-MONO-019`](ADR-MONO-019-platform-console-customer-tenant-model.md) **PROPOSED 2026-05-30** (TASK-MONO-152, PR #<this>). The console tenant switcher today lists **domain names** (`gap`/`wms`/`scm`/`erp`/`finance`), not customer names, because `ProductCatalog` binds each domain product to a `tenantSlug` *equal to the domain name* and the demo DB seeds tenant rows named after the domains; the per-domain `TenantClaimValidator` reinforces this by enforcing `tenant_id ∈ {<domain-slug>, *}` (one fixed value per domain). ADR-MONO-019 decides the production SaaS shape — a customer-tenant entity distinct from the product/domain (D1, reusing the account-service `tenants` registry), an N:M `tenant_domain_subscription` table (D2, GAP-owned entitlement authority), an explicit operator↔tenant assignment (D3, least-privilege), a subscription-driven `selectableTenants` whose registry envelope shape is **byte-stable** (D4, zero `console-web` change — the § D5 data-driven invariant's sixth confirmation), and a per-domain isolation gate evolved from `tenant_id == slug` to **entitlement-trust** (D5, GAP issues a domain-scoped token only for an entitled tenant; the domain isolates by row) — landed via a 4-step zero-regression migration with a dual-accept window (D6, BE-317 discipline). This note is **additive only** — no ADR-MONO-013 decision (D1–D8) is changed (ADR-013 ACCEPTED; HARDSTOP-04 discipline; § D5 / § D6 / § D7 / § D8 bodies byte-unchanged). ADR-MONO-019 realizes [`ADR-MONO-018`](ADR-MONO-018-platform-console-phase-8-federation-hardening.md) § 3.3's anticipation ("a new architectural axis post-Phase-8 → a new ADR") and is orthogonal to [ADR-005 (GAP)](../../projects/iam-platform/docs/adr/ADR-005-service-to-service-workload-identity.md) / TASK-BE-317 (service-to-service workload identity). ADR-MONO-019 PROPOSED is its own decision record; ACCEPTED transition + the 4-step execution roadmap (subscription model → real-customer seed → per-domain gate dual-accept → cleanup) are **separate future tasks** (sibling: ADR-MONO-014 → TASK-MONO-110; ADR-MONO-015 → TASK-MONO-112; ADR-MONO-017 → TASK-MONO-126; ADR-MONO-018 → TASK-MONO-138).

> **Additive note (2026-06-06, TASK-MONO-179 project rename)** — the OIDC authority project formerly named `global-account-platform` (short alias `gap`) is **renamed to `iam-platform` (alias `iam`)** to reflect its promotion to a full OIDC AS / Identity & Access Management platform. **Pure rename** — directory, Gradle coords, Docker/compose, CI jobs, dev hostname (`gap.local`→`iam.local`), Java identifiers, and the user-login JWT `iss` claim (`global-account-platform`→`iam`, flipped atomically across mint + all consumer `AllowedIssuersValidator`/`allowed-issuers`). **No ADR-MONO-013 decision (D1–D8) changes** (HARDSTOP-04; § D5/D6/D7/D8 bodies byte-unchanged). Two persisted-contract values are **intentionally retained** for backward compatibility (out of rename scope, checksum-locked Flyway seeds): (1) the internal workload-client `tenant_id='global-account-platform'` (V0019, inert downstream — receivers validate signature+issuer only); (2) the NextAuth OIDC provider id `gap` and its `/api/auth/callback/gap` redirect_uris (V0011/V0012). `projects/global-account-platform/...` path references and the `gap` domain-slug in this ADR's historical body remain as historical record.
