...
> **Additive note (ADR-MONO-047, 2026-07-10 — HARDSTOP-04, D1 body byte-unchanged):** D1 made `tenant` the single flat isolation key and equated one customer company with one tenant. It left **no layer between "company" and "domain"**, so a company wanting several *independently isolated* services (each with its own data boundary and its own mix of subscribed domains) had no way to express that, and no object to carry a company-wide entitlement boundary or a company-wide admin. [ADR-MONO-047](ADR-MONO-047-org-node-tenant-hierarchy.md) adds a **data-less `org_node` grouping tree ABOVE `tenant`** (D1-A: `tenant.org_node_id`, nullable) — a company becomes a node owning ≥ 1 isolated **service-tenant**. The tree **groups** tenants, it never **nests** them: `tenant_id` remains the one isolation key, every M1–M7 row-isolation guard is byte-unchanged, and the token still carries exactly one `tenant_id`. A tenant with `org_node_id = NULL` is a legal **ungrouped singleton**, so every existing row is unaffected (ADR-047 D7 — the migration is behaviourally a no-op and may even be lazy). ADR-047 also attaches a **deny-only entitlement ceiling** to each node, inherited downward as an intersection that can only *narrow* what a tenant may subscribe to (never grant). This ADR's D1–D5 bodies are unchanged; ADR-047 adds a grouping layer *above* the isolation key, not a change to it.
