...
| 4 | **fan** ← 🟢 **집행 3번째** | Server Action **5파일** — 서버 경계가 **이미 있다** | 🔴 **정정됨 (2026-08-25 `TASK-MONO-578`)**: 프록시 신설이 아니다. 주소를 만드는 지점 **1곳**(`client.ts:42`) + **D4**(움직이는 issuer). ↑ § 단계 순서 아래 **정정 ①**. 🟢 **D4 는 팬 표면에서 닫혔다**(2026-09-03 `TASK-MONO-616` 창 #3: `fan.hubwang.com` 왕복이 세션까지) ⇒ 남은 것은 **억제 몫**(`TASK-MONO-618`) + 해석기 몫(`TASK-MONO-586`) |
| 3 | **console** ← 집행 4번째 | `route.ts` 159개(이미 BFF) | 남은 절대 fetch 5건 + **OIDC/쿠키 축**. `TASK-MONO-585` 가 **BLOCKED** (선행 셋) |
