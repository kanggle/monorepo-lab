...
- **v2(b) — inventory reconciliation (built, TASK-MONO-198 — Option B delta reconciliation):** ecommerce `product-service` consumes the warehouse-origin wms events (`wms.inventory.received.v1` + `wms.inventory.adjusted.v1`) and applies the **available-quantity delta** to its sellable `stock`, narrowing the dual-bookkeeping drift (warehouse shrinkage/restock that ecommerce did not itself initiate).
  - **Sync policy = Option B (delta, keep the order-time gate)** — chosen over A (wms-as-SoT projection: removes ecommerce's order-time reservation gate → oversell window, max blast radius) and C (periodic batch pull: not event-driven). product-service keeps decrementing at order time; reconciliation only layers warehouse-origin deltas on top.
  - **Excluded by design:** the reservation lifecycle (`inventory.reserved`/`released`/`confirmed`) is **not** consumed — those movements are *caused by ecommerce orders*, which product-service already decremented at order time; re-applying them would double-count. `inventory.transferred` is **not** consumed — an intra-warehouse location move leaves the SKU total invariant.
  - **Correctness mechanism:** product-service stores a per-`inventoryId` `availableQty` snapshot; on each consumed event it applies `(new availableQty − stored)` to the mapped variant. This is correct for any adjustment reason/bucket (no need to decode wms bucket semantics) and the snapshot table doubles as the reconciliation ledger (the periodic full-resync backstop becomes a re-sum, named as a v2(b) follow-up).
  - **Reverse identity (the cost the ADR flagged):** wms events carry `skuId` (uuid), not `skuCode`. To keep wms domain-pure (D6), the resolution lives entirely on the ecommerce side: product-service consumes `wms.master.sku.v1` into a local `skuId → skuCode` snapshot, then resolves `skuCode → variantId`. **Discovered during impl: ecommerce `ProductVariant` had no SKU business key (UUID id only)** — so this task **adds a `sku` business key to `ProductVariant`** (the proper model fix, chosen by AskUserQuestion over a brittle config-uuid map), giving `skuCode == ProductVariant.sku` and a real `findVariantBySku` resolution. wms is **untouched** (consumes only already-published events).
  - **Degradation (D8):** without wms present these topics never arrive; product-service stock stays purely order-driven (today's behavior). Unmapped wms SKUs (not sold on ecommerce) → skipped silently.
- **v2(c) — operator-gated manual ship-confirm (built, TASK-MONO-305):** the manual admin ship path (`PUT /api/shippings/{id}/status` → SHIPPED) historically emitted **no** wms signal, so a WMS-routed order shipped by hand left its wms reservation `RESERVED` forever (phantom reserved stock — the **reverse direction** that v2(b), a wms→ecommerce delta, does not cover). v2(c) lets the operator **opt in** at manual-ship time (a `deductWmsInventory` flag, surfaced in console **only when** the Shipping row is `wmsRouted`) to also deduct wms physical inventory. The election is routed as `ecommerce.shipping.manual-confirm-requested.v1` (wms camelCase envelope, ACL-shaped per D6) to the **wms outbound-service**, which already owns the `orderNo → order → saga → reservation` mapping: a new `ManualShipConfirmConsumer` resolves the order by `orderNo` and, **iff the saga is `PACKING_CONFIRMED`**, calls the **existing** `ConfirmShippingUseCase` → **existing** `wms.outbound.shipping.confirmed.v1` → **existing** inventory deduction. **inventory data model untouched** — the cheaper of two designs; the alternative (teach inventory to confirm by `orderNo`) would add an `orderNo` column to `Reservation` + change the `picking.requested` contract across 5 services. Earlier saga states are a logged no-op (not yet physically picked/packed — forcing a deduction would violate the physical invariant); already-`SHIPPED` is a terminal no-op (idempotent); non-WMS-routed orders never show the toggle and a stray event misses `findByOrderNo` safely. The return-leg `markShippedByOrderId` re-entry is an idempotent no-op (no loop).
...
| 2026-06-25 | **D4 v2(c) added + ACCEPTED** — operator-gated manual ship-confirm propagates to wms inventory deduction (reverse direction, outbound-routed; inventory model untouched) | (unchanged) | (unchanged) | **reverse-confirm built** (operator opt-in) | "수동 출하 할때 관리자가 선택해서 wms도 차감해줄지 안할지 정하도록" + AskUserQuestion: 토글=경유 주문에만 노출 / 전달=Kafka 이벤트 비동기 | spec (contracts/ADR) + impl PR (TASK-MONO-305) |
