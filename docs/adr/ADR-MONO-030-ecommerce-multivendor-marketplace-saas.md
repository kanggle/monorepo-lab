...
Per HARDSTOP-04, this supersession is **recorded here** and **applied at the ACCEPTED transition** (Step 0), not silently. The PROPOSED ADR does not edit `PROJECT.md`.
...
4. **Deferred follow-ups** (each its own ADR-extension or task as it lands): seller settlement/commission; console integration (`domain_key` subscription seed + catalog render — the original ask); the remaining 11 services; ADR-022 fulfillment `tenant_id` threading. Model = **Opus** (settlement/isolation) / **Sonnet** (console render, pattern-repeat services).
