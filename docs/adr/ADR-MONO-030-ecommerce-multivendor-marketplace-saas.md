...
4. **Deferred follow-ups** (each its own ADR-extension or task as it lands): seller settlement/commission; console integration (`domain_key` subscription seed + catalog render — the original ask); the remaining 11 services; ADR-022 fulfillment `tenant_id` threading; **seller onboarding flow + real IAM provisioning (facet f)**. Model = **Opus** (settlement/isolation) / **Sonnet** (console render, pattern-repeat services).
   - **facet d (ADR-022 fulfillment `tenant_id` threading) — REALIZED** by [ADR-MONO-022](ADR-MONO-022-ecommerce-wms-fulfillment-integration.md) §6 facet-d row / TASK-MONO-296: the M5 async-propagation invariant now reaches the ecommerce↔wms fulfillment loop. The forward leg already threaded `tenantId`; the return leg now does too — wms captures the inbound envelope `tenantId`, carries it as an **opaque correlation** column on the outbound order (NOT an isolation key — wms stays single-tenant per §1.1; no NOT NULL / row filter / gate change), and echoes it on `wms.outbound.shipping.confirmed.v1` + `wms.outbound.order.cancelled.v1`. The ecommerce return consumers bind it into `TenantContext` (local-row fallback by `orderNo` when absent, D8) and clear it in a `finally`. Realized via the established ADR-022 D5 correlation-round-trip pattern (not a new decision).

     > **[Correction — TASK-BE-583, 2026-08-14, factual] The parenthetical above (*"NOT an isolation key — wms stays single-tenant per §1.1; no NOT NULL / row filter / gate change"*) described wms as of 2026-06-12 and is no longer true.** [`ADR-MONO-064`](ADR-MONO-064-wms-outbound-tenant-visibility-plane.md) (ACCEPTED 2026-08-13) made `outbound_order.tenant_id` **the** isolation axis of the wms outbound REST plane — D1 stamps the creating caller's signed tenant on the order, D2 filters list queries by it — and [`ADR-MONO-065`](ADR-MONO-065-wms-admin-read-plane-tenant-axis.md) (ACCEPTED 2026-08-14) extends that axis to the `admin-service` order/shipment projections. The column is still **nullable** (no `NOT NULL`) and the entry **gate** is still unchanged, so the correction is specifically to *"opaque correlation / not an isolation key / no row filter"*.
     >
     > 🔵 **No decision here is reversed, and ecommerce is unaffected.** facet d's subject is the *round trip* of the ecommerce tenant, and that round trip is byte-identical — the same value captured at intake, echoed on the same two return-leg topics. What changed is what **wms** additionally does with the column on its own side (it now also stamps `MANUAL` orders from the caller's JWT, so those return-leg envelopes carry a tenant where this text says they would not). §1.1's *"wms does not [carry row-level `tenant_id`]"* is stale for the same reason; both are left **byte-unchanged** as the record of what was true when written, and this note is the pointer.
...
| 2026-08-14 | ACCEPTED — factual correction (facet d + §1.1 wms tenancy) | **No decision reversed; ecommerce unaffected.** `ADR-MONO-064` (2026-08-13) then `ADR-MONO-065` (2026-08-14) turned `outbound_order.tenant_id` from an opaque correlation column into wms's **isolation axis** (stamped from the creating caller's signed JWT; list queries filtered by it; extended to the `admin-service` order/shipment projections). That makes facet d's parenthetical *"NOT an isolation key … no row filter"* and §1.1's *"wms does not carry row-level `tenant_id`"* **stale**. The round trip facet d actually decides is byte-identical — same value at intake, same two return-leg topics — so only wms's own use of the column changed (incl. `MANUAL` orders now carrying a tenant). Recorded as an additive correction note under facet d; the accepted text is left byte-unchanged. | (TASK-BE-583 AC-4 / `ADR-MONO-065` D5 — factual correction, not a decision) | #<this PR> (TASK-BE-583) |
