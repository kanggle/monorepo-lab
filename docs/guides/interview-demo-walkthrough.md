...
| 🔴 팬 **아티스트 글은 어떤 실제 호출자도 쓸 수 없다** | 피드는 `posts.author_account_id ⋈ follows.artist_account_id`(= 아티스트 엔티티 id)로 잇는데 `PublishPostUseCase` 는 저자를 호출자 sub 으로 고정한다. 시드가 직접-DB 를 쓰는 이유. 🟢 **결정 완료 (2026-08-07)**: `ADR-MONO-059` **ACCEPTED — A**(아티스트에게 실제 계정 `artists.account_id`) ⇒ 이 티켓은 **착수 가능**하고 범위는 스키마 + 온보딩 + 조인 검증이다 | `TASK-FAN-BE-045` |
| 🔴 팬 도메인에 **발급 가능한 운영자 역할이 없다** | `FAN_OPERATOR` 를 받는 코드는 iam·artist·community 전부에 있는데, `fan` 도메인을 구독한 테넌트가 없어 그 역할이 발급될 수 없다. 🟢 **결정 완료 (2026-08-07)**: `ADR-MONO-059` A 가 **B(운영자 대리 저작)를 배제**했으므로 *"B2C 테넌트를 운영자가 assume 하는가"* 는 **열지 않는다**로 답이 났다 ⇒ 남은 범위는 `ARTIST` **역할 발급 하나**로 좁아졌다 | `TASK-MONO-512` |
...
| 🔴 SCM **공급사를 등록할 API 가 없다** | `POST /po` 가 `SUPPLIER_NOT_FOUND` 인데 저장소 전 컨트롤러에 suppliers 생성 매핑이 **0건**이다(그 도메인의 e2e 도 DB 픽스처로 직접 넣는다). 공급사 관리 화면 자체가 없다 ⇒ 시드의 유일한 경로가 직접-DB 다. 🟢 **결정 완료 (2026-08-07)**: `ADR-SCM-001` **ACCEPTED — A**(운영자용 등록/조회 엔드포인트) + **자격증명은 v2 `supplier-service` 로 유보** ⇒ 착수 가능. 🔴 유보의 결과로 **자격증명 미보유 공급사가 v1 의 정상 상태**가 된다 | `TASK-SCM-BE-059` |
...
| 🔴 콘솔 **`/erp/delegation` 의 read-model 위임 뷰만 여전히 빈다** | 릴레이가 살아나자 **그 아래의 결함**이 드러났다: approval 봉투에 최상위 `aggregateId`·`tenantId` 가 없는데(masterdata 봉투는 둘 다 싣는다) 소비자가 그것을 **필수**로 요구해 첫 메시지가 곧바로 **DLT** 로 갔다. `erp.approval.*` **여섯 토픽이 같은 운명**이다. 🔴 **`TASK-ERP-BE-041` 이 닫히자 예측이 실측으로 확인됐다** — 상신 2건이 발행되며 `erp.approval.submitted.v1` end-offset **0 → 2 · DLT 4**(소비자 2 × 2, retry 0 직행). 이제 `delegated`·`submitted` 두 토픽이 실측 대상이고, 나머지 넷이 0 인 것은 **막혀서가 아니라 그 전이를 아직 안 눌러서**다. 원본 위임 목록(`/api/erp/approval/delegations`)은 정상(1건) | `TASK-ERP-BE-043` |
