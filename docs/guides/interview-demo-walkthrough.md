...
| SCM · Finance 도메인 데이터 시드 없음 | 미착수. SCM(5)·Finance(3) 은 **이미지도 없어** 빌드가 선행한다 | `TASK-MONO-510` |
| 🔵 콘솔 **`/erp/masters` · `/erp/approval` 목록 · `/erp/delegation` 은 실제로 찬다** | 이 셋은 프로듀서 DB 를 직접 읽어 프로젝션에 의존하지 않는다(라이브 원소 수 3·4·3·3·3 / 3 / 1). ERP 6화면 중 **4개 충족** | — |
...
| 🔴 콘솔 **`/erp/orgview` 는 시드와 무관하게 빈다** | erp 아웃박스 릴레이가 **한 번도 돌지 않는다** — `@Scheduled` 는 있는데 `@EnableScheduling` 이 없다(erp 5앱 중 notification 하나만 보유; wms 6/6·scm·finance 는 전부 보유). 아웃박스 UNPUBLISHED 17, **kafka end-offset 전 토픽 0** ⇒ read-model 프로젝션 5종이 전부 영구 0행이고 `read-model-service` 는 healthy 인데 하는 일이 없다 | `TASK-ERP-BE-042` |
