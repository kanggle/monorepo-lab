...
| 🔵 팬 도메인에 **발급 가능한 운영자 역할이 없다 — 그리고 그것은 결정이다** (2026-08-11 절반 해소) | `FAN_OPERATOR` 를 받는 코드는 iam·artist·community 전부에 있는데, `fan` 도메인을 구독한 테넌트가 없어 그 역할이 발급될 수 없다. 🟢 **결정 완료 (2026-08-07)**: `ADR-MONO-059` A 가 **B(운영자 대리 저작)를 배제**했으므로 *"B2C 테넌트를 운영자가 assume 하는가"* 는 **열지 않는다**로 답이 났다 ⇒ 남은 범위는 `ARTIST` **역할 발급 하나**였고, `TASK-MONO-512` 가 그것을 닫았다(account `R__06` 이 역할을, auth `R__02` 가 자격증명을 심는다). 🔴 **`FAN_OPERATOR` 는 여전히 발급 불가이고, 이제 그것은 공백이 아니라 `ADR-MONO-059` A 의 결과다** — 열려면 새 ADR 이 필요하고, 디렉터리 쓰기 라우트 9개를 어떻게 할지는 `TASK-MONO-522` 가 판정한다 | `TASK-MONO-522` |
| 🔴 API 가 **팬에게 유료 글 발행을 허용**한다 | `PublishPostUseCase` 가 `postType` 만 검사하고 `visibility` 는 **아무도 검사하지 않는다** ⇒ 일반 팬이 멤버십 게이트된 글을 만들 수 있다. ⏸ **fan-platform `ADR-003` PROPOSED — 소유자 결정 대기**(정확형 `ADR-003 ACCEPTED — <안>`) | `TASK-FAN-BE-047` |
...
| 🔴 콘솔 **`/erp/delegation` 의 read-model 위임 뷰만 여전히 빈다** | 릴레이가 살아나자 **그 아래의 결함**이 드러났다: approval 봉투에 최상위 `aggregateId`·`tenantId` 가 없는데(masterdata 봉투는 둘 다 싣는다) 소비자가 그것을 **필수**로 요구해 첫 메시지가 곧바로 **DLT** 로 갔다. `erp.approval.*` **여섯 토픽이 같은 운명**이다. 🔴 **`TASK-ERP-BE-041` 이 닫히자 예측이 실측으로 확인됐다** — 상신 2건이 발행되며 `erp.approval.submitted.v1` end-offset **0 → 2 · DLT 4**(소비자 2 × 2, retry 0 직행). 이제 `delegated`·`submitted` 두 토픽이 실측 대상이고, 나머지 넷이 0 인 것은 **막혀서가 아니라 그 전이를 아직 안 눌러서**다. 원본 위임 목록(`/api/erp/approval/delegations`)은 정상(1건). ✅ **결정 완료 (2026-08-12)**: `ADR-ERP-001` **ACCEPTED — D** — 관문을 값-등호에서 존재검사로 낮춘다. 남은 것은 구현뿐  ✅ **구현·라이브 검증 완료 (2026-08-12 UTC, `TASK-ERP-BE-043`, 머지 대기)** — 가산 프로브로 위임 grant 1건을 새로 만들자 `delegation_fact_proj` **0 → 1**(`tenant_id=demo-corp`), read-model 위임 목록 `totalElements 1`, **`.DLT` 전 토픽 0 유지**. 🔴 다만 **기존 볼륨에서는 수정 이전에 발행된 위임 2건이 소급 투영되지 않는다**(원본 `delegation_grant` 4 vs 투영 1) — 완전한 데모는 볼륨 초기화 + 재시드가 필요하다. 이 행은 impl PR 머지 + close chore 에서 제거한다. | `TASK-ERP-BE-043` |
...
| 🔴 ERP **알림함(두 번째 결재함)은 여전히 0** — 결재함과 **다른 원인** | `NotificationInboxController.recipient(jwt) = jwt.getSubject()` 라 신원이 둘이 되면 알림 수신자도 갈리는데, 그 앞에서 이벤트가 죽는다. 실측(2026-08-12, 승인자 sub 로): `GET /api/erp/notifications` **200 / totalElements 0**, `erp_notification_db.notification` **0행**, 그리고 소비자 로그가 원인을 그대로 적는다 — `Out-of-contract tenantId 'demo-corp' on topic erp.approval.submitted.v1 (single-tenant invariant: erp)`. 세 토픽(`delegated`·`submitted`×2·`approved`)이 **전부 DLT**. 🔴 봉투는 이미 고쳐져 있다(실측한 실물에 최상위 `tenantId`/`aggregateType`/`aggregateId` 전부 있음) — 막는 것은 `EnvelopeToCommandMapper` 의 **테넌트 관문**이고, 이것은 아래 `/erp/delegation` 행과 **같은 관문 하나**다. ✅ **결정 완료 (2026-08-12)**: `ADR-ERP-001` **ACCEPTED — D**(소유자 정확형) — 두 관문이 `required-tenant-id` 를 안 읽고 봉투는 사실을 싣는다. 🔴 **ACCEPT 로 차지 않는다 — 구현이 차게 한다**(`TASK-ERP-BE-043` AC-8)  ✅ **구현·라이브 검증 완료 (2026-08-12 UTC, `TASK-ERP-BE-043` AC-8, 머지 대기)** — `notification` **0 → 2** · `notification_delivery` **0 → 2**(둘 다 `demo-corp`), `GET /api/erp/notifications` → **`totalElements 1`**(APPROVAL_SUBMITTED, 승인자 sub 수신). 🔴 **관문은 둘이 아니라 셋이었다** — 이벤트 관문 2곳 외에 이 컨트롤러 자신이 **질의 테넌트로 `required-tenant-id` 상수를 넘기고 있었다**. 그것을 안 고쳤다면 테이블은 차는데 알림함은 그대로 0 이다. 이 행은 impl PR 머지 + close chore 에서 제거한다. | `TASK-ERP-BE-043` |
