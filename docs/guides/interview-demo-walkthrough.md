...
| 🔴 콘솔 **`/erp/delegation` 의 read-model 위임 뷰만 여전히 빈다** | 릴레이가 살아나자 **그 아래의 결함**이 드러났다: approval 봉투에 최상위 `aggregateId`·`tenantId` 가 없는데(masterdata 봉투는 둘 다 싣는다) 소비자가 그것을 **필수**로 요구해 첫 메시지가 곧바로 **DLT** 로 갔다. `erp.approval.*` **여섯 토픽이 같은 운명**이다. 🔴 **`TASK-ERP-BE-041` 이 닫히자 예측이 실측으로 확인됐다** — 상신 2건이 발행되며 `erp.approval.submitted.v1` end-offset **0 → 2 · DLT 4**(소비자 2 × 2, retry 0 직행). 이제 `delegated`·`submitted` 두 토픽이 실측 대상이고, 나머지 넷이 0 인 것은 **막혀서가 아니라 그 전이를 아직 안 눌러서**다. 원본 위임 목록(`/api/erp/approval/delegations`)은 정상(1건). ✅ **결정 완료 (2026-08-12)**: `ADR-ERP-001` **ACCEPTED — D** — 관문을 값-등호에서 존재검사로 낮춘다. 남은 것은 구현뿐 | `TASK-ERP-BE-043` |
