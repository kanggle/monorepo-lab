...
> ✅ ~~**여기서 눌리지 않는 것 두 가지** — 멤버십 **구독 버튼**은 PortOne 키가 없으면
> "결제 모듈이 설정되지 않았습니다" 로 막힌다(화면 문구는 "모의 PG" 라고 약속하지만 그
> 스위치가 없다 — `TASK-FAN-FE-015`). **팬 게시물 작성 화면은 아예 없다**(API 는 되고
> 시드가 그 경로로 글을 하나 넣어 둔다 — `TASK-FAN-FE-016`).~~ **(2026-08-16 고침 —
> 둘 다 눌린다)**
>
> 🔴 **이 문단은 열흘 동안 §6 과 정면으로 모순이었다.** §6 의 두 ✅ 행(`TASK-FAN-FE-015` ·
> `TASK-FAN-FE-016`)은 **2026-08-06 에 이미 "고침" 으로 바뀌어 있었다** — 고쳐지지 않은
> 것은 코드도 대장(§6)도 아니고 **여기, 사람이 실제로 읽는 절**이었다. 위험한 쪽이 하필
> 안 고쳐진 쪽이다: §3 은 데모를 **안내하며 읽는** 절이고 §6 은 **나중에 찾아보는** 절이라,
> 안내자는 모순을 만나지 못한 채 "안 된다" 만 듣고 그 화면을 건너뛴다. ⇒ **한 사실이 두
> 절에 있으면 한쪽만 고치는 것이 기본값이다.** 다음에 이 두 항목의 상태가 또 바뀌면
> **§6 의 짝 행도 함께** 고쳐라(그 반대도 마찬가지다).
>
> - **멤버십**: `자동 갱신 등록` 을 누르면 **성공**하고 버튼이 `자동 갱신 등록됨` +
>   `해지` 로 바뀐다. "결제 모듈이 설정되지 않았습니다" 는 **한 번도 뜨지 않는다.**
>   없다던 스위치는 `DEMO_PAYMENT_MOCK` 이고, 빌드타임에 인라인되는 `NEXT_PUBLIC_*` 이
>   아니라 **런타임 라우트**(`/api/payment-config`)가 읽는다 — 그래서 한 번 구운
>   이미지가 프로파일에 따라 다르게 답할 수 있다. 🔵 **판정 범위**: 실제로 누른 것은
>   **자동 갱신 등록(빌링키 경로)** 이다. 체크아웃 경로는 아래 이유로 일부러 누르지
>   않았고, 그럼에도 이 행이 성립하는 근거는 **두 경로가 같은 런타임 스위치를 읽는다**는
>   것이다.
> - **팬 게시물 작성**: 화면이 **있다** — `/compose` 다. 헤더의 `글쓰기` 가 그 진입점이고
>   제목·본문 작성 폼이 렌더된다(진입점은 로그인 상태에서만 노출된다 — 익명 방문자에게
>   제출할 수 없는 폼을 권하지 않기 위해서다).
>
> 🔴 **그래도 `프리미엄으로 업그레이드` 는 누르지 마라 — 이유가 바뀌었다.** 예전에는
> *막혀서 못 눌렀고*, 지금은 **눌리기 때문에** 위험하다. 데모 계정이 `PREMIUM` 이 되는
> 순간 세 가시성이 전부 열려 위 표의 잠금 행(`PREMIUM` 글은 제목·본문이 빈다)이 **거짓이
> 된다** — 이 표면이 보여주려는 것이 바로 그 게이팅이다. 되돌리려면 멤버십을 해지하고
> 다시 시드해야 하고 이력에 잡음이 남는다.
...
| 🟡 **스토어가 «40시간 전의 헬스 판정» 을 서빙한다 — 데모를 켠 뒤에도 「데모 서버가 꺼져 있어」 배너가 남는다** | **고쳤고 라이브로 확인했다** (`TASK-MONO-654`, 2026-09-11). 🟢 배포 직후 `Age: 0`(직전 사본은 40시간짜리였다)이고, **데모가 `stopped` 인데도** 프리렌더 HTML 에 배너 문구가 **0건**이다 — 판정이 더는 거기서 내려지지 않는다. 탐침 라우트는 익명 **200** · `no-store` · `X-Vercel-Cache: MISS`. 기전: 배너 판정이 `app/(store)/layout.tsx` 의 **서버 컴포넌트**에 있어 프리렌더된 정적 사본에 **구워졌다**. 🔴🔴 그리고 그 사본은 **재생성되지 않는다** — 이틀에 걸친 표본 5개가 전부 사본 생성 시각 `2026-09-09T21:59Z` 로 수렴했고 `Age` 가 벽시계와 함께 단조증가하며 `X-Vercel-Cache: STALE` 이 안 풀린다 ⇒ **40시간 동안 재생성 0회**(`revalidate = 60` 인데도). 🔵 그래서 「트래픽이 없어서」 갈래는 **반증됐다**(요청은 계속 있었다) — 캐시 수명을 줄이는 처방은 이 상태를 **안 고친다**. 🔴 **모집단이 두 번 틀렸었다**: 선언 grep 은 3, 살아 있는 URL 셋만 재고 적은 「런타임 1」도 틀렸다 — 빌드 산출물의 프리렌더 라우트는 **19개**이고 그 레이아웃이 감싸는 **15개**에 판정이 구워진다(옛 코드로 재현한 bite 실측). 고침: 판정을 **방문 시점의 클라이언트 탐침**으로 옮기고(`/api/demo/backend-state`, `force-dynamic` + `no-store`), `scripts/check-prerendered-demo-verdict.sh` 가 **빌드 산출물의 프리렌더 목록**을 모집단으로 그것을 지킨다. 🔵 캐시는 여전히 STALE 이어도 **상관없다** — 판정을 캐시 밖으로 뺐기 때문이다. 🔴 부수 피해 — 그 배너가 커밋된 썸네일 `store-1-home.jpg` 에 박혀 있어 론처 캐러셀이 상시 광고한다(`3690792eb`); 재촬영해야 한다 🔵 **행 상태가 🟡 인 이유**: 결함은 고쳐졌고 라이브로 확인됐지만 티켓이 아직 `in-progress/` 다. 닫을 때 이 행을 ✅ + 취소선으로 바꿔라(§ 6 규칙 (b)). | `TASK-MONO-654` · `TASK-MONO-648`(썸네일 재촬영) |
...
| 🔵 **WMS 도메인 데이터 시드 있음** — 입고 흐름 한 벌 + 출고 주문 1건 | `seed-wms.sh` 가 ASN → 검수 → 적치 → **재고 반영**(`available_qty=95`, `system:putaway-consumer`) → 출고 주문 → **피킹 확정 → 포장 → 봉인 → 출하(`SHIPPED`)** 까지 **실제 API 로** 심는다(출하 구간은 `TASK-BE-586` 이 추가). 2회 실행 수렴 확인(2회차 = 생성 0 · 기존 2 · 실패 0). 🔴 **출하까지 가는 것은 빈 볼륨에서만 참이다**(2026-08-15 실측 정정): `PickingRequest` 는 **예약 회신 이벤트를 소비할 때만** 생기므로(`ADR-MONO-066` B), 그 코드가 있기 **전에** 예약이 끝난 주문은 영원히 `PICKING` 에 멈춘다 — 시드는 멱등이라 새 예약이 일어나지 않고, 계약 §2.1(수동 재진입)은 `BE-586` R2-b 가 삭제했으므로 **되살릴 API 도 없다.** 같은 DB 에서 그 뒤에 만들어진 주문 둘은 `SHIPPED` 까지 갔다 ⇒ **코드가 아니라 데이터 이력의 문제.** 복구는 볼륨 초기화뿐이고 시드가 그 명령을 찍는다(`TASK-MONO-535`) | `TASK-MONO-510` · `TASK-MONO-535` |
...
| 🔵 **Finance 도메인 데이터 시드 있음** — 계좌 2건(ACTIVE/FULL) **+ 입금 2 · 이체 1 · 원장 전기** | `seed-finance.sh` 가 계좌 개설 2 + KYC 승급 2 + **입금 2 + 이체 1** 을 **전부 실제 API 로** 심는다(직접-DB **0건**). 🔵 원장은 **일부러 손으로 넣지 않는다** — 이체가 낳은 전기라서 `/ledger` 가 *투영이 동작한다* 는 증거가 된다(손으로 넣으면 *시드가 넣었다* 는 증거밖에 안 된다). 🔴 **판정은 응답 코드가 아니라 잔액 재조회**다: 고정 `Idempotency-Key` 라 2회차는 서버가 저장된 2xx 를 재생하므로 200 으로는 *"이번 실행이 돈을 넣었나"* 를 알 수 없다. 실측(2026-08-16) 1회차 `ledger 400000 → 900000` · 2회차 `재생, 이중 입금 없음`(잔액 불변) | `TASK-MONO-510` · `TASK-MONO-537` |
| ✅ ~~Finance 시드가 **입금 경로가 없다고 말한다**~~ (2026-08-16 고침) | 시드가 매 실행 *"입금 API 가 존재하지 않는다 ⇒ `/ledger` 는 이 스택에서 채울 수 없다"* 를 인쇄했는데, 그 경로는 **9일 전에 열렸다**(`TASK-FIN-BE-068`, 위 ✅ 행). 🔴 피해는 빈 화면이 아니라 **읽는 사람을 없는 갭으로 보내는 것**이었다 — `TASK-MONO-535` 가 wms 에서 고친 것과 같은 결함(*경고가 정상 코드를 감사하게 만든다*)의 두 번째 사본. 왜 9일이나 남았나: 그 티켓의 시드 AC 가 분류기 차단으로 **패치 인계**된 뒤 티켓이 `done` 으로 닫혔고, **이어받는 티켓이 어느 큐에도 없었다**(의무가 티켓과 함께 사라졌다) | `TASK-MONO-537` |
| ✅ ~~Finance 시드가 **콜드 스택에서 6/6 실패**한다~~ (2026-08-16 고침) | `demo-up.sh finance` 직후 자동 시드가 계좌 2 · KYC 2 · 이체 · 시산표를 **전부 HTTP 500** 으로 실패했다. 원인은 준비성 술어 — `wait_http` 는 **엣지**만 재고 401/403 을 "살아 있다" 로 세므로 백엔드에 대해 아무것도 증명하지 않는다. 형제 시드(scm 3 · erp 3 · wms)는 전부 백엔드별 대기를 갖는데 **finance 만 0개**였다. 🔴 `account-service` 에는 id 없이 2xx 를 내는 GET 이 없어 그대로는 형제 방식을 못 쓴다 ⇒ 술어를 *"백엔드가 **답했는가**"* 로 잡았다: 없는 id 의 **404 는 도달의 증거**, 게이트웨이의 **5xx 는 미도달의 증거** | `TASK-MONO-537` |
...
| 🔵 창고 스택 **재시작 루프**(`Up 3초` 반복) — 기동 전에 막힌다 | **트리거는 `.env` 부재**다(파일은 gitignore 되어 추적되지 않으므로 **새 클론·새 worktree 엔 없다**). compose 가 `${MASTER_DB_PASSWORD:-master}` 류 폴백으로 **조용히 성공**해 그 값이 볼륨에 각인되고(실측: `.env` 유무로 wms 비밀번호 **9/9 가 다르게 해소**되는데 `docker compose config` 는 **양쪽 다 rc=0·무경고**), 나중에 `.env` 를 만들면 앱만 의도한 값을 쓰다 `FATAL: password authentication failed` 로 죽는다. 🔴 DB init 은 **빈 데이터 디렉터리에서만** 돌므로 재기동으로 절대 안 고쳐지고, 증상이 **앱 크래시 루프**라 원인을 앱에서 찾게 된다. 🔵 `TASK-MONO-548` 이 `infra/demo/check-env-preflight.sh` 게이트를 넣었다 — `demo-up.sh` 가 **무엇을 띄우기 전에** 멈춘다(traefik 조차 안 뜬다). 위험 프로젝트는 실측상 **wms(7개 DB 롤) 와 ecommerce(MinIO) 둘뿐**이고, 나머지는 폴백이 곧 의도값이라 통과한다(대조군). 비파괴 복구(데이터 보존): `docker exec <pg 컨테이너> psql -U postgres -c "ALTER ROLE <롤> WITH PASSWORD '<.env 의 값>'"` | `TASK-MONO-548` |
| 🔵 콘솔 **ERP 6화면 중 5개가 찬다** | `/erp` · `/erp/guide` · `/erp/masters`(5탭, **신선 볼륨** 원소 수 3·3·3·4·3 = 직급·부서·코스트센터·사원·거래처, 실측 2026-08-16) · `/erp/approval` 목록(3) · `/erp/delegation`(1) · **`/erp/orgview`(4)**. 앞 셋은 프로듀서 DB 직독이고, `/erp/orgview` 는 `TASK-ERP-BE-042` 로 프로젝션이 살아난 뒤 **0 → 4** 가 됐다 | — |
...
| 🔵 위 잔여분에 대해 **매 실행 세는 것이 생겼다** (2026-08-16) | 위 행의 🔵 라이더가 예고한 그 상태를 시드가 이제 **직접 잰다** — `approval/delegations` 와 `read-model/delegations` 의 원소 수를 대조하고 어긋나면 마지막 요약 줄에 `⚠ 경고 N` 으로 싣는다(원인·불가역성·복구 명령 포함). 🔴 **그 전까지 아무도 세지 않았다**: 시드는 사원 프로젝션만 `4/4` 로 확인했고 위임 축은 비어 있었다. 그리고 살아남은 투영 1건이 하필 **`TASK-ERP-BE-043` 검증 세션이 자기 확인용으로 만든 프로브 행**(`reason="BE-043 라이브 프로브"`)이라, 화면은 차 있는데 **데모 시드가 만든 위임은 안 보이는** 모양이었다. 🔴 부수 수정: `seed-erp.sh` 는 6개 시드 중 **유일하게** 요약 줄을 손으로 찍고 있어서 `TASK-MONO-535` 의 경고 카운터가 erp 에는 도달하지 않았다 — `seed_summary` 로 통일했다 | `TASK-MONO-536` |
...
| 🔵 `/wms/operations` 의 **투영 지연 리포트는 운영자에게 닫혀 있다 — 설계다** | **이전 서술은 틀렸다**: *"운영자 토큰에 `projection-status` 권한이 없다"* 고 적혀 있었으나 **그런 권한은 코드에 없다**. 실제 게이트는 `OperationsController` 의 **클래스 레벨** `@PreAuthorize("hasRole('WMS_ADMIN')")` 이고, 이는 계약 (`admin-service-api.md § 6.2` — *Auth: `WMS_ADMIN` or higher*)과 **일치**한다. 운영자 티어가 ADMIN 을 안 받는 것도 결정이다(`TASK-BE-433`, `TASK-MONO-514` 재확인). 역할계층 `SUPERADMIN > ADMIN > OPERATOR > VIEWER` 때문에 운영자는 VIEWER 를 상속해 **나머지 wms 대시보드는 전부 열리고** 이 화면만 닫힌다. 🔵 **화면은 깨지지 않는다** — 콘솔이 403 을 `forbidden` 으로 매핑해 「**권한 없음**」(중립 회색)으로 렌더한다(빨간 「점검 필요」가 아니다, `TASK-PC-FE-224`). 🔵 `TASK-MONO-547` 이 게이트를 **유지**하기로 결정하고, 운영자 티어 8역할이 403 을 받는다는 것을 슬라이스 테스트로 고정했다 — 계층이나 파생표를 건드리면 이제 테스트가 먼저 빨개진다 | `TASK-MONO-547` |
...
| **`/erp/delegation` 에 시드가 만든 위임이 안 보인다** | 🔵 **시드 로그의 마지막 줄을 보라 — `⚠ 경고 N` 이 붙어 있으면 그것이 답이다.** 봉투 수정(`TASK-ERP-BE-043`) **이전에** 발행된 위임 이벤트는 버려졌고, 위임 생성은 멱등이라 **재실행으로는 낫지 않는다.** 🔴 코드 결함이 아니다 — 같은 볼륨의 나중 위임은 정상 투영된다(실측: 원본 2 ↔ 투영 1, 남은 1건은 그 수정을 검증하던 세션의 프로브 행이었다). 복구(⚠ erp 데이터 전부 삭제, 시드가 다시 심는다): `bash infra/demo/demo-down.sh erp && docker volume rm erp_mysql-data && bash infra/demo/demo-up.sh erp` — 실측(2026-08-16) 그 뒤 `위임 프로젝션 1/1 반영 확인` 으로 경고가 사라진다 |
| **한 도메인의 시드가 전부 HTTP 500 이다** (기동 직후) | 🔵 **너무 일찍 물은 것이다.** `demo-up.sh` 는 `up -d` 로 끝나므로 게이트웨이가 200 을 내는 동안에도 뒤의 서비스는 아직 뜨는 중일 수 있다. 시드 로그 **마지막 줄의 `실패 N`** 을 보고, 컨테이너가 `healthy` 가 된 뒤 그 도메인만 다시 돌려라: `bash infra/demo/seed/seed.sh <도메인>`. 🔴 재실행으로 낫는다면 그것은 **코드 결함이 아니다** — 낫지 않으면 그때 의심하라 |
