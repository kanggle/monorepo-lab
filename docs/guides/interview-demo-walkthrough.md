...
| 🔴🔴 콘솔 **`/wms/inbound` · `/wms/outbound` 목록이 빈다** — 시드가 무엇을 넣든 | **라이브 실측(7화면 스윕)**: 두 화면은 `admin-service` 프로젝션(`/dashboard/asns`, `/dashboard/orders`)을 읽는데, admin 이 **아무도 발행하지 않는 토픽**을 구독한다 — 오프셋 `wms.inbound.asn.received.v1`=**2** vs 구독 중인 `wms.inbound.asn.v1`=**0**(출고도 동일). 발행자는 토픽을 `wms.<eventType>.v1` 로 **기계 생성**하는데 admin 은 손으로 적었고, 이름이 우연히 일치하는 것들만 동작한다 | `TASK-BE-582` |
| 🔵 원시 `GET /api/v1/outbound/orders` 도 **만든 주체가 못 본다** (별개 결함) | 데모 운영자(`tenant_id=demo-corp`)는 **restricted** 로 판정돼 `tenant_id` **AND `source=FULFILLMENT_ECOMMERCE`** 인 주문만 본다. 시드가 만든 주문은 `tenant_id=NULL · source=MANUAL` 이라 조건에 안 걸린다(생성 201, 조회 `200 {"content":[]}` 실측). **콘솔 화면과는 다른 경로다** — 둘 다 고쳐야 화면이 찬다 | `TASK-BE-581` |
| 🔵 콘솔 **`/wms/inventory` 는 실제로 찬다** | 시드의 적치 확정 → Kafka → `admin_inventory_snapshot` 투영까지 도달한다(라이브 확인). WMS 7화면 중 데이터가 확인된 유일한 목록 화면 | — |
