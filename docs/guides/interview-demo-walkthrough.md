...
| 🔵 콘솔 **`/wms/outbound`** — 제품은 고쳐졌으나 **기존 볼륨에서는 재시드 전까지 빈다** | 원인이었던 것: 상류는 admin 프로젝션이 아니라 outbound-service 원시 API 이고(런타임 로그 확인), 데모 운영자(`tenant_id=demo-corp`)는 **restricted** 인데 생성 경로가 `tenant_id` 를 안 박아 자기 주문이 `NULL · MANUAL` 로 남았다 ⇒ 만든 주체가 만든 것을 못 봤다. **`ADR-MONO-064` (B) 가 닫았다** — 생성이 호출자의 서명된 테넌트를 stamp 하고 조회의 `source` pin 을 걷는다. 라이브 실측(2026-08-13): 같은 토큰으로 새 주문 생성 → `totalElements` **0 → 1**, 단건 조회가 자기 주문 **200** / B2B **403**(격리 유지). 🔴 **다만 소급 stamp 는 금지**라 기존 `SO-DEMO-0001`(`tenant_id=NULL`)은 영원히 안 보인다 ⇒ **이 화면을 채우려면 볼륨 초기화 + 재시드가 필요**하다 | `TASK-BE-581` |
