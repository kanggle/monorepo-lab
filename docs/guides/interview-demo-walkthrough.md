...
| 🔴 WMS **출고는 주문까지**만 심는다 | 예약은 `INVENTORY_RESERVE` 를 요구하는데 운영자에게 파생되지 않고, 배송은 도달 불가 TMS 스텁에 의존한다. 🔵 `TASK-MONO-514` 는 이것을 **의도적으로 묶지 않고** 닫았다 — 계약이 *"machine-to-machine … Human users do not hold it"* 라 답은 운영자 확대가 아니라 워크로드 자격증명이고, 그것은 별도 결정이다. `ADR-MONO-061`(C)이 이제 **부여 가능**하게만 만들어 뒀다(기본값은 여전히 없음) ⇒ `TASK-MONO-528`. 🔴 **예약이 풀려도 이 행은 절반만 풀린다** — 배송 쪽 TMS 스텁이 남는다 | `TASK-MONO-528` |
