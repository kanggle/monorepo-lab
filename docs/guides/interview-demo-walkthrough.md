...
| 🔴 `/wms/operations` 가 **403** | 운영자 토큰에 `projection-status` 권한이 없다. 🔵 **이 행의 티켓 인용은 처음부터 잘못돼 있었다** — `TASK-MONO-514` 는 `MASTER_WRITE` 티켓이고 `projection-status` 를 한 번도 다루지 않았다(그 티켓 본문에 그 단어가 없다). 514 가 닫히면서 드러났을 뿐이고, 이 행이 서술하는 사실은 **그것과 무관하게 여전히 참**이며 지금 **소유 티켓이 없다**. 닫힌 티켓을 가리키는 편이 소유자가 없다는 것보다 나빠 보이므로 인용을 지우고 그 사실을 적는다 | `TASK-MONO-547` |
