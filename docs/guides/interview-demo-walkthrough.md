...
| 🟡 콘솔 **ERP 결재함이 비어 있다** — 사유가 **셋에서 하나로** 줄었다 | 결재함은 "호출자가 현재 단계 승인자인 건" 만 준다. ~~② assume 토큰의 `sub` 가 `platform-console-web`~~ · ~~③ token-exchange 클라이언트가 하나뿐~~ 은 **해소**됐다(`ADR-MONO-060` A / `TASK-MONO-515`, 2026-08-07): assume 토큰의 `sub` 가 **계정 UUID** 라 actorId 가 콘솔 계정마다 하나다. 🔴 그런데 **데모 운영자 계정이 1개뿐**(`R__seed_demo_operator.sql` INSERT 1건, 실측)이라 ①(자기결재 금지)을 만족시킬 두 번째 신원이 여전히 없다 ⇒ 남은 것은 **토큰 결함이 아니라 데모 데이터 공백**이고, 운영자 계정이 둘이 되면 루프가 닫힌다. 🔵 **눈에 안 보이던 절반은 이미 고쳐졌다** — 게이트웨이 6/6 이 `X-User-Id ← sub` 이므로 필터가 0건인 도메인(finance·scm·wms)의 **감사 기록**도 `platform-console-web` 이었고, 이제 전 도메인이 실제 운영자를 적는다 | `TASK-MONO-515` |
