...
| 🟡 콘솔 `/ledger` **4피드 중 1개만 찬다** | 실측(2026-08-07, **BFF 원소 수**로 판정): 시산표 **3원소** ✅ · 기간 **빈 배열** · 불일치 **빈 배열** · 환율 **503 TIMEOUT**. 🔴 앞의 둘("물어봤고 없더라")과 마지막("물어보지도 못했다")을 **같은 0 으로 세면 안 된다** — 그래서 환율만 따로 티켓을 냈다. 🔴 **선행 함정**: 갓 로그인한 세션은 활성 테넌트가 `iam` 이라 **4피드 전부 403** 이고, 그건 "빈 화면" 과 글자만 비슷하고 결론이 정반대다. 테넌트를 `demo-corp` 로 전환한 뒤라야 위 숫자가 나온다 | `TASK-PC-FE-273` |
