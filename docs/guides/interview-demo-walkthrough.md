...
| ✅ ~~팬 **아티스트 글은 어떤 실제 호출자도 쓸 수 없다**~~ (2026-08-11 고침) | 원래 서술: 피드는 `posts.author_account_id ⋈ follows.artist_account_id`(= 아티스트 엔티티 id)로 잇는데 `PublishPostUseCase` 는 저자를 호출자 sub 으로 고정한다. 시드가 직접-DB 를 쓰는 이유. 🟢 **결정 완료 (2026-08-07)**: `ADR-MONO-059` **ACCEPTED — A**(아티스트에게 실제 계정 `artists.account_id`) ⇒ 착수했고 **닫혔다**: `artists.account_id` 신설 + 팔로우 대상 검증이 **fail-closed** 로 바뀌었고, 시드는 세 아티스트의 실제 로그인(auth `V9002` / account `V9006`)으로 글을 **API 로** 작성한다. 🔵 그 검증이 켜지자 `TASK-FAN-INT-005` 의 e2e 가 **합성 UUID 를 팔로우하던 낡은 전제**를 드러냈다 — 탈출구가 그 스테일 주석을 가리고 있었다 | `TASK-FAN-BE-045` |
| 🔵 팬 도메인에 **발급 가능한 운영자 역할이 없다 — 그리고 그것은 결정이다** (2026-08-11 절반 해소) | `FAN_OPERATOR` 를 받는 코드는 iam·artist·community 전부에 있는데, `fan` 도메인을 구독한 테넌트가 없어 그 역할이 발급될 수 없다. 🟢 **결정 완료 (2026-08-07)**: `ADR-MONO-059` A 가 **B(운영자 대리 저작)를 배제**했으므로 *"B2C 테넌트를 운영자가 assume 하는가"* 는 **열지 않는다**로 답이 났다 ⇒ 남은 범위는 `ARTIST` **역할 발급 하나**였고, `TASK-MONO-512` 가 그것을 닫았다(account `V9006` 이 역할을, auth `V9002` 가 자격증명을 심는다). 🔴 **`FAN_OPERATOR` 는 여전히 발급 불가이고, 이제 그것은 공백이 아니라 `ADR-MONO-059` A 의 결과다** — 열려면 새 ADR 이 필요하고, 디렉터리 쓰기 라우트 9개를 어떻게 할지는 `TASK-MONO-522` 가 판정한다 | `TASK-MONO-522` |
