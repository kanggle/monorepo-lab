...
- **ecommerce 주문-이행 통합의 이행자 (2026-06-08, [ADR-MONO-022](adr/ADR-MONO-022-ecommerce-wms-fulfillment-integration.md))**: ecommerce 가 **두 번째 외부 주문 소스**(ERP webhook 과 동격) — `outbound-service` 가 `ecommerce.fulfillment.requested.v1` 를 소비해 기존 `ReceiveOrderUseCase` 로 출고주문 생성(상거래 로직 0, 코드/SKU→uuid 해소만), ship/backorder 시 `wms.outbound.{shipping.confirmed,order.cancelled}.v1`(+orderNo) 으로 ecommerce 에 회신. wms 도메인은 **순수 유지**(ecommerce ACL 이 vocab 번역; §D6) — `inventory-service`/`master-service` 이벤트는 무변경, ecommerce 측 reconciliation 이 구독만. 상세=§2.3 + ADR-022.
...
- **결재 워크플로 도메인 라이브 — ADR-016 §D3 approval forward-declaration 3-증분 완성 (2026-06-05)**: **approval-service** = erp 첫 실 도메인-로직(상태기계) 서비스 — **ERP-BE-009** 단일단계 first increment(`DRAFT→SUBMITTED→APPROVED|REJECTED|WITHDRAWN`, 권한 결재자·no-self-approval·멱등 전이·불변 audit·masterdata 참조무결성·outbox `erp.approval.*.v1`) → **ERP-BE-012** 다단계 결재선(1~N) + `IN_REVIEW`(approve from SUBMITTED|IN_REVIEW → last?APPROVED:IN_REVIEW; per-stage authz; Flyway 1-stage backfill; approved=최종단계-only로 terminal-once 보존, payload stage 필드 가산) → **ERP-BE-013** 대결/위임(standing windowed `DelegationGrant` A→D, 전이시점 대결자 resolution+`onBehalfOf` 감사 L131, 신규 producer-only 토픽 `erp.approval.delegated.v1`, 위임-통한-자기결재 차단 SoD). + **read-model approval-fact 투영** (ERP-BE-010 — read-model 이 `erp.approval.*` 구독→`approval_fact_proj` latest-fact, org_scope subtree; approval→read-model 루프 완결) + **notification-service 첫 증분** (ERP-BE-011 — dual-type event-consumer+rest-api: `erp.approval.*` 4 구독→recipient 해소→in-app `Notification` 영속+recipient-scoped inbox; terminal consumer no-outbox; ADR-005 Category C; approval→notification 루프 완결). 콘솔 결재 surface = PC-FE-051(결재함)/052(notification bell)/053(다단계 UI)/054(위임 grant 관리). v2.2 deferred = approval per-request/자동부재/transitive 위임 · notification 외부채널(Slack/SMTP)+Category C retry scheduler · masterdata/permission/delegation 통지.
- **service map (v1.x)**:
...
| [ADR-MONO-016](adr/ADR-MONO-016-erp-platform-bootstrap.md) | ACCEPTED (2026-05-19, §D3 amendments 2026-06-04~05) | erp-platform 부트스트랩 — D1 Option C. domain=`erp`, traits=[internal-system/transactional/audit-heavy]. 첫 `internal-system`-primary. **§D3 forward-declaration 집행 (first-increment amendment 패턴)**: read-model-service (ERP-BE-007/010) + **approval-service 3-증분 완성** (ERP-BE-009 단일단계 → BE-012 다단계+IN_REVIEW → BE-013 대결/위임) + notification-service (ERP-BE-011). 잔여 v2 = permission-service / admin-service. |
