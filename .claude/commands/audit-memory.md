...
**① 1차 지표는 총 문자 수다. 바이트가 아니다.**

훅이 보고하는 값도, 한도 **24,576** 도 **문자**다. 이 파일들은 한글이 많아 UTF-8 로는
바이트 ≈ 1.35 × 문자이므로, **`wc -c` 로 재면 한도 아래인 파일이 초과처럼 보인다.**
...
for (const f of ['MEMORY.md','PLAYBOOKS.md','ARCHIVE.md']) {
  ...
  const path = m.reduce((a, b) => a + b.length, 0);
  console.log(f, s.length + '자', m.length + 'ptr(고유' + new Set(m).size + ')',
              '경로' + path + '자(' + Math.round(path / s.length * 100) + '%)');
}"
```

🔴 **정규식은 `[a-zA-Z0-9_.-]` 여야 한다.** 옛 `[a-z0-9_]+` 는 하이픈·마침표·대문자를 못
받아 `](ARCHIVE.md)` · `](PLAYBOOKS.md)` · 하이픈 든 `*_detail_*` 을 놓친다 —
2026-08-23 실측 **184 vs 189**. 세는 값이 틀리면 훅이 왜 우는지 설명할 수 없다.
...
**② 두 축을 섞지 마라 — 토큰 비용은 *문자*, 훅 판정은 *포인터 개수*다.**

| 축 | 재는 것 | 누가 본다 |
|---|---|---|
| **문자 수** | 매 세션 컨텍스트 토큰 비용 | 이 단계(§2-6)의 WITHIN/OVER 판정 |
| **포인터 개수** | 항목 수 | `.claude/hooks/` 의 발화 임계 |

⇒ **한쪽이 WITHIN 인데 다른 쪽이 발화하는 상태는 정상이다.** 2026-08-23 실측:
`MEMORY.md` 21,137자(여유 14.0% = WITHIN)인데 포인터 189 로 훅은 운다.

🔴 **그러니 훅이 울 때 압축부터 하지 마라 — 산문을 아무리 깎아도 훅은 안 꺼진다.**
훅을 끄는 유일한 방법은 포인터를 줄이는 것(항목 통합·아카이브)이고, 그건 아래 ③ 3번과
같은 **소유자 결정**이다.
...
### 6. 인덱스 예산
- MEMORY.md: N자 / P 포인터 (고유 U) — 한도 24,576자, 여유 R자 (X%)
- 포인터 경로 C자 (Y%) — 압축 불가 / 산문 잔량 (N−C)자
- PLAYBOOKS.md: N자 / P 포인터   ·   ARCHIVE.md: N자 / P 포인터
- 판정: WITHIN / OVER  ← **문자 축**
- 훅(포인터 개수) 발화 여부를 따로 적는다 — 두 축은 다르고, 어긋나는 것이 정상이다
...
- CLAUDE.md 중복: N
- 공통 규칙 후보: N
- 인덱스 예산: N자 / P 포인터 (한도 24,576자) — WITHIN / OVER
- 상태: PASS / NEEDS_FIX
```
