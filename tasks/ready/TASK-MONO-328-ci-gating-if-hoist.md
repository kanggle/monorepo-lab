...
- [x] **AC-0 (verify-then-act, 필수 선행)**: § DEFERRAL GUARD의 착수 트리거 2개 중 최소 1개가 실제 관측됨을 확인. **관측 없으면 STOP — 구현하지 않고 task를 그대로 둔다.** (관측 근거를 이 task 또는 PR에 기록.)
      ...
      → **2026-09-11 3차 재수행: 트리거 2개 모두 여전히 미관측 ⇒ STOP.** 근거는 § AC-0 실측 #3.
        🔴 이번 창은 `main` 이 **4번 빨갰지만** 넷 다 트리거가 아니다 — 셋은 **PR 에 안 붙는**
        잡(설계), 둘은 **skip 이 옳았다**(그 PR 이 해당 영역을 0건 건드렸다), 하나는
        **PR 에서도 빨간 채 머지**(required 가 아니라서 — `TASK-MONO-598` 의 축).
...
# 🔍 AC-0 실측 #3 (2026-09-11) — **트리거 여전히 미관측 → STOP. 보류 유지.**

창 = `2026-08-29 → 2026-09-11`(실측 #2 이후). 커밋 **238건**.

## 트리거① — 입력 자체가 0 이다

| 술어 | 결과 |
|---|---|
| 신규 **프로젝트** (`projects/*/PROJECT.md` 추가) | **0건** |
| 신규 **서비스** (`projects/*/specs/services/*/architecture.md` 추가) | **0건** |

🔴 **«없는 사건의 결과를 0 으로 적는 것» 과 «사건은 있었는데 누락이 0» 은 다르다**(실측 #2 가
이미 이 구분을 적었다). 이 창은 **전자**다 — 잡 `if:` 를 손으로 갱신할 일 자체가 없었으므로
「누락이 발생했는가」는 **물을 수 없는 질문**이다.

## 트리거② — 🔴 `main` 이 **4번 빨갰다.** 그런데 넷 다 트리거가 아니다

먼저 **사건은 있었다**. 이 창의 `main` CI 실패:

```
2026-09-09T17:49Z  998bf06bc   2026-09-09T14:30Z  5a5046578
2026-09-08T15:58Z  34e6a0ccb   2026-09-03T05:34Z  adcf4c22c
```

🔵 **그러나 트리거②의 술어는 «main 이 빨갰다» 가 아니다** — *"path-filter/`if:` **오류**로
**돌아야 할** e2e 가 **skip** 되어 회귀가 main 에 샌 사건"* 이다. ⇒ 네 건의 실패 잡이 **그 PR
에서 어떤 상태였는지** 를 하나씩 갈랐다:

| main 커밋 | 실패 잡 | 그 PR 에서 |
|---|---|---|
| `998bf06bc` (#3725) | Frontend/Console E2E full-stack · GAP logout e2e | **미부착**(PR 에 아예 안 붙는 잡) |
| `998bf06bc` | Frontend E2E smoke | **skipped** |
| `5a5046578` (#3718) | Frontend/Console E2E full-stack | **미부착** |
| `5a5046578` | Guard-count figure | 🔴 **PR 에서도 failure** — 빨간 채로 머지됐다 |
| `34e6a0ccb` (#3692) | Frontend/Console E2E full-stack | **미부착** |
| `34e6a0ccb` | Package boot jars (ecommerce) | **skipped** |
| `adcf4c22c` (#3603) | Integration (master/notification/outbound) | **미부착** |

### 🔵 «미부착» 은 오류가 아니라 **설계**다

`Frontend/Console E2E full-stack` 같은 잡은 **PR 에 안 붙고 push/nightly 에서만** 돈다.
그것은 게이팅 «오류» 가 아니라 **의도된 배치**이고, 이 저장소가 이미
*"nightly 전용 스펙이 초록으로 머지된 뒤 main 이 빨개진다"* 로 이름 붙인 **알려진 부류**다.
🔴 트리거②가 말하는 «오류» 가 아니다.

### 🔴 «skipped» 둘은 **진짜로 확인했다 — 그 skip 이 옳았다**

이 둘이 트리거②의 모양이므로 **PR 이 실제로 그 영역을 건드렸는지** 를 직접 셌다:

- **#3692 · `Package boot jars (ecommerce)`** — 바뀐 경로는 `infra/demo/**` 와 `tasks/**`
  뿐이다. 🔵 `grep ecommerce` 가 1건을 냈지만 그것은 **파일명**이었다
  (`infra/demo/aws/site/thumbnails/console-2-ecommerce.jpg`) — **ecommerce 코드 0건**.
  ⇒ skip 이 **옳다.**
- **#3725 · `Frontend E2E smoke`** — `apps/(web-store|console-web)` · `fan-platform-web`
  경로 **0건**(바뀐 것은 `docs/adr`·`infra/demo`·`tasks`). ⇒ skip 이 **옳다.**

⇒ 두 경우 다 **필터가 옳게 판정했고**, main 의 실패는 그 PR 이 흘려보낸 회귀가 아니라
**다른 사유**다. **트리거② 미관측.**

### 🔵 부수 관측 — 이 티켓의 축은 아니지만 적어 둔다

`5a5046578`(#3718)의 **`Guard-count figure` 는 PR 에서도 failure 였다** — 즉 **빨간 채로
머지됐다.** 🔵 그것이 가능한 이유는 그 잡이 **required 가 아니기** 때문이고, 그 축은
`TASK-MONO-598`(required 집합)이 이미 다룬다. 🔴 **게이팅 리팩토링과 무관**하므로 이
티켓의 트리거로 세지 않는다.

## ⇒ 판정: **STOP. 보류 유지.**

트리거 둘 다 미관측이므로 **구현하지 않는다**(no-op = 올바른 구현).
🔵 지금의 명시적 per-job `if:` 는 여전히 «동작하고 투명한» 상태다 — 이번 창에서
**`TASK-MONO-664` 가 그 `if:` 42개를 기계적으로 고쳤는데**, 그 작업이 오히려 지금 구조가
**프로그램으로 다룰 수 있다**는 것을 보였다(YAML 로 파싱해 잡 단위로 대조했고 예상 밖 0).

🔴 **다음 재측정 때 볼 것**: 이 창은 신규 프로젝트가 0 이라 트리거①을 **물을 수 없었다.**
프로젝트가 실제로 추가되는 창이 오면 그때가 첫 진짜 표본이다.
