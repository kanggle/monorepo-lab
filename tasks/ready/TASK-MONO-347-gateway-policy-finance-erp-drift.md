...
⏳ DEFERRED — 게이트웨이 정책 ↔ finance/erp 실제 구현 드리프트 해소: 정책 문서가 "모든 프로젝트에 게이트웨이"라고 선언하는데 두 도메인엔 모듈 자체가 없다
...
이 task 는 **의도적으로 보류된 백로그**다. root 리프사이클에 `backlog/` 폴더가 없어 `ready/` 에 두지만, **아래 AC-0 의 결정이 내려지기 전에는 착수하지 말 것.**
...
- [ ] **AC-0** (필수 선행) 방향 A / B 중 하나를 **사람이** 선택하고, 그 근거를 이 task 에 기록한다. 미결 시 STOP.
- [ ] **AC-1** 선택한 방향에 따라 `platform/api-gateway-policy.md` · `projects/{finance,erp}-platform/PROJECT.md` · 실제 코드 **3자가 일치**한다. 어느 문서도 존재하지 않는 게이트웨이를 있다고 말하지 않는다.
- [ ] **AC-2** 방향 B 인 경우, 예외를 **ADR 로 기록**한다(정책 완화는 task 가 아니라 아키텍처 결정). 재도입 조건(외부 노출 시점)을 명문화한다.
- [ ] **AC-3** 재발 방지 가드가 존재한다 — gateway 모듈 실재 ↔ 문서 선언 불일치 시 CI 가 RED. **음성 테스트 필수**(유령 gateway 선언을 주입 → 가드가 지목하는지 확인). `MONO-345` 의 함정 재확인: 필터를 `code-changed` 와 **AND 하지 말 것**(이 드리프트는 markdown-only 편집으로 도착한다).
- [ ] **AC-4** 문서에 "게이트웨이는 인증만, 인가는 서비스" 구분이 유지된다(현 정책 L86). 이 구분을 흐리면 도메인 롤 게이팅을 게이트웨이로 옮기려는 후속 오해가 생긴다.
