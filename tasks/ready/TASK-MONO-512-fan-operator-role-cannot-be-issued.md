...
⚠️ `FAN-BE-045` 는 현재 **AC-6 이 `ADR-004` 로 PAUSED** 다(조인 검증의 크로스서비스 기전 —
`follows` 와 `artists` 가 다른 DB 라는 사실이 `ADR-MONO-059` 에 반영돼 있지 않았다).
그 ADR 은 **이 티켓의 범위에는 영향이 없다**(역할 발급은 별개 축) — 다만 선행 티켓이
멈춰 있으므로 이 티켓도 대기한다.
