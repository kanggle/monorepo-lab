...
- [ ] **AC-1 — 빌드캐시가 나이로 잘린다 (양방향).** ① **양성**: 필터보다 **오래된** 캐시가 실제로 제거된다. ② **음성**: 필터보다 **최근** 캐시는 **남는다**(0B 축출). **두 방향을 다 보여야 한다** — 양성만 보면 "그냥 다 지우는 것" 과 구별되지 않고, 음성만 보면 "아무것도 안 하는 것" 과 구별되지 않는다. (2026-07-13 레퍼런스 실측: `until=1h`/6분된캐시 → **0B**, `until=1m`/같은레코드 → **502.6MB**.) **`--max-used-space` 로 구현하지 말 것 — 3회 반증됐다**(§ 고칠 방향).
- [ ] **AC-2 — 상한 없는 컨테이너를 이름으로 지목한다.** 스크립트 출력이 `LogConfig.Options` 에 `max-size` 가 없는 컨테이너를 **목록으로** 찍는다. **크기가 아니라 상한의 부재로 판정한다**(AC-4 가 이 차이를 강제).
- [ ] **AC-3 — 로그 총량을 보고한다.** `/var/lib/docker/containers` 총량이 출력에 나온다(`docker system df` 는 이 값을 0 으로 보고하므로, 이 줄이 **유일한 가시성**이다).
- [ ] **AC-4 — mutation: 감시가 실제로 문다.** 상한 없는 컨테이너를 하나 만들고(`docker run --rm -d --name probe alpine sleep 300` — LogConfig 상한 없음) 스크립트 실행 → **그 컨테이너가 목록에 뜬다**. 로그가 **작아도(≈0B) 떠야 한다** — 크기 기준이면 안 뜬다. ⇒ 이 mutation 이 AC-2 의 "부재로 판정" 을 증명한다. **적용 여부(컨테이너가 실제로 떠 있는지)를 먼저 출력하고 결과를 읽는다.**
- [ ] **AC-5 — 기본 동작은 비파괴.** `--logs` 없이 실행하면 **어떤 로그도 truncate 되지 않는다**(실행 전후 로그 크기 동일). 관측성은 명시 요청 시에만 포기한다.
- [ ] **AC-6 — 예약작업이 여전히 초록.** 수정된 스크립트를 `DockerWeeklyCleanup` 로 1회 수동 실행 → `LastTaskResult=0`. 경고 상황에서도 비-0 로 죽지 않는다(RED 예약작업은 무시된다).
- [ ] **AC-7 — dry-run 보존.** `--dry-run` 이 여전히 아무것도 안 지우고 회수 가능량만 보고한다(+ 새 로그 섹션 포함).
...
- [ ] AC-1~7
- [ ] `scripts/docker-cleanup.sh` 헤더에 두 함정(`RECLAIMABLE` 의 의미 · `LogConfig` 생성시점 고정) 명기
