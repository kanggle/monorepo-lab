...
ready — **⏸ DEFERRED (2026-09-24 UTC · 소유자 결정)**. 🔴 **착수하지 마라.** V0019 의 바이트는 고치지 않는다 — 정정은 `V0037` 헤더에 이미 있다. 아래 § 보류 결정. ‖ 직전: ready (2026-09-23 UTC — ⏳ SCHEDULED / 조건 게이트)
...
|---|---|---|---|
| 2026-09-24 UTC | 데모 14차 AMI 신선 볼륨(`i-008d1ac1ce23665ad`, 인스턴스 교체 직후) `auth_db` | **있음** — `0019` · checksum `1009669918` · installed_on `2026-09-24 05:45:27` | 🔴 **멈춤** — 파일 안 고침, `ready/` 유지 |

> 🔴🔴 **관측 (2026-09-24) — 이 술어는 떠 있는 스택에서는 참이 될 수 없다.** 신선 볼륨도
> auth-service 가 **부팅하면서** `V0019` 를 적용하므로, 조회할 수 있을 때쯤이면 행은 언제나 있다
> (위 행의 installed_on 은 부팅 05:4xZ 그 자체다). 즉 «행이 없다» 는 **쿼리할 DB 가 아직 없을 때**만
> 참이다. 🔵 실제로 필요한 조건은 «**옛 checksum 으로 V0019 를 적용한 볼륨이 고친 파일을 만나지
> 않는가**» — 고친 파일이 구워진 이미지의 신선 볼륨은 새 checksum 을 적용하므로 안전하다.
> ⇒ AC-0 의 게이트 문장을 그 조건으로 다시 쓸지는 **소유자 결정**이다(이 측정 기록은 게이트를
> 고치지 않는다 — 고치면 그것이 곧 «지금 해도 된다» 의 판정이 되기 때문이다).
...
# ⏸ 보류 결정 (2026-09-24 UTC · 소유자 «추천대로 진행»)

## 결정

**V0019 의 바이트를 고치지 않는다. 이 티켓은 DEFERRED 로 `ready/` 에 남긴다**(`TASK-MONO-328` 과 같은 모양 — 착수 금지 백로그).

## 왜 — 얻는 것과 잃는 것이 비대칭이다

| | |
|---|---|
| 얻는 것 | 적용된 마이그레이션의 **주석 한 문단**이 사실과 맞게 된다 |
| 이미 있는 것 | 🔵 그 문장의 반증은 `TASK-MONO-721` 이 **`V0037` 헤더에** 인용·반증해 두었다(ADR-MONO-076 D5-2). V0019 를 읽는 사람은 바로 다음 파일에서 정정을 만난다 |
| 잃는 것 | 🔴 Flyway 체크섬이 주석까지 덮으므로, **V0019 를 이미 적용한 모든 볼륨**(데모 · 각자의 로컬 `iam-mysql` · CI 캐시)이 `validateOnMigrate` 에서 기동 실패. 증상은 «auth-service 가 안 뜬다», 원인은 몇 주 전 주석 한 줄 |

## 왜 게이트를 고쳐 쓰지 않았나

2026-09-24 측정이 보인 대로 AC-0 의 술어(«`version='19'` 행이 없다»)는 **떠 있는 스택에서는 참이 될 수 없다** — 신선 볼륨도 부팅하면서 V0019 를 적용한다. 대안 술어 «옛 체크섬으로 V0019 를 적용한 볼륨이 고친 파일을 만나지 않는가» 는 옳지만 **잴 수 없다**: 각자의 로컬 볼륨은 저장소에서 보이지 않는다. ⇒ 영원히 열리지 않는 게이트를 새로 만드는 대신 **보류**를 명시한다.

## 🔴 다시 열 조건 (둘 다일 때만)

1. V0019 의 그 문장이 **V0037 헤더로도 반증되지 않는 새 오해**를 실제로 만든 기록이 생긴다(추측이 아니라 사건).
2. 그 시점에 «기존 볼륨을 전부 버린다» 가 **소유자 결정으로** 허락된다(예: 스키마 전면 재기준화).

그 전에는 **어떤 이유로도** 적용된 마이그레이션의 바이트를 고치지 않는다 — `flyway repair` · `validateOnMigrate: false` 우회도 § Edge Cases 가 이미 기각했다.
