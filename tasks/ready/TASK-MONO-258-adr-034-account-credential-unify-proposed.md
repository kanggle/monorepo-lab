...
- **AC-5** ADR-003a § 3 audit row #37 appended (append-only; rows #1-#36 byte-unchanged).
- **AC-6** Doc-only diff (no `apps/` code, no `platform/contracts/` change, no migrations).
- **AC-7** ADR-034 is **PROPOSED only** — NO self-ACCEPT, NO code (the registry/link/provisioning are post-ACCEPTED execution tasks per § 3.3). ACCEPTED is a separate user-explicit-intent task (sibling ADR-032/MONO-254, ADR-033/MONO-257).
...
- If ADR-034 is self-ACCEPTED in the same task → violates the staged-child pattern (ACCEPTED is a separate user-explicit-intent task). PROPOSED only.
