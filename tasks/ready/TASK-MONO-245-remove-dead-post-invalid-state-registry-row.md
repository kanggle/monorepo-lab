...
- [ ] **AC-1 (소비처 0 재확인)**: `grep -rn "POST_INVALID_STATE"` 가 제거 후 `error-handling.md` 0건, 잔여는 `tasks/done/TASK-MONO-244...`(이력) 뿐임을 확인.
- [ ] **AC-2**: `POST_INVALID_STATE` 레지스트리 행 삭제됨.
- [ ] **AC-3**: `POST_STATUS_TRANSITION_INVALID` 행이 dangling alias 참조 없이 canonical 단독 설명으로 정리됨.
- [ ] **AC-4 (scope-lock)**: `git diff origin/main` = `platform/error-handling.md`(-1 행 + 1 행 설명 정리) + task lifecycle 만.
- [ ] **AC-5**: `validate-rules` 재-scan 시 신규 broken-ref / dangling alias 0.
...
- 미수행(ready). 구현 시: grep 0 확인 + `git diff origin/main --stat`(error-handling.md + task lifecycle 한정) 첨부.
