...
After the fixes + quarantines, the lane gates the genuinely-passing order + payment ITs (incl. the BE-438 reconciliation IT, multi-tenant isolation, optimistic-locking, event-publish, etc.). The singleton-container refactor was **not** needed for Phase 1 (tests run single-fork/sequential; the connection-refused signal was the Docker-env override, not simultaneous-container exhaustion) — left as an optional future optimisation. AC-4 quarantines all reference their fix task; AC-2 the lane is a genuine gate (no `continue-on-error`).
