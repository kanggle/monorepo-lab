...
2. 🔴 **정규화 동일성이 성립하는지 먼저 판정한다.** 고유 4줄(마커 · `SERVICE_PREFIX` ·
   폴백 env 2개)을 정규화하면 나머지가 같아지는가? **안 되면 STOP** — 그것은 실패가 아니라
   **`B` 의 근거가 실측됐다는 뜻**이고, D5 를 고쳐야 한다. 🔴 **가드를 느슨하게 만들어
   통과시키지 마라.**
3. 🔴 **가드를 같은 커밋에서 교체한다** — 「사본 세기(상한 1)」 → 「정규화 동일성」.
   두 번째 사본이 landed 인데 **어느 가드도 없는 창**이 생기면 안 된다.
