...
- [x] 🔴 **위 실측을 다시 재라.** 특히 `scm-platform` 배지 색과 각 사본의 최근 CI 결론 —
...
- [x] 🔴 **`erp`·`finance` 에 배지가 정말 없는지 다시 확인**하라. 있으면 이 티켓의 긴급도가
...
- [x] 결정에 맞춰 `sync-portfolio.sh` 의 kept/excluded paths 또는 후처리를 고친다.
- [x] 🔴 **바꾼 뒤 한 사본에 실제로 돌려서 재라.** 술어는 「rc=0」이 아니라 **«그 사본의
      Actions 에 새 런이 생겼는가 / 배지가 무슨 색인가»** 다. 🔴🔴 어제 내가 «푸시 성공»
      만 읽고 사본 CI 를 안 봐서 이 결함이 하루 늦게 발견됐다 — **같은 술어를 쓰지 마라.**
- [x] 🟡 **`scm` 만 했다 — 소유자가 「scm 만」으로 대상을 한정했다**(사유는 § 소급 적용 절). 원문: 🔴 이미 밀린 `scm`·`erp`·`finance` 세 사본에 **소급 적용**하라(다시 밀어야 한다).
      🔴🔴 그것은 force-push 이므로 **`TASK-MONO-657` AC-0 과 같은 승인 축**이다.

## AC-3 — `TASK-MONO-657` 의 틀린 문장을 고친다

- [x] 🔴 657 § *"무엇이 사본에 없는가"* ③ 의 *"그 배지는 **5주 전 실행**을 보여준다"* 는
      **틀렸다** — 실행이 **아예 없다**(회색). 657 이 그때 `review/`·`done/` 에 있으면
      **본문을 고치지 말고** `## CORRECTION` 을 덧붙여라(frozen).
- [x] 🔵 657 이 아직 `in-progress` 면 본문에서 고쳐도 된다. **그랬다 — 취소선으로 정정했고 657 은 그 뒤 `done/` 으로 닫혔다.**
...
# 🟢 소급 적용 — 소유자 승인 (2026-09-10 UTC)

소유자의 말 그대로: **「scm 만」**

🔴 **이것이 force-push 승인이다.** `TASK-MONO-657` AC-0 ①과 같은 축이고, 대상을
**`scm-platform` 하나로 한정**한 것이다. `erp`·`finance` 는 **안 민다.**

## 왜 셋이 아니라 하나인가 — 사유를 적어 둔다

| 사본 | 다시 미는 이득 |
|---|---|
| `scm-platform` | 🔴 **빨간 배지가 사라진다.** 지금 빨간 배지를 든 **유일한** 사본이고, 하필 지원자가 건네는 링크다 |
| `erp` · `finance` | 🔵 **없다** — 배지가 애초에 0개라 방문자에게 이 축이 안 보인다 |
| `wms`·`iam`·`ecommerce`·`fan` | 🔵 **안 밀렸으므로 해당 없음.** 다음에 밀 때 새 배지가 **자동으로** 간다 |

🔵 그리고 **`TASK-MONO-664` 의 결정이 나면 한 번 더 밀어야 할 수 있다** — 지금 셋을 다
미는 것은 그 일을 두 번 하는 것이다.

## 🔴 순서 — 머지가 먼저다

`sync-portfolio.sh` 는 **로컬 `main` 이 `origin/main` 과 정확히 같을 것**을 요구한다
(326–343행의 가드: *"local main has diverged … the sync would publish the wrong state"*).
⇒ 배지 수정이 **`main` 에 들어간 뒤에** 동기화해야 한다. 브랜치에서 밀면 가드가
막거나(같지 않으므로) 막지 못하고 **엉뚱한 상태를 발행**한다.

## 🔴 판정 술어 — 「푸시 성공」을 쓰지 않는다

AC-2 둘째 칸이 못 박은 그대로:

- ① 사본 README 의 배지가 **`kanggle/monorepo-lab`** 을 가리키는가
- ② 사본에 **`vercel-deploy.yml` 이 없는가**
- ③ 사본의 `nightly-e2e.yml` · `federation-hardening-e2e.yml` 에 **`schedule:` 이 없는가**
- ④ 🔴 **이번 푸시가 만든 CI 런의 결론** — 🔵 ⓒ+ⓑ 는 `ci.yml` 을 안 건드리므로
  **여전히 `failure` 가 나오는 것이 정상이다.** 바뀌는 것은 «아무도 그것을 안 본다» 이고,
  🔴 그것을 「고쳤다」로 적으면 거짓이다(`TASK-MONO-664` 가 그 잔여를 든다)
- ⑤ 🔴 **예약 런이 더는 안 생기는가** — 이것은 **하루 뒤에야** 판정된다(다음 cron 시각이
  지나야 한다). ⚪ 오늘은 «파일에 `schedule:` 이 없다» 까지만 재고, **런이 안 생겼다는
  것은 안 쟀다고 적는다.** 둘은 다른 명제다.

---

# 🟢 소급 적용 실행 — `scm-platform` 하나 (2026-09-11 UTC)

```
✓ Sync complete: scm-platform → https://github.com/kanggle/scm-platform.git
  Commits pushed: 719        5e09b6c...5b885ab main -> main (forced update)
  scm rc=0
```

🔴 rc 는 파이프가 아니라 **로그에서** 읽었다.

## 🔴 다섯 술어 — 「푸시 성공」을 판정으로 쓰지 않았다

🔵 **밀기 «전» 기준선을 먼저 찍어 뒀다.** 안 찍었으면 「바뀐 것」과 「원래 그랬던 것」을
가를 수 없다:

| 술어 | 밀기 전 | 밀고 난 뒤 | |
|---|---|---|---|
| ① 배지 | `kanggle/scm-platform/…` | **`[![CI (monorepo-lab)](…/kanggle/monorepo-lab/…)`** | 🟢 |
| ② `vercel-deploy.yml` | **있다** | **404** | 🟢 |
| ③ `schedule:` (nightly · federation) | 각 **1** | **0 · 0** | 🟢 |
| ④ CI 결론 | `failure` | **`failure` 예정**(지금 `in_progress`) | 🟢 **정상이다** |
| ⑤ 예약 런이 안 생기는가 | — | ⚪ **안 쟀다** | |

### 🟢🟢 ②는 «파일이 없다» 보다 강하게 갈렸다 — **효과**를 쟀다

두 푸시가 만든 런을 비교하면:

```
2026-09-10T13:11:39Z  |  CI (failure)
2026-09-10T13:11:39Z  |  Nightly E2E (skipped)
2026-09-10T13:11:39Z  |  Vercel deploy hooks (failure)   ← 있었다
2026-09-11T05:27:35Z  |  CI (in_progress)
2026-09-11T05:27:35Z  |  Nightly E2E (skipped)
                                                          ← 사라졌다
```

⇒ 🔵 **런이 3개에서 2개로 줄었다.** 「파일이 404 다」는 선언이고, 이것은 **런타임 관측**이다.
사본이 남의 배포 훅 5개를 쏘려다 실패하던 것이 **더는 일어나지 않는다.**

### 🔴 ④ 를 「고쳤다」로 적지 않는다

CI 는 **여전히 실패한다.** ⓒ+ⓑ 는 `ci.yml` 을 **안 건드렸기 때문**이고, 그것이 설계다.
바뀐 것은 **아무도 그것을 안 본다**는 것뿐이다 — 배지가 모노레포를 가리키므로.
🔴 정확한 서술은 **«보이지 않게 했다»** 이고, Actions 탭을 직접 여는 사람에게는 그대로
빨갛다. 그 잔여는 **`TASK-MONO-664`** 가 든다.

### ⚪ ⑤ 는 오늘 판정할 수 없다

「예약 런이 더는 안 생긴다」는 **다음 cron 시각(UTC 18:00 · 19:00)이 지나야** 재진다.
🔴 오늘 잰 것은 **«파일에 `schedule:` 이 없다»** 까지이고, 그 둘은 **다른 명제**다.
🔵 다음에 이 리포를 볼 일이 있으면 `created_at` 이 2026-09-11T18:00Z 이후인
`event=schedule` 런이 **0건**인지 확인하면 닫힌다.

## ⚪ 안 민 것 — 사유와 함께

`erp` · `finance` 는 **안 밀었다.** 소유자가 **「scm 만」** 으로 대상을 한정했고, 사유는
그 둘이 **배지가 0개**라 이 축에서 얻는 것이 없기 때문이다. 🔵 `wms`·`iam`·`ecommerce`·`fan`
넷은 **애초에 안 밀린 상태**라 다음에 밀 때 새 배지가 **자동으로** 간다.
