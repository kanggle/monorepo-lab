...
- [x] **AC-0 (verify-then-act)** — 결함을 **재현**한다: 어떤 서비스의 jar 를 소스보다 낡게
...
- [x] **AC-1 (문다)** — 같은 상태에서 수정 후 검사가 **새 판정 칸으로 잡는다.**
      복원 후에는 다시 조용하다(정상 상태에서 안 짖는다).
- [x] **AC-2 (모집단)** — 새 축이 **서비스별 표**로 보고된다. 대표 1건 금지.
      `libs/` 는 533 과 같이 **귀속 불가 한 줄**로 남긴다(gradle 의존을 읽지 않는다).
- [x] **AC-3 (판정 불가 ≠ 초록)** — jar 가 아예 없는 경우(한 번도 빌드 안 함)는 **낡음이
      아니라 판정 불가**로 분리해 센다. 533 이 세운 규칙 그대로.
- [x] **AC-4** — 종료 코드는 **여전히 0**(자문). `verify-demo-wrapper.sh` ·
...
- [x] AC-0~AC-4 닫힘.
- [x] `check-image-freshness.sh` 안내 문구에 순서의 이유 기록.
...
## AC-0 · AC-1 — 대조군 → 판정 (**비파괴 재현**)

결함을 재현하려면 `이미지 ≥ 소스` 이면서 `jar < 소스` 인 상태가 필요하다. 이미지를 다시
굽는 대신 **jar 의 mtime 만 되돌렸다**(끝나고 복원 — 빌드 산출물을 건드리지 않는다):

- 대상: `erp-platform/gateway-service` (이미지 08-16 05:36Z · 소스 커밋 08-04 07:04Z)
- `touch -t 202608010000` → jar 07-31 15:00Z (**소스보다 이르다**)

| 칸 | 검사 | 결과 |
|---|---|---|
| **대조군** | 수정 전(`origin/main`) | **`요약 — 신선 5 · 낡음 0 · 판정 불가 0`** ← 옛 바이너리를 **초록으로 보고** |
| **판정** | 수정본 | **`옛-jar 1`** · `erp-platform/gateway-service:local  이미지 08-16 05:36Z (새로움) · jar 07-31 15:00Z < 소스 08-04 07:04Z` |
| **복원** | 수정본 | `요약 — 신선 5 · 낡음 0 · 옛-jar 0` ← **정상 상태에서 안 짖는다** |

🔴 **대조군 칸이 이 AC 의 전부다.** 판정 칸만 봤다면 *"새 축이 뭔가를 찾았다"* 로 읽혔을
뿐, **수정 전에는 그것이 초록이었다**는 사실 — 즉 무엇을 못 보던 것인지 — 이 빠진다.

🔵 대조군을 돌릴 때 한 번 헛발질했다: `origin/main` 판을 `/tmp` 에 꺼내 실행했더니
`projects.sh` 를 자기 디렉터리 기준으로 찾다가 죽었다(`unbound variable`). **저장소 안**에
두고 돌려야 같은 조건이다.

## AC-2 — 모집단 · AC-3 — 판정 불가 ≠ 초록

- 새 축도 **서비스별로** 보고한다(대표 1건 금지). 요약 줄에 `옛-jar N` 칸을 새로 뒀다.
- jar 이 **없으면** 낡음이 아니라 **판정 불가**로 센다(빌드 산출물을 청소했을 수 있다) —
  그 경우 *"이미지가 무엇으로 구워졌는지 판정 불가"* 로 문구를 구분했다.
- `libs/` 는 533 과 같이 **귀속 불가 한 줄** 그대로 둔다(gradle 의존을 읽지 않는다).

## AC-4 — 자문 유지

종료 코드는 **여전히 항상 0**. 기동을 막지 않는다(막으면 사람들이 `demo-up.sh` 를 우회하고,
그러면 검사 전체가 무력해진다 — 533 이 그렇게 설계한 이유).

## 그 밖에 — 검사가 스스로를 지키게 만든 두 곳

1. **안내 문구에 순서의 이유를 박았다.** 전에는 두 명령이 나란히 있을 뿐이었다:
   ```
   ① 먼저:    ./gradlew …:bootJar
   ② 그 다음:  DEMO_BUILD=1 bash infra/demo/demo-up.sh …
   ⚠ 순서를 지킬 것 — ②만 실행하면 **옛 jar 로 이미지만 새로 구워져** 이 검사가 그 서비스를 영영 못 잡는다
   ```
2. **`DEMO_BUILD=1` 에서도 검사를 건너뛰지 않는다.** 전에는 *"지금 새로 구우므로 물어볼
   것이 없다"* 며 건너뛰었는데, 그 전제가 틀렸다 — `demo-up.sh` 는 **컴파일하지 않으므로**
   지금 굽는 것은 디스크의 jar 그대로다. **굽기 직전이 그것을 말할 수 있는 마지막 순간**이다.
   같은 자리에서 **어느 도메인을 굽는지 이름을 댄다**(하드 의존이 함께 구워져 사용자가
   요청한 것보다 넓다 — 이 결함을 발견한 경로가 정확히 그것이었다).

## 가드

`bash -n` (두 스크립트) OK · `check-index-queue-drift` OK ·
`check-walkthrough-ledger-drift` OK · `verify-demo-wrapper.sh` rc=0
