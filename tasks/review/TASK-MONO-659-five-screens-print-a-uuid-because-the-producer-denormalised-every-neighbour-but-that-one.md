...
- [x] 🔴 **먼저 결정이다, 구현이 아니다.** 감사 행과 org-admin 행에 운영자 표시명을
      싣는 것은 ①생산자가 조인해서 싣는가 ②소비자가 별도 조회로 해석하는가 의 선택이고,
      감사 로그는 **그 시점의 이름**이 필요할 수 있다(지금 이름으로 해석하면 과거 기록이
      바뀐다). 🔴 그 판단을 적지 않고 필드만 더하지 마라.
      → 🟢 **동사=결정하고 판단을 적어라. 적혔다.**
      [`ADR-MONO-073`](../../docs/adr/ADR-MONO-073-org-admin-rows-carry-the-operator-display-name.md)
      **ACCEPTED — ⓐ 생산자 조인**(소유자, 2026-09-11). 🔵 그 ADR 이 「그때의 이름」 위험을
      **감사 행 vs 현재 소속 목록**으로 갈랐다 — 전자는 이미 철회됐고 후자엔 그 위험이 없다.
- [x] 결정이 「싣는다」면 `GroupMember.displayName` 과 **같은 필드명**을 써라. 열두 번째
      이름을 만들지 마라.
      → 🟢 **닫혔다 (`TASK-MONO-670`, 2026-09-11).** 필드명은 `displayName` — 형제
      `GroupMemberResponse`/`AddGroupMemberResponse` 와 **같은 것**이고 새 이름을 안 만들었다.
      🔵 앞선 주석이 *"지금 찍으면 안 쓴 것을 썼다고 하는 것"* 이라고 적어 뒀는데, **이제
      실제로 썼다** — `OrgAdminGrant` · `OrgAdminResponse` · 계약 · 콘솔 zod 넷 다.
...
> 🔴🔴 **이 절에는 체크박스를 두지 않는다** (2026-09-11 정정). 원래 여기에 `- [ ]` 를 하나
> 두었는데, 그것은 **§ AC-2 첫 칸의 사본**이었다 — 같은 의무가 두 곳에서 상태를 갖게 되고,
> 그러면 한쪽만 고쳐진다. 🔵 이 저장소가 이름 붙인 함정이고 **`TASK-MONO-654` 가 정확히 그
> 모양으로 창 하나를 기다렸다**. ⇒ **닫는 자리는 § AC-2 하나뿐**이고 이 절은 기록이다.

- 🔴 **무엇을 정해야 하나**: 감사 행과 org-admin 행에 운영자 표시명을 ①생산자가 조인해서
  싣는가 ②소비자가 별도 조회로 해석하는가. 🔴 감사 로그는 **그 시점의 이름**이 필요할 수
  있다 — 지금 이름으로 해석하면 **과거 기록이 바뀐다.**
- 🟢 **그 결정을 한 장으로 올려 두었다**: [`ADR-MONO-073`](../../docs/adr/ADR-MONO-073-org-admin-rows-carry-the-operator-display-name.md)
  (**PROPOSED**, 2026-09-11). 🔴🔴 그 ADR 이 찾아낸 것: **AC-2 를 막고 있던 「그때의 이름」
  위험은 감사 행의 것이고, 그 자리는 이미 철회됐다.** 남은 org-admin 행은 «현재 소속 목록»
  이라 **지금 이름이 맞다** ⇒ 그 위험이 적용되지 않는다.
- 🔵 그리고 **같은 서비스의 형제가 이미 하고 있다** — `GroupAdminUseCase:153-161` 이
  `adminOperators.findById(...).getDisplayName()` 로 조인하는데 바로 옆
  `OrgNodeAdminUseCase:85-89` 만 안 한다. `AdminOperatorJpaRepository` 는 **이미 주입돼 있다**.
...
# 🟢 AC-2 닫힘 (2026-09-11 UTC · `TASK-MONO-670`)

이 티켓의 **마지막 열린 칸**이었다. 두 칸이 서로 다른 시점에 닫혔고, 그 구분이 요지다:

| 칸 | 동사 | 언제 |
|---|---|---|
| 첫째 — 먼저 **결정**이다 | 결정하고 판단을 적어라 | `ADR-MONO-073` ACCEPTED ⓐ (소유자, 2026-09-11) |
| 둘째 — **같은 필드명**을 써라 | **써라**(구현) | `TASK-MONO-670` 이 랜딩하면서 |

🔵 **둘째 칸을 결정 시점에 같이 찍지 않은 것이 옳았다** — 그때는 아직 아무것도 안 썼다.

## 🔵 구현이 ADR 의 예상보다 더 작았다

ADR 이 ⓐ 의 대가로 *"목록 크기만큼 조회 증가(N+1)"* 를 적었다. 🔴 **실제로는 조회가 하나도
안 늘었다** — `listNodeAdmins` 는 `operatorId` 를 만들려고 `externalOperatorId(...)` 안에서
**이미 그 행을 읽고 있었고** 표시명만 버리고 있었다. grant 경로도 마찬가지로 `target` 을
이미 들고 있었다.

⇒ 🔵 이 티켓의 ①②③(wms)이 *"코드를 쥐었다가 버린다"* 였는데, **iam 도 같은 모양**이었다.
🔴 ADR 의 그 문장은 «틀렸다» 기보다 **보수적이었다** — 그리고 그 차이는 소스를 열어야 보였다.

⇒ **`TASK-MONO-659` 의 모든 AC 가 닫혔다.** 🔴 `done/` 으로 옮기는 것은 별도 close chore 다.
