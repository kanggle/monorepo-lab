...
## CORRECTION — 🔴 이 티켓은 `done/` 으로 가지 않는다. 만든 분기가 프로덕션에서 도달 불가능하다 (2026-09-08 UTC)

4-dim 검증에서 (a)(b)(c) 는 통과했고 AC 체크박스도 전부 켜져 있다. 그러나 **(d)** —
「일이 실제로 랜딩했나」 — 가 닫히지 않았다.

이 티켓의 핵심은 두 상태를 **구별**하는 것이었다:

| 상태 | 이 티켓이 정한 문구 |
|---|---|
| `/bundles` → **404** | 「이 기능이 제어 API 에 **아직 배포되지 않았습니다**」 |
| `/bundles` → 무응답 | 「제어 API 가 **응답하지 않습니다** — 잠시 후 자동으로 다시 시도합니다」 |

**라이브에서 방문자가 보는 것은 아래쪽이고, 그것은 틀렸다.**
...
`Origin` 을 붙여 직접 재니 원인이 나왔다 — `GET /bundles` 는 **404 를 주지만
`access-control-allow-origin` 이 없다**(`/status` 에는 있고, `OPTIONS /bundles` 프리플라이트에도
있다). API Gateway 가 라우트 없음에 스스로 내는 기본 404 에는 CORS 가 안 붙는다. 브라우저가
응답을 차단하므로 `fetch` 가 **던지고**, `catch` 가 `down` 으로 처리한다 — `r.status === 404`
를 보는 줄까지 **가지도 못한다.**

🔴 제어 API 는 살아 있고(`/status` 200) 재시도로는 영원히 안 풀린다. **이 티켓이 고치려던
부류의 거짓을, 이 티켓을 배포한 화면이 저지르고 있다.**

🔵 `TASK-MONO-636` 이 실제로 고친 것은 남아 있다 — 사유 없는 죽은 버튼은 사라졌고, 잠금과
사유 표시는 동작한다(`.bnote` 3장 · 배지 🔴 확인 불가 · 버튼 disabled). 틀린 것은 **어느
사유를 말하는가** 다.

⇒ **`TASK-MONO-644`** 로 기안했다. 이 티켓은 그때까지 `review/` 에 남는다 — `done/` 은
frozen 이고, 거기 넣으면 이 남은 것이 다시 안 읽힌다.
