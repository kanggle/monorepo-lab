...
## CORRECTION (2026-09-24 UTC · 14차 AMI 창) — AC-6 **닫힘**: 재촬영했고 위·아래 가격이 같다

- 촬영: `scripts/capture-portfolio.mjs --app fan`(데모 계정 로그인 · 테넌트 `fan-platform`) → 계획 11 · 찍음 11 · 실패 0.
- 🔴 **이미지를 열어 봤다** — 위(가입 카드) `월 7,900원` / `월 17,900원`, 아래(요금제) `7,900원 / 월` / `17,900원 / 월`. **같다.** 옛 값 `4,900`·`12,900` 없음.
- 업그레이드 일할 문구(`-₩7,636 → ₩10,264`)도 그대로 — 옛 README 캡션이 유효하다.
- `projects/fan-platform/README.md` 에 **04 로 다시 실었다**(옛 캡션 복원, 헤더 주석을 «재촬영» 으로 교체). `docs/portfolio.md` 는 03 을 그대로 둔다(725 가 검증된 03 으로 바꿔 둔 자리 — 다시 바꿀 이유 없음).
- 🔵 아래 절 바닥에 `샘플 데이터 (아직 발행 전)` 이 보인다 — AC-5 CORRECTION 이 적은 **번들 스냅샷** 판이고, 가격이 맞으므로 이 AC 의 판정과 무관하다.
