...
# 🟢 소유자 결정 — **ⓒ 「적용 안 함」으로 계약서 정정** (2026-09-18 UTC)

AC-0 이 내놓은 세 갈래 중 소유자가 **ⓒ** 를 골랐다. 🔴 내 추천이 ⓒ 였지만 **추천은 추천일 뿐이고,
698 § AC-3 의 「iam gateway 포함」을 부분 정정하는 것이라 소유자 결정이었다** — 그 결정이 왔다.

## 집행한 것 (스펙 먼저, 이 PR 에 포함)

`platform/contracts/jwt-standard-claims.md`:

1. **§ JWT Validation rule 5 *Implementation status*** 를 **측정된 현재 상태**로 갱신했다 —
   6 게이트웨이 shadow(696/697) · **console-bff 는 거절, 섀도 없음**(712 착지) · 남은 하나인
   iam gateway 는 **의도적 미적용**.
2. **새 불릿**: *"The iam gateway declares no audience allowlist, and that is a measured decision,
   not an omission"* — 그 엣지가 **엣지인 것은 맞다**는 것부터 적고, allowlist 는 «호출자를
   admit 하는 것» 인데 **admit 할 측정된 호출자가 없다**는 것을 근거로 적었다. 빈 목록은 기동
   실패이므로 «덮으려면 내용을 지어내야 하고, 지어낸 값이 운영 거절 기준이 된다» 까지.
3. **§ Change log** 에 항목 신설(계약서 자신의 관례) + 698 의 «Micrometer 가 없어 섀도 불가» 를
   **그 자리에서 정정**했다.

🔴 **면제는 «오늘의 측정» 에 매여 있고 그 측정과 함께 만료된다** — 계약서 본문에 그렇게 적었다.
검증 표면에 client 가 하나라도 배선되는 순간 rule 5 가 **그대로** 적용되고 이 면제는 무효다.
🔵 모집단 한계(저장소 인구조사는 런타임 설정을 못 본다)도 계약서와 이 파일 양쪽에 남겼다.

## AC 처분

| AC | 처분 |
|---|---|
| **AC-0** | 🟢 **닫힘** — 측정 완료(위 § AC-0 측정). 결과가 «모집단 0» |
| **AC-1** | 🟢 **닫힘** — 코드 재확인을 AC-0 과 같은 창에서 했다(`TokenValidator` 의 1-인자 생성자 · `iss` 직접 allowlist · `expectedAudience` 가 `String` 단일값). 🔵 JJWT `requireAudience` 의 배열 의미론 ⚪ 는 **질문 자체가 사라졌다** — 다중값 판을 내지 않으므로 |
| **AC-2** | ⚪ **대상 소멸** — 다중값 판을 내지 않는다. allowlist 가 없으므로 표현할 집합이 없다 |
| **AC-3** | ⚪ **대상 소멸** — 거절이 없으므로 403 도 섀도도 없다. 🔵 이 AC 의 **정지 조건**이 이 티켓을 여기까지 끌고 온 장치였다 |
| **AC-4** | 🟢 **닫힘(공허하지 않게)** — *"allowlist 의 각 항목 옆에 근거를 적는다"* 를 **항목 0개**로 만족한다. 🔴 이것이 공허한 만족이 아닌 이유: 「있을 법해서」 넣은 항목이 **하나도 없다**는 것이 이 AC 가 지키려던 바로 그 성질이고, 이 티켓은 그것 때문에 멈췄다 |
| **AC-5** | ⚪ **대상 소멸** — 코드 변경이 0이라 검증할 빌드 산출물이 없다. 🔵 계약서 문서 변경만 있으므로 CI 의 문서/가드 잡이 판정한다 |

## 🔵 남기는 두 갈래 (소유자가 **고르지 않은** 것)

지금 정정이 그 둘을 **닫지 않는다** — 면제가 만료되면 다시 이 자리로 돌아온다:

- **ⓐ 창에서 재기** — 데모 게이트웨이 로그/액추에이터. 🔴 697 AC-0 의 **분모 규율**(트래픽 0 이면
  «관측 0» 은 «부재» 가 아니다)이 그대로 걸리므로, 창을 열어도 판정이 안 날 수 있다.
- **ⓑ `/internal/tenants/**` 로 좁히기** — 그 경로의 호출자를 **게이트웨이 경유로 배선**하는 것이
  선행이고, 그것은 아래 곁발견 ①과 같은 작업이다.

## ⏳ 넘긴 것 — 곁발견 ① 은 **이 티켓이 닫혀도 살아 있어야 한다**

`ACCOUNT_SERVICE_BASE_URL` 미설정(product-service 셀러 프로비저닝이 `localhost:8081` 로 떨어지고,
그 호출이 **fail-soft** 라 실패가 조용하다)은 이 티켓의 범위 밖이지만 **관측은 진짜다**.
🔴 `done/` 은 다시 안 읽히므로, 이 관측은 **`TASK-MONO-672`**(창이 필요한 측정의 수령처)가 든다 —
판정에 창이 필요하기 때문이다. 이 티켓을 닫기 전에 그쪽에 적는다.
