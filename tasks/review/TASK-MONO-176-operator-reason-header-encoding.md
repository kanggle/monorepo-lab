...
- [ ] **AC-1** console 이 `X-Operator-Reason` 을 `encodeURIComponent` 로 전송(operators + accounts; 단위 테스트 — 한글 사유가 ASCII-only 헤더로 인코딩되고 round-trip).
- [ ] **AC-2** admin-service 필터가 percent-encoded 헤더를 원문 UTF-8 로 디코드하여 downstream 컨트롤러에 노출(필터 단위 테스트; 한글·공백 포함 ASCII·비-escape·malformed 케이스).
- [ ] **AC-3** 데모 라이브: `multi-operator` 가 globex 운영자 생성 시 사유 "테스트 1" → 성공, `admin_actions.reason` 에 "테스트 1". [live]
- [ ] **AC-4** console `pnpm test` GREEN(기존 reason 단언 round-trip 갱신 포함), `tsc`/`lint`/`build` GREEN; admin-service `:test` GREEN; admin-service boots healthy; GAP Testcontainers IT GREEN.
...
- [ ] console encode(2 파일 3 지점) + admin-service 필터 + contract + 단위 테스트.
- [ ] console `pnpm test`/`tsc`/`lint`/`build` + admin-service `:test` GREEN; admin-service healthy; GAP IT GREEN.
- [ ] Local 재빌드+재기동; 한글 사유 운영자 생성 성공 + `admin_actions.reason` 원문 한글 확인.
- [ ] One atomic cross-project PR.
- [ ] Task md + root `tasks/INDEX.md` 갱신.
- [ ] Reviewed + merged.
