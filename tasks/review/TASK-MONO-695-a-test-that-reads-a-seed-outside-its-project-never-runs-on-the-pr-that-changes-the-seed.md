...
🔴🔴 **이 표는 또한 `apps/*/build.gradle` 만 스캔한 결과였다 — 리뷰에서 좁다고 지적받았다.** `projects/**/build.gradle` 전체(프로젝트-레벨 `subprojects {}` 파일·`libs/<m>`·`tests/e2e` 포함, 56건 = 기존 44 + 신규 12)로 다시 스캔해도 **outside 행은 그대로 2건**(신규 12건 중 `rootProject.file(` 선언 0건) — 표의 숫자는 안 바뀌지만 그 모집단을 낸 스캔 범위가 지금은 `projects/**/build.gradle` 다(AC-1/AC-3 정정 참고).
...
- [x] **AC-1 — 모집단을 둘 다 센다.** ① `inputs.(file|dir|files)(rootProject.file(…))` 전수 ② 테스트 소스(`src/test/**`)가 **선언 없이** 모듈 밖 경로(`infra/` · 저장소 루트 상대 경로 · `../..` 류)를 읽는 자리 전수. 🔴 0건 판정엔 양성 대조군을 넣는다(① 의 `seed-scm.sh` 를 읽는 테스트가 ② 의 술어에도 걸려야 한다). 🔴🔴 **리뷰 정정 (2026-09-17, 머지 전)**: ① 의 최초 스캔 모집단이 `projects/*/apps/*/build.gradle` **뿐**이었다 — AC-3 가드 자신의 주장(«프로젝트 전체를 본다»)보다 좁았다. `projects/<p>/build.gradle`(`subprojects {}` 이 그 프로젝트의 모든 모듈에 적용되는, **가장 유력한 미래 자리**) · `projects/<p>/libs/<m>/build.gradle`(project-scoped shared module) · `projects/<p>/tests/e2e/build.gradle` 세 자리가 안 잡혔다 — 정확히 이 티켓이 닫으려던 «population hole» 모양이 가드 자신에게도 있었다. `git ls-files -- 'projects/**/build.gradle'` 로 재스캔: **56건**(기존 44 + 신규 12 — 프로젝트 루트 7·tests/e2e 4·libs 1). 12건 전수를 `grep -l rootProject.file(` 로 재확인 — **outside 선언 0건 추가**(밖 2건은 여전히 wms/iam apps/* 두 곳뿐). 🔵 **알려진 제외**: 저장소 루트 `libs/**` 와 루트 `build.gradle` 은 population 밖이다 — 그 둘은 애초에 어떤 `projects/<p>/**` 프로젝트 필터에도 안 걸리므로 "어느 프로젝트 필터가 커버해야 하는가" 질문 자체가 성립하지 않는다(다른 축인 `libs` 출력 플래그가 그 자리다, 이 가드의 소관 아님).
...
- [x] **AC-3 — 가드.** `scripts/` 에 «선언된 모듈 밖 테스트 입력 ⊆ 그 프로젝트 필터» 를 재는 가드를 둔다(또는 기존 가드 확장). `ci.yml` 의 필터를 **파싱해서** 판정하고, 경로를 손으로 복제하지 않는다. bite: AC-2 의 한 줄을 되돌리면 그 모듈:줄을 지목하며 rc≠0, 복원하면 rc=0. 🔴 `scripts/` 에 파일을 더하면 **전체 가드**를 돌린다(`check-ls-files-guard-count.sh` 분모가 움직인다). 🔴🔴 **리뷰 정정 (2026-09-17, 머지 전) — 가드 자신의 모집단이 AC-3 의 문구·자기 헤더 주장보다 좁았다.** `list_build_gradle_files()` 가 `projects/*/apps/*/build.gradle` 만 봤고, `outside-module-input-guard` changes 필터도 같은 패턴이었다 — `projects/<p>/build.gradle`(`subprojects {}`) · `projects/<p>/libs/<m>/build.gradle` · `projects/<p>/tests/e2e/build.gradle` 세 자리는 outside 선언이 생겨도 **가드가 계속 초록**이었을 것이다. 고침: 스캔·필터 둘 다 `projects/**/build.gradle` 로 확장(git ls-files 패턴 + `find` 폴백 동일하게 확장). 저장소 루트 `libs/**`·루트 `build.gradle` 은 **의도적으로 계속 제외**(AC-1 정정 참고 — 별도 `libs` 축). `--self-test` 케이스 2개 추가: 프로젝트-레벨(`scm-platform/build.gradle`) · `tests/e2e`(`fan-platform/tests/e2e/build.gradle`) 각각의 outside 선언이 bite 하는지, 양성 대조군(기존 wms/iam apps/* 케이스가 확장 후에도 여전히 통과)까지 — **총 `--self-test` 15/15 PASS**(기존 13 + 신규 2), 실제 트리 `rc=0`(outside 2건, 변동 없음 — 새로 스캔되는 12개 파일 중 outside 선언 0건 추가 확인). 개발 중 자기 회귀도 하나 잡았다: 기존 bite 케이스가 `inbound-service/build.gradle:75` 를 하드코딩하고 있었는데 그 줄은 AC-5 코멘트 추가로 이미 `:85` 로 밀려 있었다 — 파일명은 지목하되 줄번호는 정규식으로 바꿔 고침(같은 클래스의 결함이 다시 날 것을 구조적으로 막음).
- [x] **AC-4 — PR 에서 실제로 돈다.** ✅ **관측으로 닫음 (2026-09-17 KST · 오케스트레이터).** 소유자가 #3878 을 머지(`c99cc2d50`)한 뒤, 그 `main` 에서 가지를 친 시험 PR [#3883](https://github.com/kanggle/monorepo-lab/pull/3883) 을 열었다 — diff = **`infra/demo/seed/seed-scm.sh` 주석 한 줄, 파일 1개**(`gh pr diff --name-only` 로 확인), head `444cad42f`. 롤업: `changes` SUCCESS · **`Build & Test (JDK 21, Linux)` = SUCCESS**([잡 로그](https://github.com/kanggle/monorepo-lab/actions/runs/35176310795/job/105058760961)) · `Outside-module Gradle input filter coverage` = SKIPPED(`build.gradle`/`ci.yml`/스크립트를 안 건드렸으니 정상). 🔵 대조: 수정 전 표본 #3246(같은 모양의 시드-만 diff)은 `Build & Test` **SKIPPED** 였다 ⇒ 이 PR 에서 그 잡을 깨운 것은 새 필터 항목뿐이다(diff 에 다른 경로가 없다). #3883 은 **머지하지 않고 닫았다**(`CLOSED`, `mergedAt=null`, 브랜치 삭제). 아래는 차단 당시의 기록(#3880)이다 — 지우지 않는다.
  이전 기록 — 🔴 차단됨 — 소유자 결정 필요. 구현 PR(#3878, CI 전부 초록 — `Outside-module Gradle input filter coverage` 포함) 이 돈 뒤, 구현 브랜치를 베이스로 `seed-scm.sh` 주석 한 줄만 바꾸는 시험 브랜치(`task/mono-695-ac4-test-pr`)를 만들어 PR #3880 을 열었다. **CI 가 아예 안 돌았다** — 몇 분을 기다려도 `gh run list` 에 이 브랜치의 런이 0건(같은 시각 다른 PR 은 초 단위로 런이 붙었다). 원인을 `ci.yml:1-7` 에서 확인: `on.pull_request.branches: [main]` — **PR 의 base 가 `main` 이 아니면 워크플로 자체가 트리거되지 않는다.** ⇒ base=구현 브랜치인 시험 PR 은 **구조적으로 관측 불가**. 대안도 막힌다: base=`main` 으로 열면 CI 는 돌지만, `main` 은 아직 이 티켓의 `ci.yml`/`build.gradle` 수정을 안 갖고 있어 **diff 가 seed-scm.sh 하나로 격리되지 않는다**(구현 커밋 전체가 같이 잡혀 `wms`=`projects/wms-platform/**`(build.gradle 직접 변경)로도 참이 되므로, Build & Test 가 돌아도 «필터의 새 seed-scm.sh 항목 덕분» 이라는 증명이 안 된다). `workflow_dispatch` 도 `ci.yml` 에 없어 우회 불가. **PR #3880 은 머지 없이 닫았다**(CI 가 안 붙으니 열어 둘 이유가 없다, 브랜치도 삭제). **닫히려면 `main` 이 이미 고침을 갖고 있어야 한다** — 즉 구현 PR #3878 머지가 **선행 조건**인데, 자기 PR 셀프머지는 명시적 승인이 필요하다(`CLAUDE.md` § git-workflow, "Self-merge … need explicit authorization"). 이 AC 는 그 승인 없이는 닫을 수 없다 — **소유자가 ① #3878 머지를 승인하거나 ② 이 관측을 생략하는 대안(예: 이미 있는 표본 PR #3246 의 역사적 SKIPPED 기록 + 코드 리뷰로 대체)을 결정**해야 한다. 머지가 승인되면: `main` 에 머지 → 그 시점 바로 `main` 기반 새 브랜치에서 seed-scm.sh 주석 1줄 PR → `Build & Test` SKIPPED 아님 확인 → 머지 없이 닫기. 몇 분이면 끝나는 절차다.
- [x] **AC-5 — 무소유 기록을 닫는다.** ✅ **소유자 결정으로 닫음 (2026-09-17 KST).** `build.gradle` 두 곳은 구현 PR 에서 «가드 이름으로 닫힘» 으로 고쳤다. `ADR-MONO-063:187` 은 — 이 저장소에서 ACCEPTED ADR 을 정정하는 유일한 규약이 **새 ADR 작성**이라 — 선택지를 소유자에게 물었다. 추천: «CI 틈 하나로 새 ADR 은 과하다, 티켓 기록으로 둔다». 소유자 답(원문): **「ADR-MONO-063:187 정정 추천대로해」** ⇒ ADR 본문·새 ADR 없음. 이 항목의 해소 기록은 **이 티켓**이 들고 간다(`ADR-MONO-063:187` 의 «미결·무소유» → `TASK-MONO-695` 로 해소, 가드 `scripts/check-outside-module-input-filter-coverage.sh`). 아래는 그 전의 기록이다.
  이전 기록 — 고친 뒤 `auth-service/build.gradle` 의 «Residual gap» 주석과 `inbound-service/build.gradle:70-78` 주석이 **틈이 닫혔다(가드 이름)** 로 바뀐다. `ADR-MONO-063:187` 은 ACCEPTED 본문이라 고치지 않는다 — ADR 정정 규약(`docs/adr/INDEX.md` · CORRECTION 절)이 있으면 그 규약대로 «`TASK-MONO-695` 로 해소» 를 덧붙인다(규약을 먼저 읽는다). **build.gradle 두 곳은 닫음. ADR 쪽은 아래 § 구현 기록 참고 — 부분 닫힘.**
...
### AC-3 리뷰 정정 (2026-09-17, 머지 전) — 가드 자신의 모집단이 좁았다

**결함**: 위 초판이 스캔한 것은 `projects/*/apps/*/build.gradle` 뿐이었다(`outside-module-input-guard` changes 필터도 동일 패턴). AC-3 문구("전체 가드")와 가드 자신의 헤더 주장을 스스로 어겼다 — 프로젝트-레벨 `projects/<p>/build.gradle`(`subprojects {}` 이 그 프로젝트 모든 모듈에 적용되는, **가장 유력한 미래 선언 자리**) · `projects/<p>/libs/<m>/build.gradle`(CLAUDE.md project-scoped shared module) · `projects/<p>/tests/e2e/build.gradle` 세 자리에 outside 선언이 생겨도 가드는 계속 초록이었을 것 — 이 티켓이 닫으려던 population-hole 결함 클래스가 가드 자신에게 있었다.

**고침**: `list_build_gradle_files()` 의 스캔 패턴과 `make_case()` 의 픽스처-카피 패턴을 `git ls-files -- 'projects/**/build.gradle'`(+ 동일하게 확장한 `find` 폴백)로, `ci.yml` 의 `outside-module-input-guard` changes 필터를 `projects/**/build.gradle` 로 **함께** 확장했다(셋 다 갈라지면 이 드리프트가 다시 돌아온다는 것이 가드 자신의 헤더 논지). 저장소 루트 `libs/**` 와 루트 `build.gradle` 은 **의도적으로 제외 유지** — 그 둘은 애초에 어떤 `projects/<p>/**` 프로젝트 필터에도 걸리지 않으므로 "어느 프로젝트 필터가 커버하는가"라는 질문이 성립하지 않는다(별도 `libs` 출력 플래그의 소관, 이 가드는 다루지 않는다 — 알려진 제외로 기록).

**재측정**: `git ls-files -- 'projects/**/build.gradle'` = **56건**(기존 44 + 프로젝트-루트 7 + `tests/e2e` 4 + `libs/<m>` 1). 신규 12건 전수를 `grep -l rootProject.file(` 로 확인 — **outside 선언 추가 0건**(population A 는 여전히 wms/iam 2건). 실제 트리 `rc=0`.

**`--self-test` 2건 추가**: (a) `projects/scm-platform/build.gradle`(프로젝트-레벨, `subprojects {}` 안에 outside `inputs.file` 주입) → 필터 미포함 → bite, `scm-platform/build.gradle:<line>` 정확히 지목. (b) `projects/fan-platform/tests/e2e/build.gradle`(e2e 모듈에 동일 패턴 주입) → bite, `fan-platform/tests/e2e/build.gradle:<line>` 지목. 둘 다 기존 wms/iam 과 무관한 프로젝트를 골라 케이스 간 간섭이 없게 했다. 양성 대조군: 확장 후에도 기존 wms/iam apps/* 케이스가 여전히 통과하는지 별도로 재확인. **`--self-test` 총 15/15 PASS**(기존 13 + 신규 2).

**개발 중 잡은 자기 회귀 하나**: 기존 bite 케이스(④)가 `"inbound-service/build.gradle:75"` 를 문자열로 하드코딩하고 있었는데, 이 세션 앞부분에서 AC-5 주석을 추가하며 그 선언 줄이 이미 `:85` 로 밀려 있었다 — 최초 재실행에서 15개 중 1개가 FAIL 했다(다른 14개는 전부 관련 없이 통과). 파일명은 지목하되 줄번호는 정규식(`[0-9]+`)으로 바꿔 고쳤고, 재실행에서 15/15 로 확인했다. 이 결함 자체가 "줄 번호를 하드코딩하지 마라" 는 케이스 ⑥b/⑥c 의 설계 원칙을 사후적으로 재확인해 줬다.
