...
| **프리뷰 URL 상실** | 🔴 `TASK-MONO-588` AC-4 가 이미 적었다 — 프런트 e2e 가 `nightly-e2e.yml` 에만 있어 **머지 시점에 화면을 안 보는데**, 프리뷰까지 없으면 «머지 전에 화면 보는 수단» 이 **0** 이 된다. `TASK-FAN-FE-018` 이 같은 구멍의 다른 얼굴. 🔴🔴 **08-28 재계수가 이 행을 좁혔다** — 그날 브랜치 이벤트 32 개가 **프리뷰를 0 개** 만들었다(28 개는 ignore 스텝이 굽기 전에 취소). 대가는 «프리뷰 상실» 이 아니라 **«web 경로를 건드리는 PR 의 프리뷰 상실»** 이고 그런 PR 은 16 중 0 이었다 — 아래 §재계수 ③ 。🔴🔴 **AC-6 이 이 행을 소비하고 그 「0」을 반증했다(08-29)** — 「16 중 0」은 성질이 아니라 그날의 작업 구성이었다. 40 PR 전수의 최종 값은 **40 PR 당 프리뷰 2개, 전부 `kanggle-fan`, 전부 회수 가능**(론처 몫은 40 중 0). 아래 §AC-6 판정 |
...
# ✅ AC-0 판정 (2026-08-29 UTC) — **결과 A. 불리언은 production 생성까지 막는다.**

🔴 **AC-0 만 닫혔다. 이 티켓은 아직 `in-progress` 다** — AC-1 은 소유자 선행이 있다(아래).

## 착수 조건부터 — 「한도가 풀렸는가」를 먼저 쟀다

이 티켓은 하루 전 **착수 불가**였다. AC-0 의 대조군(`kanggle-portfolio`)이 계정 한도로
이미 빨강이었고, 그 상태에서 재면 *«억제는 부재로 나타나고, 한도도 같은 부재를 낸다»* 에
정면으로 걸린다.

🔴🔴 **그때 내가 적은 «가장 이른 창 = 08-29 14:48Z» 는 틀렸다.** Vercel 문구
*"retry in 24 hours"* 를 실패 시각에 **더한 값**이었지 측정이 아니었다. 실측: **04:53Z** 에
이미 배포가 생성되고 있었다(약 10시간 오차 — 롤링 24h 가 아니라 **UTC 달력일** 리셋에 부합).
[[feedback_verify_claims_against_the_source_itself]]

## 전제 확인 (이게 없으면 「배포 없음」이 두 가지를 뜻한다)

fan 의 트리거 spec 에 **`:/projects/fan-platform/web`** 이 있다 ⇒ `vercel.json` 편집은
**fan 트리거가 맞다.** 이것을 먼저 확인하지 않았다면 fan 의 부재가 «키 때문» 인지
«ignore 스텝이 건너뛴 것» 인지 **구별되지 않는다.**

## 🔴🔴 티켓이 fan 을 대상으로 고른 안전 근거는 **오늘 거짓이 됐다**

티켓 원문: *"지금 프로덕션이 어차피 `500`(OIDC env 미설정, `TASK-FAN-FE-018`)이라 배포가 한
번 빠져도 잃을 것이 없다"*. `TASK-FAN-FE-018` → `019` → `MONO-600` 을 거치며 **프로덕션은
건강하다**(가드 307 · `/api/auth/*` 200 · 서빙 커밋 = 트리거 tip, 전부 실측).

⇒ 실제 위험은 **「되돌리기 전까지 fan 이 새 배포를 못 받는다」** 였고, 창을
**06:15:59Z ~ 06:23:30Z (7분 31초)** 로 묶었다. 그동안 서빙본은 `608fe0f46` 그대로라
방문자에게 보이는 변화는 없었다. 🔵 **티켓의 근거를 그대로 인용해 진행하지 않고 다시 잰
것이 이 문단이다** — 근거가 낡으면 대상 선택도 다시 물어야 한다.

## 측정 — 3단, 대조군 포함, 계측기 2개 교차

| 단계 | 커밋 | `kanggle-fan` (대상) | `kanggle-portfolio` (대조군) |
|---|---|---|---|
| **A** 키 있음 · 프리뷰 | `0dab1dcb6` | **0건** (statuses·check-runs 둘 다) | pending → success |
| **A** 키 있음 · **production** | `e6f6a6d59` | **0건** (둘 다) | pending 06:16:04Z → success 06:17:46Z |
| **B** 키 없음 · 역방향 | `2be8f3adc` | pending 06:23:30Z → **success `Deployment has completed`** | pending → success |

- 🔵 **대조군이 세 단계 모두에서 전 생애를 마쳤다** ⇒ «푸시가 정상이었다» 가 독립 확인되고,
  fan 의 부재가 **그 키에** 귀속된다.
- 🔵 **역방향(A–B–A)이 닫혔다** ⇒ «이 커밋들의 다른 무언가» · «그날 Vercel 이 느렸다» 가
  배제된다. 대조군 하나로는 부족하다는 티켓의 요구가 여기서 값을 했다.
- 🔵 대기: 이력상 도착 간격(fan 이 portfolio 보다 ~11초 뒤)의 **30배 이상**. production
  단계는 대조군이 **1분 47초** 만에 끝났고 **06:22:53Z** 까지 기다린 뒤 «없다» 라고 했다.
- 🔵 계측기 2개(commit **statuses** 복수 + **check-runs**)가 **일치**했다 — 한쪽만 보고
  «없다» 라고 하지 않았다. [[env_empty_detector_output_is_not_absence]]

### 결론 = **A**

`git.deploymentEnabled: false`(불리언)는 **프리뷰와 production 양쪽 모두 배포의 «생성»을**
막는다. `Canceled by Ignored Build Step` 조차 남지 않는다 — `ignoreCommand` 의
「생성 후 취소」와 **다른 지점**이고, 그것이 이 설계가 노린 «판정을 생성 앞으로» 다.

🔴 `TASK-MONO-588` 을 상속하지 않은 것이 값을 했다. 588 이 잰 것은 **객체 형태 + 브랜치
이름 직접 지정**이라 `main` 을 건드리지 않았고 588 자신이 이전을 금지했다.
**불리언 형태가 production 생성까지 막는다는 것은 오늘 처음 측정됐다.**

🔵 **키는 끈적이지 않는다.** 착수 전 위험으로 적었던 *"vercel.json 이 아니라 프로젝트
설정으로 눌러앉아 소유자 대시보드가 필요해진다"* 는 **반증됐다** — 커밋에서 키를 빼자
1분 만에 배포가 돌아왔다.

## 🔵 곁가지 관측 — main 의 «취소» 를 «되돌림 실패» 로 읽지 마라

되돌림의 스쿼시 커밋 `64aef3738` 에서 fan 은 `Canceled by Ignored Build Step` 이었다.
정상이다: `scripts/vercel-should-build.sh` 는 `VERCEL_GIT_PREVIOUS_SHA`(**직전 배포**)를
기준으로 트리를 비교하는데, 프로브가 배포를 막았으므로 직전 배포는 `608fe0f46` 이고
그 범위에서 `vercel.json` 은 **더했다 뺐으므로 트리 차이가 0** 이다. 구워도 같은 산출물이다.

⇒ 되돌림이 먹혔다는 증거는 «구워졌는가» 가 아니라 **«상태가 다시 나타나는가»** 다
(키가 있는 동안엔 **0건**이었다). 그리고 브랜치 head 는 실제로 구워졌다.
...
|---|---|
| **AC-0** | ✅ **완료 — 결과 A** |
| **AC-1** Deploy Hook 배선 | 🔴 **소유자 선행**: 각 프로젝트 `Settings → Git → Deploy Hooks` 에서 훅 생성 → URL 을 GitHub Actions **secret** 으로 등록. 그 전에는 워크플로 쪽 절반을 써도 **발사할 대상이 없다** |
| **AC-2** 훅 미발사 가드 | AC-1 뒤 |
| **AC-3** `check-vercel-build-triggers.sh` 전제 재작성 | AC-1 뒤 |
| **AC-4** 안 고쳐지는 것 표 | AC-1 뒤 |
| **AC-5** 효과 재측정 | AC-1 뒤 |
| **AC-6** 브랜치 축 | 🔵 **AC-0 과 독립**이고 지금 잴 수 있다(한도 해제됨). 🔴 다만 ktg 의 결과를 이 계정의 다른 프로젝트로 **이전하지 마라** — `kanggle-fan`·`kanggle-portfolio` 각각에서 다시 물어야 한다 |

🔴 **`web-store` 는 Vercel 프로젝트가 아직 없다**(`TASK-MONO-582`). AC-1 은 그 `vercel.json`
을 어떻게 둘지 **명시적으로 정해야** 하고, 582 가 프로젝트를 만드는 순간 자동 배포가 다시
켜진다는 것이 그 이유다.

## 🔴 이 실측이 부수적으로 드러낸 것 — 별 티켓

`TASK-MONO-601` 로 분리했다. 이 티켓의 되돌림 PR(#3523)에서 **required 4종 중 3종이
`SKIPPED`** 였는데도 머지가 통과했다. 산문으로 남기지 않고 티켓으로 뺀다.

---

# 🔬 AC-6 측정 (2026-08-29 UTC) — 진행 중

## 계측기와 판정 술어를 **먼저** 못박는다

AC-0 과 **같은 계측기**를 쓴다: 커밋에 붙는 commit-status 행 중 context 가 `Vercel – *` 인 것.
🔴 판정 술어는 «구워졌는가» 가 **아니라 «행이 존재하는가»** 다. `ignoreCommand` 가 취소한
배포도 행을 남기므로(오늘 origin/main tip 이 그렇다), `git.deploymentEnabled` 의 억제는
**행 자체의 부재**로만 나타난다. 두 기전은 다른 지점에서 작동한다.

## 🔴 이 저장소에서는 `*` 가 **한 개도 못 잡는다** — 실측으로 확인하고 시작한다

ktg 가 경고한 minimatch 규칙(`*` 는 `/` 를 넘지 않는다)이 여기서는 더 강하게 걸린다.
최근 머지 PR 20 건의 head 브랜치 이름을 세었다:

| 형태 | 건수 |
|---|---:|
| 슬래시 정확히 1개 (`docs/…` `task/…` `chore/…` `feat/…` `spec/…`) | **20 / 20** |
| 슬래시 없음 | **0 / 20** |

⇒ `{"*": false}` 만 넣으면 **설정은 들어간 것처럼 보이면서 아무것도 물지 않는다.**
`**` 가 이 저장소에서는 선택이 아니라 **유일하게 무는 규칙**이다.
[[feedback_why_a_guard_does_not_bite]]

## 착수 조건 — 한도가 열려 있는가 (AC-0 과 동일)

`origin/main` tip `84f852ed4` 에서 **두 프로젝트 모두 06:44Z 에 행이 생성**됐다
(`success :: Canceled by Ignored Build Step`). ⇒ 생성이 되고 있다 ⇒ 부재를 키에 귀속시킬 수 있다.

## 오늘의 예산 — 재고 시작한다

`08-29T00:00Z ~` 이 저장소 **main 축만 9 행**(커밋 5개: 2·2·1·2·2 — `e6f6a6d59` 가 1인 것은
AC-0 이 fan 을 막고 있던 그 커밋이다). 🔴 브랜치 축과 `korea-travel-guide` 는 여기 없다.
계정 전체 100/일 카운터의 잔량은 이 계측기로 알 수 없다(`TASK-MONO-587` 이 소유자 사안인 이유).

## 설계 — 3단, **매 푸시마다 동시 대조군**

| 푸시 | 브랜치 | fan 키 | 론처 키 | fan 기대 | portfolio 기대 |
|---|---|---|---|---|---|
| **P1** | `task/mono-590-ac6-branch-axis` | 없음 | 없음 | 행 **있음** | 행 **있음** |
| **P2** | 같은 브랜치 | **객체** | 없음 | 행 **없음** | 행 **있음** ← 동시 양성 대조군 |
| **P2'** | `preview/mono-590-ac6-fan-hatch` | **객체** | 없음 | 행 **있음** ← 해치 | 행 **있음** |
| **P3** | 같은 브랜치 | 제거 | **객체** | 행 **있음** ← A–B–A | 행 **없음** |
| **P3'** | `preview/mono-590-ac6-portfolio-hatch` | 제거 | **객체** | 행 **있음** | 행 **있음** ← 해치 |

- 🔵 **키를 두 프로젝트에 동시에 넣지 않는다.** 그러면 부재의 원인이 키인지 한도인지 다시
  구별 불가가 된다 — 한쪽이 항상 배포를 **받는 것**이 그 푸시의 대조군이다.
- 🔵 **P2' / P3' 는 P2 / P3 와 같은 시각에 민다.** 같은 순간에 한 브랜치는 막히고 다른
  브랜치는 열리는 **반대 쌍**이 「그때 Vercel 이 그냥 다 굽고 있었다」를 배제한다.
- 🔵 **P3 가 fan 의 A–B–A 를 닫는다** — 키를 빼면 행이 돌아오는지. AC-0 에서 키가 끈적이지
  않음을 확인했지만 객체 형태는 **다른 키 모양**이라 다시 묻는다.
- 🔴 `kanggle-fan` 과 `kanggle-portfolio` 를 **각각** 묻는다. ktg 의 결과도, 서로의 결과도
  이전하지 않는다.

## 판정 — 8칸, **모든 칸에 같은 순간의 대조군**

| # | 브랜치 | fan 키 | 론처 키 | `kanggle-fan` | `kanggle-portfolio` | 이 칸이 답하는 것 |
|---|---|---|---|---|---|---|
| **P1** `e22a1d9fd` | `task/…branch-axis` | 없음 | 없음 | **있음** | **있음** | 기준선. PR 없이도 브랜치 푸시가 양쪽을 만든다 · 한도 열림 |
| **P2** `b7336386a` | `task/…branch-axis` | 객체 | 없음 | **없음** | **있음** | fan 억제. 슬래시 1개 이름이므로 문 것은 `**` |
| **P2'** `91f77274a` | `preview/…fan-hatch` | 객체 | 없음 | **있음** | **있음** | fan 해치. P2 와 **같은 푸시·2초 차** |
| **P3** `c246c5ed6` | `task/…branch-axis` | 없음 | 객체 | **있음**(빌드 완료까지) | **없음** | portfolio 억제 + fan 의 A–B–A 되돌림 |
| **P3'** `3a5b4c101` | `preview/…portfolio-hatch` | 없음 | 객체 | **있음** | **있음** | portfolio 해치 |
| **P4** `592347083` | `mono590ac6noslash` | 없음 | **`**` 만** | **있음** | **없음** | `*` 없이도 **슬래시 없는** 이름이 잡힌다 |
| **R1** 같은 SHA | `preview/…same-sha` | 없음 | `**` 만 | (기존 행 그대로) | **없음** | 같은 커밋을 두 번째 ref 로 밀어도 **안 살아난다** |
| **R2** `47f844124` | 같은 브랜치 + 새 커밋 | 없음 | `**` 만 | **있음** | **있음** (15초) | 작동하는 우회로 |

🔵 **부재 칸마다 같은 순간에 배포를 받는 대상이 있었다.** P2 에서 fan 이 조용한 그 2초 뒤에
fan 은 옆 브랜치에서 배포를 **만들고 있었다** — 「그때 Vercel 이 조용했다」·「한도였다」가
시간이 아니라 **구조로** 배제된다. 행 도착은 통상 2초이고, 부재 창은 P2 4분+ · P3 11라운드
(3분40초) · P4 7라운드였다. [[feedback_absence_verdict_from_a_proxy_is_not_a_measurement]]

### 결론 넷

1. **객체 형태는 브랜치 배포의 «생성»을 막는다.** `kanggle-fan`·`kanggle-portfolio`
   **각각에서** 물었고 각각 참이다. ktg 의 결과도, 서로의 결과도 이전하지 않았다.
2. 🔴 **`"*": false` 는 이 저장소에서 죽은 규칙이다 — 넣지 않는다.** 최근 브랜치 20/20 이
   슬래시를 1개 가져 `*` 는 하나도 못 잡고, P4 가 `**` **단독으로** 슬래시 없는 이름까지
   잡는 것을 보였다. ktg 는 둘을 같이 넣어 귀속이 불가능했고, 여기서 분리해 쟀다.
   죽은 규칙을 남기면 「설정은 들어갔는데 무는 것은 하나뿐」인 상태가 문서에 숨는다.
   [[feedback_why_a_guard_does_not_bite]]
3. **동점 규칙(하나라도 `true` 면 배포)이 두 프로젝트에서 성립한다.** `preview/…` 는
   `**: false` 와 `preview/*: true` 에 **동시에** 걸리고 배포됐다. `main` 이 `main: true` 로
   살아남는 것도 같은 규칙에 기댄다 — 🔴 다만 아래 §되돌림 계획을 보라.
4. 🔴🔴 **순진한 우회로는 틀렸다.** 「같은 커밋을 `preview/` ref 로 다시 민다」는 **작동하지
   않는다**(R1 — Vercel 은 커밋 SHA 로 중복 제거한다). 작동하는 것은 **`preview/` 브랜치 위의
   새 커밋**(R2)뿐이다. 이 문장을 재지 않고 규칙에 적었다면, **정말로 프리뷰가 필요한
   순간에만** 실패하는 처방이 됐을 것이다. [[feedback_a_verifiable_mechanism_is_not_the_cause]]

## 🔴 대가 — 08-28 의 「잃을 것이 이미 없다」는 **오늘 반증됐다**

§재계수 ③ 은 「오늘 만들어진 프리뷰 URL 은 0 개다」로 이 대가를 0 으로 매겼다. 상속하지 않고
다시 셌더니 **오늘 머지 5건 중 3건이 web/론처 경로를 건드렸고, 그중 2건은 실제로 구워졌다**
(`#3520`·`#3523` = `Deployment has completed` + 실 URL). 「16 중 0」은 성질이 아니라
**그날의 작업 구성**이었다. [[feedback_recount_population_dont_inherit_scope]]
[[feedback_local_proves_behaviour_not_performance]]

🔵 **그런데 표본을 넓히니 결정은 바뀌지 않는다.** 머지 PR 40건
(`08-27T06:48Z ~ 08-29T06:44Z`)의 브랜치 축 전수:

| 프로젝트 | 이벤트 | 🔨 구워짐 | 생성 후 취소(순낭비) | 한도 거절 |
|---|---:|---:|---:|---:|
| `kanggle-fan` | 39 | **2** (5.1%) | 23 | 14 |
| `kanggle-portfolio` | 40 | **0** (0%) | 28 | 12 |
| **합** | **79** | **2** | **51** | **26** |

- **79 중 77(97.5%)이 아무것도 만들지 않았다.**
- **26(33%)은 한도 거절 그 자체였다** — 브랜치 축이 자기가 만든 혼잡의 피해자이기도 하다.
- 🔴 **론처는 40 중 0 을 구웠다.** 이 프로젝트에 대해서는 억제 대가가 **측정상 0** 이다.
- fan 의 대가는 **40 PR 당 2 프리뷰**이고, 그 2 는 `preview/` 브랜치로 회수 가능하다(R2).

## ✅ 결정 — 두 프로젝트 모두 랜딩, `preview/*` 해치를 남긴다

```json
"git": { "deploymentEnabled": { "**": false, "main": true, "preview/*": true } }
```

- 🔴 **이것은 사람 규약을 만든다. 규약은 지켜지지 않는다** — AC-6 이 미리 적으라고 한 그 전제다.
  화면을 봐야 하는 PR 의 작성자가 브랜치를 `preview/…` 로 이름 지을 것을 **기대하지 않는다.**
  대신 회수 절차를 여기 못박는다 — 🔵 **사후에도 된다는 것이 R2 의 값이다**: 빈 커밋 하나를
  얹어 `preview/<이름>` 으로 밀면 프리뷰가 15초 만에 돌아온다.
  🔴 **ref 만 옮기는 것으로는 안 된다**(R1). 반드시 **새 커밋**이어야 한다.
- 🔴 **가드는 만들지 않는다, 지금은.** 「web 경로를 건드리는데 `preview/` 가 아니다」를 무는
  가드는 오늘 3건·40PR 표본 2건의 모집단을 갖는다 — 만들 수는 있으나, 그 가드가 **막을 손해**는
  「프리뷰 없이 머지」가 아니라 「프리뷰를 원했는데 없다」이고 그건 작성자만 안다.
  회수가 두 줄이므로 가드보다 **절차의 거처**가 먼저다. 별 티켓으로 올리지 않고 여기 적는다.

## 🔴🔴 세 번째 `vercel.json` — 이 PR 에 **못 들어갔다**(분류기 차단). 사라지지 않게 여기 적는다

가드가 드러냈다: 이 저장소의 `vercel.json` 은 2개가 아니라 **3개**다 —
`projects/ecommerce-microservices-platform/apps/web-store/vercel.json` 이 있고,
Vercel **프로젝트가 아직 없어서 불활성**일 뿐이다(`TASK-MONO-582`).

🔴 이 티켓의 Edge Case 가 이미 못박았다: *「`web-store` 프로젝트가 생겼다 → 🔴 자동으로 켜지게
두지 마라」*. 지금 그 파일에는 키가 없으므로, 소유자가 582 AC-4 로 프로젝트를 만드는 **그 순간
브랜치 축이 억제 없이 켜진다** — 그리고 `TASK-MONO-575` AC-2 는 이미 「계정이 세 번째 프로젝트를
못 견딘다」로 판정돼 있다.

동일한 최소 형태를 그 파일에도 넣으려 했으나 **자동 모드 분류기가 차단했다**(허용되는 것을
바꾸는 편집이라 게이트에 걸린다 — 경로가 아니라 **편집의 축**이 기준인 그 규칙). 셸로 우회하지
않았다. 패치는 소유자에게 그대로 넘긴다:

```json
// projects/ecommerce-microservices-platform/apps/web-store/vercel.json
// "$schema" 줄 바로 다음에 삽입
  "git": { "deploymentEnabled": { "**": false, "main": true, "preview/*": true } },
```

🔵 **넣어도 오늘은 아무 일도 일어나지 않는다** — 프로젝트가 없으면 Vercel 은 이 파일을 읽지
않는다. 그래서 이 항목은 **측정된 적이 없고**, 프로젝트가 생기는 날 P1–P4 와 같은 방식으로
다시 물어야 한다. 🔴 fan·론처의 결과를 그리로 이전하지 마라 — 이 티켓이 두 번 못박은 그 금지가
세 번째 프로젝트에도 그대로 걸린다.

## 🔴🔴 되돌림 계획 — `main: true` 는 **머지 전에는 잴 수 없다**

브랜치 푸시로는 `main` 규칙이 행사되지 않는다. 동점 규칙 자체는 두 프로젝트에서 실측했지만
(P2'·P3'), 그것은 `preview/*` 로 잰 것이고 `main` 은 **다른 패턴 항목**이다. 추론을 판정으로
쓰지 않는다.

⇒ 이 PR 이 랜딩하는 **스쿼시 커밋에서 두 프로젝트 모두 행이 생기는지**를 즉시 확인한다.
🔴 판정 술어는 「구워졌는가」가 아니라 「행이 있는가」다 — 이 PR 은 론처 pathspec 을 건드리지
않으므로 ignore 스텝이 취소하는 것이 **정상**이다. 행이 **하나라도 없으면** 즉시 되돌린다
(되돌림 커밋을 PR 로 올려 랜딩, 창 목표 10분 이내). 되돌림이 먹혔다는 증거는 「구워졌는가」가
아니라 **「상태가 다시 나타나는가」**다 — AC-0 이 같은 함정을 이미 한 번 밟았다.

## 🔴 AC-4 의 「프리뷰 URL 상실」 행을 여기서 값 매긴다 (AC-6 이 그 행을 소비한다)

그 행의 최종 값: **40 PR 당 프리뷰 2개**, 전부 `kanggle-fan`, 전부 회수 가능(위 두 줄).
`kanggle-portfolio` 의 몫은 **0** 이다. 08-28 이 적은 「0 이라 잃을 것이 없다」도,
착수 시 적었던 「머지 전에 화면 볼 수단이 0 이 된다」도 **둘 다 틀렸다** — 전자는 그날의
구성이었고, 후자는 회수 절차를 몰랐다.

## 측정에 쓴 예산

브랜치 축 프로브 **8 푸시**(P1·P2·P2'·P3·P3'·P4·R1·R2 — 이 중 R1 은 ref 만 이동). 생성된
배포는 **9**(억제로 안 생긴 것 4). 측정 시작 시점에 오늘 이 저장소 main 축은 이미 **9** 였다.
🔴 계정 전체 100/일 중 **잔량은 이 계측기로 알 수 없다**(`TASK-MONO-587` 이 소유자 사안인 이유).

## 프로브 브랜치 정리

`preview/mono-590-ac6-fan-hatch` · `preview/mono-590-ac6-portfolio-hatch` ·
`preview/mono-590-ac6-same-sha` · `mono590ac6noslash` — **머지되지 않는다.** 이 PR 랜딩 후 삭제.
위 표의 SHA 가 근거의 거처이고, 상태 행은 커밋에 남는다.
