...
## ~~AC-3 — 론처: 스토어 링크를 Vercel 로~~ → 🔴 **분리했다 (2026-08-26, 착수 중 실측)**

**이 AC 는 이 티켓에서 뺀다.** 착수 전 조사에서 나는 *"가드 (t) 만 보면 된다"* 고 적었는데,
구현하려고 그 자리를 열어 보니 **가드가 하나 더 있고 그쪽이 훨씬 크다.**

`index.html` 의 `surfaceUrl` 위에 **`GUARD-Z14-BEGIN`** 마커가 있고,
`verify-demo-wrapper.sh` **(z14)** 가 그 구간을 **문자열 grep 이 아니라 실행해서** 네 칸을 본다:

| 칸 | 단언 |
|---|---|
| (1) 도메인 `up` | 링크 활성 + href 가 `demoHost()` 파생 |
| (2) `down`/`unknown` | **비활성, href 자체가 없음** (bite) |
| (3) `partial` | 명시된 결정대로 **비활성** |
| (4) `health_stale` | `up` 이어도 **비활성** (`TASK-MONO-551` 이 만든 필드) |

그리고 (z14) 는 **`data-domain` 이 실재하는 compose 도메인인지**와 **본문에 sslip 호스트가
직접 적히지 않았는지**도 단언한다.

🔴 **스토어 링크를 「항상 살아 있는 정적 Vercel 주소」로 바꾸면 (2)(3)(4)의 전제가 그 행에서
깨진다** — Vercel 판은 데모가 꺼져 있어도 떠야 하기 때문이고, 그것이 이 이관의 **목적**이다.
⇒ **(z14) 를 뒤집어야** 하며, 그 작업은 `TASK-MONO-579` 의 (z9) 뒤집기와 **같은 크기**다.
각주가 아니라 티켓이다.

⇒ **후속 티켓으로 넘긴다.** 이 티켓은 **앱 쪽 (B) 실증**까지만 한다.

🔵 **그래서 방문자 경로는 아직 안 바뀐다** — 여전히 데모 호스트의 스토어를 본다. 숨기지 않는다.
🔵 **191번 산문도 그때 함께 고친다** — 지금 고치면 코드는 그대로인데 주석만 미래를 말하게 된다.
