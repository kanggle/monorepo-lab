...
## CORRECTION (2026-08-29) — AC-0 ①이 답을 받았다. **남은 것은 ② 와 클릭, 둘 다 소유자다**

**정정 대상은 이 파일의 AC-0 과 그것을 인용하는 Failure Scenario 행이다:**
...
**지금 참인 것 — AC-0 ① 은 닫혔다.** 소유자가 **2026-08-29 에 Pro 로 올렸다.**
이 티켓이 그 질문을 세운 이유는 *"세 번째 프로젝트가 한도를 넘길까"* 였고, 그 한도는
`TASK-MONO-575` AC-2 가 **「못 견딘다」**(하한 74 생성배포/24h @ 2개 → 4개면 ≈148 vs 100)로
판정한 그 값이다. **그 우변이 이 계정에 더 이상 걸려 있지 않다.**

🔴 **하지만 «넘길 수 없다» 로 적지 않는다.** 새 한도의 **계정 적용 값**은 여전히 소유자
대시보드가 유일한 출처이고(575 AC-0 의 *"문서 값을 적지 마라"*), 이 저장소는 Vercel 토큰이
없어 카운터를 못 읽는다. ⇒ 정확한 표현은 **「이 축은 더 이상 착수를 막지 않는다」**이지
**「한도가 6,000 이다」가 아니다.**

🔵 **좌변도 같은 날 줄었다.** `TASK-MONO-590` AC-6 이 세 `vercel.json` 전부에
`{"**": false, "main": true, "preview/*": true}` 를 랜딩시켰고(스쿼시 `93a8ce5c8` ·
web-store 몫은 `438fb38e6`), 브랜치 축은 08-28 재계수 기준 이 저장소 몫의 **정확히 절반**
이었다. 🔴 **그러나 web-store 의 그 키는 아직 «측정된 적이 없다»** — 프로젝트가 없으면
Vercel 은 그 파일을 읽지 않는다. 프로젝트가 생기는 날 형제들과 **같은 방식으로 다시 물어야**
하고, fan·론처의 결과를 그리로 이전하지 않는다(590 이 두 번 못박은 금지).

## 🔴 그래서 남은 선행은 **둘, 전부 소유자**다

| # | 남은 것 | 왜 저장소가 못 하나 |
|---|---|---|
| **AC-0 ②** | web-store 를 **자기 프로젝트**로 만들지, 기존 프로젝트의 경로로 얹을지 | 계정 구성 결정이다. 🔵 이 티켓의 기본값은 **자기 프로젝트**(형제 둘이 각자 프로젝트) |
| **AC-4** | Vercel 대시보드에서 **프로젝트 생성 클릭** + 환경변수 입력 | 대시보드 조작이고, `TASK-MONO-562` 가 *"Vercel 대시보드 조작·플랜 변경은 사용자 승인 대상"* 으로 이미 규정했다 |

⇒ **이 티켓의 저장소 몫은 끝나 있었고, 08-29 이전까지 그것을 막던 세 축 중 하나(플랜)가
빠졌다.** 🔴 나머지 둘은 그대로이므로 이 티켓은 **계속 `review` 에 있고 닫히지 않는다.**

🔴 **본문을 지우지 않는다.** AC-0 ① 은 08-26 에 물을 **필요가 있었던** 질문이고, 그것을
물었기 때문에 575·587·590 이 나왔다. 이 절은 덮어쓰지 않고 **답한다** — 정정은 추가이지
재작성이 아니다. [[feedback_one_fact_in_two_sections_only_one_gets_fixed]]
