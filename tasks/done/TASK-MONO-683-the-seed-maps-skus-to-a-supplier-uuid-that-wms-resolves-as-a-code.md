...
done (2026-09-23 UTC — 4차원 검증 · impl PR #3860 squash `a7a8f4e67` · 🔵 AC-4 는 ⚪ **측정 불가 + 갈 곳** 이 AC 자신이 정한 닫는 방식이다 → `TASK-MONO-672` 항목 5)
...
## AC-4 — ① 에서 멈췄다: 승인할 **대상이 없다** (⚪ 측정 불가, 실패 아님)

🔴 **쓰기 승인을 받아 놓고 쓸 자리가 없었다.** `TASK-MONO-672` 항목 5 가 순서를 못박았다 —
*"① `/scm/replenishment` 에 보충 제안이 **한 건이라도 생기는가**… 0건이면 그 사실을 적는다.
🔴 앞 칸이 안 되면 뒤 칸은 «측정 불가» 로 적는다 — «실패» 가 아니다."*

**관측 (08:14 UTC, 테넌트 `demo-corp` 적용 확인 후):**

```
SCM 보충 운영 (재고보충 추천)
  상태: 전체 / SUGGESTED / APPROVED / MATERIALIZED / DISMISSED
  → 표시할 보충 추천이 없습니다.        (표 행 0)
```

⇒ ②(승인 → DRAFT → confirm → `scm.procurement.inbound-expected.v1`)와 ③(wms ASN)은
**측정 불가**다. 승인할 제안이 없으므로 그 경로에 진입할 수 없다.

🔵 **화면이 이유를 직접 말한다**: *"**wms 저재고 알림이 만든** 보충 추천을 검토하고 승인(→
DRAFT 발주 생성) 또는 기각합니다."* ⇒ 제안은 **wms 저재고 알림 또는 IVS 야간 스윕**에서만
생기고, 신선 볼륨 + 시드만 돈 데모에는 그 트리거가 **일어난 적이 없다.**

🔴 **그러므로 다음 창에서 그냥 다시 봐도 또 0건이다.** 이 칸을 열려면 **먼저 제안을 만드는
경로**가 있어야 한다 — 후보 둘: ⓐ 시드가 저재고 상태를 만들어 알림을 유발한다 ⓑ IVS 스윕을
수동 트리거한다. 🔴 어느 쪽도 이 티켓의 범위가 아니다(683 은 «UUID↔코드» 티켓이다).
⇒ **그 선행을 `TASK-MONO-672` 항목 5 에 되돌려 적는다** — 항목 5 는 «측정» 이 아니라
«측정의 선행을 먼저 만들어야 하는 일» 이었다.

## 🔴 이 창에서 내가 한 번 무효 측정을 했다

첫 시도는 테넌트를 `selectOption` 하고 2초 기다린 뒤 바로 `/scm/replenishment` 를 열었고,
화면은 **«테넌트를 먼저 선택하세요»** 였다(본문 172자). `TASK-MONO-707` 이 고친 바로 그
함정을 **내 임시 스크립트가 그대로 밟았다** — 고친 것은 `capture-portfolio.mjs` 이고 그날그날
쓰는 일회용 스크립트는 그 교훈을 물려받지 않는다. 적용 판정(`셀렉트값 == 요구값 && «테넌트를
선택» 부재`) + 경유 재시도를 넣자 한 번에 통과했고(본문 295자) 그 결과가 위 관측이다.
🔵 교훈: **일회용 측정 스크립트에도 적용 판정을 복사해 넣어라.**

---

## CORRECTION (2026-09-23 UTC · close chore 의 4차원 검증이 붙인다)

### ① 왜 `review/` 를 거치지 않았나 — **impl PR 이 그 이동을 빠뜨렸다**

impl PR [#3860](https://github.com/kanggle/monorepo-lab/pull/3860)(squash `a7a8f4e67`,
2026-09-16 머지)이 Phase 1 측정 + Phase 2 구현을 담았으나 **`in-progress → review` 라이프사이클
이동을 함께 넣지 않았다**(§ PR Separation Rule 이 그 둘을 한 PR 의 별도 커밋으로 요구한다).
그래서 이 파일은 일주일 동안 `in-progress/` 에 남았다.

🔵 **없던 `review/` 단계를 지어내지 않는다.** impl 은 일주일 전에 머지됐고 검토를 기다린
적이 없다. 4차원을 지금 직접 재고 `done/` 으로 보낸다 — 거치지 않은 단계를 거친 것처럼
적는 것이 더 나쁜 기록이다.
...
| 차원 | 결과 |
|---|---|
| (a) `state` | **MERGED** (2026-09-16T13:04:59Z) |
| (b) 조상 | `git merge-base --is-ancestor a7a8f4e67 origin/main` → **rc=0** |
| (c) 필수 4종 | `changes` · `INDEX queue drift` · `Task ID collision` · `Walkthrough ledger drift` 전부 **SUCCESS**, 실패 **0** |
| (d) `# Acceptance Criteria` **열어서 읽음** | AC-0·1·2·3 `[x]` + 근거 절 · **AC-4 는 ⚪** ↓ |

### ③ 🔵 AC-4 의 ⚪ 는 **그 AC 자신이 정한 닫는 방식**이다

AC-4 의 문장: *"창이 없으면 **⚪ + 갈 곳(`TASK-MONO-672`)**. 이 칸은 체크하지 않는다 —
판정은 거기서 난다."* 그러므로 상자가 `[ ]` 인 것이 **미완이 아니라 지시대로**다
(CLAUDE.md § 4차원 (d): *"a ⚪ recording 「could not be measured, and why」 **is** the closure
when the AC asked for that"*).

🔵 그리고 그 ⚪ 는 **실제로 창에서 시도됐다** — 2026-09-18 둘째 창이 ①을 재서 **표 행 0**,
그리고 *"제안은 wms 저재고 알림 또는 IVS 야간 스윕에서만 생긴다"* 는 이유까지 기록했다.
«안 해 본 ⚪» 가 아니라 **«해 봤고 선행이 없어서 못 연 ⚪»** 다.

### ④ 🔴 이 닫기가 `TASK-MONO-672` 의 조건 하나를 **해소한다**

672 항목 5 는 수령 당시 이렇게 적었다:

> *"이 집은 «닫히면서 집을 잃는» 의무만 받는다. 683 은 수령 시점에 아직 `in-progress` 이고 …
> **683 이 닫히지 않고 살아 남는다면 이 항목은 683 으로 되돌려야 한다**(의무 이중 보유 금지)."*

683 이 일주일째 살아 있었으므로 그 조건은 **발동 직전**이었다. 이 닫기로 전제가 참이 되고,
항목 5 는 672 가 정당하게 보유한다.

### ⑤ 🔵 항목 5 의 남은 모양 (같은 날 재측정으로 확정)

- ② ③ (승인 → confirm → wms ASN) — **선행이 생기기 전까지 측정 불가.** 다시 열지 마라.
- ① — **0 은 이미 기록됐다**(09-18). 남은 것은 항목 본문이 요구한
  *"`seed-wms.sh` 이후 `SKU-APPLE-001` 가용재고"* **한 줄**뿐이고, 그것이 선행 경로
  (ⓐ 시드가 저재고 유발 / ⓑ IVS 스윕 수동 트리거) 설계의 입력이다.
- 🔴 **신선 볼륨은 필요 없다** — `seed-scm.sh:109` 가 *"683 이전 볼륨에서는 `SUP-001` 이 **새
  행**으로 생긴다"* 라고 적어 뒀고, 그 시드는 구워진 클론(`a7a8f4e67` ⊂ `1d47c5ad6`)에 있다.
