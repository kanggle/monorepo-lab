...
done (2026-09-18 UTC — 4차원 검증 · impl PR #3912 squash `28bb47056`)
...
| 차원 | 값 |
|---|---|
| (a) PR 상태 | `state=MERGED` · `mergedAt=2026-09-18T05:56:32Z` · PR [#3912](https://github.com/kanggle/monorepo-lab/pull/3912) |
| (b) main 팁 | `28bb47056` = 그 PR 의 squash 커밋 |
| (c) 머지 직전 체크 | **실패 0** — 버킷 합 검산까지 했다(44 skipping + 19 pass = **63 = 총계**, 분류 못 한 몫 0). 필수 넷 전부 **실행**(task 파일을 건드린 PR) |
| (d) AC 절 | 아래 — **열어서** verb 별로 읽었다 |

## (d) — verb 별 판독

| AC | verb | 닫힌 근거 |
|---|---|---|
| AC-1 | «고른다(소유자 결정)» | 🟢 소유자가 **ⓐ** 를 골랐다. 내 추천을 결정으로 적지 않았다 |
| AC-2 | «고친다 + 되돌리면 빨개진다» | 🟢 구현 + bite **5종**, 각각 **자기 칸만** 빨강 |
| AC-3 | «창 판정 — IAM 폼이 다시 뜨는가» | 🟢 창에서 쟀고, 그 답이 **교란돼 있었다**는 것까지 대조군으로 갈랐다 |
| AC-0 (2) | «로그아웃 뒤 다시 로그인» | 🟢 위와 같은 측정 |
| AC-0 (1) | 🔴 «**응답 본문의 키 목록**으로 본다» | 🟡 **그 방법으로는 안 했다** — 아래 |

## 🔴 정직성 고지 — AC-0 (1) 은 **AC 가 이름 대지 않은 길**로 답했다

그 칸은 방법을 못박았다: *"응답 본문의 키 목록으로 본다 — 로컬 IAM(Testcontainers/슬라이스)으로
재현되면 그것이 첫 판정, 안 되면 창."* 실제로 한 것은 **둘 다 아니다**:

1. **코드 독해**(2026-09-17, #3907) — 커스텀 프로바이더가 `Map.of()` 를 돌려준다.
2. **창에서의 쿠키 부재**(2026-09-18) — 갱신 뒤 `console_id_token` 이 안 선다.
3. 수정 후 **단위 단언** — `additionalParameters` 에 `id_token` 키가 있다.

🔵 **질문의 답은 확정됐다**(셋이 서로를 지지한다). 🔴 그러나 **HTTP 응답 본문을 뜬 적은 없다** —
3은 프로바이더 경계이지 엔드포인트 본문이 아니다. 그 구별을 지우고 닫지 않는다.
⇒ **넘긴다: `TASK-MONO-672` 항목 6.**

## ⏳ 이 티켓이 들고 있던 남의 의무 — 집을 줬다

`done/` 은 얼어 있고 거기 적은 남은 일은 다시 안 읽힌다. 그래서 **닫기 전에** 넘겼다:

- **`TASK-MONO-672` 항목 6** — ① 토큰 엔드포인트 **응답 본문의 키 목록** ② 수정 후 창 재판정
  (로그인 → 31분 유휴 → 갱신 → 로그아웃 → 다시 로그인). 🔴 **AMI 재굽기가 선행**한다(백엔드 변경).
  🔴 그리고 그 재판정은 **대조군 없이는 또 교란된다** — 이 티켓이 그것으로 판정이 뒤집힌 티켓이다.
