...
- **후속 (ACCEPT 전에는 생성 금지)**: `TASK-MONO-351`~`354` (D7 로드맵). **ADR 이 ACCEPTED 되기 전에는 어떤 코드도 쓰지 않는다.**
...
4. D7 로드맵(`TASK-MONO-351`~`354`)을 **PAUSED** 로 명시.

## Out of Scope

- **`libs/java-gateway` 코드 작성 — 일체 금지.** ACCEPT 전 코드는 곧 ADR 게이트 우회다.
- **`TASK-MONO-351`~`354` 생성** — ACCEPT 후에 spawn 한다.
...
- [x] `ADR-MONO-048` **PROPOSED** 로 작성 (D1~D7 + 대안 + 부정적 결과) — PR #2411 (`ba014464f`)
- [x] D5 4정책 권위 확증 기록
- [x] D6 행동-불변 증명 의무 명문화
- [x] `libs/` 코드 0줄 (AC-7)
- [x] `tasks/INDEX.md` 갱신
- [x] **사용자에게 ACCEPT 여부를 물음** — 자율 진행하지 않았다. **결과: 사용자 명시 승인 `ADR-MONO-048 ACCEPTED` (2026-07-11)** → ADR Status ACCEPTED, § 6 UNPAUSED, `TASK-MONO-351`(D7 step 1) spawn.

**게이트가 실제로 작동했다는 기록 — 이 task 의 진짜 산출물**: "2단계, 추출 착수" 로 진입하려던 순간 `shared-library-policy.md` § Change Rule 에 걸렸고, **코드 대신 결정 문서를 냈다.** PROPOSED 로 물었고, 사람이 결정했다. 에이전트 self-ACCEPT 는 일어나지 않았다.

**task-id 충돌 기록 (동시 세션)**: 이 task 와 다른 세션의 iam auth 계약 task 가 **동시에 `TASK-MONO-349` 를 점유**했고, 그 뒤 **양쪽이 서로에게 번호를 양보하려 했다**(나는 349→350 으로 물러나려 했고, 상대는 자기 것을 349→350 으로 옮겼다). 최종 랜딩한 해법(PR #2412)이 옳다 — **"의존이 걸리지 않은 티켓이 움직인다"**: 이 ADR 은 review/ 에서 PR 체인이 물려 있었고 상대는 미착수 백로그였으므로, 상대가 `350` 으로 이동하고 이 ADR 이 `349` 를 유지했다. 그래서 **D7 추출 체인은 `351` 부터 시작**하고 `350` 은 비어 있다. **교훈**: 동시 세션 환경에서 task-id 는 픽 직전에 `git ls-tree origin/main` 으로 재확인해도 부족하다 — **머지 순간까지 경합한다.** 양보 규칙("의존 없는 쪽이 이동")을 미리 합의해 두는 편이 서로 물러나다 다시 충돌하는 것보다 낫다.
