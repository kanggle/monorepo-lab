...
> **Renumbered.** Authored as `TASK-MONO-349`; a concurrent session landed a different `TASK-MONO-349` first (iam auth contract contradictions, PR #2408), so it keeps the number and this one moved to `350`. The impl commit `ba014464f` (PR #2411) still carries the old id in its subject — un-rewritable post-merge.
...
**게이트가 실제로 작동했다는 기록**: 2단계("추출 착수")로 진입하려던 순간 `shared-library-policy.md` § Change Rule 에 걸렸고, **코드 대신 결정 문서를 냈다.** 에이전트가 self-ACCEPT 하지 않았고, 사람이 결정했다. 이게 이 task 의 진짜 산출물이다 — ADR 본문보다도.
