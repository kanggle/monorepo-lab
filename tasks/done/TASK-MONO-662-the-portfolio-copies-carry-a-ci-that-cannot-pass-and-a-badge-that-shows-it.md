...
# 🟢 Close chore (2026-09-11 UTC) — 네 축 통과 · ⑤ 는 여전히 ⚪ (경계 미도래)

## 네 축

| 축 | 재는 것 | PR #3760 (구현) | PR #3761 (review 이동) |
|---|---|---|---|
| (a) | `state` / `mergedAt` | `MERGED` · `04:54:00Z` | `MERGED` · `05:34:25Z` |
| (b) | squash 커밋이 `origin/main` 의 **조상** | `7fe80a287` 🟢 | `839760f70` 🟢 |
| (c) | required 4종 실패 0 | `changes` · `Task ID collision …` · `INDEX queue drift …` · `Walkthrough limitation ledger drift …` **전부 SUCCESS**, 전체 실패 0 | 같음 |
| (d) | **AC 절을 열어서** 읽음 | 체크박스 전부 `[x]`, ⑤ 는 ⚪ + 사유가 본문에 적혀 있다 ⇒ AC 가 요구한 형태의 닫힘 | — |

🔵 (b) 는 «tip 이 squash 커밋과 같다» 가 아니라 **`git merge-base --is-ancestor`** 로 쟀다 —
그 사이 다른 PR 이 머지됐으므로 tip 동일성 술어는 여기서 성립하지 않는다.

🔵 (d) 의 «임시로 들고 있던 남의 의무» 도 grep 했다. 662 가 다른 티켓에 얹어 둔 항목은
**없고**, 잔여 하나(`ci.yml` 이 여전히 빨갛다)는 이미 **`TASK-MONO-664`** 가 이름으로 들고 있다.

## 🟢🟢 새 증거 — 밀고 난 뒤 **한 번 더 재동기화가 지나갔고, 셋 다 살아남았다**

close chore 를 하면서 사본의 **현재 HEAD** 를 다시 쟀더니 `5b885abfd`
(`2026-09-11T05:27:28Z`, *"restore root build.gradle + rewrite paths for standalone layout"*)
로, **ⓒ+ⓑ 푸시(`7d7241d8c` · `04:53:59Z`) 보다 34분 뒤**였다.

🔴 이것은 이 티켓이 가장 조용히 깨질 수 있는 자리다 — 재동기화는 monorepo-lab 의 내용을
밀어넣고, **monorepo-lab 의 워크플로에는 `schedule:` 이 그대로 있다.** 「사본에서 지웠다」가
다음 동기화 한 번에 되돌아오면 이 티켓은 초록인 채로 틀린다.

**그 HEAD 에서 다시 쟀다:**

| 술어 | `5b885abfd` (재동기화 이후) |
|---|---|
| ① 배지 | 🟢 `[![CI (monorepo-lab)](…/kanggle/monorepo-lab/…)` |
| ② `vercel-deploy.yml` | 🟢 404 |
| ③ `schedule:` (nightly · federation) | 🟢 **0 · 0** |

⇒ 🔵 **기전까지 확인했다**: 그 셋은 사본에 손으로 바른 것이 아니라
[`scripts/sync-portfolio.sh`](../../scripts/sync-portfolio.sh) 의 후처리(`vercel-deploy.yml` 삭제 ·
들여쓰기 인식 `schedule:` 블록 제거)에 있다 ⇒ **생성기가 매번 다시 적용한다.**
「지금 0 이다」보다 **「재동기화를 한 번 통과했는데도 0 이다」** 가 강한 술어이고,
이 칸은 그 강한 쪽으로 닫혔다.

## ④ — 「예정」을 **실측**으로 올린다

밀기 직후 기록은 *"`failure` 예정(지금 `in_progress`)"* 이었다. 그 런은 끝났고
**`2026-09-11T05:27:35Z  ev=push  CI  concl=failure`** 다. 🔴 **이것은 정상이고 고친 것이
아니다** — ⓒ+ⓑ 는 `ci.yml` 을 안 건드렸다. 잔여는 `TASK-MONO-664`.

## ⚪ ⑤ — 다시 쟀고, **여전히 못 잰다.** 그리고 이 티켓이 적어 둔 경계 시각이 틀렸다

- 지금(`2026-09-11T05:49Z`) 기준 `event=schedule` 런 중 ⓑ 푸시(`04:53Z`) **이후**의 것은 **0건**.
- 🔴 **그 0 은 공허하다** — 다음 cron 경계가 아직 안 지났다. 「0 이니까 닫혔다」로 읽으면
  이 저장소가 이름 붙인 *«0건 ≠ 없음»* 을 그대로 밟는다.
- 🔴🔴 **정정**: 위 § 에 *"다음 cron 시각(UTC 18:00 · 19:00)"* 이라고 적었는데 **틀렸다.**
  실제로 관측되는 예약 런은 그 시각이 아니라 **`20:45Z`~`21:35Z`** 대다(GitHub 의 cron 지연
  포함, 5일 전수):

```
2026-09-10T20:45:29Z / 21:24:30Z      2026-09-09T20:51:05Z / 21:29:40Z
2026-09-08T21:01:55Z / 21:34:35Z      2026-09-07T21:25:40Z / 21:54:30Z
2026-09-06T20:18:24Z / 21:02:40Z
```

  ⇒ **경계는 2026-09-11 ~21:35Z** 이고, 그 전에는 몇 번을 재도 공허하다.

- 🔵 **닫는 법**(경계를 지난 뒤 한 줄):

```
gh api "repos/kanggle/scm-platform/actions/runs?per_page=60" \
  --jq '[.workflow_runs[]|select(.event=="schedule" and .created_at>"2026-09-11T06:00:00Z")]|length'
```

  경계를 **ⓑ 푸시 시각** 으로 잡는 것이 옳다(«그 수정 이후에 예약 런이 생겼는가»가 명제다).
  `0` 이면 닫힌다. 🔴 `18:00Z` 를 경계로 쓰지 마라 — 위 정정대로 그 시각엔 cron 이 안 돈다.

- 🔴 **이 ⚪ 를 들고 `done/` 으로 간다.** 사유: 이 AC 가 *"못 쟀으면 ⚪ 로 «왜» 를 적어라"* 를
  요구했고 그것이 닫힘의 형태다. 🔵 남은 판정은 **15시간 뒤 한 줄**이고, 그것이 빨강이면
  새 티켓이지 이 티켓의 재오픈이 아니다(생성기 수정이 되돌아왔다는 뜻이 되므로 반경이 다르다).
