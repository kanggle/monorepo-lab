...
## CORRECTION — `AC-5`(만료 세션·IAM 중단)의 의무를 `TASK-MONO-645` 로 넘겼다 (2026-09-08 UTC)

`AC-5` 는 ⚪ 로 «못 쟀고 왜 못 쟀는지» 를 적었다. 그 사유(EC2 기동 창 필요)는 지금도 유효하고,
소유자가 이번 세션에 EC2 를 켜지 않기로 했다.

🔴 그런데 이 티켓이 `done/` 으로 가면 그 항목은 **다시 안 읽힌다** — `done/` 은 frozen 이고
아무도 거기서 남은 일을 찾지 않는다. 그래서 닫기 전에 **집을 줬다**:

⇒ **`TASK-MONO-645`** 가 이 측정을 넘겨받았다(그 티켓의 ①). 착수 게이트는 그대로다 —
**이것만을 위해 EC2 를 켜지 않는다.**

🔵 자동 검사가 이미 재고 있는 축은 넘기지 않았다: 콘솔 14/14 · 팬 e2e 18/18 · 세 앱 유닛.
넘긴 것은 «만료된 세션 쿠키가 있는 채로 IAM 이 죽은» 조합 **하나**다.
