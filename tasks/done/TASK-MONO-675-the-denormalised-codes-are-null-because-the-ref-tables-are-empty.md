...
done (2026-09-23 UTC — 4차원 검증 · impl PR #3816 squash `dabf5c107` · AC-3 는 **화면**으로 닫혔다(2026-09-17 창) · AC-1 은 **ⓐ 확정**(2026-09-22 창, 토픽 오프셋 0 + `.dlq` 0))
...
| 차원 | 결과 |
|---|---|
| (a) `state` | **MERGED** — impl PR [#3816](https://github.com/kanggle/monorepo-lab/pull/3816), 2026-09-15T08:22:36Z |
| (b) 조상 | `git merge-base --is-ancestor dabf5c107 origin/main` → **rc=0** |
| (c) 필수 4종 | `changes` · `INDEX queue drift` · `Task ID collision` · `Walkthrough ledger drift` 전부 **SUCCESS**, 실패 **0** |
| (d) `# Acceptance Criteria` **열어서 읽음** | AC-0·1·2·3·4 전부 닫힘 ↓ |

### ② 🔵 남아 있는 `[ ]` 넷은 **미완이 아니라 «해당 없음» 이다**

- **AC-0 셋째 칸** *"창이 필요하다 … 못 열면 `ready/` 에 그대로 둬라"* — 창이 **열렸고**
  위 두 칸이 런타임 값으로 `[x]` 가 됐다. 이 칸은 창이 **없었을 때의 지시**다.
- **AC-1 둘째 칸** *"«가장 그럴듯한 것» 으로 고르지 마라 — 술어는 **관측**이어야 한다"* —
  지켰다. ⓐ 는 산문이 아니라 **브로커 관측**으로 확정됐다(2026-09-22 창: 6개 토픽 전 파티션
  끝 오프셋 **0**, `.dlq` **0**).
- **AC-3 셋째 칸** *"창이 없으면 ⚪ + 갈 곳"* — 창이 있었으므로 **발동하지 않았다.**
- 🔴 즉 이 넷은 **「조건이 거짓이라 실행되지 않은 분기」** 이지 빠뜨린 일이 아니다.
  (이 구별을 안 적으면 다음 사람이 `[ ]` 를 세어 «미완 4건» 으로 읽는다 — 710·720·719 가
  같은 자리에서 걸렸고, 719 의 CORRECTION 이 그 대응표를 남긴 이유다.)

### ③ 🟢 AC-3 은 **화면**으로 닫혔다 — API 로 닫지 않았다

AC-3 자신이 *"API 응답에 코드가 실렸다는 것으로 닫지 마라. 콘솔 `/wms/inventory` 의 위치·SKU
열에 「이름 확인 불가」가 **사라졌는가**가 판정이다"* 라고 못 박았고, 2026-09-17 창이 그대로 했다:

```
콘솔 /wms/inventory   위치 칸  WH01-A-01-01-01
                      SKU 칸   SKU-APPLE-001        (이미지를 열어 확인)
대조군                 수정 전 API 응답 — 네 코드 **전부 null**
```

### ④ 🔵 `review/` 를 안 거쳤다 — 사유를 적는다

impl PR #3816 이 `in-progress → review` 라이프사이클 이동을 **함께 넣지 않았다**
(§ PR Separation Rule). 그래서 파일이 8일간 `in-progress/` 에 남았다. impl 은 이미 머지됐고
검토를 기다린 적이 없으므로 **없던 단계를 지어내지 않고** 4차원을 지금 재서 보낸다.
🔴 이것으로 **같은 누락이 이 세션에서 두 번째다**(`TASK-MONO-683` #3860 · 이 티켓 #3816) —
우연이 아니라 **impl PR 이 라이프사이클 이동을 잊는 것이 반복 패턴**이라는 뜻이다.

### ⑤ 🔴 이 티켓이 남긴 것 — 사본이 **다섯 개**가 됐다

고친 방식은 형제 셋이 이미 쓰던 우회(`R__seed_dev_masterref.sql` 로 같은 고정 UUID 를 직접
심기)이고, 그래서 이 UUID 세트는 이제 **master 원본 + 사본 4개**다. AC-2 가 그 위험을 적었고
**이미 갈라져 있는 것도 찾았다**(outbound 의 사본이 master 원본과 불일치)
⇒ `projects/wms-platform/tasks/ready/TASK-BE-588-…` 로 기안됐고 **머지됐다**(`1da127f3b`).
🔵 그 티켓이 살아 있으므로 이 의무는 **집을 잃지 않는다.**
