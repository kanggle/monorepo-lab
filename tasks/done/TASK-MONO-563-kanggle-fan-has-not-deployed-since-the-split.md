...
## ✅ 2026-08-21 16:41 UTC — **AC-3 을 라이브 프로덕션에서 닫았다.** 티켓 종료

`#3417` 머지(squash `8e571a750`) 후 프로덕션이 자기 커밋을 말한다:

```
$ curl -sL https://kanggle-fan.vercel.app/build-info.json
{"commit":"8e571a7503443608d03548545ef87a5fe4510a50","ref":"main"}

$ bash check-fan-fresh.sh
   서빙 커밋 = 8e571a750…   기대 커밋 = 8e571a750…   (origin/main)
   ✔ 신선                                                        rc=0

$ bash check-fan-fresh.sh --self-test
   대조 기준 = d74c5d56b (이전 판) vs 현재 8e571a750
   ✔ 신선 — origin/main (현재)                                    → 0
   ✖ 다름 — 서빙본이 이전 판보다 커밋 1 개 **앞서** 있습니다        → 1
   ── 대조군 결과: 현재기준=0  이전판기준=1                        rc=0
```

**같은 오리진·같은 판정자·기준 둘이 갈렸다.** 이제 *"이 프로젝트의 마지막 성공 배포는
어느 커밋인가"* 를 물을 수 있다 — 이 티켓이 태어난 이유가 그것을 **물을 수 없었기** 때문이다.

## 🔴 정직하게 — AC-0 은 **문자 그대로는 끝내 미충족**이다

AC-0 은 *"`npx vercel inspect <id> --logs` 로 빌드 로그를 실제로 읽어라"* 였다.
**로그는 한 번도 읽지 못했다**(Vercel 인증이 소유자 승인 대상). 원인은 **다른 계측기**로 찾았다:
공개 스키마(`$schema` 는 공개 URL 이다) + 라이브 되돌림 대조군.

🔵 다만 AC-0 이 **막으려던 것은 막혔다.** 그 AC 의 목적은 *"인계된 진단을 쓰지 마라"* 였고,
실제로 인계된 진단은 **반증됐다** — 이 티켓이 세운 *"문서 링크=설정 거부 / 배포 URL=빌드 실패"*
판별표는 두 상태가 **같은 초에 둘 다 달린다**는 원본 관측으로 무너졌다.
⇒ 인계 진단을 그대로 썼다면 엉뚱한 층을 팠을 것이다.

🔴 그래도 **"로그를 읽었다" 고 적지는 않는다.** 대체 계측이 성공한 것과 지정된 계측을 수행한
것은 다른 사건이고, 이 저장소는 그 둘을 섞어 적는 데 값을 치러 왔다.

## 최종 AC 상태

| AC | 상태 |
|---|---|
| AC-0 | **부분** — 지정 계측(빌드 로그)은 **미수행**. 다른 계측으로 원인 확정, 인계 진단은 **반증** |
| AC-1 | **충족** — `main` 프로덕션 배포 성공, `/`·`/artists`·`/login` 200 |
| AC-2 | **충족** — 원인이 저장소에 있고(`VERCEL.md`·`vercel-ignore.sh`·가드 칸 (5)), 되돌림 대조군이 **라이브**로 갈렸다 |
| AC-3 | **충족** — `build-info.json` + `check-fan-fresh.sh` + 자기시험이 **프로덕션에서** 통과 |
| AC-4 | **충족** — 판단을 문구가 아니라 *성공한 배포의 존재*로 한다 |

## 이 티켓이 남기는 한 문장

**경고문을 쓴 변경이 그 경고를 어겼다.** `VERCEL.md` 첫 문단이 *"스키마가 엄격하다"* 고
적어 두고서 같은 파일이 스키마의 **다른 조항**(길이)을 어겼고, 아무 가드도 그 축을 보고 있지
않았다. 규칙을 **적어 두는 것**과 그 규칙을 **재는 것**은 다르다.
