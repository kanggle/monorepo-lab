...
> **완료 (2026-05-31)**: impl PR #974 (squash `ef26d530`). ADR-MONO-019 **런타임 활성화 capstone**. federation-hardening-e2e full-stack(GAP + finance/wms/scm/erp + console-bff + console-web)에 실 고객 `acme-corp` operator 시드(auth_db.credentials tenant_id=acme-corp + admin_operators + role; finance_db acme-corp 계정+잔액) + `loginAsAcmeOperator`(OIDC PKCE, console_active_tenant=acme-corp) + 신규 `entitlement-trust-crossdomain.spec.ts`(overview DomainCard `data-status`: scm/erp **forbidden**[게이트 거부] / finance/wms **not-forbidden**[entitled 통과]; fresh storageState 로 '*' 마스킹 차단). **production code 0**(E2E fixtures+spec). **3차원**(MERGED `ef26d530` / tip 일치 / pre-merge 0). **AC-5 런타임 검증**: `gh workflow run federation-hardening-e2e.yml` → 1차 run 26696990585 = login fixture 이중 tracing.start throw(하니스 회귀, entitlement 미도달) → **MONO-155 fix** → 재실행 run 26697320370 = **9 passed (37.2s) SUCCESS** — 신규 entitlement spec 포함 8 spec 전부 GREEN. **ADR-019 논리 체인 full-stack 런타임 종결**(BE-322 모델 → BE-324 keystone → BE-325 시드 → step3 게이트 4/4 → PC-BE-007 BFF pass-through → **MONO-154 런타임 증명**). **⚠️ 메타(lifecycle 사고)**: 본 task 의 ready→done git mv 가 중단된 close 브랜치에서 working tree 에 carry 되어 **MONO-155 PR #975 에 leak**(done/ 위치는 #975 에, Status→done 은 본 close 에서 정정) — BE-299 re-stage hazard 의 cross-branch 변종. 교훈: 중단할 close 브랜치는 `git checkout -- .` / `git restore` 로 working tree 정리 후 전환(미커밋 git mv 가 다음 브랜치로 따라감). **잔여(별건)**: gap admin operator-scope 게이트 + step 4 cleanup + multi-고객 전환(D3-B, ADR 선행).
