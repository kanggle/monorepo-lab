...
# 🟢 종료 CORRECTION (2026-09-05 UTC) — **AC 가 전부 닫혔다**

🔴 **§ 남은 것 표(위쪽)를 그대로 읽지 마라.** 그 표는 `AC-4b` 를
*"🟡 fan 닫힘 · store 열림"* 으로 적고 있는데, 그 뒤 두 절이 그것을 뒤집었다:

| 절 | 무엇 |
|---|---|
| § AC-4b 후속 (2026-09-03) | store 의 **② 단계(배포 승격)** 가 닫혔다 — 판별은 Vercel 배포 id(`?dpl=`) |
| § AC-4b (2026-09-04, 창 #4 칸 1) | store 왕복이 **홉⑤ 세션까지** — `tenantId=ecommerce`, `roles=["CUSTOMER"]` |

⇒ **AC-0 · AC-1 · AC-2 · AC-3 · AC-4 · AC-4b · AC-5 · AC-6 전부 닫힘.**

🔵 **양성 대조군이 같은 창에 있었다** — 같은 IdP 로 fan 왕복도 세션까지 갔다
(`roles=["FAN"]`). 두 팔이 **같이** 성공했으므로 이 PASS 는 「IdP 가 우연히 관대했다」로
설명되지 않는다.

🔵 **무엇이 고쳤나 — 선언이 아니라 배포다.** env 투입은 창 #3 시점에 이미 있었고, store
경로를 건드리는 배포가 실린 **뒤에야** 왕복이 성립했다. 이 티켓이 스스로 적은
*"env 목록에 있다 = 켜져 있다가 아니다"* 가 **반대 방향으로** 확인된 것이다.

## 🔴 이 종료가 닫지 **않는** 것

- **`ADR-MONO-069` (C2) 의 나머지** — 이 티켓은 IdP 를 `auth.hubwang.com` 뒤에 두는
  구현이었고, 그 위에서 도는 팬 화면의 데이터 축은 **`TASK-MONO-622`** 가 든다.
- **V3 의 `Secure` 없음 / `SameSite` 미설정** — AC-3 이 🟡 실측으로 적어 둔 그대로다.
  평문 HTTP 데모 경로가 남아 있는 한 그 값이고, 바꾸려면 별 티켓이다.
