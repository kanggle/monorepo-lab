...
- [x] 🔴 이 결함은 **30분 이상 살아 있는 스택**에서만 재진다. PR CI 로는 판정 불가다.
- [x] 술어는 **최종 경로의 쿼리까지**다 — `/login` 만 보면 ①과 ②가 구별되지 않는다.
- [x] 🔴 **T+0 대조군을 반드시 같이** 재라(대기 전에 같은 화면이 열렸는가). 660 의 측정이
      그것을 포함했고, 없으면 «원래 안 열리는 화면» 과 구별이 안 된다.
- [x] 창을 못 열면 이 칸을 ⚪ 로 두고 **갈 곳을 적어라**(`TASK-MONO-672`).
...
🔵 **소유자 결정 (2026-09-16) — D1~D5 전부 수용.** 선택창 원문: 질문 «구현이 결정 문구와 다른 곳 5개 … 수용할까요?»
→ 답 **「전부 수용 (추천)」**. 선택지는 «전부 수용 / 일부만 수용 / 전부 되돌리기» 셋이었고 «Other» 기입은 없었다.
⇒ 위 표의 «구현» 열이 이 티켓의 기대 동작이다. 되돌리는 후속 티켓은 없다. 🔴 이것은 **구현 차이의 수용**이지
AC-2(창에서의 런타임 판정)를 닫는 것이 아니다.
...
- 🔵 곁발견(미수정, 범위 밖): `layout.tsx` 의 `getCatalog()` **401** 캐치는 여전히 마커 없는 `buildLoginRedirect()` 다 — 쿠키가 살아 있는데 백엔드가 거절하면 `/login?redirect=` → `/login` 이 쿠키를 보고 `/console` 로 되튕긴다(TASK-PC-FE-278 이 서버 401 지점 53곳에서 고친 모양). `relogin-marker.test.ts` 는 `redirect('/login')` 리터럴만 찾아서 이 호출을 못 본다.
  → 🔵 **받는 티켓 = `tasks/ready/TASK-MONO-690-the-console-layout-catalog-401-still-bounces-without-the-session-expired-marker.md`** (2026-09-15 같은 PR 에서 기안, INDEX ready 행 확인 — 산문 «범위 밖» 으로만 남기지 않는다). 🔴 **번호가 두 번 밀렸다**: 처음 684 로 기안 → 동시 세션 #3822 가 684 를 먼저 머지 → 685 로 옮김 → CI 를 기다린 10분 사이 #3831 이 685 를 먼저 머지(머지 게이트가 «main 에 685 있음» 으로 막았다) → 동시 세션이 빠르게 번호를 가져가는 구간이라 max+1 대신 **버퍼를 두고 690**.
...
세션마다 **따로 로그인해 손대지 않은 저장 상태**를 썼다(한 세션을 열어 재면 토큰이 갱신돼 더는 유휴가 아니다). 모두 테넌트 `demo-corp`(다른 테넌트로 갔다가 돌아오는 왕복 선택). 쿠키 만료 실측: `console_access_token` · `console_id_token` = 로그인 +30분, `console_operator_token` = +60분, `console_refresh_token` = +30일.

| 시나리오 | 로그인 | 측정 (UTC) | 최종 URL(쿼리까지) | 판정 |
|---|---|---|---|---|
| **T+0 대조군** | 09:02:47 | 09:03:18 | `/wms/inventory` | 🟢 대기 전 같은 화면이 열렸다 |
| **T+30 유휴 뒤 링크 클릭** | 09:02:47 | 09:36:06 (액세스 토큰 만료 09:32:59 뒤) | `/wms/inventory` · 실제 데이터(`SKU-APPLE-001` 1행, 이미지 확인) | 🟢 조용한 갱신 |
| **훼손한 리프레시 쿠키** (T+30, 값 끝 6자 변조) | 09:04:37 | 09:36 | `/login?error=session_expired&redirect=%2Fwms%2Finventory` | 🟢 표지 + 돌아갈 경로(D1) — 루프 없음 |
| **두 탭 동시 이동** ×2회 (T+30) | 09:04:58 | 09:36 | 두 탭 모두 요청한 화면(`/wms/inventory` · `/scm/procurement`) | 🟢 동시 갱신(`reuse=false`)에도 세션 유지 |
| **T+60 유휴 뒤 링크 클릭** (운영자 토큰까지 만료) | 09:04:15 | 10:06:07 | `/wms/inventory` · 실제 데이터(이미지 확인) | 🟢 조용한 갱신 |
