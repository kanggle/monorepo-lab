...
- 손대지 않은 트리(파킹된 main 체크아웃)에서 `build-bundled-snapshots.mjs --check` → **`rc=0`**.
- 그 트리의 `fan.json`: 글 **12** · 공개 **6** · 사진 있는 글 **0** — 기안 표와 같다.
- `imageUrls` 렌더 코드: `src/` 에서 시험·주석 외 **0건** — 기안 표와 같다.

## AC-1 — 공개 글이 사진을 갖는다

- 변경 후 `fan.json`: 공개 **6/6** 이 사진을 갖고 총 **9장**, 두 장인 글 **3**(루미·노아·유노). 잠긴 글의 사진 **0**.
- 🔴 주소는 **넣는 문자열 그대로**(`?w=1200&h=675&q=80&auto=format&fit=crop`) 찔렀다 — 9개 전부 `200 image/jpeg`(60~211KB).
- 🔴🔴 후보 **22장을 전부 열어 보고** 골랐다. 제외 2장: 관객 얼굴이 또렷한 군중 사진 · 사람이 누워 있는 사진.
  남긴 것은 악기·무대·스튜디오이고 글 내용에 맞췄다(드럼 노트 → 드럼·믹싱 콘솔, 재즈 편곡 → 색소폰·악보).
- 음성 대조군 `b004`(잠긴 글 + MinIO 주소)는 **그대로** 두었고, 새 시험이 «지울 사진이 실제로 있었다» 를 양성 대조군으로 단언한다.
- 🔵 헬퍼 `PHOTO` 를 `RAW_POSTS` **앞**에 선언했다 — 상품용 `U` 는 뒤에 선언돼 여기서 쓰면 TDZ 로 모듈 로드가 죽는다.

## AC-2 — 봉투가 이상한 사진 주소를 거부한다

`contract.mjs` — 공개 글 `imageUrls` 가 배열이 아니거나, 원소가 `https://<host>/…` 가 아니면 거부.
시험: 거부 **7칸**(`http:` · `javascript:` · `//host` · 호스트 없음 · 숫자 · 문자열 · 키 부재) + 통과 대조군 **2칸**(정상 주소 · 빈 배열).
🔵 키 부재를 «0장» 으로 읽어 주지 않는다 — 발행자의 누락이 조용히 통과한다.

## AC-3 · AC-4 — 화면

| 칸 | 결과 |
|---|---|
| 잠긴 카드·상세에 사진 실린 **불가능한 입력** | `<img>` **0**, HTML 에 주소 문자열 **없음** |
| 대조군: 같은 입력을 `locked:false` | 카드 1장 + alt, 상세 전부 + 장마다 다른 alt |
| 카드에 3장 | `<img>` **1** + `+2` |
| 사진 0장 공개 글 | 틀 없음, 본문 그대로 |
| 실제 시드로 그린 피드 | `<img>` 수 = 사진 있는 공개 글 수(**6**), 전부 `data-locked="false"` 안 |
| `onError` | 틀까지 사라지고 카드 제목·본문은 그대로 |

🔴 `PublicPostImage` 는 `src`·`alt`·`frameClassName`·`badge` 만 받는다 — `post` 가 RSC 페이로드에 안 실린다.

## AC-5 — 공개 경로의 성질

- `public-pages.test.tsx`(익명 렌더 fetch **0건** 포함) 그대로 초록 — 아래 264 에 들어 있다.
- `next.config.ts` 는 이 diff 에 **없다**(`images.unoptimized: true` 유지).
- 생성기 누출 대조군 fan **29건** 그대로 문다.

## AC-6 — 게이트 (각각 독립 실행, 명시 rc)

| 게이트 | 결과 |
|---|---|
| `node --test infra/demo/public-data/tests/public-data.test.mjs` | ✅ **33/33** (기존 31 + 신규 2) |
| `build-bundled-snapshots.mjs --check` | ✅ rc=0 (store·console 은 줄바꿈만 달라 스테이지 diff 0) |
| `fan-platform-web` `vitest run` | ✅ 파일 **32/32** · 시험 **264/264** |
| `tsc --noEmit` | ✅ rc=0 |
| `next lint --dir src` | ✅ 경고·오류 0 |
| `next build` | ✅ rc=0 |
| 필수 가드 3종(스테이지 후) | ✅ rc=0 |
| 공개 카드 선택자에 기대는 e2e(나이틀리 포함) | 저장소 전체 grep **0건** |

## ⚪ 미측정 — 머지 전에는 잴 수 없다

1. **라이브 `fan.hubwang.com` 에서 사진이 깨지지 않고 그려지는가** — 머지 → Vercel 배포 뒤. 🔴 배포 시각이 머지 시각 뒤인지 먼저 본다.
   이 칸의 결과는 **close chore 에서** 기록한다(`review/` 는 frozen).
2. 실제 브라우저 렌더 — 위 시험은 jsdom 이다. 로컬 `next start` 는 띄우지 않았다: 이 호스트에서 남은 서버가
   디스크를 잠그고 에이전트가 종료할 수 없는 사고가 있었다. 1번이 이 칸을 함께 잰다.

## CORRECTION — 위 § 미측정 두 칸은 라이브로 닫혔다 (2026-09-15 UTC, close chore)

위 절의 «⚪ 미측정» 은 그때의 사실이고 지우지 않는다. 지금 참인 것은 다음이다.
impl PR [#3808](https://github.com/kanggle/monorepo-lab/pull/3808) · 스쿼시 `d78a8fb2e` · 머지 `06:51:55Z`.

### 🔴 «배포 워크플로 성공» 은 «배포됨» 이 아니었다 — 서빙 중인 판으로 쟀다

`vercel-deploy.yml` 의 `kanggle-fan` 잡은 머지 **15초 뒤** 성공했는데, 로그를 열어 보니 한 일은
배포 훅 호출뿐이었다(`HTTP 201` · `state: PENDING`). 빌드는 Vercel 쪽에서 따로 돈다.
⇒ 판정은 **`fan.hubwang.com/build-info.json` 의 `commit`** 으로 했다 — `06:53:09Z` 에 `d78a8fb2e` 와 **일치**.
🔵 이 커밋이 빌드를 건너뛰지 않는 근거: `vercel-ignore.sh` 의 SPECS 에 `projects/fan-platform/web` 과
`infra/demo/public-data` 가 **둘 다** 있다.

### ① 라이브 HTML — 수가 시드에서 유도한 값과 정확히 같다

| 화면 | 실측 | 기대(유도) |
|---|---|---|
| `/` 첫 페이지 | 카드 10 · 사진 **5** | 발행일 내림차순 10개 중 공개 글 = 유노·리오·하린·세아·루미 **5** |
| `/?page=1` | 카드 2 · 사진 **1** | 노아(공개) + 루미 잠긴 글 ⇒ **1** |
| 합계 | 사진 있는 카드 **6** | 공개 글 **6** |
| `/posts/…b001`(루미) | 사진 **2** | 2장 |
| `/posts/…b004`(잠긴 글) | 사진 **0** · `minio` 문자열 **0** | 음성 대조군의 MinIO 주소가 HTML 에 없다 |
| 사진 주소 직접 요청 | `200 image/jpeg` 154,591B | — |

### ② 실제 브라우저(헤드리스 Chromium) — «HTML 에 있다» ≠ «그려진다»

`img.complete && naturalWidth > 0` 으로 쟀다(끝까지 스크롤해 `loading="lazy"` 까지 요청시킨 뒤):

| 화면 | 사진 | 디코딩 | 깨짐 |
|---|---|---|---|
| `/` 1280px | 5 | **5** | 0 |
| `/posts/…b001` 1280px | 2 | **2** | 0 |
| `/` 400px | 5 | **5** | 0 |

스크린샷 셋을 **열어서** 봤다: 공개 카드에 16:9 사진 + `+1`, 잠긴 카드는 사진 없이 «멤버십 전용»,
상세는 두 장이 나란히, 400px 에서 사진이 카드 폭에 맞춰 줄어든다.

### 곁발견 (이 티켓 밖 — 기존 결함)

- 🔵 400px 에서 **상단 내비가 글자 단위로 줄바꿈**된다(«피/드» · «아티스/트» · «로그/인»). 헤더 컴포넌트의 기존
  모양이고 이 diff 는 헤더를 건드리지 않았다 — 기록만 한다.

⇒ **AC 전부 닫힘 · ⚪ 0.** 로그인 경로의 사진은 `TASK-MONO-679`.
