...
- [x] 🔴🔴 **자격증명이 있는가.** `gh secret list` 로 **재라**(위 실측은 낡는다). 다른 리포에
...
# 🟢🟢 § 동사별 대조 (2026-09-11 UTC · close chore)

> 🔴 이 절에는 체크박스를 두지 않는다 — 위 § Acceptance Criteria 가 유일한 체크 자리다.

🔵 **아홉 칸 전부 «이미 한 일이 기록돼 있는데 칸만 안 찍힌» 부류였다.** 그래도 칸마다
**실물로** 대조했다 — 체크박스나 산문이 아니라 파일과 명령 출력으로.

| 칸 | 동사 | 실물 확인 |
|---|---|---|
| 자격증명이 **있는가**(`gh secret list` 로 **재라**) | 재라 | 🟢 **`PORTFOLIO_SYNC_TOKEN`**(`2026-09-11T09:35:09Z`) 이 실재한다 — 이 close chore 에서 **다시** 쟀다 |
| 토큰 스코프를 적어 **요청하라** | 요청하라 | 🟢 요청했고 소유자가 **fine-grained PAT** 로 발급했다 |
| `command -v docker` 무조건 fail 을 **폴백으로 바꾼다** | 바꾼다 | 🟢 `docker info` 기반 native→docker→둘 다 이름 대며 실패. 🔵 첫 구현이 `command -v docker` 였고 **그 자리에서 잡았다**(CLI 존재 ≠ 데몬 생존) |
| `checkout` 아래에서 가드가 **통과하는지 확인** | 확인 | 🟢 두 잡 모두 `ref: main` + `fetch-depth: 0` — 🔵 **가드를 안 지우고 환경을 맞췄다** |
| 소요 시간을 **적어라** | 적어라 | ⚪ **못 쟀다** — 아래 § |
| 트리거를 **소유자에게 묻는다** | 묻는다 | 🟢 물었고 **ⓐ `workflow_dispatch` 만** |
| 답을 **말 그대로** 적는다 | 적는다 | 🟢 인용부로 |
| `monorepo-workflow.md` § 5 에 **CI 경로 추가** | 추가한다 | 🟢 실재(2곳) |
| 호스트 실행법을 **지우지 마라** | (금지) | 🟢 **안 지웠다**(9곳 남아 있다) |

## ⚪ 남는 것 하나 — 소요 시간, 그리고 **그 집**

🔴 이 호스트에 **네이티브 `git-filter-repo` 도 docker 데몬도 없어** 어느 경로도 못 돌린다.
🔵 그리고 AC 가 *"호스트 실측과 **같은 프로젝트**로 재라"* 고 못 박았으므로 **아무 수나 적지
않았다** — 두 열의 술어와 모집단을 맞춰야 비교가 성립한다.

⇒ **집**: 이 워크플로의 **첫 `workflow_dispatch` 실행**이 이 칸을 닫는다. 그 실행은
7개 리포에 푸시하므로 **소유자가 누르는 동작**이고, 그때 Actions 로그의 잡 소요가 곧 답이다.
🔵 그리고 `TASK-MONO-664` 의 ⚪(«사본 Actions 에서 `skipped` 인가»)도 **같은 실행이 닫는다** —
그 티켓이 이미 이 워크플로를 자기 ⚪ 의 집으로 지목해 뒀다.

🔴 ⇒ **이 티켓을 닫아도 두 ⚪ 가 갈 곳을 잃지 않는다.** 둘 다 «첫 실행» 이라는 같은 사건에
묶여 있고, 그 사건은 소유자의 버튼이다.
