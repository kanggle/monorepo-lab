...
| 축 | 결과 |
|---|---|
| (a) | 수확 **#3765** `90a79db3f` · 정정 **#3768** `7a3b5d5c8` 둘 다 `MERGED` |
| (b) | 둘 다 `origin/main` 의 **조상**(`git merge-base --is-ancestor`) |
| (c) | 둘 다 실패 **0**, required 4종 전부 SUCCESS |
| (d) | 🔴 **AC 절을 열어서 읽었다** — 6칸 전부 `[x]`, 열린 칸 **0** |

🔵 (d) 는 `TASK-MONO-665` 의 close chore 가 **21칸이 열린 채 `done/` 갈 뻔한 것**을 잡은
뒤라 이번엔 **옮기기 전에** 켰다(#3768 에서 수행 기록과 하나씩 대조).

## ⚪ 이 티켓이 남기는 ⚪ 셋 — 전부 «못 쟀다 + 왜» 형태다

1. **AC-3 의 A′(팬발) 토큰** — Auth.js **암호화 세션 쿠키**라 클레임을 복호하지 못했다.
   거부 자체는 재현됐고 `credentials` 가 `fan-platform` 신원을 `…fa02` 로 확정하지만
   **토큰으로 확증한 것은 아니다.** 🔵 `ADR-MONO-072` § Consequences 가 이 ⚪ 를 그대로 인용한다.
2. `/posts/abc-123` 의 404 — 🔵 그것은 **정상**이고 ⚪ 가 아니다(팬 e2e 스펙이 그렇게 적는다).
3. § 수확에 적은 나머지는 전부 닫힌 판정이다.

## 🔵 임시로 들고 있던 남의 의무 — grep 했고, **집이 있다**

| 본문의 표현 | 어디로 갔나 |
|---|---|
| *"결함으로 판정되면 **별도 티켓**으로 기안한다"*(2곳) | 🟢 **`TASK-MONO-666`** — 그 티켓이 `ADR-MONO-072`(ACCEPTED — ⓒ)까지 끌고 갔다 |
| *"다음에 데모가 켜지는 창에 얹어라"* | 🟢 착수 게이트 문구였고, **그 창이 2026-09-11 에 열렸다**(60분) |

🔴 **스코프 표의 틀린 칸은 `done/` 으로 가기 전에 고쳤다**(#3768, `TASK-MONO-666` AC-3) —
`done/` 은 frozen 이라 그 뒤에는 못 고친다.
