...
**2026-09-07 UTC · close chore (review → done)** — 닫으면서 두 가지를 정정한다. 둘 다 이 파일이
`done/` 으로 얼기 **전에** 적어야 하는 것이다(얼면 다시 못 고친다).

### ① 🔴🔴 AC-2 의 「순서」 지시가 **틀렸다** — 핀 커밋은 `apply` **뒤**여야 한다
...
**실행은 이 지시를 따르지 않았고, 안 따른 것이 옳았다**. 구현 § AC-2 가 그 이유를 적고 있지만
「AC 가 시킨 것과 **반대로** 했다」고는 안 적어서, 이 파일만 읽는 다음 사람은 **AC 표의 지시**를
권위로 읽는다. 그리고 그 지시는 해로운 쪽이다:

| | 핀 커밋을 `apply` **앞**에 (AC-2 의 지시) | 핀 커밋을 `apply` **뒤**에 (실제로 한 것) |
|---|---|---|
| 그 창에서 인스턴스가 문 AMI | 옛 세대 | 새 세대 |
| 그 창의 plain 판정 = **CI 가 보는 것** | 🔴 **rc=0 초록** | rc=1 빨강 |
| 그 창의 `--with-aws` 판정 | rc=1 | rc=0 |
| **틀리는 방향** | 🔴 **덜 일하는 쪽** — 초록인데 호스트는 옛 세대를 서빙 중 | 🔵 **더 일하는 쪽** — 빨갛지만 호스트는 이미 새 세대 |

⇒ AC-2 의 지시를 따랐으면 **CI 초록 + 데모 호스트는 옛 세대**라는 창이 열렸을 것이다.
이 창은 추정이 아니라 **관측된 것**이다 — 굽기 직후·`apply` 전에 두 모드를 실제로 돌렸고
plain=rc0 / `--with-aws`=rc1 로 갈렸다. **러너에는 AWS 자격이 없어 CI 는 plain 으로만 돈다.**

🔵 AC-2 의 걱정 자체는 맞다 — 「저장소와 서빙이 어긋나는 창」은 실재한다. 틀린 것은 **어느 쪽이
그 창을 만드느냐**다. 서빙을 바꾸는 것은 `apply` 이므로 핀은 `apply` 를 **따라가야** 한다.
🔴 원인은 추론 실패가 아니라 **안 열어 본 것**이다: `bake.sh` 의 단계 순서
(1 `tfvars` → 2 `apply` → 3 핀 커밋 → 4 판정)가 이미 옳게 적혀 있었고, AC 를 쓸 때 그 스크립트를
열었으면 이 지시를 안 썼다.

### ② 「다음 nightly」가 실제로 무엇이었나 (§ 남은 것 의 마지막 문장)

🔴 **머지가 발사한 nightly 는 그 잡을 안 돌렸다.** `nightly-e2e.yml` 의 세 「watch」 잡은
`if: … && github.event_name != 'push'` 로 막혀 있어서, 머지 push 런 `34056787421` 에서
`Launcher freshness` · `AMI generation` · `Deployed fan surface` 셋이 전부 **skipped** 였다.
런 자체는 `success` 이고 GitHub 은 skip 을 실패로 안 센다 ⇒ **여기서 「초록이었다」와
「돌았다」가 갈린다**. 잡을 세지 않고 런 결론만 봤으면 못 봤을 구별이다.
🔵 skip 은 결함이 아니라 설계다 — 세 잡 다 라이브 배포 상태를 찌르므로 매 push 마다 돌리지 않는다.

🔵 **그 14분 뒤 스케줄 런 `34057525496` 이 같은 sha `e4d4880d2` 에서 돌았고, 14/14 success ·
skipped 0** 이다. `AMI generation` 잡 로그:

```
[자가검사] 같은커밋=0  주입=1  없는커밋=2  오버레이=1(분기적중=1)   (기대 0 / 1 / 2 / 1·1)
           주입 커밋 27fb75425 에서 어긋남 4건 재현 — 판정자가 뭅니다
[실판정]   핀 ami-029c40fb18c63816b · 구운 커밋 3bc182ecd4e8 · 출처 = ami-tag
           구운 세대 3bc182ecd  vs  서빙 세대 e4d4880d2  → ✔ 같은 세대
```

🔴 **초록만 보고 닫지 않았다** — 자가검사가 옛 커밋을 주입해 **옛 어긋남 4건을 그대로 재현**한
다음의 초록이다. 「판정자가 물 수 있다」와 「지금은 안 문다」를 한 런에서 같이 잰다.
⇒ § 남은 것 의 그 문장은 **닫혔다**. CI 축에 남은 확인 없음.

### 남는 의무

**없다.** ① 은 이 파일 안에서 끝나는 기록 정정이고(AC-2 는 요구한 일 자체는 닫혔다 — 틀린 것은
그 안에 끼워 넣은 순서 조언이다), ② 는 실측으로 닫혔다. AC-5 가 적은 대로 8차 AMI
`ami-0fc6e4b7ac34c4ef5` 를 롤백 경로로 유지하는 것은 **상태이지 열린 작업이 아니다**.
