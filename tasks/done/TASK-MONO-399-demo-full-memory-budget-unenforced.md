...
**AC-0 ~ AC-6 전부 완료.** 이 티켓은 `ready/` 에 남아 있었지만 그것은 라이프사이클 미종결이지
미완 작업이 아니었다 — 마지막 AC(AC-2 clean 재측정)는 2026-08-21 에 끝났다.

| AC | 상태 | 어디서 |
|---|---|---|
| AC-0 인계 숫자 재측정 | ✅ 달랐다(리밋 선언 2개 프로젝트/35 lines, full 86 선언/98 컨테이너) | § AC-0 |
| AC-1 재시작 근인 분류 | ✅ erp = **Flyway 체크섬 충돌**(메모리 아님) → `TASK-ERP-BE-035`(done) | § AC-1 |
| AC-2 `full` 실제 메모리 | ✅ **clean 재측정 2026-08-21** (크래시루프 0) | § AC-2 |
| AC-3 512M→1G 인과 | ✅ 데모 호스트에서 확인 | § AC-3 |
| AC-4 KRaft 손상 인과 | ✅ **미증명으로 종결**(세 번째 추측 없음) | § AC-4 |
| AC-5 비용 | ✅ | § AC-5 |
| AC-6 재굽기 | ✅ **2026-08-17** (`TASK-MONO-477` AC-8 과 한 bake 로 병합) | 아래 § |

## 🔴 이 파일을 읽을 다음 사람에게 — 아래 본문에 **낡은 기록 두 개**가 있다

같은 사실이 두 절에 있으면 한쪽만 고쳐진다. 지우지 않고 **무효 표시**만 한다(경위가 근거다).

1. **§ "AC-6 — 연기 (blocked on ERP-BE-035)"** — **무효.** 그 제목이 붙은 뒤 게이트가 두 번
   갱신됐고(2026-07-28 · 2026-08-15) AC-6 은 **2026-08-17 에 실제로 완료**됐다. `ERP-BE-035` 는
   `projects/erp-platform/tasks/done/` 에 있다. 제목만 보고 *"아직 막혀 있다"* 로 읽지 마라.
2. **§ AC-6 완료 기록의 "남은 뒷정리(사용자 결정 대기)"** — **해소됨.** `TASK-MONO-477`(done)이
   기록: 여기 지목된 옛 AMI `ami-0b6b962d3f3f23865` + 스냅샷 `snap-0e96353c6bb20a2e8` 은
   **이미 존재하지 않았다**(실측). `terraform destroy` 는 **하지 않는 것으로 명시 결정**(온디맨드
   설계가 이미 비용을 묶는다). 뒤집으려면 새 티켓.

## 🔴🔴 AMI 세대 — 이 티켓이 적은 AMI 는 **현재 것이 아니다**

이 파일 안에서만 AMI 가 세 개 등장하고 전부 낡았다. **목록을 물려받지 말고 `terraform.tfvars`
에서 다시 읽어라.**

| 세대 | AMI | 대상 커밋 | 기록한 티켓 |
|---|---|---|---|
| 동결(as-baked) | `ami-0b6b962d3f3f23865` | `f5288a7b1` | 이 티켓 § 실측 결과 |
| AC-6 재굽기 | `ami-008695099ec898477` | `d7ded4429` | 이 티켓 § AC-6 완료 |
| AC-2 clean 측정 | `ami-0c768f12eb9a024ce` | `270ed172f` | 이 티켓 § AC-2 |
| **현재** | **`ami-08df900798bc99c83`** | **`e632e3b54`** | `TASK-MONO-552` AC-4 (done) |

## 🔵 후속 소비자 — `TASK-BE-581` AC-4 가 재굽기를 기다린다

2026-08-22 UTC 실측: 현재 AMI 의 대상 커밋 `e632e3b54` 이후 `origin/main` 은 **5 커밋** 앞서 있고,
그중 `projects/` 를 건드린 것은 **정확히 1건** — `def5fe981`(`TASK-BE-581`, auth-service)이다.

⇒ 데모 호스트는 앱 소스 기준 **딱 그 한 커밋만큼** 낡았고, `TASK-BE-581` AC-4(콘솔 로그인 화면에
회원가입 링크가 없는지 브라우저로 판정)는 **그 커밋이 이미지에 들어가야** 잴 수 있다. 지금 켜면
581 이전 화면이 떠서 *"고쳐지지 않았다"* 와 **구별되지 않는다**(`check-image-freshness.sh` 가
겨누는 바로 그 모양). 다음 `packer build` 승인 시 581 AC-4 를 같은 기동에 묶으면 기동 1회로
끝난다. 🔵 이 티켓은 그 재굽기의 **소유자가 아니다** — 사실만 남긴다.
...
## ~~AC-6 — 연기 (blocked on ERP-BE-035)~~ — 🔴 **무효 (2026-08-22)**

> 이 절은 2026-07-23 시점의 판단이다. **AC-6 은 2026-08-17 에 완료**됐고 `ERP-BE-035` 는 `done` 이다.
> 아래 본문은 경위 보존용으로 남긴다 — **현재 상태로 읽지 마라.** 파일 상단 § 종결 참조.
