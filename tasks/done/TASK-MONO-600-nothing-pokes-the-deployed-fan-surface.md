...
- 🔴🔴 **첫 야간 런의 ②축은 빨간색이 «맞다»**, 그리고 그것은 이 가드의 결함이 아니다.
  ...
  🔵 **해소 조건**: 팬 트리거 경로를 건드리는 커밋이 하나 **빌드까지** 되면 ②가 초록이 된다.
  ①축은 지금도 초록이다.
  🔴🔴 **[정정] 이 자리에 처음 적었던 «가장 이른 창 = 2026-08-29 14:48Z» 는 틀렸다.**
  그 값은 Vercel 이 준 문구 *"retry in 24 hours"* 를 실패 시각(08-28 14:48Z)에 그대로
  더한 것이었다 — **문구를 재지 않고 믿었다.** 실측: 08-29 **04:53Z** 에 이미 배포가
  다시 **생성**되고 있었다(PR #3520 의 `Vercel – kanggle-fan` = 생성 후
  `Canceled by Ignored Build Step`). ⇒ 카운터는 롤링 24h 가 아니라 **UTC 달력일**로
  리셋되는 것에 부합하고, 내 추정은 약 **10시간** 늦었다.
...
# 🟢 close chore 기록 (2026-08-29 UTC) — **위에 적어 둔 «첫 야간 런은 RED» 는 이미 해소됐다**

🔴 **판정 문단의 「남은 것」 첫 항목이 틀린 채로 굳지 않도록 여기서 정정한다.**
그 항목은 *"matcher 고침이 배포돼야 ②축이 초록이 되는데 팬 배포가 한도로 막혀 있으니 첫
야간 런은 RED 이고 그것이 맞다 — 끄지 마라"* 였다. **그 조건은 머지 직후 사라졌다.**

무엇이 바뀌었나: 이 티켓의 impl PR([#3520](https://github.com/kanggle/monorepo-lab/pull/3520))
자체가 팬 트리거 경로를 건드렸고, 한도가 이미 풀려 있어 스쿼시 커밋 `608fe0f46` 의 배포가
**실제로 구워졌다** — `Vercel – kanggle-fan` = `Deployment has completed`
(2026-08-29 **05:17:05Z**). 🔵 대조: 같은 푸시의 `kanggle-portfolio` 는
`Canceled by Ignored Build Step`(그 경로를 안 건드렸으므로 정상).

**그래서 두 축을 라이브에서 다시 쟀다**(추정하지 않고):

```
①축  /nonexistent-xyz 307 → /login?from=%2Fnonexistent-xyz
     /artists 307 · /me 307 · /login 200(음성 대조군) · /api/auth/providers 200(설정 대조군)
     ⇒ exit 0
②축  서빙 커밋 = 608fe0f467db616f94e59940ffea96c3bace03c9
     기대 커밋 = 608fe0f467db616f94e59940ffea96c3bace03c9  (origin/main 트리거 tip)
     ⇒ exit 0  «신선»
```

🔵 **②축이 초록이 됐다는 것 자체가 이 티켓이 고친 결함의 증거다** — `build-info.json` 의
matcher 제외가 배포에 도달했다는 뜻이고, 그 전까지 그 판정자는 로그인 HTML 을 읽으며
08-27 부터 «판정 불가» 였다.
...
| 차원 | 값 |
|---|---|
| (a) | `state=MERGED`, `mergedAt=2026-08-29T05:16:10Z`, mergeCommit `608fe0f46` |
| (b) | `origin/main` tip = `608fe0f46` — 스쿼시와 일치 |
| (c) | 머지 전 실패 **required 0건** (전체 40 SUCCESS · 실패/보류 0) |

머지 후 `main` CI = **실패 0건**(non-skipped 전부 success). 🔵 머지된 blob 이
**실행으로 검증한 로컬 파일과 바이트 동일**함도 `cmp` 로 확인했다 — 백틱 수정이 그대로 갔다.

## 🔴 이 티켓을 하며 내가 틀린 것 둘 (다음 사람을 위해 남긴다)

1. **편집한 셸 분기를 로컬에서 한 번도 실행하지 않았다.** 큰따옴표 안 백틱이 명령 치환으로
   실행돼 `/login`·`src/middleware.ts`·`build-info.json` 을 **실제로 실행**했고, 메시지가
   깨진 채 나왔다. 🔴 `bash -n` 은 이것을 못 본다(문법상 유효하다). 잡은 것은 디스패치 런이고,
   형제 전수를 돌리니 **저장소의 다른 모든 스크립트는 이미 이스케이프하고 있었다** — 어긋난
   것은 내 4줄뿐이었다. 그중 하나는 결핍 발견 분기라 **가장 필요한 순간에** 터졌을 것이다.
   [[feedback_grep_the_siblings_before_fixing_it_yourself]]
2. **문구를 재지 않고 믿었다.** Vercel 의 *"retry in 24 hours"* 를 실패 시각에 더해
   «가장 이른 창 = 08-29 14:48Z» 라고 **두 번** 말했다. 실측은 **04:53Z** — 약 10시간
   빗나갔고, 카운터는 롤링 24h 가 아니라 **UTC 달력일** 리셋에 부합한다.
   [[feedback_verify_claims_against_the_source_itself]]

## 남은 것 (이 티켓이 안 고친 것)

- 🔴 `korea-travel-guide` · `web-store` · `console` 표면은 이 잡이 **안 잰다**. 팬에서 한 번
  돌려 본 뒤 일반화하기로 한 범위 그대로다 — 그리고 이제 «한 번 돌려 봤다» 가 충족됐다.
- 🔴 **인증에 성공한 사용자 경로**는 여전히 아무도 안 잰다. ①축이 재는 것은 «미인증이
  막히는가» 뿐이고, 로그인이 끝까지 도는지는 `TASK-MONO-574`(OIDC 왕복)의 축이다.
- 🔵 실패 알림의 강도는 이 워크플로의 다른 잡들과 **같다**(배지 + 소유자 메일). 이슈
  자동생성/웹훅은 `ADR-MONO-011 § 6.1` 의 미해결 v2 로 남는다.
