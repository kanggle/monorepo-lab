...
## CORRECTION (2026-08-29) — 선행 표의 1번은 **이제 거짓이다**

이 파일은 동결이므로 위를 고치지 않는다. 아래는 **지금 참인 것**이고, 위에 적힌 관측을
반증하지도 대체하지도 않는다 — 그때는 그것이 사실이었다.

### 무엇이 거짓이 됐나

§ 선행 표 1번은 *「`ADR-MONO-068` 승격 트리거가 이 티켓에서 발화한다 → 🔴 소유자 결정 사안」*
이라 적고, § 「이 티켓은 `ADR-MONO-068` 의 승격 트리거를 발화시킨다」는 *"지금 해석기를 가진
앱은 **하나**(web-store)"* + *"이 티켓은 고르지 않는다 ⇒ AC-0 에서 멈추고 올린다"* 로 끝난다.

**셋 다 해소됐고, 해소한 것이 이 티켓 자신이다.**

| 그때 적은 것 | 오늘 |
|---|---|
| 트리거가 **발화할 것이다** | ✅ **발화했다** — 이 티켓이 두 번째 해석기를 썼다 |
| 🔴 소유자 결정 사안 | ✅ **2026-08-26 에 `C` 로 지정됨** (`ADR-MONO-068` History (2) · § D5) |
| 「AC-0 에서 멈추고 올린다」 | ✅ 올렸고 답을 받았다. 그 뒤 **가드가 교체됐다** — 「사본 세기(상한 1)」 → 「정규화 동일성 + 3에서 승격」(`PROMOTE_AT_APPS=3`) |
| 해석기를 가진 앱 = **1** | **2** (web-store · fan-platform-web). 가드 실측 `rc=0` · 정규화 비교 **1쌍** |

### 🔴 그래서 이 티켓이 아직 `review/` 인 이유는 1번이 **아니다**

남은 것은 **선행 2(`ADR-MONO-067` AC-0 ③ OIDC 왕복 = `TASK-MONO-574`)** 와 **AC-3** 이다.
1번을 블로커로 읽고 「소유자 결정을 기다리는 중」이라고 판단하면 **엉뚱한 것을 기다리게 된다** —
이 정정이 존재하는 이유가 그것이다.

### 🔵 왜 이 정정을 남기는가

`TASK-MONO-601` 을 닫은 직후 남은 큐를 소유자에게 보고하면서, **이 표를 그대로 읽고
「`ADR-MONO-068` 승격 형식」을 아직 안 정해진 결정으로 올렸다.** ADR 자신을 열지 않고 티켓의
표만 읽은 결과다. 소유자가 답을 고른 뒤에야 ADR 을 열어 **이미 2026-08-26 에 정해져 있었다**는
것을 발견했고, 아무것도 바꾸지 않고 철회했다.

⇒ 교훈은 이 저장소가 이미 이름 붙여 둔 것들이다: **내 티켓이 인용한 스펙을 직접 열어라**,
그리고 **「결정 대기」 목록을 감사하라** — *결정 대기* 로 적힌 항목은 **이미 결정됐을 수 있고**,
그 상태로 남아 있으면 다음 사람도 같은 것을 다시 묻는다.
[[feedback_my_own_ticket_cited_a_spec_that_says_otherwise]]
[[feedback_ac0_remeasure_is_ungated_work_that_resizes_decisions]]

🔴 같은 낡음이 **`TASK-MONO-585` 에도 있었다** — 그쪽은 `ready/` 라 본문을 고쳤고, 거기서는
사실이 하나 더 붙는다: **585 가 세 번째가 되므로 § D5.4 의 「`B` 로 승격」이 발동한다.**


## CORRECTION (2026-09-01 UTC) — D4 의 «결정» 절반이 닫혔다. 🔴 «해제»는 아니다

이 티켓은 선행 2(`ADR-MONO-067` AC-0 ③ OIDC 왕복)를 **❌ D4 축에 걸린다** 로 적어 두었다.
그 D4 는 이제 이름과 답을 갖는다:

> **`ADR-MONO-069` ACCEPTED — `C2`** (2026-09-01, 소유자 정확형 지정, rider = *"apex 쿠키는 호스트 한정"*)

`C2` = `auth.hubwang.com` 을 **Vercel 프로젝트 하나**가 받아 그날의 IdP 로 서버측 포워딩한다.
축 ①(스킴)·②(범위)가 **둘 다** 사라지고, 🔵 이 티켓이 노출돼 있던 **쿠키 미측정 축(R2)에도
노출되지 않는다**(B·C 는 R2 에 무조건 안전하다).

🔴 **그러나 이 행을 «✅» 로 읽지 마라.** 바뀐 것은 블로커의 **이름**이다:

| | 전 | 후 |
|---|---|---|
| 무엇에 막혀 있나 | «D4 가 결정되지 않았다» | **«`TASK-MONO-610` 이 아직 배선되지 않았다»** |
| 언제 풀리나 | 소유자 지정 | 610 의 AC-3(V1–V7) 통과 — 🔴 그중 **V5 는 부팅 2회** 를 요구한다 |

🔴 **그리고 610 의 AC-0 이 C2 를 탈락시킬 수 있다.** `ADR-MONO-069` § 선택지 C 의 미측정
**2**(`Host`/`Location` 재작성)·**3**(`Set-Cookie` 의 `Domain`)이 거짓이면 **C2 탈락, C1 재지정**이고
이 티켓은 다시 «결정 대기» 로 돌아간다. 그 측정은 **아직 안 됐다**.
...
# 🔴🔴 CORRECTION (2026-09-04 UTC) — **이 티켓의 제목이 여전히 참이다. 그리고 원인이 한 줄로 좁혀졌다**

`review/`(frozen) 이라 순수 추가한다. 🔴 **창 #3 의 «라이브 축 PASS» 를 이 티켓이 닫혔다는
뜻으로 읽지 마라** — 그 PASS 는 **로그인 왕복** 축이었고, **데이터 축은 그때 안 쟀다.**

## 기동 창 #4 가 데이터 축을 처음 쟀다 — **못 닿는다**

```
GET https://fan.hubwang.com/         200  "피드를 불러올 수 없습니다 … 백엔드 게이트웨이가 응답하지 않을 수 있습니다"
GET https://fan.hubwang.com/artists  200  "디렉토리를 불러올 수 없습니다"
   도메인 개체 0
```

⇒ **이 티켓의 제목(«팬은 이미 Vercel 에 있는데 백엔드에 못 닿는다»)이 문자 그대로 참이다.**

## 🔵 원인이 아닌 것 (전부 같은 창 실측)

데모 백엔드 **UP** · 게이트웨이 인증 정상(401 은 토큰 없는 정상 응답) ·
데이터 있음(`[seed:fan] 생성 9 · 실패 0`) · 로그인 **세션까지 성립**.

## 🔴🔴 원인 — **`kanggle-fan` 에 `DEMO_API_BASE` 가 없다**
...
🔵 **이 티켓이 예상한 것과 다르다.** 이 티켓은 *"주소를 만드는 지점은 **1곳**"* 이라며
**코드 작업**(`client.ts`)을 상정했다. 그런데 그 코드는 **이미 들어가 있다** —
`src/shared/config/demo-backend.ts` 가 `@demo/backend-resolver` 를 import 한다.
⇒ **남은 것은 코드가 아니라 그 해석기의 입력값 한 줄**이다.

🔴 **필요조건이지 충분조건은 아니다** — 넣고 재배포한 뒤 다시 재야 확정된다.

## 🔴 이 티켓은 `review/` 라 AC 를 못 얻는다 — **후속 티켓이 필요하다**
...
# 🟢 CORRECTION (2026-09-05 UTC) — **제목의 명제가 거짓이 됐다. 이 티켓은 닫힌다**

`review/`(frozen) 이라 순수 추가한다. 09-04 CORRECTION 이 적은 close 조건
— *"`TASK-MONO-622` AC-1 의 판정 하나"* — 가 **기동 창 #5 에서 PASS 했다.**

## 판정

| 이 티켓의 제목 | 창 #5 실측 |
|---|---|
| *"팬은 이미 Vercel 에 있는데 **백엔드에 못 닿는다**"* | 🟢 **닿는다** — `fan.hubwang.com/` 도메인 개체 **4** · `/artists` **3** · 에러 문구 **0** · 음성 대조군 404 · 양성 대조군(같은 창 store) 상품 8 |

**끊겼던 곳**은 코드가 아니라 **해석기의 입력**이었다 — `kanggle-fan` 에만
`DEMO_API_BASE` 가 없었다(창 #4 의 대조군이 한 칸으로 지목). 소유자가 2026-09-04 에
투입했고, `622` 의 PR `#3625` 가 팬 배포를 트리거해 라이브 커밋이 `e07e9dad1`(= main tip)
이 되면서 **「env 는 있는데 배포에 안 실렸다」 갈래까지 제거된 상태**로 판정됐다.

## 🔴 이 CORRECTION 이 주장하지 **않는** 것

- **「팬 이관이 끝났다」가 아니다.** `ADR-MONO-067` 단계 4 의 억제 몫은 `618` 이, 판정
  몫은 `622` 가 들었다. 이 티켓이 닫는 것은 **자기 제목의 명제** 하나다.
- **창 밖에서는 여전히 안 보인다.** 팬 표면은 `middleware.ts` 가 fail-closed 라 데모가
  꺼져 있으면 `/login` 으로 간다 — 그것은 결함이 아니라 `TASK-FAN-FE-019` AC-1 이 근거를
  들어 고른 (A) 다.
- **소화율은 소급되지 않는다.** 창 #1~#4 의 판정은 그때의 값으로 남는다.

---

# ✅ 종료 (2026-09-05 UTC) — 4-dim

## (a)(b) — PR 축

| impl PR | 스쿼시 | state |
|---|---|---|
| `#3479` (해석기 + 가드) | `8e43a4db9` | MERGED |
| `#3489` (AC-3 · `DemoBackendNotice`) | `4e750c183` | MERGED |

## 🔴 (c) — **required 집합이 비어 있던 시기라 그냥 통과시키지 않았다**

두 PR 모두 **FAILURE 2건**이 있다: `Vercel – kanggle-fan` · `Vercel – kanggle-portfolio`.
🔴 당시(08-26/27)는 `TASK-MONO-598` 이 required 넷을 등록하기 **전**이라 required 집합이
**비어 있었고**, (c)는 «어떤 상태에서도 참» 이었다. `CLAUDE.md` 가 경고한 그 자리다
⇒ **실패한 것을 직접 열어 봤다.**

| 축 | 실측 |
|---|---|
| 무엇이 실패했나 | **외부 배포 status context** 둘. `ci.yml` 잡은 **0건 실패** |
| 이 PR 탓인가 | ❌ — 🔵 **같은 날 PR 6건 전수(`#3474`·`#3479`·`#3480`·`#3488`·`#3489`·`#3490`)가 똑같이 그 둘만 빨갛다** |
| 언제 사라졌나 | 08-30(`#3550`)에는 **0건** ⇒ 기간 한정 사건(Vercel 일일 배포 한도) |
| 오늘은 | 🟢 두 프로젝트 다 정상 — 팬 라이브 `e07e9dad1`(= main tip) · 론처 **200** |

⇒ **main 회귀 없음.** 🔵 이 판정은 «required 가 비어서 초록이었다» 가 아니라
**«무엇이 빨갰는지 열어 보고, 대조군으로 귀속했다»** 이다.
...
|---|---|
| AC-0 (선실측) | ✅ 2026-08-26 |
| AC-1 (서버가 주소를 **런타임에** 얻는다) | ✅ `#3479` — `@demo/backend-resolver` |
| AC-2 (마커 `ADR-MONO-068` D2) | ✅ `#3479` |
| AC-3 (데모가 꺼져 있을 때 **그렇다고 말한다**) | ✅ `#3489` — `widgets/demo-notice/DemoBackendNotice.tsx` |
| AC-4 (검증 · 산출물 재계수) | ✅ |

🔵 § *"🔴 남은 것 — AC-3 … 그래서 이 티켓은 `ready` 로 남는다"* 는 **`#3489` 이전의
기록**이다. 그 뒤 위젯이 실재한다.

## 🟢 그리고 제목의 명제가 **거짓이 됐다** — 기동 창 #5

*"팬은 이미 Vercel 에 있는데 **백엔드에 못 닿는다**"* → 🟢 **닿는다**
(개체 `/` **4** · `/artists` **3** · 에러 문구 **0** · 음성 404 · 양성 store 8).
끊겼던 곳은 코드가 아니라 **해석기의 입력**(`DEMO_API_BASE` 부재)이었다.

## 🔵 `DemoBackendNotice` 의 도달 범위 — 좁지만 죽지 않았다

위젯은 `(main)` 셸에만 붙고, 팬은 `middleware.ts` 가 fail-closed 라 데모가 꺼져 있으면
보통 `/login` 으로 간다(그 위젯이 **덮지 않는다고 자기 헤더에 적어 둔** 자리다).
🔵 그래도 **죽은 코드가 아니다** — 창 안에서 로그인한 방문자의 세션은 30일 유효하므로
(실측: `expires` = 창 시각 + 30일), **데모가 꺼진 뒤 다시 오는 방문자**가 정확히 그
위젯을 본다. AC-3 이 말한 *"화면은 뜨고 데이터만 없다"* 가 그 경로다.
🔴 fail-closed 는 `TASK-FAN-FE-019` AC-1 이 근거를 들어 고른 (A) 이고, 재론하지 않는다.

## 🔴 이 종료가 주장하지 **않는** 것

- **「팬 이관이 끝났다」가 아니다.** `ADR-MONO-067` 단계 4 의 억제 몫은 `618`,
  판정 몫은 `622` 가 들었다. 이 티켓이 닫는 것은 **자기 제목의 명제** 하나다.
- **소화율은 소급되지 않는다.** 창 #1~#4 의 판정은 그때의 값으로 남는다.
