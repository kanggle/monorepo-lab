...
## CORRECTION — AC-2 의 **인라인 주석이 낡았다** (2026-09-22 UTC, close chore 의 4차원 (d) 에서 발견)

AC-2 의 체크박스는 `[x] 🟢` 인데 바로 아래 줄이 아직 이렇게 말한다:

> → **이 세션에서 닫을 수 없다.** … 그래서 이 티켓은 `in-progress/` 에 남는다 — `review/` 로 옮기지 않았다.

🔴 **그 문장은 2026-09-18 의 상태이고 지금은 거짓이다.** AC-2 는 2026-09-22 데모 창에서
촬영 매니페스트로 닫혔고(`✔ /ecommerce/orders/[id] → …/9bce4a2b-…`, ecommerce 21장 빈 장 0),
이 파일은 `review/` 에 있다. 실제 결말은 이 파일 아래 § *"🟢 AC-2 판정 — 데모 창에서 닫았다"*
에 있다.

🔵 **고쳐 쓰지 않고 여기에 덧붙이는 이유**: `tasks/INDEX.md` § Review Rules 가
*"Do not modify a task file after it moves to `review/` or `done/` — 편집은 무엇이 리뷰됐고
무엇이 결정됐는지의 기록을 다시 쓴다"* 고 못박는다. 그래서 낡은 줄은 **그대로 두고** 정정을
추가한다.

🔴🔴 **원인은 내 편집이 반쪽이었다는 것**이다 — 체크박스와 하단 §은 고치고 **인라인 주석은
안 고쳤다**. 이 저장소가 이름 붙인 «한 사실이 두 절에 있으면 한쪽만 고쳐진다» 그대로다.
🔵 **4차원 (d) 가 그것을 잡았다** — (d) 가 «INDEX 행이 아니라 AC 절을 열어서 읽어라» 인
이유가 정확히 이것이다. (a)(b)(c) 는 셋 다 통과했고 이 모순을 보지 못했다.
