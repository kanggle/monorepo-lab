...
## CORRECTION (2026-09-01 UTC) — AC-4 사후검증 결과

`TASK-MONO-573` 이 머지 +9일 시점에 산출물로 실측했다(측정 `2026-09-01T06:58Z`,
계측기 = artifacts API 의 `expired==false` 240행).

**판정: 효과가 있었다.** 살아있는 `wms-boot-jars` 는 **1벌**, 가장 오래된 `created_at` 이
`2026-09-01T05:23:40Z`(1시간 35분 전)이고 수명은 **23:59:49** ⇒ AC-4 의 PASS 두 조건 모두 참.

🔴 **이 티켓의 두 예측을 정정한다.**

| 예측 | 실측 | |
|---|---|---|
| 잔존 «≈11벌 / ≈3.2 GB» | **1벌 / 0.29 GB** | 개수는 **11배** 빗나갔다 |
| 회수 «≈19.5 GB» | **−22.76 GB** (23.05 → 0.29) | 조금 더 회수했다 |

🔵 **개수가 11배 빗나간 것은 결함이 아니라, 이 티켓이 옳았다는 증거다.** 이 티켓은
*«11 은 예측이지 측정이 아니다 … 판정은 「≈11 인가」가 아니라 「가장 오래된 것이 24시간 이내인가」로 한다»*
라고 미리 적었다. 개수로 판정했다면 정상 상태가 빨간불이 됐을 것이다.

🔴 **전체 합계를 이 변경의 공으로 돌리지 마라.** 전체 살아있는 아티팩트는 60.33 GB/374개 →
**5.75 GB/240개**(−54.58 GB)지만, 이 변경이 바꾼 것은 `retention-days` **한 줄**이다. 정직한 몫은
**−22.76 GB(41.7%)** 이고 나머지 −31.8 GB 는 이 변경이 닿지도 않은 항목에서 나왔다 — 원인은 안 쟀다.

🔵 **모집단을 넓혀서도 확인했다** — 살아있는 boot-jar 계열 **10벌 전수**가 1일 수명이고
**7일짜리 boot-jars 는 0벌**이다.

✅ **AC-3 의 가드는 도달했다.** 이 티켓 머지 이후 `.github/workflows/` 를 건드린 머지 커밋 **7건 전부**에서
`Artifact retention (build outputs expire in 1 day, not 7)` 이 SUCCESS 이고, 워크플로를 안 건드린
`#3542` 에서는 SKIPPED 다(음성 대조군 — 필터가 「항상 켜짐」이 아니다). 물기는 매 실행의 `--self-test`
스텝이 증명한다.

🔴 **573 의 AC-4 는 «566 을 review/ → done/ 로 함께 옮긴다»고 적었지만 이 티켓은 2026-08-27(#3490)에
이미 닫혀 있었다.** 그래서 이 CORRECTION append 로 대신하고, 라이프사이클을 타는 것은 573 하나다.
