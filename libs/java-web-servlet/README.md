...
| Class | Purpose |
| --- | --- |
| `com.example.web.exception.CommonGlobalExceptionHandler` | Abstract `@ControllerAdvice` base providing default handlers for `MethodArgumentNotValidException`, `HttpMessageNotReadableException`, `MissingRequestHeaderException`, `MissingServletRequestParameterException`, `IllegalArgumentException`, `ObjectOptimisticLockingFailureException`, `NoResourceFoundException` / `NoHandlerFoundException` (404), `HttpRequestMethodNotSupportedException` (405 + RFC 7231 `Allow`), `HttpMediaTypeNotSupportedException` (415), and a fallback `Exception` handler (500). Subclasses (per service) annotate themselves `@RestControllerAdvice` and add service-specific `@ExceptionHandler` methods. **Overridable validation status**: `protected HttpStatus validationFailureStatus()` (default `400`) is what the `@Valid` and `IllegalArgumentException` arms answer with — a service whose published contract uses `422` for that case overrides this one method instead of re-implementing both arms (ADR-MONO-058 § D2). Overriding it does not affect any other arm. |
...
| `com.example.web.idempotency.IdempotencyKeyFilter` | Configurable REST Idempotency-Key filter (ADR-MONO-038) carrying the unified replay / 409-conflict / 503-processing / fail-open control flow. Parameterised by `IdempotencyFilterConfig` (methods, path predicate, max key length, TTLs), an `IdempotencyStore`, a `BodyCanonicalizer`, an `IdempotencyErrorWriter`, and optional `IdempotencyMetrics`. |
| `com.example.web.idempotency.IdempotencyStore` | Storage contract (`lookup` / `tryAcquireLock` / `put` / `releaseLock`) the per-service Redis/in-memory adapters implement; returns/stores `StoredResponse`. |
| `com.example.web.idempotency.StoredResponse` | Cached-response record (`requestHash`, `status`, `bodyJson`, `contentType`, `storedAt`) the filter replays. |
| `com.example.web.idempotency.BodyCanonicalizer` (+ `JsonValueBodyCanonicalizer`, `JsonTreeBodyCanonicalizer`) | Pluggable canonical-hash strategy. `JsonValue…` = Family A (`readValue` via `BodyHashUtil`); `JsonTree…` = Family B (`readTree` tree-sort, lenient raw-byte fallback). |
| `com.example.web.idempotency.IdempotencyErrorWriter` | Service-owned 409 / 503 / 400 error-response writer strategy (keeps the error envelope service-side; no shared error DTO). |
| `com.example.web.idempotency.IdempotencyMetrics` | Micrometer-free optional metrics SPI (`recordLookup`, `recordStoreFailure`) with a `NO_OP` default. |
