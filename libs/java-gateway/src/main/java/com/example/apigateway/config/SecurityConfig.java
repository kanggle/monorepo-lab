...
/**
 * The reactive security chain shared by the wms / scm / fan gateways. All three had
 * byte-identical copies of this class, down to the same three public paths.
 *
 * <p><strong>Registration:</strong> this {@code @Configuration} lives outside every
 * consuming service's base package, so the component scan must name
 * {@code com.example.apigateway} explicitly. ecommerce deliberately does <em>not</em>
 * scan it — its own {@code SecurityConfig} encodes route knowledge (public product
 * reads, the carrier webhook, CORS preflight) and stays in the service
 * (ADR-MONO-048 § D4: domain ownership over reuse convenience).
 */
@Configuration
...
public class SecurityConfig {
    ...
    private ServerAuthenticationEntryPoint unauthorizedEntryPoint(GatewayErrorHandler errorHandler) {
        return (exchange, ex) -> {
            ...
            if (oauthError != null
                    && GatewayErrorCodes.TENANT_MISMATCH.equals(oauthError.getErrorCode())) {
