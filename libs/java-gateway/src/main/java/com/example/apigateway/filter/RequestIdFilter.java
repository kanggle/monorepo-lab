 ...
 *
 * <p><strong>Registration:</strong> this class lives outside every consuming
 * service's base package, so {@code @Component} alone will NOT pick it up. Each
 * gateway must name {@code com.example.apigateway} in its component scan. Miss
 * that and the filter silently never registers — unit tests still pass while the
 * running gateway is missing it (TASK-MONO-351 § Edge Cases).
 */
@Component
