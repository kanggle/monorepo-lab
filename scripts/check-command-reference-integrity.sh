#!/usr/bin/env bash
#
# check-command-reference-integrity.sh — TASK-MONO-468
#
# Fails when a slash-command definition under `.claude/commands/*.md` points at a
# repo path that does not exist, in either of the two shapes a command uses to
# name a file:
#
#   markdown link   [text](../../platform/refactoring-policy.md)  — a clickable
#                   link. Resolved RELATIVE TO THE COMMAND FILE, exactly the way
...
#                   `config/`, `hooks/` name directories that exist ONLY under
#                   `.claude/`, so a bare `skills/INDEX.md` is by construction a
#                   dead reference — that is the exact defect this guard was born
#                   from (below).
#
# WHY THIS EXISTS
# ---------------
...
#
# Scope is `.claude/commands/*.md` only. Agent and skill reference integrity have
# the same gap and are a natural follow-up, but widening now risks starting red on
# an unrelated pre-existing drift, and a guard must be green the day it lands.
#
# WHAT IS AND IS NOT A CHECKABLE REFERENCE (the false-positive frontier)
# ---------------------------------------------------------------------
# A command file is mostly prose and code fragments. Only two token shapes are
# treated as references; everything else is ignored ON PURPOSE:
#
...
#
# USAGE
#   scripts/check-command-reference-integrity.sh              # guard the repo
#   scripts/check-command-reference-integrity.sh --selftest   # assert the extractor
...
# The first nine are real repo roots; the last six are `.claude/` subdirectories
# that exist nowhere else, so writing them bare is itself the drift.
ANCHOR_RE='^(platform|rules|libs|tasks|docs|scripts|projects|\.claude|\.github|skills|agents|commands|workflows|config|hooks)/'
...
A directory ref `.claude/config/` is anchored.
...
PATH	skills/INDEX.md'
  ...
  if [[ $st_fail -eq 0 ]]; then
    echo "check-command-reference-integrity --selftest: OK (extractor frontier pinned)."
...
# ---------------------------------------------------------------------------
# Population from the tree, not enumerated here.
# ---------------------------------------------------------------------------
mapfile -t commands < <(git ls-files '.claude/commands/*.md' | LC_ALL=C sort)

if [[ ${#commands[@]} -eq 0 ]]; then
  echo "check-command-reference-integrity: no .claude/commands/*.md found."
...
for cmd in "${commands[@]}"; do
  ...
  while IFS=$'\t' read -r kind ref; do
    ...
    case "$kind" in
      LINK)
        # Resolve relative to the command file, the way GitHub renders the link.
        # `[ -e ]` collapses ../ at the filesystem level, so any depth works.
        target="${ref%/}"
        if [ ! -e "$cmd_dir/$target" ]; then
          echo "DEAD LINK  $cmd"
          echo "           [..]($ref) resolves to '$cmd_dir/$ref', which does not exist."
          findings=$((findings + 1))
          fail=1
        fi
        ;;
  ...
  done < <(extract_refs "$cmd")
done

echo
if [[ $fail -eq 0 ]]; then
  echo "check-command-reference-integrity: OK — ${#commands[@]} command file(s), ${checked} reference(s) checked, all resolve."
  ...
  echo "check-command-reference-integrity: FAILED — ${findings} dead reference(s) across ${#commands[@]} command file(s)."
  echo "  Fix the reference in the command; never invent a file to satisfy it."
fi
