...
- admin-service `infrastructure/client/AccountServiceTenantClient` (→ account `/internal/tenants/**`, BE-250) 를 Bearer 로 전환. **초안 인벤토리 누락분(코드 검증 2026-05-30) — account `/internal/**` 의 세 번째 호출자**. account 수신측 X-token 제거(BE-319) 가 깨지지 않도록 admin→account 전 호출자가 Bearer 여야 함 → 본 task 에 포함.
- 세 client 모두 `admin-service-client`(V0019, secret 기본 "secret") 사용.
...
- **AC-2b**: admin `AccountServiceTenantClient`(→account `/internal/tenants/**`) 호출이 Bearer 첨부 + X-Internal-Token 미전송.
...
- **AC-6**: 회귀 0 — 기존 admin 클라이언트 테스트(`AccountServiceClientResilienceTest`/`AccountServiceClientUnitTest`/`SecurityServiceClientCircuitBreakerTest`/`SecurityServiceClientUnitTest`) 및 account/security/tenant 호출을 trigger 하는 admin IT(`AdminIntegrationTest`/`TenantAdminIntegrationTest`/`ConsoleRegistryIntegrationTest`/`OperatorProfileIntegrationTest`/`OperatorProfileAdminIntegrationTest`) 유지.
...
- admin 의 네 client(Account/Tenant/Security/Auth)가 동일 property `admin.downstream.internal-token` 을 공유 → account(Account+Tenant)/security client 는 Bearer 로 전환하고 auth client 의 X-Internal-Token 은 유지(혼합 상태 정상). property 자체는 auth client 가 계속 사용하므로 제거하지 않음.
...
- 호출자 인벤토리(코드 재검증 2026-05-30): admin → account/security `/internal/**` 호출자는 **3종** — `AccountServiceClient`(/internal/accounts/**), `SecurityServiceClient`(/internal/security/**), `AccountServiceTenantClient`(/internal/tenants/**, BE-250). 세 client 모두 `admin.downstream.internal-token`(`${INTERNAL_API_TOKEN:}`) 으로 X-Internal-Token 전송 → 전부 Bearer 로 전환. (초안 인벤토리는 tenant client 누락 — 사용자 승인 하 scope 확장.) admin `AuthServiceClient`(/internal/auth/**) = X-Internal-Token 유지(auth 수신측 permitAll). `admin.downstream.internal-token` property 는 auth client 가 계속 사용하므로 보존.
