...
- security-service: **`SecurityConfig` 없음** (Spring Security web chain 전무, HTTP 표면 = `/internal/security/**` 2개 컨트롤러 + actuator). raw `@Component` 필터 `InternalAuthFilter`(403 반환, bypass profile = `test`/`standalone`). **구현 결정 = 옵션 (b)** — 전체 SecurityFilterChain 신규 도입(blast radius 최대) 대신 기존 raw 필터를 terminal(403) 유지하되 `JwtDecoder`(jose 모듈만 추가, resource-server starter 회피 → default lock-everything chain 방지) 주입해 Bearer JWT 도 수용. account 와 dual-accept **시맨틱 동일**, 메커니즘만 출발점 차이 반영.
...
**테스트 영향**: `InternalApiFilterTest`(8 메서드, 필터가 직접 401 작성함을 단언)는 필터 contract 변경으로 **재작성 대상** — "필터가 SecurityContext 인증을 설정/미설정" 단언으로 전환. (AC-7 의 보존 대상 = `OAuth2*IntegrationTest`/`InternalControllerTest`; `InternalApiFilterTest` 는 필터 자체 단위 테스트라 정당한 변경.) security-service 는 옵션 (b) 로 `InternalAuthFilter` 자체에 Bearer JWT 수용을 추가(terminal·403 유지) — 슬라이스 테스트는 `@MockitoBean JwtDecoder` 한 줄만 추가.

## 구현 결과 (2026-05-30, 3-dim 검증)

- **AC-1/AC-2**: auth-service IT `OAuth2AuthorizationServerIntegrationTest` 에 `account-service-client` client_credentials → 200 + JWKS 공개키로 서명 검증 + iss + `tenant_id=global-account-platform`/`tenant_type=INTERNAL` + `sub=account-service-client` 단언. 7/7 PASS.
- **AC-3/4/5**: account `InternalDualAuthSliceTest`(Bearer JWT 200 / X-Internal-Token 200 / 자격증명 없음 401) + `InternalApiFilterTest` 재작성(비종결 단언). PASS.
- **AC-6**: security `InternalAuthFilterTest`(JWT/X-token/neither, mock decoder) + 슬라이스 2종 + `SecurityServiceIntegrationTest` 회귀. PASS.
- **AC-7**: OAuth2 IT 회귀 + `InternalControllerTest` + account provisioning IT 3종(Tenant/Bulk/Role) GREEN.
- **AC-8**: 계약 spec(`*.md`) 인증 절 불변(읽기만).
- **degenerate edge**(X-token 유효 + Bearer 동시 + JWT 무효): account 는 bearer 필터가 먼저 401, security 는 X-token 우선 통과 — 어느 쪽도 X-token-only 호출자 회귀 아님(소프트 케이스, AC 아님). 정상 호출자는 한 가지 메커니즘만 전송.
- seed 토큰 TTL = 1800s(기존 포맷 재사용); 단축/캐시 정책은 BE-318.
