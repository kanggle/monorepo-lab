...
- `TASK-BE-289-fix-TASK-BE-288.md` — TASK-BE-288 review fix (2 finding). **Finding 1 (BLOCKING, PR #555 merge gate)**: `PatchOperatorRoleUseCase` role-binding `tenant_id` 이 legacy 4-arg `AdminOperatorRoleJpaEntity.create` (하드코딩 `"fan-platform"`) → 5-arg `operator.tenantId()` 로 전환 — non-fan-platform 대상 운영자(WMS 테넌트 / SUPER_ADMIN cross-tenant role patch)의 persisted `admin_operator_roles.tenant_id` 가 변경됨. multi-tenant+regulated+audit-heavy 서비스에서 TASK-BE-288 의 "behavior 0 / byte-identical" 보증 위반. 해소 = Option A (legacy `"fan-platform"` 복원, BE-288 strictly behavior-neutral 유지 + 별 correctness task) **또는** Option B (correction 비준 + Flyway backfill 마이그레이션 + 격리 회귀 테스트 + done-note 정정). **Finding 2 (minor, fix-forward 가능)**: `OperatorRoleResolver.java` 삭제됐으나 `architecture.md` L75/L87/L141 가 여전히 참조 — spec→deleted-code dead-ref (ADR-MONO-012 / MONO-085/086 policing 대상). architecture.md 를 port/adapter/`AuditReasons` shape 로 갱신 + L141 naming-collision caveat 재작성/삭제. 분석=Opus 4.7 / 구현 권장=Opus 4.7 (multi-tenant persisted-state + spec governance 판단).
