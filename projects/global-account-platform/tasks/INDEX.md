...
- `TASK-BE-289-fix-TASK-BE-288.md` — TASK-BE-288 review follow-up. **Finding 1 (Option A) 는 PR #555 에서 이미 해소·머지됨** — `PatchOperatorRoleUseCase` role-binding `tenant_id` 를 legacy 하드코딩 `"fan-platform"` (`LEGACY_BINDING_TENANT_ID` 상수) 로 pin → TASK-BE-288 strictly behavior-neutral (main `1ef970bf`, CI run `25919652146` 17/17 PASS). 본 task **잔여 2 항목**: (1) **Finding 2 (minor, fix-forward)** — `OperatorRoleResolver.java` 삭제됐으나 `architecture.md` L75/L87/L141 가 여전히 참조 (spec→deleted-code dead-ref, ADR-MONO-012 / MONO-085/086 policing 대상); architecture.md 를 port/adapter/`AuditReasons` shape 로 갱신 + L141 naming-collision caveat 재작성/삭제. (2) **Deferred tenant_id correctness** — `PatchOperatorRoleUseCase` 를 `LEGACY_BINDING_TENANT_ID` → `operator.tenantId()` 로 전환(= `CreateOperatorUseCase` 와 정합) + 기존 mis-stamped `admin_operator_roles` row Flyway backfill + 테넌트 격리 회귀 테스트 (PROJECT.md mandate). 두 항목 독립 → 별 PR 분리 가능 (`feedback_pr_bundling`). 분석=Opus 4.7 / 구현 권장=Opus 4.7 (multi-tenant persisted-state + Flyway migration + spec governance 판단).
