...
/**
 * Unit tests for {@link GapClientCredentialsTokenProvider} (TASK-BE-318d):
 * fetches a client_credentials token via Basic auth and caches it (AC-2).
 */
@DisplayName("GapClientCredentialsTokenProvider 단위 테스트")
class GapClientCredentialsTokenProviderTest {
    ...
    private GapClientCredentialsTokenProvider provider() {
        return new GapClientCredentialsTokenProvider(
                wireMock.baseUrl() + "/oauth2/token", "membership-service-client", "membership-service-secret");
    }
    ...
    void fetchesToken_withBasicAuth() {
        ...
        String expectedBasic = "Basic " + Base64.getEncoder()
                .encodeToString("membership-service-client:membership-service-secret".getBytes());
    ...
    void cachesToken_singleFetchForMultipleCalls() {
        ...
        GapClientCredentialsTokenProvider provider = provider();
