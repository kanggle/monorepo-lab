 ...
 * membership-service's outbound {@code /internal/accounts/**} calls with {@code Authorization: Bearer}
 * (TASK-BE-318d, ADR-005 단계 3). Used by {@link AccountStatusClient}.
 *
 * <p>Hand-rolled rather than {@code spring-boot-starter-oauth2-client} on purpose: membership-service
 * runs only an OAuth2 <em>resource server</em> chain (it validates inbound JWTs on its own
 * {@code /internal/membership/**}); introducing the OAuth2 <em>client</em> autoconfiguration to attach
 * outbound tokens would add machinery this single call does not need. This provider uses only a plain
 * {@link RestClient} + Jackson — no new dependency — and preserves {@link AccountStatusClient}'s
 * existing {@code ResilienceClientFactory} RestClient + circuit-breaker structure (parity with the
 * security/auth blueprints, TASK-BE-318 / 318c).
 *
...
@Slf4j
@Component
public class GapClientCredentialsTokenProvider {
    ...
    public GapClientCredentialsTokenProvider(
            @Value("${gap.internal-client.token-uri:http://localhost:8081/oauth2/token}") String tokenUri,
            @Value("${gap.internal-client.client-id:membership-service-client}") String clientId,
            @Value("${gap.internal-client.client-secret:membership-service-secret}") String clientSecret) {
