 ...
 * {@code AccountServiceTenantClient} (→ account) and {@code SecurityServiceClient} (→ security).
 * {@code AuthServiceClient} (→ auth) intentionally keeps the static {@code X-Internal-Token}
 * (auth receiver is still {@code permitAll}).
 *
...
public class GapClientCredentialsTokenProvider {
    ...
    public GapClientCredentialsTokenProvider(
            ...
            @Value("${gap.internal-client.client-id:admin-service-client}") String clientId,
            @Value("${gap.internal-client.client-secret:secret}") String clientSecret) {
