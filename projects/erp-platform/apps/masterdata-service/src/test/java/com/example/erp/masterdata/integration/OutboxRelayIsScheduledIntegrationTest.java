...
import com.example.erp.masterdata.infrastructure.outbox.MasterdataOutboxPublisher;
 ...
 *
 * <p><b>Why this test exists at all.</b> {@code MasterdataOutboxPublisher} carries
 ...
 *       {@code MasterdataOutboxPublisher#publishPending} — precise enough to name the
 *       broken thing in the failure message;</li>
 *   <li>a pending row written to {@code masterdata_outbox} becomes published without
...
class OutboxRelayIsScheduledIntegrationTest extends AbstractMasterdataIntegrationTest {

    @Autowired
    private ObjectProvider<ScheduledAnnotationBeanPostProcessor> scheduling;

    @Autowired
    private MasterdataOutboxJpaRepository outboxRepository;

    @Test
    @DisplayName("the scheduler holds a task targeting MasterdataOutboxPublisher#publishPending")
    void the_relay_method_is_registered_with_the_scheduler() {
        ...
        // version-dependent and was already false here on the first run — a brittle
        // predicate that would have reported "the relay is not registered" while the
        // relay was demonstrably running. `Task#toString` delegates to the runnable,
        // and for an annotation-scheduled method that is the qualified method name.
        assertThat(tasks.stream().map(t -> t.getTask().toString()).toList())
                ...
                .anySatisfy(description -> assertThat(description)
                        .contains(MasterdataOutboxPublisher.class.getSimpleName())
    ...
    void a_pending_row_drains_without_anyone_calling_the_publisher() {
        ...
        outboxRepository.saveAndFlush(MasterdataOutboxJpaEntity.create(
                id,
                "Department",
                aggregateId,
                ...
                MasterdataEventPublisher.EVENT_DEPARTMENT_CHANGED,
                "{\"departmentId\":\"" + aggregateId + "\",\"guard\":\"TASK-ERP-BE-042\"}",
                aggregateId,
                Instant.now()));
