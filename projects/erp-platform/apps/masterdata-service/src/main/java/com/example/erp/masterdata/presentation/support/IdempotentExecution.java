package com.example.erp.masterdata.presentation.support;

import com.example.erp.masterdata.application.port.outbound.IdempotencyStore;
import com.example.erp.masterdata.domain.error.DomainErrors.IdempotencyKeyConflictException;
 ...
 * mutating action, so a concurrent same-key burst executes the action
 * <em>exactly once</em>.
 *
 * <ol>
 *   <li>EXECUTE — this request won the claim: run the use case, then
 ...
 *       stored response (NO re-mutation);</li>
 ...
 *       the bound → 409 (deterministic, no re-mutation — never a faked
 *       success).</li>
 * </ol>
 */
@Slf4j
