package com.example.erp.masterdata.domain.jobgrade;
...
public class JobGrade {
    ...
    public static JobGrade create(String id, String tenantId, String code, String name,
                                  int displayOrder, EffectivePeriod period, Instant now) {
        ...
        g.status = MasterStatus.ACTIVE;
        g.effectiveFrom = period.effectiveFrom();
        g.effectiveTo = period.effectiveTo();
        g.createdAt = now;
        g.updatedAt = now;
        return g;
    }
    ...
    public void updateAttributes(String newName, Integer newDisplayOrder, Instant now) {
        if (newName != null) this.name = newName;
        if (newDisplayOrder != null) this.displayOrder = newDisplayOrder;
        this.updatedAt = now;
    }

    public void retire(Instant now) {
        MasterStatusMachine.ensureRetireAllowed(this.status, "JobGrade " + id);
