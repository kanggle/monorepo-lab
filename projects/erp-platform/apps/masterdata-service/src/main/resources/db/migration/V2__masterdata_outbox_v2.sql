-- erp-platform masterdata-service outbox v1 -> v2 (TASK-ERP-BE-026).
-- MySQL 8, InnoDB, utf8mb4 — parity with V1. Migrates masterdata-service onto the
...
-- (TASK-SCM-BE-032).
--
-- masterdata_outbox — pending rows have published_at IS NULL; the relay
-- (MasterdataOutboxPublisher) forwards them to Kafka in created-order and stamps
...
-- matches MasterdataOutboxJpaEntity's UUID field mapped via @JdbcTypeCode(SqlTypes.CHAR).
...
CREATE TABLE masterdata_outbox (
    id              CHAR(36)     NOT NULL,
...
CREATE INDEX idx_masterdata_outbox_unpublished ON masterdata_outbox (published_at, created_at);
...
-- NOT dropped: masterdata-service RETAINS the libs OutboxAutoConfiguration (it is
...
-- (low-volume, re-derivable in demo/fed-e2e/CI — TASK-ERP-BE-026 Edge Case
-- "cutover" / F1).
