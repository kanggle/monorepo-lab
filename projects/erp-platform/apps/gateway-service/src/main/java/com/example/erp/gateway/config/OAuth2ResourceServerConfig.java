 ...
 * five. What stays here is what is erp's to decide: the property keys, and
 * {@link #tenantGate()}.
 */
@Configuration
public class OAuth2ResourceServerConfig {
    ...
    static final String GATEWAY_NAME = "erp";
    ...
    public OAuth2ResourceServerConfig(
            ...
            @Value("${erpplatform.oauth2.allowed-issuers}") String allowedIssuersCsv,
            @Value("${erpplatform.oauth2.required-tenant-id:erp}") String requiredTenantId,
            @Value("${erpplatform.oauth2.allowed-audiences}") String allowedAudiencesCsv,
            @Value("${erpplatform.oauth2.audience-mode}") String audienceMode,
    ...
    /**
     * erp's tenant gate: equality <strong>or</strong> the {@code "*"} SUPER_ADMIN wildcard,
     * plus entitlement-trust dual-accept (ADR-MONO-019 § D5).
     *
     * <p><strong>This is not a new decision.</strong> It is the gate erp's own services
     * already declare — every {@code ServiceLevelOAuth2Config} under `erp-platform` builds
     * `new TenantClaimValidator(erpplatform.oauth2.required-tenant-id:erp)` with the
