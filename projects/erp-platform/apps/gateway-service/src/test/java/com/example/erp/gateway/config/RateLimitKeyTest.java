...
class RateLimitKeyTest {
    ...
    @Test
    @DisplayName("인증 요청 → rate:erp-platform:<route>:acct:<sub>")
    void authenticatedRequestKeysOnTheAccount() {
        ...
        assertThat(key).isEqualTo("rate:erp-platform:unknown:acct:operator-a");
    }
    ...
    void anonymousRequestFallsBackToTheIpKey() {
        ...
        assertThat(key).isEqualTo("rate:erp-platform:unknown:203.0.113.7");
    }
    ...
    void usesTheFirstForwardedForHop() {
        String key = config.clientIpKeyResolver()
                .resolve(MockServerWebExchange.from(
                        MockServerHttpRequest.get("/api/erp/masters")
    ...
    void degradesGracefullyWithoutARouteAttribute() {
        String key = config.clientIpKeyResolver()
                .resolve(MockServerWebExchange.from(
                        MockServerHttpRequest.get("/api/erp/masters").build()))
                .block();

        assertThat(key).startsWith("rate:erp-platform:unknown:");
    }

    // --- helpers --------------------------------------------------------------

    private static MockServerWebExchange get() {
        return MockServerWebExchange.from(MockServerHttpRequest.get("/api/erp/masters")
