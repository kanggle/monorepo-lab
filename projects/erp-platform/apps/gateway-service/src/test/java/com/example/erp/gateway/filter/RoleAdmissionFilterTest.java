...
/**
 * erp's rule-6 admission (TASK-MONO-416), asserted against the {@link RoleAdmissionFilter} this
 * gateway actually wires — constructed from the production {@link GatewayIdentityConfig} bean,
 * not hand-rolled, so removing or weakening the bean turns this red.
 *
 * <p>The <em>negative</em> case is the reason this file exists: a gateway tested only with
 * admitted tokens stays green even if admission is dropped entirely (ADR-MONO-049 § D5-8). erp's
 * suite is Docker-free by design, so this exercises the production filter directly rather than
 * through a Testcontainers HTTP chain (that authority lives in fan/scm's integration suites).
 */
@DisplayName("erp 역할 admission — role/scope 있으면 통과, 둘 다 없으면 403")
class RoleAdmissionFilterTest {
    ...
    void admitsOperatorRoleToken() {
        assertThat(runAuthenticated(jwt(Map.of("roles", List.of("ERP_OPERATOR")))).called).isTrue();
    }

    @Test
    @DisplayName("scope 만 있는 머신 토큰은 통과 (erp.read/write)")
    void admitsScopeOnlyMachineToken() {
        assertThat(runAuthenticated(jwt(Map.of("scope", "erp.read erp.write"))).called).isTrue();
    }
    ...
    private static MockServerWebExchange get() {
        return MockServerWebExchange.from(
                MockServerHttpRequest.get("/api/erp/masterdata/employees/1"));
    }
