...
/**
 * read-model-service Spring Security configuration.
 *
 * <ul>
 *   <li>{@code /actuator/{health,info,prometheus}} — public (prometheus
 *       network-isolated to the internal docker network, E7)</li>
 *   <li>{@code /api/erp/read-model/**} — bearer token required (RS256, GAP JWKS)</li>
 *   <li>everything else — denied</li>
 * </ul>
 *
 * Read-only API (E5) — no mutating endpoints. The READ authorization gate is
 * enforced per-request by {@code ReadAuthorizationGate} in the controller; the
 * tenant gate is enforced by the decode-time validator + {@code TenantClaimEnforcer}
 * filter (both dual-accept). No public webhook surface (erp internal-only, E7).
 */
@Configuration
@EnableWebSecurity
public class SecurityConfig {
     ...
     * pattern, its 401/403 writers, and the closed
