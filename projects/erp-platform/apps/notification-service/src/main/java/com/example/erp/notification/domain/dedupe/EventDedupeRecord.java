package com.example.erp.notification.domain.dedupe;
 ...
 * presence of a record means the event has been dispatched, so re-delivery is
 * skipped without creating a second notification (the inbox stays
 * byte-identical). Captures the dispatch provenance ({@code eventId} +
 * {@code topic} + {@code aggregateId} + {@code processedAt}). Distinct from
 * libs/java-messaging's {@code ProcessedEventJpaEntity} (whose outbox
 * auto-config is excluded — notification is a no-outbox terminal consumer).
 *
