...
/**
 * notification-service Spring Security configuration.
 *
 * <ul>
 *   <li>{@code /actuator/{health,info,prometheus}} — public (prometheus
 *       network-isolated to the internal docker network, E7)</li>
 *   <li>{@code /api/erp/notifications/**} — bearer token required (RS256, GAP JWKS)</li>
 *   <li>everything else — denied</li>
 * </ul>
 *
 * The READ authorization gate is enforced per-request by
 * {@code ReadAuthorizationGate} in the controller; the tenant gate is enforced
 * by the decode-time validator + {@code TenantClaimEnforcer} filter (both
 * dual-accept). No public webhook surface (erp internal-only, E7).
 */
@Configuration
@EnableWebSecurity
