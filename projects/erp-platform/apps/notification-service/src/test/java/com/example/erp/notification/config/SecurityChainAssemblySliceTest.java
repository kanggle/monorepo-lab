...
/**
 * ADR-MONO-058 § D4 for notification-service — the assembled chain's verdicts, exercised end to end
 * with a really RSA-signed JWT against a JWKS this test actually serves.
 *
 * <p>notification-service has no {@code ActorContext} and no actor converter: it authorises off
...
@DisplayName("notification-service — the assembled resource-server chain (ADR-MONO-058 D4)")
class SecurityChainAssemblySliceTest {

    private static final String ISSUER = "http://test-issuer";
    private static final String INBOX = "/api/erp/notifications";
    ...
    static {
        try {
            RSA_KEY = new RSAKeyGenerator(2048).keyID("erp-be-037-notification").generate();
    ...
    void captureOnInvocation() {
        lenient().when(queryInbox.list(anyString(), anyString(), nullable(Boolean.class),
                        anyInt(), anyInt()))
    ...
    void principalIsAPlainJwtNotAnActor() throws Exception {
        ...
        assertThat(((Jwt) liveAuthentication.get().getPrincipal()).getSubject())
                .isEqualTo("recipient-1");
    }
