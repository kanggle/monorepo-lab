...
/**
 * Decode-time {@link TenantClaimValidator} — entitlement-trust dual-accept gate
 * (ADR-MONO-019 § D5). Accept on legacy tenant_id OR signed entitled_domains;
 * reject only when both fail (fail-closed).
 */
class TenantClaimValidatorTest {
    ...
    void erpTenantPasses() {
        assertThat(validator.validate(jwt(Map.of("tenant_id", "erp", "sub", "u"))).hasErrors())
                .isFalse();
    }
    ...
    void crossTenantWithoutEntitlementFails() {
        OAuth2TokenValidatorResult r = validator.validate(jwt(Map.of("tenant_id", "scm", "sub", "u")));
    ...
    void absentTenantIdFails() {
        assertThat(validator.validate(jwt(Map.of("sub", "u"))).hasErrors()).isTrue();
    }
