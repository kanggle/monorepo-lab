package com.example.erp.approval.presentation.filter;

import com.example.erp.approval.infrastructure.security.TenantClaimValidator;
 ...
 * depth — the decode-time {@link TenantClaimValidator} and this filter each
 * independently enforce the invariant). Applies the same entitlement-trust
 * dual-accept gate (ADR-MONO-019 § D5), reusing
 * {@link TenantClaimValidator#isEntitled} so both points share one source of
 * truth. Mirrors masterdata-service exactly.
 */
@Slf4j
@Component
