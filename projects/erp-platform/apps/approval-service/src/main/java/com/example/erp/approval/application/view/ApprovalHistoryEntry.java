 ...
 * {@code ApprovalHistoryEntry}). {@code reason} is ABSENT when none
 * ({@code @JsonInclude(NON_NULL)}).
 */
@JsonInclude(JsonInclude.Include.NON_NULL)
public record ApprovalHistoryEntry(String transition, String actor, Instant at, String reason) {

    public static ApprovalHistoryEntry from(ApprovalAction a) {
        return new ApprovalHistoryEntry(a.getTransition().name(), a.getActor(),
                a.getOccurredAt(), a.getReason());
    }
}
