...
- `TASK-ERP-BE-006-masterdata-response-effectiveperiod-contract-conformance.md` — **REVIEW (2026-06-03, impl on `task/erp-be-masterdata-effectiveperiod-contract-conformance`)**. masterdata-service response **contract conformance**: the 5 master `*View` records emitted FLAT `effectiveFrom`/`effectiveTo`/`createdAt`/`updatedAt`, violating `masterdata-api.md` § Common shapes which (and the platform-console PC-FE-010 consumer) require nested `effectivePeriod: { effectiveFrom, effectiveTo }` (+ detail `audit`). **Surfaced by TASK-MONO-170** live demo: console ERP 운영 received 200 from all 5 masters but every response failed the consumer Zod parse (`erp_ok`→`erp_error`) → degraded. Latent because the console client had only mocked unit tests + the producer slice test asserted only `$.data.code`/`status` — no test ever exercised the real producer→console JSON. **Fix**: new `EffectivePeriodDto` + `AuditDto` view records; 5 Views (`Department`/`Employee`/`JobGrade`/`CostCenter`/`BusinessPartner`) reshaped flat→nested + `from()` factories; updated `DepartmentControllerSliceTest` (constructor + **new nested-shape regression assertion**: `$.data.effectivePeriod.effectiveFrom` exists, `$.data.effectiveFrom` doesNotExist) + `MasterdataLifecycleIntegrationTest` accessors. **NO domain/persistence/auth/contract/ADR change** (contract already specifies nested; producer brought into conformance). **Verified**: `:check` GREEN (slice test passes real Jackson serialization through MockMvc = definitive nested-shape proof); rebuilt masterdata-service in the MONO-170 demo stack (healthy + reachable). AC-7 (console ERP 운영 globex renders 5 masters live) = user re-click confirm. 분석=Opus 4.8 / 구현=Opus(직접, 라이브 재배포).
