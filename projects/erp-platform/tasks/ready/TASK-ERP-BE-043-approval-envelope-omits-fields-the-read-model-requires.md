...
- [ ] **AC-7 (래칫이 실제로 도는가)** — `ADR-ERP-001` D 가 명명한 래칫(erp 전체에서 distinct
...
- [ ] **AC-8 (두 번째 관문 + 알림함)** — `notification-service` 의 `EnvelopeToCommandMapper`
...
- [ ] **AC-9 (계약 리터럴)** — 두 계약 문서의 봉투 예시 `"tenantId": "erp"` 를 플레이스홀더로
