...
| Data store | MySQL `erp_approval_db` (its **own database** on the shared MySQL instance — TASK-ERP-BE-035; tables `approval_request` / `approval_route` / `approval_action` / `approval_audit_log` / `outbox` / `processed_events` / `idempotency_keys`). Previously shared `erp_db` with masterdata/notification, but each service ships its own `V1__init` + generic `outbox` / `processed_events` / `idempotency_keys` → one `flyway_schema_history` + identical table names collided (crash-loop). A DB per service (mirroring read-model's `erp_read_model_db`) gives each its own Flyway history and tables; no cross-service JOIN. |
