...
### Local (Docker Compose, hostname routing)

```bash
# 1) 한 번만 — *.local hosts 등록 + 공유 Traefik 기동 (모노레포 루트에서)
bash scripts/dev-setup.sh           # Linux/macOS
.\scripts\dev-setup.ps1              # Windows (Run as Administrator)
pnpm traefik:up

# 2) 전체 서비스 실행
pnpm ecommerce:up                    # docker compose up -d 의 단축 명령
...
> 호스트네임 라우팅은 [ADR-MONO-001](../../docs/adr/ADR-MONO-001-port-prefix-scaling.md) Option C 정책에 따른다. 이전의 `PORT_PREFIX` 규약은 TASK-MONO-024 에서 폐지되어 더 이상 사용되지 않는다.
