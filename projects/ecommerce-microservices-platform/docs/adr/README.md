...
| [ADR-008](ADR-008-internal-endpoint-auth-boundary.md) | ~~내부 전용 엔드포인트(`/api/internal/**`) 인증 경계를 네트워크-단독 → 앱-레이어 방어심층으로 승격~~ (철회 — 전제 phantom: 트리거가 decommissioned auth-service, 라이브 order-service 는 이미 BE-412 client_credentials JWT 로 해결) | Withdrawn |
