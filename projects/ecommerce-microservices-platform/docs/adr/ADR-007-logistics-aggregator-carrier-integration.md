...
1. **TASK-BE-358** — 중개사(aggregator) 어댑터 매핑 + credential 배선. D2 실현의 헤드라인.
...
- **리스크 표면**: 중개사 응답 vocab 이 `CarrierStatusMapper` 매핑을 벗어나면 no-op(상태 정체)로 나타난다 — best-effort 설계상 크래시가 아니라 미동작이므로, 미매핑 상태는 메트릭/로그로 가시화해야 한다(BE-358 범위).
