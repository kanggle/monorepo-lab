...
| ID | Title | Service | Tags |
|---|---|---|---|
| TASK-BE-435 | **READY**. Stuck payment-pending 주문 종결을 `STUCK_RECOVERY_FAILED`(운영 핸드오프) → **`CANCELLED(PAYMENT_TIMEOUT)`** 자동취소로 전환 + 늦게 도착한 결제의 money-safe 보상(payment-service: COMPLETED→자동환불 / PENDING→void 가드 + 캡처후 자동역환불). 재고복원 불요(stuck=미예약, BE-428). 상태머신/이벤트 계약(`OrderCancelled.cancelReason` 추가) + ADR-MONO-005 §D3 Category A 종결 refine. 분석=Opus 4.8 / 구현 권장=Opus(결제정합 레이스). | order-service, payment-service | code, event, saga, payment, adr |
