...
| TASK-BE-443 | **READY (money-safety)**. BE-442가 노출한 잠복 제품 결함: confirm() post-capture re-read(`findByOrderId`, 동일 `@Transactional` Hibernate Session)가 **stale L1-managed PENDING** 엔티티를 반환해 동시 커밋된 VOIDED 컬럼을 버림(세션 엔티티 동일성=DB 격리수준 위) → 동시 void 미감지 → 자동역환불·stranded 스킵 → `@Version` 없어 last-writer-wins로 **취소주문에 COMPLETED+PaymentCompleted**(자금 보유, BE-435/437 가드가 그 race에서 dead). 단위테스트는 mock-repo 2-stub(PENDING→VOIDED)라 가짜 green. 수정=`PaymentRepository.findByOrderIdFresh`(impl `entityManager.refresh`로 fresh 재SELECT) + L88 swap, 성공경로 byte-equivalent. durability IT(BE-442 흡수)는 REQUIRES_NEW로 VOIDED-during-capture 결정론적 주입해 해제=회귀증명. | payment-service | code, bug, payment, money-safety |
