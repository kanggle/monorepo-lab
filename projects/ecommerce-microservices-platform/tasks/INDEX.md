...
| ID | Title | Service | Tags |
|---|---|---|---|
| TASK-BE-435 | **REVIEW** (impl in PR). Stuck payment-pending 주문 종결을 `STUCK_RECOVERY_FAILED` → **`CANCELLED(PAYMENT_TIMEOUT)`** 자동취소로 전환(ADR-MONO-005 §2.3 D3 auto-resolving refinement, 선행 TASK-MONO-306 머지됨). order: `CancelReason` enum + `Order.cancel(reason)` + `OrderCancelledEvent.cancelReason`(additive) + stuck-handler cancel+OrderCancelled+informational alert 공동커밋. payment: `PaymentStatus.VOIDED` + consumer `handleOrderCancelled`(COMPLETED→환불/PENDING→void) + confirm 전·후 가드(캡처후 re-read 자동역환불)로 양쪽 순서 money-safe. product: 무변경(NEW release no-op, AC-4 테스트). 3서비스 unit GREEN(order 87/payment 68/product 10), AC-2/3 IT는 CI-Linux 권위. 분석=Opus 4.8 / 구현=2×Opus backend-engineer + Opus 검수. | order-service, payment-service | code, event, saga, payment, adr |
