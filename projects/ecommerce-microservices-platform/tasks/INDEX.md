...
| TASK-BE-438 | **READY**. BE-437 후속 — 묶인 환불(stranded refund) 자동 재조정 sweeper(Category-A saga, ADR-MONO-005 §2.3 D3). `stranded_refund` 테이블 영속화 + `@Scheduled` sweeper가 **PG 상태 선조회**(이미 취소면 RESOLVED, 이중환불 방지) 후 재취소, bounded-retry→terminal `UNRESOLVED`+에스컬레이션. 신규 세션 구현 권장(Opus). | payment-service | code, event |
