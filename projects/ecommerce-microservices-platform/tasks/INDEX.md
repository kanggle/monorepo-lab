...
| ID | Title | Service | Tags |
|---|---|---|---|
| TASK-BE-365 | **READY**. ecommerce **marketplace settlement / commission** (ADR-MONO-030 **Step 4 facet b**). 신규 `settlement-service`(event-consumer + rest-api, terminal — outbox 제외)가 `order.order.placed`(라인 스냅샷)+`payment.payment.completed`(accrual 트리거)+`payment.payment.refunded`(reversal)를 소비 → order-line 단위 **플랫폼 수수료** vs **셀러 net** 분할(`commission=round(gross×rate_bps/10000)` HALF_UP, 셀러별 율+플랫폼 기본), append-only 불변 ledger. **★tenant 권위=OrderPlaced 스냅샷**(payment 봉투엔 tenant 없음). 셀러-스코프 ABAC read(net-zero, Step3 재사용). 기간마감/payout/뱅킹/부분환불/multi-currency=보류. Flyway V1. SoT=`specs/features/marketplace-settlement.md` + `specs/services/settlement-service/architecture.md`. 분석=Opus 4.8 / 구현 권장=Opus. ⚠️큰 세션 JDT.LS OOM→새 세션 구현 권장. | settlement-service (신규) | code, api, event, multi-tenant, marketplace |
