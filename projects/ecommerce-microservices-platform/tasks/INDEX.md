...
| TASK-BE-362 | **READY**. shipping-service 물류 중개 플랫폼(aggregator) 어댑터 매핑 + credential 배선 (ADR-007 **D2** 헤드라인). `CarrierStatusMapper` 중개사 통일코드 매핑표 보강 + 미매핑 `carrier_status_unmapped` metric/WARN + credential env-only(하드코딩 0). mock/sandbox 실증, net-zero. **구 BE-358 — `done/` webhook-repo-scan 와 ID 충돌하여 재번호.** 분석=Opus 4.8 / 구현 권장=Opus. | shipping-service | code, api, adr |
| TASK-BE-359 | **READY**. shipping-service `carrier-webhook` 경로 gateway public-route 노출 (ADR-007 **D5-2**) — 현재 미노출이라 중개사 inbound webhook 도달 불가. (선행: BE-362.) 분석=Opus 4.8 / 구현 권장=Sonnet — gateway route 설정. | gateway-service (+shipping 계약) | code, api, deploy |
| TASK-BE-360 | **READY**. shipping-service 무인 auto-collect tracking 스케줄러 (ADR-007 **D5-3**, ShedLock; in-flight 배송 폴링). ⚠️ShedLock IT 트랩=use-case 직접 호출로 락 우회. (선행: BE-362.) 분석=Opus 4.8 / 구현 권장=Opus. | shipping-service | code, test |
| TASK-BE-361 | **READY**. shipping-service carrier webhook dedup-marker(`processed_carrier_webhooks`) retention/cleanup sweep (ADR-007 **D5-4**, ShedLock). (선행: BE-362/359.) 분석=Opus 4.8 / 구현 권장=Sonnet. | shipping-service | code, test |
