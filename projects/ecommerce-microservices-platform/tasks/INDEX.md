...
| TASK-BE-442 | **READY (money-safety, test-rework)**. BE-440 후속: `PaymentRefundStrandedDurabilityIntegrationTest`의 순차 arrange가 BE-440(OrderCancelled→VOIDED 커밋) 이후 도달 불가 — confirm()의 pre-capture VOIDED 가드가 capture 전에 reject → post-capture stranded 경로 미도달(0 rows). 진짜 불변식(BE-437 AC-2 REQUIRES_NEW durability)은 **VOIDED-during-capture 동시 interleave**(confirmPayment mock Answer가 별도 커넥션에서 VOIDED 커밋) 필요. production 무변경(confirm()은 모든 순서에서 money-safe), 테스트측 동시성 주입 rework + 격리 해제. deterministic(sleep 금지). | payment-service | code, test, payment, money-safety |
