...
| TASK-INT-024 | **DONE (2026-06-02)**. ADR-MONO-021 §3.3 step 3(D4 step3) — web-store GAP e2e 가 `account_type=CONSUMER` 클레임의 full OIDC 왕복(GAP id_token→NextAuth profile→jwt→session→`/api/auth/session`)을 단언. `account-type-claim.spec.ts`(`shouldSkipGap()` 게이트→기존 CI skip, nightly GAP suite 실행) + `gap-consumer-seed.sql` account_type='CONSUMER' 명시(+구식 "미발급" 주석 정정). 정적검증: tsc clean + compose config valid. 클레임 발급은 BE-329 `FormLoginIntegrationTest`(실 MySQL, 실제 SAS 토큰 디코드)로 결정론적 증명 — 본 spec 은 NextAuth 전파 레이어 추가(AC-1 라이브=nightly GAP CI 위임; 실행 중 GAP 이미지가 account_type 이전이라 로컬 재빌드는 호스트 fragility 대비 비례성 낮음). impl PR #1014 squash `aa3ec383`. 3차원 ✓. **ADR-MONO-021 §3.3 steps 1–3 완료.** 분석=Opus 4.8 / 구현=Opus. | web-store | test, e2e, auth, multi-tenant |
