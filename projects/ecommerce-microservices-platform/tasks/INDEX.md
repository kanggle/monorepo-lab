...
| TASK-BE-441 | **READY**. TASK-MONO-307 잔여 triage(systemic 2버그 외): order IT assertion/behavioral 5건 — 403 error-code drift(`$.code` UNAUTHORIZED vs ACCESS_DENIED, Query/Cancellation 2), seller-scope ABAC AC-3, `@Version` optimistic-lock 미발생 + saveAll IllegalStateException 미발생, refund-on-PENDING DLQ 예외 미전파. 대개 테스트 기대 drift(인스코프 수정) 추정, 일부 제품 회귀 가능→분류 후 수정/재티켓. 메서드별 `@Disabled` 격리됨(통과 테스트 보존). | order-service | code, test, triage |
