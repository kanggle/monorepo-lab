...
- [ ] **AC-1** — `account-type-guard.spec.ts` 에서 `test.describe.configure({ timeout: ... })` 와 그 근거 주석이 제거됐다. 남은 주석은 **CI 가 실증한 것만** 말한다(가드가 무는 이유, 왜 이 스펙이 예전엔 못 돌았는지). 성능에 대한 인과 주장은 남기지 않는다.
...
- [ ] **AC-4 (무손실)** — `tsc --noEmit` exit 0, `next lint` 경고/에러 0. 나머지 11개 테스트 통과 유지.
