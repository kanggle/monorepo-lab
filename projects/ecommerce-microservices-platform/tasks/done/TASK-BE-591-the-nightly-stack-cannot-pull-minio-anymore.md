...
- [x] 🔴 **이 잡은 PR 에서 안 돈다** — `nightly-e2e.yml` 은 `push` 로 `main` 에서만 돈다.
      ⇒ PR 초록은 이 결함에 대해 **아무것도 증명하지 않는다**. 머지 후 그 커밋의
      `Frontend E2E full-stack` 결론을 **직접 열어서** 확인하라.
      → 🟢 머지 커밋 `c06c4ca76` 의 그 잡(`job=103430349177`) = **success**. 결론만 읽지 않고
      **스텝 27개를 전부 열었다**.
- [x] 실패가 **같은 자리(스택 기동)에서 사라졌는지**로 판정하라. 🔵 그 뒤에 다른 자리에서
      깨지면 그건 **이 티켓이 가려 두고 있던 다음 결함**이지 회귀가 아니다 — 따로 기안하라.
      → 🟢 **그 자리에서 사라졌다**: 죽던 `#14 Start docker compose stack` = **SUCCESS**.
      🔴 그리고 **`#21 Assert the required specs actually ran` 도 SUCCESS** 다 — 「잡이 초록」과
      「스펙이 실제로 돌았다」는 다른 명제이고, 이 결함은 **테스트를 0개 돌리고도 잡을 끝낼 수
      있는** 부류였으므로 후자를 따로 확인해야 판정이 성립한다. 🔵 뒤에서 새로 깨진 자리는 없다.
...
| 차원 | 결과 |
|---|---|
| (a) PR | 🟢 `#3799` `state=MERGED` |
| (b) 커밋 | 🟢 `c06c4ca764f0c5f73296a6a0bcfe78614f0e01d9` 가 `origin/main` 에 있다 |
| (c) 머지 전 required | 🟢 4/4 SUCCESS · 전체 FAILURE **0** (SKIPPED=45 SUCCESS=18) |
| (d) AC | 🟢 AC-0·1·2·3 전부, **그 칸이 쓴 동사로** 닫았다 |

## 🔴 (c) 가 이 티켓에서는 판정이 아니다 — 그게 이 티켓의 요지다

이 결함을 만든 잡은 **required 4종에 없다**. ⇒ 여섯 커밋이 전부 (c) 를 통과하며 머지됐고,
**PR 단위 검증으로는 영원히 안 보였다.** 실제 판정은 (b) 의 커밋에서 그 잡을 **직접 연 것**이다:

```
#14 Start docker compose stack …            SUCCESS   ← 죽던 자리
#21 Assert the required specs actually ran  SUCCESS   ← 테스트가 진짜 돌았다
```

🔵 **두 번째 줄이 없으면 판정이 아니다.** 이 결함은 테스트를 **0개 돌리고도** 잡을 끝낼 수 있는
부류였다(실제로 그렇게 여섯 번 끝났다) — 「잡이 초록」과 「스펙이 돌았다」를 구별해야 한다.

## 🔵 이 티켓이 실제로 알려준 것

결함의 내용(레지스트리 4줄)이 아니라 **어떻게 발견됐는가** 다.

§0 의 *"세션 끝에 이번 세션의 머지 전부를 **커밋별로** CI 확인해라"* 가 없었다면 이건 안 보였다.
🔴 **머지마다 초록이었고, 마지막 tip 도 «required 는» 초록이었다.** 커밋별로 열어야만
«required 밖의 잡이 여섯 커밋 연속 빨갛다» 가 보인다.

⇒ 🔵 `CLAUDE.md` 의 *"dimension (c) passing does NOT mean the suites were green"* 이
**이번에 실물로 한 번 더 참이 됐다.**

## ⚪ 닫으면서 들고 나가는 것 — 없다

- ⚪ Hub 판↔quay 판 바이트 동일성 / 로컬 pull — 둘 다 **AC-3 이 「적어라」로 요구한 ⚪** 이고
  적었다. 🔴 **후속 집을 만들 이유가 없다**: Hub 가 닫혀 원본이 영영 없으므로 그 측정은
  «다음 창에» 가능해지는 종류가 아니다. 창을 기다리는 ⚪ 와 **원리상 못 재는 ⚪** 는 다르다.
- 🔵 **quay.io 도 언젠가 닫힌다**는 축은 이 티켓이 해결하지 않았고 § Edge Cases 가 그걸 적었다.
  근본 처방(사내 미러 / pull-through 캐시)은 **비용이 드는 소유자 결정**이라 여기 안 끼웠다.
