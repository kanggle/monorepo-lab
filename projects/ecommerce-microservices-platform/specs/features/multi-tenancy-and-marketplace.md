...
| **평면 — roles** | 소비자/운영자 분리 | `roles` (소비자 `CUSTOMER` / 운영자 `ECOMMERCE_OPERATOR`) | 이미 존재 | **재사용** — IAM 발급 + gateway `AccountTypeEnforcementFilter` roles-only 강제 (TASK-BE-131; ADR-MONO-032/035 가 레거시 `account_type` 평면을 roles 로 대체) |

**복합 신원 = `tenant_id`(어느 스토어) × `roles`(소비자 `CUSTOMER` / 운영자 `ECOMMERCE_OPERATOR`) × `seller_id`(어느 셀러; 운영자 평면 한정).**
...
| 평면 | `roles` (소비자 `CUSTOMER` / 운영자 `ECOMMERCE_OPERATOR`) **이미 존재** (IAM 발급, gateway 강제) | 변경 없음 (재사용). 셀러 = 운영자(`ECOMMERCE_OPERATOR`) 평면 내부 `seller_id` |
...
- **per-tenant rate limit** (`gateway-service`): route 의 `RequestRateLimiter` 필터가 **`(tenant_id, route_id)` 튜플 키**로 동작 (`tenantRouteKeyResolver`, 키 = `rate:ecommerce-gw:<routeId>:t:<tenantId>`). 한 테넌트의 burst 가 다른 테넌트의 버킷을 소모하지 않음 (M7 line 84). tenant 는 JWKS-검증된 JWT 의 `tenant_id` claim 에서 **reactive security context** 로 추출 (ThreadLocal 아님, non-blocking).
  ...
  - **limit source**: route 당 config 기본값(`replenishRate`/`burstCapacity`, application.yml) **+ 옵션 per-tenant override 구현**(TASK-BE-405 AC-2). config = `ecommerce.ratelimit.overrides.<tenantId>.<routeId>.{replenish-rate,burst-capacity}`(`RateLimitOverrideProperties`). 체인 = `FailOpenRateLimiter → OverrideAwareRateLimiter → RedisRateLimiter`: `OverrideAwareRateLimiter` 가 키에서 `<tenantId>` 를 파싱해 `(tenantId, routeId)` 유효 override(`burst ≥ replenish ≥ 1`) 존재 시 override 한도를 seed 한 캐시 `RedisRateLimiter` 로 위임, 없으면 route 기본값으로 위임. override·default 림미터가 동일 Redis 템플릿·Lua 스크립트를 공유 → 동일 `(tenant, route)` 버킷(크기만 다름, 격리 유지). **override 미설정 시 net-zero**(기본값) — 전 호출이 default 경로, config-default-only 와 동치. entitlement 평면과 decouple(`tenant_domain_subscription` 무결합). 위반 → **429 `TOO_MANY_REQUESTS`**.
...
### 2.7 `ADR-MONO-058` §2 D8 — `TenantClaimEnforcer` 미채택, defer (TASK-BE-571, 2026-08-01)

> **주의 — D8 이름 충돌**: 이 절의 "D8" 는 `ADR-MONO-058` §2 의 D8 항목(`libs/java-security-servlet.TenantClaimEnforcer` 채택)을 가리킨다. §2.5/§2.6 및 여러 service architecture.md 가 쓰는 "(D8)"(`ADR-MONO-030` 의 default-tenant net-zero degradation)과는 **다른 D8** 이다 — 항상 전체 인용(`ADR-MONO-058 §2 D8`)으로 구분할 것.

`ADR-MONO-058` §2 D8 은 "나머지 fleet 이 이미 한 번 거친 통합(`ADR-MONO-049`) 을 ecommerce 만 안 했다 — 새 결정이 아니라 이미 내려진 결정에 대한 gap 을 닫는 것"이라고 서술한다. `TASK-BE-571` 의 조사 결과 이 프레이밍은 ecommerce 에 그대로 적용되지 않아 — 옵션 (a)/(b)/(c) 중 명시적으로 **(c) defer** 를 선택했다:

- **현행 유지**: ecommerce 의 tenant 전파 메커니즘은 여전히 이 문서 §2.2 M2 가 서술하는 3-layer 구조 — 서비스별 로컬 `TenantContext`(plain `ThreadLocal`, `DEFAULT_TENANT_ID` net-zero fallback 포함) + `TenantContextFilter`(gateway 가 forward 한 `X-Tenant-Id` **헤더**를 읽어 바인딩) 10-copy 그대로다. 코드 변경 없음.
- **왜 `TenantClaimEnforcer` 가 오늘 안 맞는가**: `TenantClaimEnforcer`(`libs/java-security-servlet`)는 요청-거부 필터로, `SecurityContextHolder` 에 이미 올라온 `JwtAuthenticationToken` 의 `tenant_id` claim 을 읽는다 — 즉 서비스가 **자체 Spring Security OAuth2 Resource Server JWT 디코드 체인**을 이미 돌렸다고 가정한다. ecommerce 의 10 개 서비스는 오늘 그 체인이 없다 — `gateway-service` 만 RS256 JWT 검증을 하고, 나머지는 gateway 가 enrich 한 헤더(`X-Tenant-Id` 등)를 신뢰하는 구조다(`PROJECT.md` Service Map). `ADR-MONO-058` 자신의 D4("보안-체인 조립") 테이블도 이 로컬 JWT 체인 채택 대상에 `scm/erp/wms/fan` 만 올렸을 뿐 **ecommerce 는 확인된 바 없다**. `TenantClaimEnforcer` 도 propagation(다운스트림에 tenant 를 노출하는 메커니즘)이나 net-zero fallback 을 제공하지 않는다 — 순수 게이트일 뿐, `TenantContext`/`TenantContextFilter` 가 겸하는 두 역할(바인딩/전파 + net-zero 기본값)을 대체하지 못한다.
- **선행 조건(전환 시 필요한 것)**: `ADR-MONO-058` §2 D4 급의 작업 — ecommerce 의 10 개 서비스에 로컬 Spring Security OAuth2 Resource Server JWT 체인을 세우는 것 — 이 먼저 확정·적용되어야 한다(`ADR-MONO-058` §4 가 스스로 "D1/D4 는 fleet 전체에서 가장 위험도 높은 카테고리"라 명시). 그 전까지는 `TenantClaimEnforcer` 를 그대로 붙이면 `SecurityContextHolder` 에 `JwtAuthenticationToken` 이 전혀 없어 필터가 매 요청 `chain.doFilter` 로 조용히 통과시키는 **100% no-op 상태**가 된다(`TenantClaimEnforcer` 자신의 `if (!(auth instanceof JwtAuthenticationToken jwtAuth))` 조기 반환) — 이것이 defer 를 선택한 핵심 이유.
- **`TASK-BE-564` 선례**: `ADR-MONO-058` §2 D8 텍스트가 언급하는 "`TASK-BE-557`"(settlement-service `TenantContext.set()` 의 `.trim()` divergence 수정) 은 이 프로젝트 tasks 에 실재하지 않는 phantom ID 다 — 같은 결함을 고친 실제 작업은 `tasks/done/TASK-BE-564-settlement-tenantcontext-trim-divergence-fix.md`(DONE, 3-dim 검증, impl PR #3081) 이며, 향후 이 선례를 인용할 때는 `TASK-BE-557` 이 아니라 **`TASK-BE-564`** 를 참조한다. `TASK-BE-571`(2026-08-01) 이 재확인한 바, 이 fix 는 여전히 유효(no regression) — `settlement-service`'s `TenantContext.set()` 은 `.trim()` 없이 verbatim 바인딩한다.
- **재평가 트리거**: (1) `ADR-MONO-058` §2 D4 급 로컬 JWT 체인 도입이 ecommerce 에 대해 별도로 확정되거나, (2) `TenantClaimEnforcer` 자체가 헤더-신뢰(header-trust) 모드를 갖도록 `libs/java-security-servlet` 이 확장되면(`platform/shared-library-policy.md § Review Rule` 대상, 프로젝트 내부 태스크 권한 밖) — 그때 이 절을 재검토한다.
...
| 운영자/셀러-어드민 | `roles ∋ ECOMMERCE_OPERATOR` | `/api/admin/**` | IAM (assume-tenant 도메인 롤 파생, ADR-035 4a) |

- **단일 IdP(IAM), 평면 2개 = `roles` (`CUSTOMER` / `ECOMMERCE_OPERATOR`)**. 별도 ecommerce auth-service 아님 (auth-service 는 제거됨, TASK-BE-132).
- **셀러 축은 운영자(`ECOMMERCE_OPERATOR`) 평면 내부**: `seller_id` 는 운영자 토큰/스코프에만 의미 있음. 소비자(`CUSTOMER`)는 `seller_id` 권위 없음(상품을 *볼* 뿐, 셀러로서 행위 불가).
