...
- `account_type` 클레임 (`CONSUMER` | `OPERATOR`) 으로 web-store / platform-console 권한을 분리한다 (gateway `AccountTypeEnforcementFilter` 가 `/api/admin/**` 경로에 `OPERATOR` 강제 — TASK-BE-131).
...
| `ecommerce.operator` | platform-console operator 권한 (IAM `account_type=OPERATOR` 토큰만 발급) |
