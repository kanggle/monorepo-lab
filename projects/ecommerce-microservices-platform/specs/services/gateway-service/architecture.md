...
- **Limit source** — per-route default `replenishRate`/`burstCapacity` in the route filter args **plus an optional per-tenant override** (implemented, TASK-BE-405 AC-2). Override config (`config/RateLimitOverrideProperties`, `@ConfigurationProperties("ecommerce.ratelimit")`):
  ```yaml
  ecommerce:
    ratelimit:
      overrides:
        <tenantId>:                # the tenant_id claim
          <routeId>:               # the Spring Cloud Gateway route id
            replenish-rate: 500
            burst-capacity: 1000
  ```
  Resolution chain is `FailOpenRateLimiter → OverrideAwareRateLimiter → RedisRateLimiter`: `OverrideAwareRateLimiter` (`ratelimit/OverrideAwareRateLimiter`) parses `<tenantId>` back out of the resolved key (`...:t:<tenantId>[:ip:<ip>]`) and, when a **valid** override (`burst ≥ replenish ≥ 1`) exists for `(tenantId, routeId)`, delegates to a small cached `RedisRateLimiter` seeded with the override limits; otherwise it delegates to the route/config default. Override and default limiters share the same Redis template + Lua script, so `getKeys(id, routeId)` targets the **same per-`(tenant, route)` bucket** — only its size differs, keeping `(tenant, route)` isolation intact. **Net-zero when no overrides are configured** (the default): every call hits the default limiter, behaviour identical to config-default-only. No coupling to `tenant_domain_subscription` (entitlement plane stays decoupled). Breach → **429 `TOO_MANY_REQUESTS`**.
- **Degrade** — `tenant_id` absent (standalone / no IAM) → default-tenant single bucket (D8 net-zero). **Redis unavailable → fail-open** via `FailOpenRateLimiter` (`ratelimit/FailOpenRateLimiter`, the scm/wms/fan precedent): only Redis-class errors are allowed through with sentinel `X-RateLimit-Remaining: -1` + `gateway_ratelimit_redis_unavailable_total`; non-Redis errors propagate (5xx) + `gateway_ratelimit_unexpected_error_total`. `FailOpenRateLimiter` is the **outermost** decorator (wraps `OverrideAwareRateLimiter`), so the per-tenant-override path and the default path inherit identical fail-open semantics. Rate limiting is additive, never a hard dependency.
