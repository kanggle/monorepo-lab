...
1. **JWT validation 통과 없이는 어떠한 private `/api/...` 요청도 downstream 에 도달하지 않는다** — GAP JWKS 서명 + `aud=ecommerce` + `tenant_id=ecommerce` 모두 통과 필수.
...
- Token issuance / refresh — `auth-service-deprecated` (v1 legacy) 또는 GAP 가 직접 (전환 진행 중).
