...
- **JWT validation** — OAuth2 Resource Server against IAM JWKS (`/.well-known/jwks.json`); validates RS256 signature, `aud` ∩ client-id allowlist (`ecommerce.oauth2.allowed-audiences`; shadow mode until TASK-MONO-696 phase 2), `tenant_id=ecommerce` (see [`../../integration/iam-integration.md`](../../integration/iam-integration.md)).
...
1. **JWT validation 통과 없이는 어떠한 private `/api/...` 요청도 downstream 에 도달하지 않는다** — IAM JWKS 서명 + `aud` ∩ client-id allowlist (`ecommerce.oauth2.allowed-audiences`; shadow mode until TASK-MONO-696 phase 2) + `tenant_id=ecommerce` 모두 통과 필수.
