...
| Order state machine | `PENDING → CONFIRMED → SHIPPED → DELIVERED`; `BACKORDERED` (TASK-BE-428: payment-reservation short → held; `BACKORDERED → CONFIRMED` on restock re-reservation, `BACKORDERED → CANCELLED` on operator cancel); `CANCELLED` (from `PENDING\|CONFIRMED\|BACKORDERED`); terminal `STUCK_RECOVERY_FAILED`. Operator-initiated cancel via internal API (`/api/internal/orders/{orderId}/cancel`). |
