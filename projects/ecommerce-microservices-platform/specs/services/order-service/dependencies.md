...
- all inbound interactions arrive through gateway-service — **with one exception, the gateway-excluded `/api/internal/orders/**` route**, called directly on the internal network by batch-worker (`client_credentials` Bearer, fail-closed). Two endpoints live there: (1) the system-command `POST /api/internal/orders/confirm-paid-stale` — order-service evaluates the predicate + performs the `PENDING → CONFIRMED` transition server-side and emits the standard `OrderConfirmed`. Contract: `specs/contracts/http/internal/order-confirm-paid-stale.md` (TASK-BE-410 decision; impl TASK-BE-412). (2) the **read-only** `POST /api/internal/orders/existence` — which of the given order ids exist in any tenant, so batch-worker can release coupons held by orders that were never saved. Contract: `specs/contracts/http/internal/order-existence.md` (TASK-INT-028).
