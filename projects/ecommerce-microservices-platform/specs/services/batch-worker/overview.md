...
- ~~**Daily sales aggregation** — order / payment 합산 일일 요약 (downstream 분석 dashboard 용).~~ **DEFERRED — 미구현·제거 (TASK-BE-411 결정 2026-06-20)**: 소비자가 없다. 유일한 매출 대시보드(admin 홈, TASK-FE-063)는 백엔드 집계 테이블을 명시적으로 out-of-scope 로 두고 `GET /api/orders` 에서 **FE-side 집계**를 택했다. 미래에 실 분석 소비자가 spec 화되면, 일일 money 롤업의 자연스러운 home 은 batch-worker 가 아니라 settlement-service(이미 `occurred_at` 인덱스 money 행 + forward-declared period-close 보유)다. 소비자 부재 상태로 `daily_sales_summary` 테이블을 만드는 것은 speculative infrastructure 이므로 도입하지 않는다.
- **Orphan-coupon release** (TASK-INT-028) — 저장된 적 없는 주문에 `USED` 로 묶인 쿠폰을 푼다. 주문 롤백 뒤 order-service 의 release 가 promotion-service 에 도착하지 못하면 아무것도 그 쿠폰을 풀지 않는다(취소 복원은 주문이 있어야 발화). 흐름: promotion-service `POST /api/internal/coupons/stale-used`(`USED` ≥ 60분) → order-service `POST /api/internal/orders/existence`(테넌트 무관) → **존재하지 않는** 주문의 쿠폰만 promotion-service release(쿠폰 자신의 `X-Tenant-Id`). 🔴 **「모름」≠「없음」**: 존재 조회가 실패하거나 본문이 비면 그 회차는 아무것도 풀지 않고 `FAILED`. 🔴 **전부-없음 브레이크**: 한 회차에 `min-scanned-for-all-absent-abort`(기본 20) 건 이상을 봤는데 존재하는 주문이 **0건**이면, 설정이 빈 환경을 가리킨다고 보고 아무것도 풀지 않고 `FAILED`. 한 건의 release 실패는 나머지를 멈추지 않는다(다음 회차가 재시도).
...
| Spring Scheduler | `orphanCouponReleaseJob` (`@Scheduled(cron = "0 5,35 * * * *")`) | client_credentials (outbound to order-service); none (promotion-service internal network) | every 30 min: release coupons `USED` by orders that do not exist in any tenant (TASK-INT-028). Never releases on an unknown answer; aborts on an all-absent batch of ≥ 20. |
...
| HTTP outbound | `order-service` (`POST /api/internal/orders/confirm-paid-stale`) | client_credentials Bearer JWT (`ecommerce-internal-services-client`) | stale paid-order forward-confirm — internal system-command (TASK-BE-410 decision) |
| HTTP outbound | `order-service` (`POST /api/internal/orders/existence`) | client_credentials Bearer JWT (same client) | read-only existence check for orphan-coupon release (TASK-INT-028) |
| HTTP outbound | `promotion-service` (`POST /api/internal/coupons/stale-used`, `POST /api/internal/coupons/{couponId}/release`) | none — internal network only | orphan-coupon release: read-only list, then release with the coupon's `X-Tenant-Id` (TASK-INT-028). The second non-read-only internal call. |
...
- `order-service` — HTTP system-command (`POST /api/internal/orders/confirm-paid-stale`, `client_credentials` Bearer) for stale paid-order forward-confirm (order-service evaluates the predicate server-side), and HTTP read-only `POST /api/internal/orders/existence` for orphan-coupon release (TASK-INT-028)
- `promotion-service` — internal network: read-only `POST /api/internal/coupons/stale-used` and system-command `POST /api/internal/coupons/{couponId}/release` for orphan-coupon release (TASK-INT-028)
