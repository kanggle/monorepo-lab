...
| Replaced by | GAP (Global Account Platform) OIDC |
...
responsibility has been transferred to the Global Account Platform (GAP), which
...
1. **TASK-MONO-027** (PR #145) — ecommerce gateway switched from auth-service
   JWKS to `GAP /oauth2/jwks`. `TenantClaimValidator` + `AllowedIssuersValidator`
   added to enforce tenant isolation.
2. **TASK-FE-067** (PR #148) — `web-store` and `admin-dashboard` migrated from
   direct auth-service calls to NextAuth v5 + GAP OIDC
