...
| IAM (iam-platform) | topic `account.created` (IAM `account-events.md`; subscription `specs/contracts/events/account-lifecycle-subscriptions.md`, ADR-MONO-037) | Trigger WELCOME notification on new account registration (no PII personalization — emailHash-only event) |
