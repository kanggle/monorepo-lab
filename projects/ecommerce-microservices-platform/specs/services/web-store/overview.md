...
| Backend dependencies | `gateway-service` (all API traffic), GAP IdP (auth, indirect via gateway) |
...
- **Auth flows** — login / signup / refresh via `/api/auth/...` (currently `auth-service-deprecated`; GAP OIDC 전환 진행 중).
