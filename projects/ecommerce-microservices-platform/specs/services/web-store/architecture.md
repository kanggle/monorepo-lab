...
## Styling & Theming
- **No CSS framework.** Styling is a hand-rolled **design-token** system, not Tailwind. Tokens are CSS custom properties declared on `:root` in `app/globals.css` (colour, typography, spacing, radius, shadow, transition, layout) and consumed three ways: global utility classes (`.btn`, `.card`, `.input`, …) in `globals.css`, per-component **CSS Modules** (`*.module.css`), and inline `style={{ … 'var(--token)' }}`. `@repo/ui` primitives are style-light (inline styles), so the storefront owns its visual language.
- **Semantic colour tokens** decouple components from raw hex. Surfaces use `--color-bg` (page canvas) vs `--color-surface` (elevated: cards, header, inputs, dropdowns); text uses `--color-text` / `--color-text-secondary` / `--color-text-muted`; actions use `--color-primary` (with `--color-on-primary` for its label) and `--color-accent`. Two tokens are deliberately **theme-invariant literal white** — `--color-white` and `--color-text-inverse` — for foreground on coloured/accent fills, the hero, footer, and image overlays; they must NOT be remapped per theme.
- **Light / dark theme.** The active theme is `data-theme="light" | "dark"` on `<html>`. `globals.css` `:root` holds the light palette; `:root[data-theme='dark']` overrides only the semantic colour + shadow tokens (typography / spacing / radius are theme-agnostic). In dark mode `--color-primary` inverts to a light neutral (so it stays legible as heading / logo **text**) and pairs with a near-black `--color-on-primary` for filled buttons / badges — the "near-white primary" pattern (mirrors `platform-console`).
- **No-flash bootstrap.** `shared/ui/ThemeScript.tsx` renders a tiny synchronous inline `<head>` script (in `app/layout.tsx`) that sets `data-theme` before first paint — from the persisted choice (`localStorage['webstore-theme']`) else the OS `prefers-color-scheme`. `<html>` carries `suppressHydrationWarning` because the script mutates an attribute React also renders. Deliberately **dependency-free** (not `next-themes`) to fit the plain-CSS token system.
- **Runtime toggle.** `shared/ui/ThemeToggle.tsx` (header) is self-contained — no context provider: it reads the applied theme after mount and flips + persists the explicit choice via `shared/lib/theme.ts` (`applyTheme` / `readAppliedTheme`). It renders a neutral placeholder icon until mounted to avoid an SSR/client hydration mismatch.
...
- Consumer-role guard: only a token carrying `roles ∋ CUSTOMER` may sign in. An operator (whose ecommerce assumed token carries `ECOMMERCE_OPERATOR` / no `CUSTOMER`) is rejected at the `signIn()` + `session()` callbacks (returned session has no `accountId`) and bounced to `/login?error=account_type_mismatch` (legacy error-code string retained for UI compatibility). Role-based per ADR-MONO-035 4b — the legacy `account_type` claim was removed in ADR-MONO-032 D5 step 4.
