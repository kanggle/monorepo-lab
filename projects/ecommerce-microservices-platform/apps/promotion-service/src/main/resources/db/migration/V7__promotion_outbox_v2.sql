-- TASK-BE-444: migrate promotion-service outbox v1 -> v2 (AbstractOutboxPublisher).
--
-- Adds the v2-shaped `promotion_outbox` table consumed by PromotionOutboxPublisher
-- (extends AbstractOutboxPublisher<PromotionOutboxEntity>). Columns match
-- libs/java-messaging OutboxRowEntity — the @MappedSuperclass that
-- PromotionOutboxEntity extends (UUID event_id PK, occurred_at as the domain
-- event timestamp, retries + last_error for the v2 backoff/failure tracking).
-- Mirrors the CI-validated master_outbox table (TASK-BE-438, wms master-service).
--
...
-- fails to boot. V7 therefore ADDS `promotion_outbox` and leaves the v1 `outbox`
-- (now unused — no scheduler drives the v1 publisher anymore) and
-- `processed_events` (still the OrderCancelled-consumer dedupe table) in place.
--
...
-- `promotion_outbox`). They are deliberately ABANDONED: promotion's CouponUsed /
-- CouponExpired events have no live consumer (promotion-events.md) and are
-- re-derivable in the demo / fed-e2e / CI usage this monorepo targets.
-- (See TASK-BE-444 Edge Case "Cutover" / Failure Scenario F1.)
...
CREATE INDEX idx_promotion_outbox_pending
    ON promotion_outbox (occurred_at)
