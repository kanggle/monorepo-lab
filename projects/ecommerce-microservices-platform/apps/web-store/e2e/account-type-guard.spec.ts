 ...
 *
 * GAP V0012 시드는 web-store 가 `ecommerce.consumer` scope 만, admin-dashboard 가
 * `ecommerce.operator` scope 만 요청하도록 등록한다. 토큰의 `account_type` claim 이
 * `OPERATOR` 인 사용자가 web-store 로 진입하면:
 ...
 *
 * 이 테스트는 GAP 컨테이너 + admin/consumer 시드 모두 필요하므로 (TASK-MONO-014
