 ...
 *
 * ⚠️ 보류 중 — TASK-MONO-381. 다른 스펙들처럼 `SKIP_GAP_E2E` 로 게이트하지 않는 이유:
...
test.describe('consumer-role 교차 앱 차단 (web-store)', () => {
  test.fixme(true, 'TASK-MONO-381 — lean IAM 스택은 CUSTOMER 없는 토큰을 발급할 수 없다 (RoleSeedPolicy 가 클라이언트 platform 으로 키잉됨)');
