 ...
 * {@link #isRestricted()} to (optionally) narrow OPERATOR reads to a single
 * seller's rows — always <em>inside</em> the tenant filter (isolate-then-attribute,
 * AC-6).
 *
 * <p>Framework-free on purpose (a plain {@link ThreadLocal}): the infrastructure
 * repositories depend on it without dragging in Spring or servlet types.
 *
 ...
 * forwards the header to the shared-catalog plane, F5), a tenant operator with no
 * seller restriction, a background thread, or a unit test — or when the scope is
 * the wildcard {@code '*'}, {@link #isRestricted()} is {@code false} and reads
 * return the <em>full tenant view</em>. The seller axis NEVER fail-closes to an
 * empty result.
 */
public final class SellerScopeContext {
     ...
     * the read must add an {@code AND seller_id = :scope} predicate. {@code false}
     * for absent/blank/{@code '*'} (fail-OPEN, full tenant view).
     */
