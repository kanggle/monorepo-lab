...
interface ProductJpaRepository extends JpaRepository<ProductJpaEntity, UUID> {
    ...
    // no ILIKE) and null-guarded like categoryId/status — absent name returns the
    // full (tenant- and seller-scoped) view. LOWER(...) LIKE LOWER(CONCAT('%',?,'%'))
    // is portable across Postgres (prod) and H2 PostgreSQL-mode (tests). A leading
    // wildcard cannot use a B-tree index; a pg_trgm GIN index on name is a future
    // follow-up if catalog scan cost matters.
