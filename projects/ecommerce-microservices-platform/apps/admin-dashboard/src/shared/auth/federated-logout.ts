 ...
 * credential form. To actually sign the operator out we redirect the browser to
 ...
 *
 * admin-dashboard logout is client-driven (`next-auth/react` `signOut`), so this
 ...
 *
 * The registered `post-logout-redirect-uris` for `ecommerce-admin-dashboard-client`
 * (GAP V0012) are the app roots `http://localhost:3001/` + `http://admin.ecommerce.local/`
 ...
 * no query) so SAS matches it exactly; the app middleware then bounces the
 * now-anonymous request to `/login`.
 *
 * Mirrors the platform-console (TASK-PC-FE-033 / BE-328), fan-platform-web
 * (TASK-FAN-FE-002) and web-store (TASK-FE-070) RP-initiated logout.
 */
