...
@DisplayName("ShippingStatusChangedEventConsumer 단위 테스트 (ADR-MONO-058 D7)")
class ShippingStatusChangedEventConsumerTest {
    ...
    void stubDedupeAppliesByDefault() {
        lenient().when(eventDedupePort.process(any(), eq("ShippingStatusChanged"), any()))
    ...
    void handle_duplicate_skips() {
        ShippingStatusChangedEvent event = event("order-1", "SHIPPED");
        given(eventDedupePort.process(any(), eq("ShippingStatusChanged"), any()))
