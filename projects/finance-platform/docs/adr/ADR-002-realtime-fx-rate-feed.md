...
5. **`TASK-FIN-BE-040`** (DONE) — per-pair FX rate history **read/drill** REST 엔드포인트. `GET /api/finance/ledger/fx-rates/{foreignCurrency}/history` (base=KRW 고정, `?limit` default 50/cap 500/floor 1, `fetched_at DESC, id DESC` 정렬, unknown pair→200 empty). 도메인 포트 `findHistory(Currency,Currency,int)` Spring-free; JPA 어댑터 `PageRequest.of(0,limit)` 변환; `GetFxRateHistoryUseCase`; `FxRateHistoryResponse` DTO(F5 rate string). additive·net-zero. 잔여 deferred(당시): 콘솔 "환율 대시보드 + 수동 refresh" 표면(별도 PC-FE task) · per-tenant override · ShedLock 단일리더.
6. **`TASK-FIN-BE-041`** (DONE) — **ShedLock 단일리더** (D4 deferred 항목 실현). `SchedulerConfig` (`@EnableSchedulerLock(defaultLockAtMostFor="PT5M")` + `JdbcTemplateLockProvider` `.usingDbTime()`) + `@SchedulerLock(name="ledger-fx-rate-poll", lockAtMostFor="PT10M", lockAtLeastFor="PT5S")` on `FxRateFeedPoller.poll()` + V14 `shedlock` 테이블(MySQL InnoDB/utf8mb4). 단일 인스턴스 = 무경합 pass-through(net-zero). 잔여 deferred: 콘솔 "환율 대시보드 + 수동 refresh" 표면(별도 PC-FE task).
7. **`TASK-FIN-BE-042`** (DONE) — **per-tenant override(특수 계약환율)** 실현. V15 신규 테이블 `fx_rate_override`(PK `(tenant_id, base_currency, foreign_currency)`, `rate DECIMAL(20,8)` > 0, audit `updated_by`/`updated_at`; **no backfill** — 행 부재 = override 없음 = 피드 경로 byte-identical, net-zero) + `FxRateOverride` 도메인(@IdClass) + `FxRateOverrideRepository` 포트 + JPA 어댑터. `ResolveEffectiveFxRate` 가 **tenant-scoped** 로 우선순위 **manual > per-tenant override(`source=override:contract`, `fromFeed=false`) > feed > `FX_RATE_UNAVAILABLE`** 해석(override 시장환율 shadow; 부재 시 피드 경로 unchanged). 운영자 REST `GET`/`PUT /api/finance/ledger/settlements/fx-rate-override/{foreignCurrency}`(tenant-scoped, audit `FX_RATE_OVERRIDE_SET`, last-write-wins; 비양수/무효 rate·미지원 통화 → `VALIDATION_ERROR` 400). 테넌트 격리(tenant A override 가 tenant B 에 미적용 — AC-3). money=no float, F5 rate string. **V14 = TASK-FIN-BE-041 ShedLock 테이블 — 본 태스크는 V15 사용.** additive·net-zero. 잔여 deferred: 콘솔 "환율 대시보드 + 수동 refresh" 표면(별도 PC-FE task).
