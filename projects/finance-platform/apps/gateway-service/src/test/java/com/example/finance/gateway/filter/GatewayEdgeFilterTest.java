 ...
 *
 * <p>The filters themselves are the library's and are tested there. What is finance's — and what
 ...
 */
@DisplayName("finance 엣지 필터 — strip 집합 · 주입 헤더 · 순서 불변식")
class GatewayEdgeFilterTest {
    ...
    /**
     * Nothing in finance reads these headers today (census: zero {@code X-*} identity headers
     * consumed across account-service and ledger-service). That is exactly why the strip has to
    ...
    void stripsEveryClientSuppliedIdentityHeader() {
        MockServerHttpRequest request = MockServerHttpRequest.get("/api/finance/accounts/1")
                .header("X-User-Id", "forged-user")
                .header("X-User-Email", "forged@example.com")
                .header("X-User-Role", "FINANCE_ADMIN")
    ...
    void injectsVerifiedIdentityHeaders() {
        HttpHeaders headers = enrich(jwt(Map.of(
                "email", "ops@example.com", "role", "FINANCE_ADMIN", "tenant_id", "finance")));
        ...
        assertThat(headers.getFirst("X-User-Role")).isEqualTo("FINANCE_ADMIN");
        assertThat(headers.getFirst("X-Tenant-Id")).isEqualTo("finance");
    }
    ...
    void alwaysWritesTheRoleHeaderEvenWhenEmpty() {
        HttpHeaders headers = enrich(jwt(Map.of("tenant_id", "finance")));
    ...
    @Test
    @DisplayName("finance 는 X-Roles / X-Scopes / X-Token-Type 을 주입하지 않는다")
    void doesNotInjectHeadersOtherDomainsUse() {
        HttpHeaders headers = enrich(jwt(Map.of("tenant_id", "finance", "role", "FINANCE_ADMIN")));
    ...
    @Test
    @DisplayName("매핑 목록이 finance 정책 그대로다")
