...
class ReconciliationMatcherTest {
    ...
    @Nested
    @DisplayName("11th increment: base (FX) leg (TASK-FIN-BE-017, multi-currency)")
    class BaseLegFx {
    ...
    @Nested
    @DisplayName("13th increment: configurable base-leg FX tolerance (TASK-FIN-BE-020)")
    class Tolerance {
    ...
    /**
     * 14th increment: cross-currency base-leg matching (TASK-FIN-BE-021). A base-currency
     * (KRW) external line with NO same-currency candidate falls back to a FOREIGN internal
     * line whose carrying base (baseMoney) is within tolerance of the external KRW amount.
     * Same-currency matching takes precedence; the base comparison is the match key (no
     * AMOUNT_MISMATCH); beyond tolerance → UNMATCHED_EXTERNAL.
     */
    @Nested
    @DisplayName("14th increment: cross-currency base-leg matching (TASK-FIN-BE-021)")
    class CrossCurrencyForward {
    ...
    /**
     * 19th increment: reverse cross-currency matching (TASK-FIN-BE-027). The MIRROR of the
     * 14th increment. A FOREIGN external line (currency != KRW) carrying a declared
     * baseAmount, with NO same-currency candidate, falls back to a BASE-currency (KRW)
     * internal line whose KRW amount (money) is within tolerance of the external's
     * baseAmount. Same-currency matching takes precedence; the base comparison is the match
     * key (no AMOUNT_MISMATCH); a foreign external without a baseAmount, or beyond tolerance
     * → UNMATCHED_EXTERNAL. The two cross passes are mutually exclusive (KRW external vs
     * foreign).
     */
    @Nested
    @DisplayName("19th increment: reverse cross-currency matching (TASK-FIN-BE-027)")
    class CrossCurrencyReverse {
