 ...
 *
 * <p><b>Precedence (manual wins; feed fills the gap):</b>
 * <ol>
 *   <li>the operator <b>supplied</b> a rate → {@link ResolvedFxRate#fromFeed()} {@code = false},
 *       {@code rate} = exactly the supplied value, {@code sourceDescription = "manual"} — the
 *       cache is never consulted (<b>net-zero</b>: the provided-rate path is byte-identical to
 *       before this increment);</li>
 *   <li>the rate is <b>omitted</b> ({@code null}) — the feed must supply it (regulated
 *       fail-closed; an estimated/stale rate must never recognise P&amp;L):
 *       <ul>
 ...
 * a cached rate ≤ 0 still surfaces SETTLEMENT/REVALUATION_RATE_INVALID downstream, edge case).
 */
@Service
...
public class ResolveEffectiveFxRate {
    ...
    /**
     * The resolved rate plus its provenance. {@code fromFeed=false} = the operator's manual rate
     * ({@code sourceDescription = "manual"}); {@code fromFeed=true} = a fresh cached quote
     * ({@code sourceDescription = "feed:<source>@<asOf>"} — recorded in the audit reason for
     * traceability, AC-2).
     */
    ...
    /**
     * Resolve the rate for the {@code (base, foreign)} pair, preferring the operator-supplied
     * {@code providedRate} and falling back to the fresh cached quote only when it is omitted.
     *
     * @param base         the reporting/base currency (KRW)
     * @param foreign      the position's foreign currency
     * @param providedRate the operator-supplied rate, or {@code null} to fall back to the feed
     * @return the resolved rate + provenance
     * @throws FxRateUnavailableException when the rate is omitted and no fresh quote can supply it
     *                                    (feed disabled / no quote / stale) — 422, nothing persists
     */
    public ResolvedFxRate resolve(Currency base, Currency foreign, BigDecimal providedRate) {
