package com.example.finance.ledger.presentation.filter;

import com.example.finance.ledger.infrastructure.security.TenantClaimValidator;
 ...
 * depth — decode validator + this filter each independently enforce the
 * invariant). Applies the same <strong>entitlement-trust dual-accept</strong>
 * gate as {@link TenantClaimValidator}: pass when legacy {@code tenant_id ∈
 * {expectedTenantId, "*"}} <em>or</em> the signed {@code entitled_domains} claim
 * contains {@code expectedTenantId}; otherwise 403 {@code TENANT_FORBIDDEN}.
...
public class TenantClaimEnforcer extends OncePerRequestFilter {
    ...
    public TenantClaimEnforcer(
            @org.springframework.beans.factory.annotation.Value(
                    "${financeplatform.oauth2.required-tenant-id:finance}")
    ...
    protected void doFilterInternal(HttpServletRequest request,
        ...
        if (auth instanceof JwtAuthenticationToken jwtAuth) {
            ...
            if (!legacyOk && !entitled) {
                log.warn("TenantClaimEnforcer rejected cross-tenant request: "
                        + "tenant={} path={}", tenantId, request.getRequestURI());
