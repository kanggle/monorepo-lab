...
/**
 * <strong>finance's tenant policy</strong>, pinned — what account-service's gate admits and,
 ...
 */
@DisplayName("finance/account-service — the tenant gate's policy, both halves")
class FinanceTenantGatePolicyTest {
    ...
    class FilterAdmits {
        ...
        void exactTenantPasses() throws Exception {
            assertThat(filter(jwt("finance", null), "/api/finance/accounts/acc-1").called).isTrue();
        }

        @Test
        @DisplayName("tenant_id=* passes — the wildcard switch is ON in the wiring")
        void wildcardPasses() throws Exception {
            assertThat(filter(jwt("*", null), "/api/finance/accounts/acc-1").called).isTrue();
        }

        @Test
        @DisplayName("tenant_id=acme + entitled_domains=[finance] passes, 200")
        void entitledCrossTenantPasses() throws Exception {
            Outcome outcome = filter(jwt("acme", List.of("finance")), "/api/finance/accounts/acc-1");
    ...
    class FilterRefuses {
        ...
        void crossTenantBlocked() throws Exception {
            Outcome outcome = filter(jwt("wms", null), "/api/finance/accounts/acc-1");
        ...
        void nonEntitledCrossTenantBlocked() throws Exception {
            Outcome outcome = filter(jwt("acme", List.of("wms")), "/api/finance/accounts/acc-1");
            assertThat(outcome.called).isFalse();
            assertThat(outcome.status).isEqualTo(403);
            assertThat(outcome.body).contains(TenantClaimEnforcer.CODE_TENANT_FORBIDDEN);
        }
        ...
        void noTenantContextIs401() throws Exception {
            Outcome outcome = filter(jwt(null, null), "/api/finance/accounts/acc-1");
            assertThat(outcome.called).isFalse();
    ...
    void theTwoLayersAgree() throws Exception {
        ...
        for (Jwt token : tokens) {
            ...
            boolean filterAdmits = filter(token, "/api/finance/accounts/acc-1").called;
