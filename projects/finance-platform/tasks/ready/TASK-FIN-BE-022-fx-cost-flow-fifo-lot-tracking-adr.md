...
Author **ADR-001 (finance-platform) PROPOSED** — 외화 포지션 원가흐름(cost-flow)을 가중평균에서 FIFO 로트(lot) 추적으로(per-tenant 설정·opt-in·net-zero). ledger-service 15번째 증분의 방향 결정으로, 14증분(FIN-BE-007~021)까지의 가중평균 pool 처분원가 모델을 회고적으로 기록하고 FIFO 취득-로트 원가추적으로의 전환을 결정한다. cost-flow method(FIFO vs 가중평균)는 **실현손익 숫자를 바꾸는 회계정책 결정**이라 코드로 묵시 선택하면 HARDSTOP-09 + 기존 테넌트 reconciliation 드리프트 → 결정을 먼저 기록하고 PAUSE. Doc-only; ACCEPTED + 구현은 별도 user-explicit-intent 태스크(staged-child).
...
- **AC-7** ADR-001 은 **PROPOSED only** — self-ACCEPT 금지. ACCEPTED 전환과 실행(FIN-BE-023~026)은 별도 user-explicit-intent 태스크임을 § 6 가 명시.
...
- ADR-001 이 같은 태스크에서 self-ACCEPTED 되면 → staged-child 패턴 위반(ACCEPTED 는 별도 user-explicit 태스크). PROPOSED only.
