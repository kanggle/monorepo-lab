...
- `TASK-FIN-BE-004-idempotency-concurrency-exactly-once-fix.md` — (impl PR) **Fix issue found in TASK-FIN-BE-001** — concurrent same-Idempotency-Key fund movement exactly-once (surfaced by MONO-115 run `26071043853`, finance IT 11/12 residual). Root cause (BE-301): production `IdempotentExecution.run` was non-atomic check-then-act → a concurrent same-key burst let N threads MISS+move funds; architecture.md already specified "Redis primary (SET NX-EX)" atomic claim the impl never realised (production design gap, neither clean H1 nor H2). **Fix = claim-before-execute, DB-authoritative**: port `claim()→{EXECUTE|REPLAY|CONFLICT|IN_PROGRESS}`+`complete()`+`release()`; `IdempotencyKeyJpaRepository` native atomic `insertClaim` (composite PK = concurrency gate; `save` merges so can't fail-on-dup)+`markCompleted`+`delete`; `RedisOrDbIdempotencyStore` DB-PK authoritative atomic gate (sentinel `response_status=0`, dup→CONFLICT/REPLAY/IN_PROGRESS, expired sentinel reclaimable, Redis=best-effort REPLAY accelerator, fail-CLOSED on DB down, `@Transactional(REQUIRES_NEW)` ×3 = claim commits+visible to burst, ComplianceFailureRecorder precedent); `IdempotentExecution.run` (signature UNCHANGED → 4 controller call sites untouched) claim-before-execute loop (EXECUTE→action→complete(2xx)/release(fail/non-2xx); REPLAY→stored; CONFLICT→409; IN_PROGRESS→bounded poll ≤5s≪req timeout→REPLAY, stall→409 deterministic no-re-movement); IT rewritten to drive the **real** wrapper, assertion **STRENGTHENED** to `actionExecuted==1` (green only via genuine prod guarantee — green-wash prohibited). impl=Opus 직접(correctness-critical, agent 미사용) / **dispatcher BE-301**: scope=port+repo+store+IdempotentExecution+IT only (no V1/migration/contract/architecture.md[impl now COMPLIES with existing SET-NX-EX spec]/ADR/ci.yml/controller/FIN-BE-002·003); `:check --rerun-tasks` 117/0/0/0 (11-XML, all sources compile incl. rewritten IT); controllers untouched. 행위증명=impl PR 의 MONO-115 `finance-integration-tests` (real MySQL+Redis; local Docker blocker → CI authoritative; green-wash 금지로 **12/12 GREEN** 확인 전 미선언). 머지 ⇒ main `finance-integration-tests` RED→GREEN = honest green-wash chain **true terminal** (FIN-BE-001 gap → MONO-115 CI → FIN-BE-002 schema → FIN-BE-003 behavioural → FIN-BE-004 concurrency). close chore = impl PR 머지 후.
