...
- Chain (assembled by the shared `GatewayJwtDecoders.validatorChain`, in order): timestamps →
  ...
  `TenantClaimValidator` (finance's gate, above) → Spring defaults → **then**, only on a token
  those passed, `AllowedAudiencesValidator` (TASK-MONO-696, `jwt-standard-claims.md` rule 5).
...
- Audience: `aud` (the issuing **client id**) ∩ `financeplatform.oauth2.allowed-audiences`
  (shipped: `platform-console-web`, the client measured to reach this edge) ≠ ∅. Empty or absent
  allowlist fails the boot. `financeplatform.oauth2.audience-mode` ships **SHADOW**: a mismatch is
