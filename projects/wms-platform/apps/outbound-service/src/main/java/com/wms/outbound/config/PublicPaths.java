...
/**
 * outbound-service's public (unauthenticated) request paths (ADR-MONO-058 § D5).
 *
 ...
 * only the data (outbound-service's own policy of what is public); the
 ...
 *
 * <p>{@code /webhooks/erp/order} is public at the JWT filter-chain level —
 * HMAC signature verification is performed inside the controller instead
 * ({@code specs/contracts/webhooks/erp-order-webhook.md} § Endpoint). This
 * matches outbound-service's pre-existing {@code PUBLIC_PATHS} array, which
 * carried the same 5th entry; TASK-BE-570 re-verified this divergence from
 * the other 4 wms services at implementation time rather than assuming
 * byte-identical data across all 5 (AC-3).
 *
