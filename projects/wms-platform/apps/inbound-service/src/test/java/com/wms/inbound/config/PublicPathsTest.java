...
/**
 * Pins inbound-service's {@link PublicPaths} (ADR-MONO-058 § D5, TASK-BE-570).
 *
 * <p>Unlike master/inventory/admin, inbound-service's pre-existing
 * {@code PUBLIC_PATHS} array carried a 5th entry, {@code /webhooks/erp/asn}
 ...
 */
@DisplayName("inbound-service PublicPaths")
class PublicPathsTest {
    ...
    void exactSetUnchanged() {
        assertThat(PublicPaths.EXACT).containsExactlyInAnyOrder(
                "/actuator/health", "/actuator/info", "/actuator/prometheus", "/webhooks/erp/asn");
    }
    ...
    void isPublic_exactEntriesAndHealthSubPathAndWebhook_true() {
        ...
        assertThat(PublicPaths.isPublic("/webhooks/erp/asn")).isTrue();
    }
    ...
    void isPublic_nonPublicPaths_false() {
        ...
        assertThat(PublicPaths.isPublic("/api/v1/inbound/asns")).isFalse();
    }
}
