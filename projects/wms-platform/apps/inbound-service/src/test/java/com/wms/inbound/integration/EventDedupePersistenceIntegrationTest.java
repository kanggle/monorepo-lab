 ...
 */
class EventDedupePersistenceIntegrationTest extends InboundServiceIntegrationBase {
    ...
    void redeliveredEventIdAppliesWorkExactlyOnce() {
        ...
        EventDedupePort.Outcome first = tx.execute(status ->
                dedupe.process(eventId, "inbound.putaway.completed", applied::incrementAndGet));
        ...
        Integer rows = jdbc.queryForObject(
                "SELECT COUNT(*) FROM inbound_event_dedupe WHERE event_id = ?",
