...
- [ ] AC-1 — `Math.random()` 기반 `shortSuffix()` 가 master-service 테스트에서 **0건**(`grep` 확인).
- [ ] AC-2 — 6개 클래스가 **하나의 공유 카운터**를 쓴다(클래스별 사본 금지). 발급된 코드는 `^WH\d{2,3}$` 및 `^Z-[A-Z0-9]+$` 를 만족.
- [ ] AC-3 — **유니크성 단위 테스트**가 존재하고 Docker-free 로 통과: N 회 호출 → N 개 distinct, 소진 시 예외.
- [ ] AC-4 — 프로덕션 코드(`apps/master-service/src/main/**`) **byte-unchanged**(`git diff --numstat` 빈 출력).
- [ ] AC-5 — 의도적 중복 테스트를 깨뜨리지 않는다. (사전 확인: 실 DB 에서 `warehouseCode` 중복을 의도하는 IT 는 **없다**. 409 를 기대하는 IT 2건은 *idempotency-key 불일치* 와 *stale-version PATCH* 로 코드 충돌과 무관. `WAREHOUSE_CODE_DUPLICATE` 테스트는 `WarehouseControllerTest` 의 **mock 단위테스트**라 실 DB 미사용.)
- [ ] AC-6 — CI `Integration (master-service + …)` 레인 GREEN.
...
- [ ] 공유 발급기 신설 + 6개 클래스 치환 + `Math.random()` 0건
- [ ] 유니크성 단위테스트 통과(Docker-free)
- [ ] 프로덕션 코드 byte-unchanged
- [ ] CI 레인 GREEN
- [ ] `projects/wms-platform/tasks/INDEX.md` done entry
