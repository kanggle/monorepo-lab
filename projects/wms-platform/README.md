...
> 캡처 7장을 검토했고 **싣지 않기로 했다.** 두 가지가 겹친다:
>
> - 🔴 **UUID 노출** — `WMS 재고` 의 「위치」·「SKU」가 코드가 아니라 UUID 원문이다
>   (`01910000-0000-7000-8000-…`). 콘솔에서 못 고친다 — 생산자가 옆 필드는 비정규화해 놓고
>   그 하나만 빼놨다. 추적: `TASK-MONO-659`(그 티켓이 지목한 다섯 중
>   `InventorySnapshotResponse` · `AsnSummaryResponse` · `OrderLineResponse` 셋이 이 프로젝트다).
> - 🔴 **데모 시드가 얇다** — 재고 1행 · 출고 주문 1건. 그리고 그 1건의 saga 상태가
>   `STUCK_RECOVERY_FAILED` 다. 포트폴리오에 **막힌 사가**를 광고할 수는 없다.
>
> 🔵 **스크린샷은 광고다.** 코드가 아니라 데모 데이터의 문제이지만, 방문자는 그 구분을
> 하지 않는다. 659 가 닫히고 시드가 두터워진 뒤 재촬영해서 싣는다.
>
