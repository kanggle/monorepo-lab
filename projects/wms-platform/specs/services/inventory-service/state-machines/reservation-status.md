...
| `RESERVED` | `RELEASED` (`MANUAL`) | `Reservation.release(MANUAL)` | `INVENTORY_ADMIN` REST manual release (`POST /api/v1/inventory/reservations/{id}:release`, `Idempotency-Key` required) | per-line `Inventory.release(qty, reservationId, reason)` → 2 Movement rows (`RESERVED -N` + `AVAILABLE +N`, `reason_code=PICKING_CANCELLED`); set `released_reason=MANUAL`; bump `version` | `inventory.released` |
