...
> `CONFIRMED`)은 상태 진행이 잘 보이지만 **「공급사」 칸이 UUID 원문**이다.
> 2026-09-12 에 새 코드로 AMI 를 다시 구운 데모에서 다시 열어도 **그대로였다**(3건 모두 UUID).
>
> 🔴 **처음 적은 추적이 틀렸다.** 원래 `TASK-MONO-659` 로 적었지만 그 티켓은 이 칸을
> **다루지 않았다**(«다른 부류» 로 남겼다) — 659 가 닫힌 뒤에도 그대로인 이유가 그것이다.
> 기전도 wms 와 다르다: 공급사 마스터는 **있다**(코드·이름을 가진 등록 API) — 그런데 발주 응답은
> 공급사를 **서버가 만든 UUID `supplierId` 하나로만** 싣고 코드·이름 필드가 없어, 콘솔은 그 id 를
> 그대로 보여 준다. 추적: `TASK-MONO-677`.
>
> 🔵 **스크린샷은 광고다.** 알려진 결함이 박힌 화면을 거는 것은 그 결함을 광고하는 일이라,
> `TASK-MONO-677` 이 닫혀 공급사 칸이 읽히는 값이 된 뒤 재촬영해서 싣는다.
