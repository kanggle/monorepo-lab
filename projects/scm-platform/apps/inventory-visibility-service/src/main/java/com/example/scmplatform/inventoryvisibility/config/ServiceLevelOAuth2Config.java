 ...
 * held as a class, written inline. Fleet-wide only this service and {@code demand-planning} did
 * that, which is precisely why no count ever saw them: <strong>a detector that looks for
 * {@code public class TenantClaimValidator} cannot see a lambda.</strong>
 *
 * <p>The old Javadoc on {@code isEntitled} said the helper <em>"cannot be shared across
 * modules"</em>. {@code libs/java-security} is that module. The sentence was true when it was
 * written and had quietly become false — the exact class of defect this ADR exists to argue
 * against, sitting in the file it was arguing about.
 *
 * <p>Both layers still dual-accept, and now they cannot disagree, because they are configured
 * from the same switches a few lines apart (ADR-MONO-049 § 1.9 — an enforcer that refuses what
 * the decoder just admitted is not defence in depth).
 */
@Configuration
public class ServiceLevelOAuth2Config {
    ...
    public OAuth2TokenValidator<Jwt> jwtTokenValidator() {
        ...
        validators.add(TenantClaimValidator.forTenant(requiredTenantId)
                ...
                .trustEntitledDomains()      // entitlement-trust dual-accept (TASK-MONO-162)
                .build());
     ...
     * {@code /actuator/} while SecurityConfig permitted three paths (ADR-MONO-049 § 1.8). Nothing
     * was reachable through that gap, but it was a gap held open by a fact about a different
     * file. Now the permit list and the exemption are one object.
     */
    @Bean
