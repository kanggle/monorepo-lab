...
/**
 * TASK-MONO-420 / TASK-MONO-421 behaviour (an unmapped path is 404 and a wrong method is
 * 405, neither a catch-all 500), re-pinned after TASK-SCM-BE-055 (ADR-MONO-058 § D2)
 * moved those arms out of this service and into {@code libs/java-web-servlet}'s
 ...
 * also slip past it). Routing the request through
 * {@code ExceptionHandlerExceptionResolver} asks the artifact instead of a proxy for it:
 * an unregistered inherited arm resolves to the catch-all 500 and this test goes RED.
 */
...
class GlobalExceptionHandlerNotFoundTest {
    ...
    @Test
    @DisplayName("지원 안 하는 메서드가 405 METHOD_NOT_ALLOWED + RFC 7231 Allow 헤더로 처리된다 (500 아님)")
    ...
    @Test
    @DisplayName("지원 안 하는 Content-Type이 415 UNSUPPORTED_MEDIA_TYPE로 처리된다 (500 아님)")
