...
/**
 * {@code accountKeyResolver} — the bean every scm route actually references
...
class AccountKeyResolverTest {
    ...
    @Test
    @DisplayName("인증 요청 → rate:scm-platform:<route>:acct:<sub>")
    void authenticatedRequestKeysOnTheAccount() {
        String key = resolver.resolve(exchangeFor("10.0.0.5", routeWithId("procurement-service")))
                .contextWrite(authContext("operator-a"))
                .block();

        assertThat(key).isEqualTo("rate:scm-platform:procurement-service:acct:operator-a");
    }
    ...
    void sameIpDifferentAccountsProduceIndependentKeys() {
        ...
        String a = resolver.resolve(exchangeFor(ip, routeWithId("procurement-service")))
                .contextWrite(authContext("operator-a")).block();
        String b = resolver.resolve(exchangeFor(ip, routeWithId("procurement-service")))
                .contextWrite(authContext("operator-b")).block();

        assertThat(a).isEqualTo("rate:scm-platform:procurement-service:acct:operator-a");
        assertThat(b).isEqualTo("rate:scm-platform:procurement-service:acct:operator-b");
    ...
    void aTokenWithoutASubjectFallsBackToTheIpKey() {
        ...
        String key = resolver.resolve(exchangeFor("192.0.2.5", routeWithId("procurement-service")))
                .contextWrite(authContext(noSubject))
                .block();

        assertThat(key).isEqualTo("rate:scm-platform:procurement-service:192.0.2.5");
    }

    @Test
    @DisplayName("보안 컨텍스트 부재 → IP 키로 폴백 (pre-auth 경로)")
    void anonymousRequestFallsBackToTheIpKey() {
        String key = resolver.resolve(exchangeFor("198.51.100.7", routeWithId("procurement-service")))
                .block();
    ...
    private static MockServerWebExchange exchangeFor(String ip, Route route) {
        MockServerHttpRequest request = MockServerHttpRequest.get("/api/v1/procurement/orders")
