-- TASK-SCM-BE-032: migrate procurement-service outbox v1 -> v2 (AbstractOutboxPublisher).
--
-- Adds the v2-shaped `procurement_outbox` table consumed by ProcurementOutboxPublisher
-- (extends AbstractOutboxPublisher<ProcurementOutboxJpaEntity>). Columns match
-- libs/java-messaging OutboxRowEntity — the @MappedSuperclass that
-- ProcurementOutboxJpaEntity extends (UUID event_id PK, occurred_at as the domain
-- event timestamp, retries + last_error for the v2 backoff/failure tracking).
-- Mirrors the CI-validated Postgres v2 reference (wms master_outbox, TASK-BE-438;
-- ecommerce promotion_outbox, TASK-BE-444).
--
...
-- by the lib EntityScan). V3 therefore ADDS `procurement_outbox` and leaves the
...
-- `procurement_outbox`). They are deliberately ABANDONED: procurement events are
-- re-derivable in the demo / fed-e2e / CI usage this monorepo targets.
-- (See TASK-SCM-BE-032 Edge Case "Cutover" / Failure Scenario F1.)
...
CREATE INDEX idx_procurement_outbox_pending
    ON procurement_outbox (occurred_at)
