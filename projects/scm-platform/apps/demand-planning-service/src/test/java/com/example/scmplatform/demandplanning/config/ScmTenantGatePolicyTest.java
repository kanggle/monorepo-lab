...
/**
 * <strong>scm's tenant policy</strong>, pinned — what demand-planning's gate admits, what it
 ...
 */
@DisplayName("scm/demand-planning — the tenant gate's policy, both halves")
class ScmTenantGatePolicyTest {

    private static final String ISSUER = "http://iam.local";
    private static final String API_PATH = "/api/demand-planning/suggestions/s-1";
    ...
    class ExemptionEqualsThePermitList {
        ...
        @Test
        @DisplayName("a request carrying no JWT is passed through — this filter only judges tokens")
        void nonJwtRequestPassesThrough() throws Exception {
            // demand-planning has no /internal/** surface (inventory-visibility does). The
            // property that matters here is the same one: the enforcer is not what stands
            // between a tokenless caller and the app — Spring Security is.
            ...
            assertThat(chain.called)
                    .as("authorization is Spring Security's job")
                    .isTrue();
        }
    }
