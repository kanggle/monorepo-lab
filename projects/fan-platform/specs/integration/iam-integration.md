...
> 본 문서는 `fan-platform` 의 모든 서비스가 IAM 를 표준 OIDC IdP 로 사용하는 방식을 1쪽으로 요약한다.
> ADR-001 (D1=A) 와 IAM 의 [consumer-integration-guide.md](../../../iam-platform/specs/features/consumer-integration-guide.md) 의 fan-platform 적용본이며,
...
- 일반 팬 사용자 계정은 IAM 의 [self-service signup endpoint](../../../iam-platform/specs/contracts/http/auth-api.md) 로 생성된다.
- B2C 운영자 (artist 등록 / 모더레이션 / B2C admin 콘솔 사용자) 의 계정은 IAM 의 [account internal provisioning API](../../../iam-platform/specs/contracts/http/internal/account-internal-provisioning.md) 를 통해 생성된다.
...
`<tenant>.<resource>.<action>` 명명 규칙. 모두 `tenant_id=fan-platform` scope 으로 IAM 의 `oauth_scopes` 에 등록.
...
OIDC 표준 scope (`openid`, `profile`, `email`, `offline_access`) 는 IAM 의 시스템 scope 에서 자동 적용.
...
`membership-service` (spec 확정 FAN-BE-008, 구현 FAN-BE-009) 는 두 방향으로 IAM 를 소비한다:

1. **End-user 요청** — 다른 fan-platform 서비스와 동일하게 OAuth2 Resource Server 패턴으로 IAM 의 RS256 access token 을 검증하고 `tenant_id ∈ { fan-platform, * }` 만 통과시킨다 (위 § Token 검증 규칙 동일 적용). 적용 경로 = `/api/fan/memberships/**`.
2. **Workload identity (피호출)** — `membership-service` 의 내부 access-check 엔드포인트 `GET /internal/membership/access` 는 community-service 가 **IAM `client_credentials` JWT** (ADR-MONO-005 workload identity) 로 호출한다. membership-service 는 수신측으로서 issuer + 서명 + 인식된 internal client 신원/role 을 검증하며, end-user 토큰·무토큰은 401/403 으로 거부한다. 이 경로는 게이트웨이 미노출 (내부 docker 망 전용). 위 표의 `fan-platform-internal-services-client` (`client_credentials`) 가 그 호출자 client 의 등록 슬롯이다.
...
- [ ] IAM 의 `fan-platform` 테넌트 등록 (admin-service [Tenant Lifecycle API](../../../iam-platform/specs/contracts/http/admin-api.md#tenant-lifecycle-task-be-256)).
