...
- shared libs:
  ...
  - `libs:java-web-servlet` — `CommonGlobalExceptionHandler`. **Adopted by ADR-MONO-058 § D2 / TASK-FAN-BE-038**: `AbstractDomainExceptionHandler extends CommonGlobalExceptionHandler`, inheriting the framework arms (400 malformed-body / missing-header — this is the missing-`Idempotency-Key` guard — / missing-parameter, 404, 405 + RFC 7231 `Allow`, 409 `ObjectOptimisticLockingFailureException`, 415, catch-all 500) instead of hand-copying them. fan-platform's published **422** for `@Valid` / `IllegalArgumentException` is carried by overriding the base's `validationFailureStatus()` hook. The service-local `ApiErrorBody` survives **only** as the `details`-carrying envelope extension `platform/error-handling.md § Error Response Format` permits, used by the one arm whose `details` payload `membership-api.md` documents (`MEMBERSHIP_STATE_INVALID`)
