...
- ~~membership-service 통합~~ — **v1 에 포함됨** (FAN-BE-010: `HttpMembershipChecker` 가 membership-service 를 호출, fail-closed). 더 이상 out-of-scope 아님. 🔴 `TASK-FAN-INT-006` 이 inert fallback 과 `community.membership-service.enabled` 를 **삭제**했다 — membership-service 부재 스택은 이제 게이트가 열리는 게 아니라 **전건 거부**된다.
