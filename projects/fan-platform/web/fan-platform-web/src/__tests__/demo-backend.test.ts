/**
 * `demo-backend.ts` — 데모 백엔드 주소의 **런타임 해석** (TASK-MONO-586 / ADR-MONO-067 D2).
 *
 * 🔴 이 표에서 가장 중요한 칸은 "있음/꺼짐" 이 아니라 **`/status` 가 실패하는 칸**이다.
 *    그 칸이 없으면 *"데모 컨트롤 플레인이 잠깐 죽으면 팬도 죽는다"* 를 아무도 모른다.
...
beforeEach(() => {
  ...
  delete process.env.GATEWAY_URL_INTERNAL;
...
describe('resolveUpstreamBaseUrl — 폴백 사슬', () => {
  it('해석되면 그 주소', async () => {
    process.env.DEMO_API_BASE = CONTROL;
    process.env.GATEWAY_URL_INTERNAL = 'http://gateway-service:8080';
    stubStatus({ state: 'running', ip: '13.125.1.2' });

    const { resolveUpstreamBaseUrl } = await load();
    expect(await resolveUpstreamBaseUrl()).toBe(
      'http://fan-platform.13-125-1-2.sslip.io',
    );
  });

  it('🔴 해석 안 되면 GATEWAY_URL_INTERNAL — 데모 호스트의 컨테이너 판이 이걸로 산다', async () => {
    process.env.GATEWAY_URL_INTERNAL = 'http://gateway-service:8080';
    process.env.NEXT_PUBLIC_GATEWAY_URL = 'http://fan-platform.local';

    const { resolveUpstreamBaseUrl } = await load();
    expect(await resolveUpstreamBaseUrl()).toBe('http://gateway-service:8080');
  });

  it('🔴 그다음 NEXT_PUBLIC_GATEWAY_URL — 로컬 개발이 이걸로 산다', async () => {
    process.env.NEXT_PUBLIC_GATEWAY_URL = 'http://fan-platform.local';

    const { resolveUpstreamBaseUrl } = await load();
    expect(await resolveUpstreamBaseUrl()).toBe('http://fan-platform.local');
  });
