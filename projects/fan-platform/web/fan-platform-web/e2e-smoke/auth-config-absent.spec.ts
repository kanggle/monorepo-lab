...
test.describe('auth config absent — 설정이 없어도 가드는 닫힌다', () => {
   ...
   */
  // 🔴🔴 TASK-MONO-635 / ADR-MONO-070 — **`/artists` 와 `/posts/:id` 를 여기서 뺐다.**
  //    그 둘은 이제 공개 경로다(공개 열람이 새 요구이고, 그 화면들은 Vercel 저장본만
  //    읽는다). 목록에 남겨 두면 이 스위트가 **이관 자체를 결함으로 판정**한다.
  // 🔴 그런데 «뺐다» 로 끝내면 이 파일이 재던 성질(설정이 없어도 가드는 닫힌다)의
  //    모집단이 줄어든다 ⇒ **개인 데이터 경로로 갈아 끼웠다.** 재는 것은 같고 대상만
  //    옮겼다: `/me`(내 정보) · `/membership/history`(내 결제 이력)는 공개될 수 없는
  //    화면이고, 후자는 공개인 `/membership` 과 **접두사를 공유**해서 가장 위험하다.
  // 🔵 열린 쪽의 단언은 `auth-guard.spec.ts` § 방향 ① 이 갖고 있다 — 칸은 줄지 않았다.
  const protectedPaths = [
    { path: '/me', label: '보호 경로(개인 정보)' },
    { path: '/membership/history', label: '보호 경로(개인 결제 이력 — 공개 /membership 과 접두사 공유)' },
