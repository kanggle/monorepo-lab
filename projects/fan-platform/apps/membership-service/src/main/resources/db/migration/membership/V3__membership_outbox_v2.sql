-- TASK-FAN-BE-020: migrate membership-service outbox v1 -> v2 (AbstractOutboxPublisher).
--
-- Adds the v2-shaped `membership_outbox` table consumed by MembershipOutboxPublisher
-- (extends AbstractOutboxPublisher<MembershipOutboxJpaEntity>). Columns match
-- libs/java-messaging OutboxRowEntity — the @MappedSuperclass that
-- MembershipOutboxJpaEntity extends (UUID event_id PK, occurred_at as the domain
...
-- by the lib EntityScan). V3 therefore ADDS `membership_outbox` and leaves the
...
-- `membership_outbox`). They are deliberately ABANDONED: membership events are
-- re-derivable in the demo / fed-e2e / CI usage this monorepo targets.
-- (See TASK-FAN-BE-020 Edge Case "Cutover" / Failure Scenario F1.)
...
CREATE INDEX idx_membership_outbox_pending
    ON membership_outbox (occurred_at)
