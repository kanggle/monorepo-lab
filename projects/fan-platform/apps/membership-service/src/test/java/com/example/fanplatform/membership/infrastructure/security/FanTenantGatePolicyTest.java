...
class FanTenantGatePolicyTest {
    ...
    private static ServiceLevelOAuth2Config wiredConfig() {
        ...
        // rejects a null URI, so the end-user validator chain can no longer be built from a config
        // that left this field unset — as the production bean never does (@Value with no default: an
        // unset property fails the context). Wiring it here completes the fixture; no assertion below
        // reads it, and nothing about the chain's behaviour depends on its value.
        ReflectionTestUtils.setField(config, "endUserJwkSetUri", ISSUER + "/oauth2/jwks");
        return config;
    }
