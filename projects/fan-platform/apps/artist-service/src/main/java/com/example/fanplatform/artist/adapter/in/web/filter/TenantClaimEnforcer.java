...
import com.example.fanplatform.artist.domain.tenant.TenantContext;
 ...
 *
 * <p>Mirrors community-service's filter — defense-in-depth final guard. The
 * {@link TenantClaimValidator} already runs during JWT validation; this
 * filter re-checks after Spring Security has populated the SecurityContext
 * so even if the JwtDecoder is misconfigured, requests carrying an
 * unexpected {@code tenant_id} are still rejected with 403
