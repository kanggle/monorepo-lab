...
/**
 * fan-platform-owned exception-handling policy for artist-service.
 ...
 * 400 missing-header / missing-parameter, 409
 ...
 *
 * <p>artist-service additionally <em>overrides</em> the inherited malformed-body arm —
 * see {@link GlobalExceptionHandler#handleMalformedRequest} — because
 * {@code artist-api.md} publishes 422 there where every other adopter publishes 400.
 */
@Slf4j
abstract class AbstractDomainExceptionHandler extends CommonGlobalExceptionHandler {
     ...
     * ({@code artist-api.md} — "422 VALIDATION_ERROR | constraint violation
     * (`@Valid`)"), where the shared default is 400. One override moves both inherited
     * arms; see {@link CommonGlobalExceptionHandler#validationFailureStatus()}.
     */
    @Override
    ...
    /**
     * Not covered by the shared base — {@code artist-api.md} publishes
     * "400 VALIDATION_ERROR | path-variable type mismatch (e.g. non-UUID `{id}`)", so
     * without this arm the case would fall through to the catch-all and regress to 500.
     */
