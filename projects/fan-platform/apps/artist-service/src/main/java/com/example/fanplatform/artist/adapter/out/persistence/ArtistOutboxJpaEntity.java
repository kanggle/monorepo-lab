...
/**
 * JPA entity backing {@code artist_outbox} (TASK-FAN-BE-022, outbox v2).
 *
 ...
 *
 * <p>Lives under {@code com.example.fanplatform.artist.adapter.out.persistence}
 * so it is picked up by the existing
 * {@code @EntityScan(basePackages = "…adapter.out.persistence")} and the
 * repository by the existing
 * {@code @EnableJpaRepositories(basePackages = "…adapter.out.persistence")} — no
 * JpaConfig change required.
 */
@Entity
@Table(name = "artist_outbox")
public class ArtistOutboxJpaEntity extends OutboxRowEntity {

    protected ArtistOutboxJpaEntity() {
        super();
    }

    public ArtistOutboxJpaEntity(UUID eventId, String eventType, String aggregateType,
