-- TASK-FAN-BE-021: migrate community-service outbox v1 -> v2 (AbstractOutboxPublisher).
--
-- Adds the v2-shaped `community_outbox` table consumed by CommunityOutboxPublisher
-- (extends AbstractOutboxPublisher<CommunityOutboxJpaEntity>). Columns match
-- libs/java-messaging OutboxRowEntity — the @MappedSuperclass that
-- CommunityOutboxJpaEntity extends (UUID event_id PK, occurred_at as the domain
...
-- by the lib EntityScan). V2 therefore ADDS `community_outbox` and leaves the
...
-- `community_outbox`). They are deliberately ABANDONED: community events are
-- re-derivable in the demo / fed-e2e / CI usage this monorepo targets.
-- (See TASK-FAN-BE-021 Edge Case "Cutover" / Failure Scenario F1.)
...
CREATE INDEX idx_community_outbox_pending
    ON community_outbox (occurred_at)
