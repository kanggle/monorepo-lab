...
/**
 * JPA entity backing {@code community_outbox} (TASK-FAN-BE-021, outbox v2).
 *
 ...
 *
 * <p>Lives under {@code com.example.fanplatform.community.infrastructure.jpa}
 * so it is picked up by the existing
 * {@code @EntityScan(basePackages = {"…domain", "…infrastructure.jpa"})} and the
 * repository by the existing
 * {@code @EnableJpaRepositories(basePackages = "…infrastructure.jpa")} — no
 * JpaConfig change required.
 */
@Entity
@Table(name = "community_outbox")
public class CommunityOutboxJpaEntity extends OutboxRowEntity {

    protected CommunityOutboxJpaEntity() {
        super();
    }

    public CommunityOutboxJpaEntity(UUID eventId, String eventType, String aggregateType,
