...
4. 실패 시 로그 위치:
   ...
   - `docker compose -p iam-e2e logs <service>` (suite JVM이 아직 살아있는 동안)
...
```bash
docker compose -f docker-compose.e2e.yml -p iam-e2e up -d
docker compose -p iam-e2e ps
...
```bash
docker compose -p iam-e2e down -v
```
