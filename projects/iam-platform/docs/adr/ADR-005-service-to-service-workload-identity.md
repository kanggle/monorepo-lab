...
- **중앙 IdP + Federation**: IAM `auth-service` = Spring Authorization Server(full OIDC — discovery/JWKS/token/userinfo/revocation/introspection) + 외부 IdP 3종(Google/Kakao/Microsoft).
...
|---|---|---|
| account-service | ✅ 검증함 | `InternalApiFilter` — `X-Internal-Token` **정적 공유 토큰** (fail-closed, 미통과 시 401) |
| security-service | ✅ 검증함 | `InternalAuthFilter`(별도 클래스) — `X-Internal-Token` 정적 공유 토큰 (미통과 시 403, profile 기반 bypass) |
...
ADR-003 회귀 매트릭스에서 **`client_credentials (test-internal-client) → 200 PASS`** 가 확인된다. 즉 IAM `auth-service` 는 이미 `client_credentials` grant 를 지원하는 OIDC AS 다. 본 ADR 의 결정은 **새 인증 인프라 신축이 아니라, 이미 있는 OIDC AS 를 서비스 간 경로에 재사용**하는 것이다.

또한 양 끝(발급·검증)이 **모두 이미 사내에서 동작 중**임이 코드로 확인된다:

- **발급측**: `client_credentials` registered client 가 이미 6개+ seed 되어 있다 — `test-internal-client`(V0008), `community-service-client`/`membership-service-client`(V0009), `wms-internal-services-client`(V0010), `scm-platform-internal-services-client`(V0013), `finance-platform-internal-services-client`(V0017), erp(V0018). 서비스 client seed 는 확립된 패턴의 반복이다.
- **수신측 검증 blueprint**: `community-service`/`membership-service` 가 이미 `oauth2ResourceServer(jwt)` + JWKS(`/oauth2/jwks`) 로 자신의 `/internal/**` 를 검증한다. 본 ADR 의 수신측 전환은 신규 발명이 아니라 **사내 검증된 패턴을 account/security 에 복제**하는 것이다. (다만 community/membership 은 JWT 단일 요구(`.authenticated()`)이고, 본 ADR 의 무중단 단계 2 는 **JWT 와 X-Internal-Token 의 이중 허용**이 추가 설계점 — 아래 단계 2 및 TASK-BE-317 참조.)
- **네이밍 컨벤션**: 기존 client_credentials client 는 모두 `<service>-service-client` 또는 `<tenant>-internal-services-client` 규약을 따른다(`svc-` prefix 선례 없음). 본 ADR 의 서비스 client 는 이 선례에 맞춰 `<service>-service-client`(`admin-service-client`, `auth-service-client`, `security-service-client`, `account-service-client`) 로 명명한다. community/membership 호출자는 **기존 client 재사용**.
...
| 옵션 | 설명 | keyless | 장점 | 단점 |
|---|---|---|---|---|
| **A. `client_credentials` 단기 JWT** | 각 서비스가 IAM 에서 `client_credentials` grant 로 단기 JWT 발급 → `Authorization: Bearer` 로 `/internal/**` 호출. 수신측은 JWKS 로 검증 | △ (client_secret 1개 정적 잔존) | IAM OIDC AS **이미 지원** → 신축 최소. edge 신원과 동일 인프라로 일관. 호출자 신원(client_id)·만료·회전 확보 | client_secret 은 여전히 정적 비밀 |
...
1. **인프라 재사용**: `client_credentials` grant 가 이미 동작(ADR-003 입증). 토큰 발급자(=AWS STS / GCP 메타데이터 역할)를 IAM 에 맡기면 추가 컴포넌트 0.
...
```
[호출측]  서비스 부팅/호출 시
  POST /oauth2/token (grant_type=client_credentials, client_id=<service>-service-client, client_secret=…)
...
[수신측]  /internal/** 보안 체인
  X-Internal-Token(InternalApiFilter)  →  OAuth2 Resource Server (JWKS 검증)
   - iss = IAM issuer, JWKS = auth-service /oauth2/jwks
   - 선택적으로 client_id allowlist (호출자 신원 검증)
```

서비스별 registered client(`admin-service-client`, `account-service-client`, `security-service-client`, … — community/membership 은 기존 client 재사용)를 `client_credentials` grant 로 등록(migration seed). 수신측은 `/internal/**` 를 permitAll/정적토큰에서 **JWT resource-server 검증**으로 전환하되, 단계 2 에서는 X-Internal-Token 과 **이중 허용**한다.

### 무중단 마이그레이션 (4단계)

| 단계 | 내용 | 무중단 보장 |
|---|---|---|
| **1. 발급 인프라** | IAM `client_credentials` 서비스 client 등록(seed) + 토큰 발급 검증 | 신규 추가, 기존 경로 불변 |
...
- **IAM auth-service**: `client_credentials` 서비스 client seed(migration), JWT 에 서비스 신원 claim.
...
- **cross-service 마이그레이션 비용**: IAM 내부 다수 서비스 + 10 계약 spec. 무중단 4단계 진행 필요.
- **IAM 가용성 의존 증가**: 서비스 간 호출 전 토큰 발급이 필요 → auth-service 장애 시 영향 확대. 토큰 캐시 + 만료 여유로 완화하되, IAM 자체는 SPOF 성격 강화.
...
- **TASK-BE-317** (본 ADR 와 함께 발행): 단계 1+2 — IAM `client_credentials` 서비스 client 인프라 + 수신측 JWT 검증 추가(X-Internal-Token 병행). 기존 경로 회귀 0 보장.
...
- ADR-001 (OIDC Adoption) — SAS 도입, IAM = OIDC AS
