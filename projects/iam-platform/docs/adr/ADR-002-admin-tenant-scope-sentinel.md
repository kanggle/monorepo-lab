...
`iam-platform`은 복수 테넌트(fan platform, game platform 등)를 단일 플랫폼에서 운영하는 멀티 테넌트 SaaS다. admin-service는 모든 테넌트 데이터에 대한 운영 명령(계정 잠금·해제, 감사 조회 등)을 처리하므로, 어떤 운영자가 어떤 테넌트 데이터에 접근할 수 있는지를 정확히 제어해야 한다.
