...
| `account-service` | entitled_domains lookup (issuance-time, **fail-soft**) | `GET /internal/tenant-domain-subscriptions?tenantId=...` | [../../contracts/http/internal/account-tenant-domain-subscriptions.md](../../contracts/http/internal/) |
| `admin-service` | **assume-tenant assignment gate** (issuance-time one-shot, **fail-CLOSED**) — TASK-BE-327 | `GET /internal/operator-assignments/check?oidcSubject=...&tenantId=...` | [../../contracts/http/internal/auth-to-admin.md](../../contracts/http/internal/auth-to-admin.md) |
...
> **fail-CLOSED vs fail-SOFT 분리 (TASK-BE-327 핵심 정확성)**: admin assignment gate 는 **fail-closed** (admin 장애 → assume-tenant 발급 거부, 토큰 없음); account entitled_domains 는 **fail-soft** (account 장애 → claim 생략, 토큰은 발급). 두 downstream, 정반대 정책 — 절대 섞지 않는다.
...
- `refresh:blacklist:{tenant_id}:{jti}` — 즉시 revoke된 토큰의 블랙리스트 (테넌트 격리; 레거시 `refresh:blacklist:{jti}` 는 읽기 전용 fallback — 정규 스키마는 [redis-keys.md](redis-keys.md))
...
| `account-service` 장애 (entitled_domains) | assume-tenant/login 토큰의 entitled_domains claim 생략 | **fail-soft** — 토큰은 발급되고 도메인은 `tenant_id` 게이트로 fallback (TASK-BE-324/327) |
| `admin-service` 장애 (assume-tenant gate) | assume-tenant 발급 불가 | **fail-CLOSED** — `invalid_grant`, 토큰 미발급. login/authorization_code 경로는 영향 없음 (TASK-BE-327) |
