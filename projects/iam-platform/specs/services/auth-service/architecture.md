...
ADR-003/004 closure 후). access/refresh token RSA 비대칭 서명 + JWKS 노출 +
Authorization Code+PKCE / refresh / revoke 흐름 제공. 로그인/로그아웃 +
로그인 실패 카운팅 + refresh token 회전·재사용 탐지 모두 본 서비스 owns.
적용되는 규칙:
[platform/service-types/identity-platform.md](../../../../../platform/service-types/identity-platform.md).
...
>
> **TASK-BE-599 갱신** — 폼-로그인 경로가 `auth.login.attempted/failed/succeeded` 발행을
> 갖게 됐다. 발행은 `application/LoginEventRecorder` 에 있고 provider 가 부른다(실패 이벤트의
> `accountId` 를 아는 곳이 provider 뿐이라서 — 실패 핸들러는 예외만 본다). **rate-limit(로그인
...
- **두 저장소를 다 닫는다.** SAS refresh 의 수락 여부는 `SasRefreshTokenAuthenticationProvider` 가
  ...
  🔵 TASK-BE-604: (2) 가 이제 **그 자체로 최종 거부**다 — SAS 기본 refresh provider 가 토큰 엔드포인트에서 제거되어, 미러 행 거부를
  더 이상 다른 provider 가 재처리하지 않는다(이전엔 (1) 만으로 통과). 두 저장소를 다 닫는 것은 그대로다(이메일 키 미러 행 배수 기간).
...
**Assume-Tenant 발급 경로 (TASK-BE-327 / ADR-MONO-020 § 3.3 step 2, D2+D3)**:

`authorization_code` 로그인 발급 경로와 **나란히** 존재하는 두 번째 domain-facing 발급 경로. `POST /oauth2/token` 의 `grant_type=urn:ietf:params:oauth:grant-type:token-exchange` (RFC 8693) 가 운영자의 base IAM OIDC 세션을 **선택된** customer tenant 로 scope 된 단명 토큰으로 교환한다 (AWS STS AssumeRole 유사; ADR-MONO-014 operator-identity exchange 의 sibling, 그쪽은 byte-unchanged).

1. `AssumeTenantAuthenticationConverter` (token endpoint, grant_type 필터 — 불일치 시 null → 기존 grant byte-unchanged) 가 `subject_token`/`subject_token_type`/`audience`(=선택 tenant) 를 파싱. `audience` 누락 → `invalid_request`.
2. `AssumeTenantAuthenticationProvider`:
   - subject_token 을 **auth-service 자기 `JwtDecoder`(자신이 서명한 동일 JWKS)** 로 검증 → `sub` 추출. 실패 → `invalid_grant` (fail-closed).
   ...
   - **refresh token 미발급** (단명, selection 마다 재발급 — ADR-MONO-020 § 3.1).
...
- ❌ refresh token rotation·logout에서 `tenant_id` 검증 누락 — 다른 테넌트의 token으로 rotation 시도는 `TOKEN_TENANT_MISMATCH`로 거부(SAS `/oauth2/token` = `400 invalid_grant`, 레거시 REST = 403 — [multi-tenancy.md § Refresh Token](../../features/multi-tenancy.md#refresh-token)). 비교 기준은 세션의 로그인 시점 테넌트이지 client 테넌트가 아니다(TASK-BE-604). **force-logout(session-revoke)** 은 credential-tenant 게이트로 confine 하되 cross-tenant 를 **no-op `200 count=0`** 로 처리(403 거부 아님 — TASK-BE-468, 아래 application/ 참조) ([specs/features/multi-tenancy.md](../../features/multi-tenancy.md))
...
- **Refresh rotation의 tenant 무결성**: `RefreshTokenUseCase`는 제출된 refresh token의 `tenant_id`(JWT claim 또는 DB row)와 새로 발급할 token의 `tenant_id`가 일치해야 한다. 불일치 시 `TOKEN_TENANT_MISMATCH` 403 + reuse-detection과 동일 수준의 보안 이벤트 발행. **SAS 경로**(`SasRefreshTokenAuthenticationProvider`, 브라우저 세션 전부)는 미러 행 테넌트를 **세션의 로그인 시점 테넌트**(principal details `tenant_id` = 토큰 claim)와 비교해 `400 invalid_grant` 로 거부하고, 그 거부는 최종이다 — SAS 기본 refresh provider 는 제거됐다(TASK-BE-604). 회전 미러 행도 같은 테넌트를 싣는다
...
> - **Legacy v1 tables (TASK-MONO-406)**: the lib `OutboxAutoConfiguration` / `OutboxJpaConfig` / `ProcessedEventJpaEntity` were deleted (TASK-MONO-312 had already removed the v1 `OutboxJpaEntity`), so **no** entity maps the v1 `outbox` (BIGINT/status) / `processed_events` tables any more. They stay in the schema because applied Flyway migrations are immutable, and `ddl-auto=validate` only validates *mapped* entities. In-flight v1 `outbox` rows at cutover are abandoned (low-volume, re-derivable).
