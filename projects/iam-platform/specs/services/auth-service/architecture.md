...
- ❌ refresh token rotation·logout에서 `tenant_id` 검증 누락 — 다른 테넌트의 token으로 rotation 시도는 403 `TOKEN_TENANT_MISMATCH`로 거부. **force-logout(session-revoke)** 은 credential-tenant 게이트로 confine 하되 cross-tenant 를 **no-op `200 count=0`** 로 처리(403 거부 아님 — TASK-BE-468, 아래 application/ 참조) ([specs/features/multi-tenancy.md](../../features/multi-tenancy.md))
