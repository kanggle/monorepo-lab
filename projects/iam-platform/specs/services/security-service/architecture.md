...
- **인증** (TASK-BE-319a, ADR-005 단계 4a): `/internal/**` 는 IAM `client_credentials` JWT(`Authorization: Bearer`) **단일** 인증 — `InternalAuthFilter` 가 JWKS 서명 + issuer 로 검증(정적 `X-Internal-Token` 제거됨). `test`/`standalone` profile 은 우회.
...
> - **Legacy v1 tables (TASK-MONO-406)**: the lib `OutboxAutoConfiguration` / `OutboxJpaConfig` / `ProcessedEventJpaEntity` were deleted (TASK-MONO-312 had already removed the v1 `OutboxJpaEntity` it re-pointed at `outbox_events` via `META-INF/orm.xml`), so the library maps no table at all. The v1 `outbox_events` (BIGINT/status) table is now unmapped legacy — it stays in the schema because applied Flyway migrations are immutable, and `ddl-auto=validate` only validates *mapped* entities. `processed_events` is still mapped, by security-service's OWN `infrastructure.persistence.ProcessedEventJpaEntity` (added by TASK-MONO-406) — do NOT drop it. In-flight v1 `outbox_events` rows at cutover are abandoned (low-volume, re-derivable).
