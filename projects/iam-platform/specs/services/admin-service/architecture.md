...
assignment 게이트를 통과시킨다 (도메인→IAM per-request callback 아님 — ADR-MONO-020
§ 3.1 은 후자만 금지).
...
- **read-only — `admin_actions` row 미생성** (ADR-MONO-014 token-exchange "not audited"
...
- **퍼시스턴스**: MySQL — `admin_actions` (append-only 감사 원장), `admin_outbox` (v2), `outbox` + `processed_events` (v1 legacy — 매핑하는 엔티티 없음, TASK-MONO-406 이후 unmapped; 적용된 Flyway 마이그레이션은 불변이므로 스키마에만 남아 있음)
