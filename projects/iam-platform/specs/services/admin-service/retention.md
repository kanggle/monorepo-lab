...
| `tenant_partnership` / `tenant_partnership_participant` | TERMINATED 후 **관계 감사 가치**로 유지 (삭제 대신 종단 상태 보존) | row 삭제 없음 — cascade 는 파생 소멸로 발효(§ 아래), TERMINATED row 는 감사 트레일로 잔존 | TASK-BE-476, A3 (cross-org 위임 이력 역추적) |
...
## `tenant_partnership` / `tenant_partnership_participant`

### Retention

- **ACTIVE/PENDING/SUSPENDED** 동안: 무기한 보유(운영 중 관계).
- **TERMINATED** 전환 후: row 삭제 없이 **종단 상태로 유지** — "언제 어느 두 테넌트가 어떤 `delegated_scope` 로 협력했고 언제 종료됐나"는 cross-org 위임 감사의 1급 증거(A3). `admin_actions` 의 `PARTNERSHIP_*` action row + `partnership.*` 이벤트가 동반 트레일.

### Destruction Path

- **row 삭제 없음** (일반 경로). cascade-revoke(D6)는 row 삭제가 아니라 **파생 소멸**로 발효한다 — 파트너십을 SUSPENDED/TERMINATED 로 마킹하면 assume-tenant 발급기가 request-time 에 파생을 0 으로 계산([rbac.md](./rbac.md) Cross-Org Partner Delegation Confinement). 따라서 접근 차단에 row 삭제가 불필요하고, row 는 감사 목적으로 보존된다.
- `tenant_partnership_participant`: 부모 `tenant_partnership` archive 시 FK `ON DELETE CASCADE`. participant 해제(B offboard)는 row 삭제이나(운영 lifecycle), `partnership.participant_removed` 이벤트 + `PARTNERSHIP_PARTICIPANT_REMOVE` audit row 가 트레일을 남긴다.
- 당사자 테넌트가 삭제/archive 되는 극단 경로는 tenant lifecycle(account-service 소유)에 종속 — 본 스코프 밖(파트너십은 두 ACTIVE 테넌트 전제).

### 근거

- audit-heavy A3 (cross-org 위임 이력 불변 역추적), TASK-BE-476 / ADR-MONO-045 D6 (cascade 는 파생 소멸이지 row 소멸이 아님).
