...
## GAP OIDC Subject-Token Validation (TASK-BE-298 / ADR-MONO-014)

`POST /api/admin/auth/token-exchange` ([admin-api.md](../../contracts/http/admin-api.md))
는 platform-console 이 보유한 **GAP OIDC `platform-console-web` access token**
...
| 2 | **`iss`** == GAP OIDC issuer (`admin.oidc.issuer`, 기본 auth-service `oidc.issuer-url`). | `401` |
...
| 5 | **GAP OIDC access token 형태 확인** — `token_type` claim 부재. GAP OIDC access token 은 `token_type` 커스텀 claim 을 싣지 않는다 ([auth-api.md Token Claims](../../contracts/http/auth-api.md)); `token_type` 가 존재하면(=admin/admin_refresh/admin_bootstrap 등 admin-service 자체 발급 토큰) **거부** — operator/bootstrap 토큰을 subject token 으로 우회 제시하는 경로 차단. | `401` |
...
- 발급되는 operator token 은 단명(≤ operator access TTL,
  ...
  남기지 않는다. console 은 자신의 GAP refresh 로 rotate 한 access token 으로
...
- `OperatorAuthenticationFilter` 는 본 검증 경로로 인해 **변경되지 않는다**.
  ...
  아니다. raw GAP OIDC 토큰을 `/api/admin/**` 의 일반 엔드포인트에 제시하면
