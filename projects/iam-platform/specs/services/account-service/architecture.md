...
> - **Legacy v1 tables (TASK-MONO-406)**: the lib `OutboxAutoConfiguration` / `OutboxJpaConfig` / `ProcessedEventJpaEntity` were deleted (TASK-MONO-312 had already removed the v1 `OutboxJpaEntity`), so the library maps no table at all. The v1 `outbox` table is now unmapped legacy — it stays in the schema because applied Flyway migrations are immutable, and `ddl-auto=validate` only validates *mapped* entities. `processed_events` is still mapped, by account-service's OWN `infrastructure.persistence.ProcessedEventJpaEntity` (added by TASK-MONO-406) — do NOT drop it. In-flight v1 rows at cutover are abandoned.
