...
> **TASK-MONO-295 (ADR-MONO-040 Phase 2 2026-06-18)** — 쿼리에 `subjectEmail` 파라미터가 **additive·optional** 로 추가되었다(운영자 row 의 **DUAL-KEY** fallback 조회). ADR-040 Phase 2 가 SAS `sub` 를 account_id 로 전환하지만 `admin_operators.oidc_subject` 는 여전히 운영자 **email** 로 시드돼 있고(account_id↔email 매핑은 cross-DB: auth_db ⟂ admin_db → 단일 Flyway backfill 불가), admin-service 는 운영자를 account_id(`oidcSubject`) **우선**, 레거시 email(`subjectEmail`) **fallback** 으로 조회한다. `required=false` 이므로 구버전 auth-service(파라미터 미전송)는 account_id 단독 조회로 byte-호환. backfill(`oidc_subject`→account_id) + fallback 제거는 deferred Phase-3 후속.
...
| `subjectEmail` | string (email) | No | **TASK-MONO-295 (ADR-040 Phase 2) — additive·optional.** 운영자의 로그인 email(subject 토큰 `email` claim). account_id 조회 miss 시 `admin_operators.oidc_subject` **레거시 fallback 키**. 미전송/blank → fallback 미수행(account_id 단독 조회) |
...
1. **DUAL-KEY 조회 (TASK-MONO-295)**: `oidcSubject`(account_id) 로 `admin_operators` row 조회 (`AdminOperatorPort.findByOidcSubject`); miss 시 `subjectEmail`(blank/null 아니면) 로 재조회(레거시 시드값 fallback). 두 키 모두 miss OR 비-`ACTIVE` → `assigned=false`, `orgScope=null` (fail-closed; 운영자 존재 여부를 boolean 너머로 노출하지 않는다; fallback 은 fail-closed 게이트를 **완화하지 않는다**).
