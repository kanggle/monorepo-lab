...
> **TASK-MONO-295 (ADR-MONO-040 Phase 2 2026-06-18)** — 운영자 row 의 **DUAL-KEY** fallback 조회를 위한 운영자 email 이 **`X-Subject-Email` 헤더**(additive·optional)로 전달된다. ADR-040 Phase 2 가 SAS `sub` 를 account_id 로 전환하지만 `admin_operators.oidc_subject` 는 여전히 운영자 **email** 로 시드돼 있고(account_id↔email 매핑은 cross-DB: auth_db ⟂ admin_db → 단일 Flyway backfill 불가), admin-service 는 운영자를 account_id(`oidcSubject`) **우선**, 레거시 email **fallback** 으로 조회한다.
>
> **email 출처 = 서버사이드 (토큰 미경유)**: assume-tenant `subject_token` 으로 쓰이는 **SAS access token 은 `email` claim 을 싣지 않는다**(email 은 `OidcUserInfoMapper` 가 userinfo/id-token 에만 emit; access token 엔 없음). 따라서 auth-service 가 검증된 `sub`(=account_id)로 **로컬 `auth_db.credentials` 스토어**(이 서비스가 로그인 인증에 쓰는 동일 스토어 — `CredentialAuthenticationProvider`)에서 `account_id → email` 을 서버사이드 resolve 해 채운다. cross-service hop 없음, **어떤 토큰에도 email(PII) 미탑재**.
>
> **PII 는 헤더로 (쿼리 금지)**: email 은 `confidential` PII([admin-service data-model.md](../../../services/admin-service/data-model.md) `admin_operators.email`)이므로 **쿼리 파라미터가 아닌 `X-Subject-Email` 헤더**로 전달한다 — 쿼리스트링은 게이트웨이/액세스 로그 URL 라인에 남지만 헤더 값은 남지 않는다. auth-service·admin-service 양측 모두 email 값을 로깅하지 않는다. 헤더 미전송/blank → fallback 미수행(account_id 단독 조회, byte-호환). backfill(`oidc_subject`→account_id) + fallback 제거는 deferred Phase-3 후속.

**Query Parameters**:

| 파라미터 | 타입 | 필수 | 설명 |
|---|---|---|---|
| `oidcSubject` | string (UUID) | Yes | 운영자의 IAM OIDC subject (`sub` = account_id, ADR-040 Phase 2). `admin_operators.oidc_subject` 로 운영자 row 를 fail-closed 조회(**1차 키**) |
...
| 헤더 | 타입 | 필수 | 설명 |
|---|---|---|---|
| `X-Subject-Email` | string (email) | No | **TASK-MONO-295 (ADR-040 Phase 2) — additive·optional.** 운영자의 로그인 email, auth-service 가 `sub`(=account_id)로 로컬 `auth_db.credentials` 에서 **서버사이드 resolve**(토큰 claim 아님). account_id 조회 miss 시 `admin_operators.oidc_subject` **레거시 fallback 키**. 쿼리 아닌 헤더인 이유 = PII(`confidential`)를 로그 URL 에서 배제. 미전송/blank → fallback 미수행(account_id 단독 조회). 값은 양측 모두 미로깅 |
...
1. **DUAL-KEY 조회 (TASK-MONO-295)**: `oidcSubject`(account_id) 로 `admin_operators` row 조회 (`AdminOperatorPort.findByOidcSubject`); miss 시 `X-Subject-Email` 헤더값(blank/null 아니면) 로 재조회(레거시 시드값 fallback). 두 키 모두 miss OR 비-`ACTIVE` → `assigned=false`, `orgScope=null` (fail-closed; 운영자 존재 여부를 boolean 너머로 노출하지 않는다; fallback 은 fail-closed 게이트를 **완화하지 않는다**).
