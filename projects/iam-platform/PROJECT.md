...
- **fan content / subscriptions / payments**: **iam 은 팬 도메인 기능을 갖지 않는다.** 과거 frozen demo consumer(`community-service`, `membership-service`)로 존재했으나 **TASK-MONO-394 로 RETIRED (2026-07-14)** — 같은 도메인을 [`fan-platform`](../fan-platform/PROJECT.md) 이 실제로 구현·배포한다.
...
| `auth-service` | identity-platform | OIDC Authorization Server (Spring Authorization Server). 로그인/로그아웃, OAuth 2.1 Authorization Code+PKCE, JWT 발급+JWKS, refresh token 회전·재사용 탐지, Redis 실패 카운트 (2026-05-11 service-type 정정, ADR-001 ACCEPTED) |
...
### Product-layer services — RETIRED 2026-07-14 (TASK-MONO-394)

| Service | Service Type | 상태 |
|---|---|---|
| ~~`community-service`~~ | ~~rest-api~~ | **RETIRED 2026-07-14** — 중복된 전시물 (TASK-MONO-394) |
| ~~`membership-service`~~ | ~~rest-api~~ | **RETIRED 2026-07-14** — 중복된 전시물 (TASK-MONO-394) |

두 서비스는 `account-service` 내부 API를 호출하는 **FROZEN product-layer demo consumer** 로, *"포트폴리오의 integration 예시"* 역할만 수행했다. **[`fan-platform`](../fan-platform/PROJECT.md) 이 같은 도메인(community·membership)을 실제로 구현하고 배포하면서 그 역할을 대체했고**, 전시물은 중복이 되었다.
