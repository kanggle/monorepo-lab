...
review — AC-0 · AC-1 · AC-2 완료(2026-09-25 UTC). 🔴 **AC-3 · AC-4 는 창(라이브 스택) 항목으로 열려 있다** — 아래 § 창 런북. 단위 테스트 초록은 AC-3 의 판정이 아니다.
🔴🔴 **머지 전에 소유자가 읽어야 할 것 두 개** — § 구현 기록의 «ⓐ 는 사용자 쪽 결과를 바꾼다» 와 «ⓒ 는 매 로그인을 새 디바이스로 만든다». 티켓 기안 시 ⓐ 를 «사용자 체감 동작 변화 없음» 이라 적었는데 **틀렸다**.
...
- [x] **AC-0 — 🔴 소유자 결정 (HARDSTOP-09 성격: 스펙에 없는 설계 선택)** — ✅ **결정됨 2026-09-25 (소유자)**. 원문:
  ...
  - **outbox 실패 처리**(Edge Case 표가 «AC-0 에 명시하라» 고 한 것): 이벤트 쓰기·디바이스 세션 등록은 **텔레메트리**다 — 실패해도 로그인 결과를 바꾸지 않는다(맞는 비밀번호는 성공, 틀린 비밀번호는 여전히 `/login?error`). 테스트로 박았다(`succeededTelemetryFailure_loginStillSucceeds` · `failedTelemetryFailure_stillBadCredentials`).
  - 원래 선택지(보존):
...
- ③ **폼 로그인은 계정 상태를 안 본다** — LOCKED/DORMANT/DELETED 계정도 `POST /login` 을 통과한다(provider 에 `getAccountStatus` 호출 0, auth-service 에 `account.locked` 소비자 0). 그래서 위 ①번 자동 잠금도 **폼 로그인을 막지는 못한다**. `LoginUseCase` 는 확인했었다(`checkAccountStatus`). 별개 결함 후보 — 이 티켓 범위 밖.
...
🔴 **공유 데모 계정에서 비밀번호 오류는 2회 이하로** — 10회/1시간이면 VelocityRule 이 계정을 자동 잠근다(§ 소유자 1).
...
2. `login_history` 에 이 계정의 `outcome='FAILURE'` 행 + velocity 키 값이 3단계 전보다 **1 증가**(3단계 전 값을 먼저 읽어 둬라 — 이미 있던 키를 «증가» 로 오독하지 않게).
