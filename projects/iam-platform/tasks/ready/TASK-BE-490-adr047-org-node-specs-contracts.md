...
- `projects/iam-platform/specs/services/admin-service/data-model.md` — `admin_operator_roles.org_node_id` nullable column (the node-scoped grant), and the mutual-exclusion rule vs `tenant_id`.
...
- [ ] **AC-3**: `rbac.md` carries the `org.manage` key row, the `ORG_ADMIN` seed role + permission set, and the `TenantScopeGuard` subtree driver, with `SUPER_ADMIN '*'` short-circuit and no-escalation preserved verbatim from ADR-024.
...
- `admin_operator_roles` gains `org_node_id` **without** the mutual-exclusion rule vs `tenant_id` → a grant row with both is ambiguous and `effectiveAdminScope` silently unions. Guard: AC-3 + data-model rule.
