...
> **완료 (2026-05-31)**: impl PR #981 (squash `ee19ff8c`). ADR-MONO-020 § 3.3 step 1(D1/D5/D6-step1) — admin-service `operator_tenant_assignment` N:M(Flyway `V0030`, admin_operator_roles mirror: 복합 PK `(operator_id BIGINT, tenant_id)`, FK→admin_operators.id CASCADE, `permission_set_id` NULL FK→admin_roles.id[D5], idx_ota_tenant_id; **시드 0**) + **dual-read** `TenantScopeResolver`(effective = assignment tenantIds ∪ {legacy `admin_operators.tenant_id`}; `'*'`→{`'*'`} sentinel 무변경; **assignment 0 → {home}** net-zero). **4 gating site 배선**(legacy 단일 equality → effective-set 멤버십; platform/`'*'` 분기 byte-unchanged): ConsoleRegistryUseCase.selectableTenants(`bound ∩ effectiveTenants`, JPA-direct resolveOperator 경로 명시 배선) + PermissionEvaluator.isTenantAllowed(**3-arg overload — AdminOperator record + isPlatformScope() byte-unchanged**, record field 추가 회피) + AuditQueryUseCase + UpdateOperatorProfileUseCase. CreateOperator/AdminActionAuditWriter/Tenant write paths 무변경. **NET-ZERO 증명**: 빈 테이블 → 전 operator {home} → 기존 동작 byte-identical, 기존 테스트(AdminOperatorTest/PermissionEvaluatorTenantScopeTest/ConsoleRegistryUseCaseTest[backward-compat seed 포함]/AuditQueryUseCaseTest/TenantAdminControllerTest) **단언 무변경 GREEN**(신규 의존 stub/ctor 조정만). 신규 TenantScopeResolverTest + OperatorTenantAssignmentJpaRepositoryTest(Testcontainers, `ddl-auto=validate` real MySQL GREEN) + multi-assignment AC-5 케이스. **66 unit + 4 IT, 0 failures.** **3차원**(MERGED `ee19ff8c` / tip 일치 / pre-merge 0). **BE-299 re-stage** ✓. **CI 1-pass**: GAP Integration GREEN 2m32s(V0030 clean-migrate + net-zero IT) + Build GREEN. **scope-lock**: admin-service(V0030 + assignment domain/persistence/port + resolver + 4 site + test) 만. **후속(별 task)**: **D2 RFC8693 assume-tenant exchange**(auth-service hot-path, **최고위험 — 새 세션 권장**) → D4 console switcher → D6 step4 cleanup. **메타**: dual-read net-zero = 빈 테이블 + {home} fallback + 기존 단언 무변경 3중; AdminOperator record 동결 위해 PermissionEvaluator 3-arg overload 선택(record field 대신).
