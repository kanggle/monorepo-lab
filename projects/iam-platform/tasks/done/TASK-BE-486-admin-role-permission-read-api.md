...
> **DONE (2026-07-08, 3-dim verified — impl PR #2331 squash `59070366`)**. `GET /api/admin/roles`(role+permission 키 집합, id ASC) + `GET /api/admin/permissions`(`Permission.catalog()` drift-proof) 신설. **권한 키 결정=기존 `operator.manage` 재사용**(grantable-roles와 동일 게이트, seed 변경 0, rbac.md 신규키 지양). `permission-sets` 생략(정당: `/roles`가 permission 집합 포함→PC-FE-228 커버). Contract-first: `admin-api.md` 두 섹션+`rbac.md` 갱신 선행. Thin-Layered+port, `@RequiresPermission` deny-default, GET 성공 무감사. **검증**: worktree `compileJava/TestJava` clean·`:admin-service:test` BUILD SUCCESSFUL(happy+401+403+empty). **3-dim**: (a) MERGED+`59070366`; (b) tip 일치; (c) pre-merge — iam Testcontainers 레인+iam E2E smoke SUCCESS(RoleCatalogIntegrationTest CI Linux 통과), WMS `master-service` IT 1-test flake(`WarehouseIntegrationTest` stale-version 409, iam 무관)는 rerun 후 clean. 분석=Opus 4.8 / 구현=Opus 4.8(backend-engineer). **→ PC-FE-227/228 언블록.**
