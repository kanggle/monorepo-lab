...
> **완료 (2026-05-31)**: impl PR #968 (squash `3e52f93a`). ADR-MONO-019 § 3.3 **keystone** — GAP auth-service 가 토큰 발급 시점에 테넌트 ACTIVE 구독을 조회해 서명 `entitled_domains` claim 주입 → step 3 의 4-도메인 dual-accept 게이트(finance/erp/scm/wms)를 inert→**active**. **account-service**: `?tenantId=` 역조회(`findByStatusAndTenantId` + port/impl/usecase 오버로드/controller param; tenantId 부재 호출 byte-identical) + contract spec. **auth-service**: `AccountServicePort.listEntitledDomains` + `AccountServiceClient`(Bearer + resilience4j accountService CB/retry, sibling 패턴) + `TenantClaimTokenCustomizer.populateEntitledDomains`(authorization_code+refresh). **3 load-bearing 제약**: ① **cc grant 스킵 = 재귀 안전성**(authcode→Bearer→`GapClientCredentialsTokenProvider` cc self-mint→customize; cc 가 account 호출 시 무한재귀 → `customizeForClientCredentials` byte-unchanged, unit `never().listEntitledDomains`) ② **fail-soft**(account down/CB-open/timeout/빈 → claim 생략, 발급 성공; 발급 hot-path 가 account 가용성에 hard-dep 안 가짐) ③ **net-zero**(slug-tenant self-subscription → `entitled_domains=[slug]` legacy 중복). claim 키 `entitled_domains`=도메인측 `CLAIM_ENTITLED_DOMAINS`, `List<String>`. **3차원**(MERGED `3e52f93a` / tip 일치 / pre-merge 0 non-success). **BE-299 re-stage** ✓. **CI 1-pass**: GAP Integration GREEN 3m28s(WireMock `OAuth2AuthCodePkceIntegrationTest` 가 발급 토큰 entitled_domains 단언; sibling auth IT 는 fail-soft 로 unstubbed account 호출→claim 생략→무영향) + Build & Test GREEN. **scope-lock**: account 역조회 + auth client/port/customizer + 4 test + contract 만(cc 경로/JwtTokenGenerator Path B/도메인 게이트/시드 무변경). **후속**: step 2 실 고객 시드(entitled_domains 가 실제 cross-domain 의미) + E2E 활성화 검증. **메타**: SAS customizer 가 발급 중 내부 HTTP 호출 시 cc self-mint 재귀 — workload(cc) grant 스킵이 재귀 차단 + 효율 + 정합(workload 토큰은 /internal/ 게이트, entitlement 게이트 아님)을 동시 달성.
