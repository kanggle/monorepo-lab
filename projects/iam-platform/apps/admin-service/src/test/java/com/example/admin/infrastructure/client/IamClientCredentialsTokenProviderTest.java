...
/**
 * admin-service-side wiring tests for the canonical {@code libs/java-security}
 * {@link IamClientCredentialsTokenProvider} (TASK-BE-568, ADR-MONO-058 § D6 — replaces the
 * previous per-service local copy, TASK-BE-318b). Exercises the provider constructed with
 * admin-service's real {@code client-id} default (seeded in auth-service V0019) and the
 * {@code internal.invoke} scope its {@code /internal/**} callers require, so the header/body
 * bytes asserted here are exactly what production admin-service sends — not just "it compiles".
 ...
 */
@DisplayName("IamClientCredentialsTokenProvider 단위 테스트 (admin-service wiring)")
class IamClientCredentialsTokenProviderTest {
    ...
    private IamClientCredentialsTokenProvider provider() {
        return new IamClientCredentialsTokenProvider(
                wireMock.baseUrl() + "/oauth2/token", "admin-service-client", "secret",
    ...
    void fetchesToken_withBasicAuth() {
        ...
        String expectedBasic = "Basic " + Base64.getEncoder()
                .encodeToString("admin-service-client:secret".getBytes(StandardCharsets.UTF_8));
    ...
    @Test
    @DisplayName("AC-3: 유효한 캐시 토큰은 재사용되어 토큰 엔드포인트를 한 번만 호출한다")
