 ...
 * resolution (login-time exchange).</b> The Phase-2 transitional DUAL-KEY (account_id
 * first, legacy email fallback resolved server-side from the validated {@code sub})
 * is removed now that the part-A email→account_id backfill (TASK-MONO-298) has
 * migrated {@code admin_operators.oidc_subject} to the account UUID. The operator is
 * resolved by the validated {@code sub} (= account_id) ONLY, via the SHARED
 * {@link OperatorOidcSubjectResolver} (the same resolver the assume-tenant gate in
 * {@link OperatorAssignmentCheckUseCase} uses, so a sub-keyed path cannot diverge).
 * The fail-closed invariant is unchanged: no matching row → 401, no token.
 */
@Slf4j
@Service
...
public class TokenExchangeService {
    ...
    public ExchangeResult exchange(String subjectToken) {
        ...
        //    account UUID (jwt-standard-claims.md) and admin_operators.oidc_subject has
        //    been backfilled to account_id (part A), so the resolver keys on it directly.
                ...
                operatorResolver.resolve(oidcSubject)
                        .orElseThrow(() -> {
                            log.debug("token-exchange fail-closed: no admin_operators row "
                                    + "for the OIDC subject (account_id)");
