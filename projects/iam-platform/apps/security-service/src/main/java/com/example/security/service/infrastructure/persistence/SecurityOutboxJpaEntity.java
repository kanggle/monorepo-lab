package com.example.security.service.infrastructure.persistence;
...
/**
 * Per-service transactional-outbox row for security-service (TASK-BE-453 — outbox
 ...
 * this table without depending on this entity. Mirrors the in-worktree
 * auth-service {@code AuthOutboxJpaEntity} + finance account-service's
 * {@code AccountOutboxJpaEntity} (the MySQL v2 precedent), minus the
 * {@code event_version} column (the iam v1 envelope never carried one).
 *
 ...
 * serialises the UUID as its 36-char canonical string, matching the {@code V0011}
 ...
 * {@code OutboxSecurityEventPublisher}); the relay never parses it.
 *
 * <p>Lives under {@code com.example.security.service.infrastructure.persistence} (the
 * {@code JpaConfig} {@code @EntityScan} / {@code @EnableJpaRepositories} base) so
 * it registers — a missing scan would only fail the full-boot IT, never a
 * mock-repo unit test (payment §27 lesson).
 */
@Entity
@Table(name = "security_outbox")
public class SecurityOutboxJpaEntity implements OutboxRow {
    ...
    protected SecurityOutboxJpaEntity() {
    }

    private SecurityOutboxJpaEntity(UUID id, String aggregateType, String aggregateId,
    ...
    public static SecurityOutboxJpaEntity create(UUID id, String aggregateType, String aggregateId,
                                                 String eventType, String payload,
                                                 String partitionKey, Instant createdAt) {
        return new SecurityOutboxJpaEntity(id, aggregateType, aggregateId, eventType,
