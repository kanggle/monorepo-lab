...
/**
 * Per-service transactional-outbox row for community-service (TASK-BE-455 — outbox
 ...
 * serialises the UUID as its 36-char canonical string, matching the {@code V0006}
 ...
 * {@code OutboxCommunityEventPublisher}); the relay never parses it.
 *
 * <p>Lives under {@code com.example.community.infrastructure.persistence} (one of
 * the {@code JpaConfig} {@code @EntityScan} / {@code @EnableJpaRepositories} bases)
 * so it registers — a missing scan would only fail the full-boot IT, never a
 * mock-repo unit test (payment §27 lesson).
 */
@Entity
@Table(name = "community_outbox")
public class CommunityOutboxJpaEntity implements OutboxRow {
    ...
    protected CommunityOutboxJpaEntity() {
    }

    private CommunityOutboxJpaEntity(UUID id, String aggregateType, String aggregateId,
    ...
    public static CommunityOutboxJpaEntity create(UUID id, String aggregateType, String aggregateId,
                                                  String eventType, String payload,
                                                  String partitionKey, Instant createdAt) {
        return new CommunityOutboxJpaEntity(id, aggregateType, aggregateId, eventType,
