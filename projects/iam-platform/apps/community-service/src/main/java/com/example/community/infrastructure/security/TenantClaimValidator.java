 ...
 *
 * <p>TASK-BE-253: community-service is a {@code fan-platform} consumer. Tokens issued
 * for any other tenant ({@code wms}, future {@code erp}/{@code scm}/...) MUST be rejected
 * with an OAuth2 {@code invalid_token} error, which the Resource Server filter chain
 * surfaces as HTTP 401. We additionally raise a more specific error code so the
 * AuthenticationEntryPoint / AccessDeniedHandler can map cross-tenant misuse to 403
 * (the spec's "TENANT_FORBIDDEN" code).
 *
 * <p>Combined with the standard issuer + signature + expiration validators provided by
 * Spring Security, this is a defense-in-depth check that protects the service when
 * GAP issues a legitimately signed token belonging to a different tenant.
 */
