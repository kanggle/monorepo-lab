-- iam-platform community-service outbox v1 -> v2 (TASK-BE-455).
-- MySQL 8, InnoDB, utf8mb4 — parity with V1. Migrates community-service onto the
...
-- (TASK-ERP-BE-025).
--
-- community_outbox — pending rows have published_at IS NULL; the relay
-- (CommunityOutboxPublisher) forwards them to Kafka in created-order and stamps
...
-- matches CommunityOutboxJpaEntity's UUID field mapped via @JdbcTypeCode(SqlTypes.CHAR).
...
-- shape: {eventId, eventType, source="community-service", occurredAt, schemaVersion=1,
-- partitionKey, payload}).
-- NB: no event_version column (the iam v1 BaseEventPublisher path never carried one).
-- event_type VARCHAR(80) fits the longest community event ("community.comment.created"=25);
-- aggregate_id VARCHAR(64) fits the postId partition keys.
...
CREATE INDEX idx_community_outbox_unpublished ON community_outbox (published_at, created_at);

-- KEEP-auto-config stance (NOT the finance account exclude stance). The v1 `outbox`
-- (BIGINT/status, V0004) and `processed_events` (V0005) tables are intentionally NOT
-- dropped: community-service RETAINS the libs OutboxAutoConfiguration (it is NOT
...
-- demo/fed-e2e/CI — TASK-BE-455 Edge Case "cutover" / F1).
