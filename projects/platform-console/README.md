...
| IdP | IAM — OIDC **public client** (Auth Code + PKCE), 운영자 로그인 + product/tenant 레지스트리 소비 |
...
운영자가 한 번 로그인하여 `iam` · `wms` · `scm` (+ 향후 `erp` · `finance`) 도메인을 **하나의 콘솔 안에서** 운영한다. 콘솔이 각 도메인의 gateway/admin REST API를 호출해 화면을 렌더하는 **Model B (단일 UI)** — 런처가 아니다. 도메인 정의·rationale·결정 근거는 [PROJECT.md](PROJECT.md) + [ADR-MONO-013](../../docs/adr/ADR-MONO-013-platform-console-foundation.md) 참조.
...
| Service | 역할 | 후속 Task |
|---|---|---|
| `console-web` | 단일 콘솔 UI (frontend-app) | TASK-PC-FE-001 (셸+IAM SSO+카탈로그) → Phase 2 IAM 운영자 parity |
...
- **IAM OIDC client 미발행** — TASK-BE-296 (IAM project-internal) 선행 필요.
- **CI / portfolio sync 미등록** — frontend lint/build CI 편입 + `scripts/sync-portfolio.sh` 등재는 첫 셸 task 시점 별도.
- **admin-web 미폐기** — IAM `admin-web` 폐기는 ADR-MONO-013 Phase 3 (콘솔이 IAM 운영자 parity 검증 후). 본 PR과 무관.
