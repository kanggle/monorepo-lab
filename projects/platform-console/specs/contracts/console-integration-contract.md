...
- **Counts.** `GET /api/admin/operators?page=0&size=1`.`totalElements` (총계) + the `status=ACTIVE` / `status=SUSPENDED` filtered totals (split); `GET /api/admin/accounts?page=0&size=1`.`totalElements` (계정 총계 — no lock/suspended breakdown: the search endpoint has no status filter); `GET /api/admin/audit?page=0&size=5` → `totalElements` (스코프 내 총 이벤트) + `content` (recent 5 rows).
