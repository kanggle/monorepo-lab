...
/**
 * `features/wms-ops/api/inbound-state.ts` — the server-side inbound (ASN)
 * section state for the dedicated `/wms/inbound` route (TASK-PC-FE-222,
 * mirrors `wms-inventory-state.test.ts`):
 ...
 *   - eligible → seeds asns (IAM OIDC token, server-side);
...
import { getWmsInboundState } from '@/features/wms-ops/api/inbound-state';
...
describe('getWmsInboundState — eligibility gate (§ 2.4.5, TASK-PC-FE-222)', () => {
  it('not eligible → notEligible block, NO wms call fabricated', async () => {
    cookieJar.set(ACCESS_COOKIE, 'GAP-ACCESS');
    const fetchMock = vi.fn();
    vi.stubGlobal('fetch', fetchMock);

    const state = await getWmsInboundState(false);
    expect(state.notEligible).toBe(true);
    expect(state.asns).toBeNull();
    // No cross-tenant call ever fabricated.
    expect(fetchMock).not.toHaveBeenCalled();
  });

  it('eligible → seeds asns (IAM OIDC token, server-side)', async () => {
    cookieJar.set(ACCESS_COOKIE, 'GAP-ACCESS');
    const fetchMock = vi.fn((_u: string, _init?: RequestInit) =>
      Promise.resolve(jsonResponse(ASNS)),
    );
    vi.stubGlobal('fetch', fetchMock);

    const state = await getWmsInboundState(true);
    expect(state.notEligible).toBe(false);
    expect(state.degraded).toBe(false);
    expect(state.asns).not.toBeNull();
  ...
  it('surfaces the read-model-lag hint when the producer set it', async () => {
    ...
    vi.stubGlobal(
      ...
      vi.fn(() =>
        Promise.resolve(
          jsonResponse(ASNS, 200, { 'X-Read-Model-Lag-Seconds': '9' }),
        ),
      ),
    );
    const state = await getWmsInboundState(true);
