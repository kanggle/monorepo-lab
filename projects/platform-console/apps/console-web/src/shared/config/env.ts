...
const ServerEnvSchema = z.object({
  ...
  /** Outbound timeout (ms) for IAM cross-org partnership calls
   *  (TASK-PC-FE-187 / integration-heavy I1 — same convention as
   *  SUBSCRIPTIONS_TIMEOUT_MS). The invite POST + accept/suspend/reactivate/
   *  terminate colon-verbs + list GET + participant add/remove hang off
   *  `${IAM_ADMIN_API_BASE}/api/admin/partnerships...` — admin-service gates
   *  `partnership.manage` + the D2 TenantScopeGuard (BE-476/477/478 /
  ...
  /** Outbound timeout (ms) for IAM org-node hierarchy calls (TASK-PC-FE-237 /
   *  ADR-MONO-047 — integration-heavy I1, same convention as
   *  TENANTS_TIMEOUT_MS). The org-node CRUD + `/{id}/ceiling` + `/{id}/admins`
   *  + `/{id}/tenants` endpoints hang off `${IAM_ADMIN_API_BASE}/api/admin/
