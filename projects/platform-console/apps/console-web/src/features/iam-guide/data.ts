...
/**
 * IAM 작업 레시피 — 온보딩 위임 체인·구독·파트너십·접근 매트릭스라는 이 화면의
 * 실제 메뉴/개념만 참조한다. 화면에 없는 버튼·상태는 언급하지 않는다.
 */
export const IAM_RECIPES: GuideRecipeData[] = [
  {
    title: '새 직원에게 도메인 화면 권한 주기',
    intro:
      '관리 권한이 아니라 도메인 운영 권한을 주는 흐름입니다 — 권한은 구독 도메인에서 자동으로 생깁니다.',
    steps: [
      '「운영자 관리」에서 직원을 운영자로 등록합니다(모든 변경은 사유 입력이 필수).',
      '그 운영자를 일할 회사 테넌트에 배정합니다(테넌트 배정). 원 소속이 아닌 회사에서 일하게 하는 유일한 방법입니다.',
      '「도메인 구독」에서 그 테넌트가 쓸 도메인(WMS·이커머스 등)이 켜져 있는지 확인합니다 — 구독 도메인이 곧 직원의 도메인 운영 권한이 됩니다.',
      '직원이 로그인한 뒤 그 테넌트를 고르면 해당 도메인 운영 화면이 열립니다.',
    ],
  },
  {
    title: '협력사에 우리 운영 일부를 맡기기',
    steps: [
      '「파트너십」에서 상대 회사에 초대를 보냅니다(SUPER_ADMIN 도 이 화면은 열 수 없습니다 — 당사자만 가능).',
      '상대가 초대를 수락하면, 위임받은 쪽에서 담당 운영자를 그 테넌트에 배정합니다.',
      '필요하면 「운영자 관리」의 조직 범위(부서)로 그 담당자가 볼 수 있는 데이터를 좁힙니다.',
    ],
  },
  {
    title: '운영자가 메뉴를 못 여는 이유 찾기',
    steps: [
...
/**
 * IAM 용어집 — 이 화면에 실제 렌더되는 문자열 중 일반 운영자가 모를 법한
 * 용어만 골랐다(조직 계층 도입으로 화면에 노출되는 org-node 계열 용어).
 * 역할명·권한 키·"테넌트" 등은 화면이 이미 인라인으로 설명하므로 제외.
 */
export const IAM_GLOSSARY: GlossaryEntry[] = [
  {
    key: 'ceiling',
    term: '엔타이틀먼트 상한 (ceiling)',
    full: 'entitlement ceiling',
    meaning:
      '조직 노드에 거는 도메인 사용 상한. 하위 테넌트가 쓸 수 있는 도메인을 좁히기만 하고 부여하지는 않습니다 — "상한 없음"과 "빈 상한(아무것도 불가)"은 정반대입니다.',
  },
  {
    key: 'subtree',
    term: '노드 subtree',
    meaning:
      '한 조직 노드와 그 아래 모든 하위 노드·테넌트의 묶음. ORG_ADMIN 은 자기 노드 subtree 안에서만 운영자를 관리합니다.',
  },
  {
    key: 'no-escalation',
    term: 'no-escalation',
