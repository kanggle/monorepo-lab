...
import { retireCostCenter } from '@/features/erp-ops/api/erp-api';
...
/**
 * Same-origin erp cost-center RETIRE proxy (POST — TASK-PC-FE-048). `reason`
 ...
 * reference this cost center) passes through inline-actionably.
 */
export async function POST(
  ...
  try {
    const result = await retireCostCenter(id, body.reason, body.idempotencyKey);
