...
/**
 * ecommerce notification template list route (TASK-PC-FE-089 — ADR-031 Phase 5b).
 ...
 */
export default async function EcommerceNotificationsPage() {
  ...
  if (registryDegraded) {
    return (
      <section aria-labelledby="ecommerce-notifications-heading">
        ...
        >
          ecommerce 알림 템플릿 운영 정보를 일시적으로 불러올 수 없습니다. 콘솔의
  ...
  const state = await getNotificationsSectionState(eligible);
  ...
  if (state.degraded || !state.templates) {
    return (
      <section aria-labelledby="ecommerce-notifications-heading">
        {heading}
        <div
  ...
  return <NotificationsScreen templates={state.templates} />;
}
