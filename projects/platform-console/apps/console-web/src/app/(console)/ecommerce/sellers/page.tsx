...
/**
 * ecommerce seller list route (TASK-PC-FE-090 — ADR-MONO-031 § 2.4.10 7th area).
 * An in-console nav destination under the `/ecommerce` section.
 *
 * Server component. Eligibility waterfall mirrors /ecommerce/users/page.tsx:
...
export default async function EcommerceSellersPage() {
  ...
  if (registryDegraded) {
    return (
      <section aria-labelledby="ecommerce-sellers-heading">
        ...
        >
          ecommerce 셀러 운영 정보를 일시적으로 불러올 수 없습니다. 콘솔의
  ...
  const state = await getSellersSectionState(eligible);
  ...
  if (state.notEligible) {
    return (
      <section aria-labelledby="ecommerce-sellers-heading">
        ...
        <div
          role="status"
          data-testid="seller-not-eligible"
          className="rounded-md border border-border bg-muted px-4 py-6 text-sm text-muted-foreground"
        >
          <p className="mb-2 font-medium text-foreground">
            ecommerce 셀러 운영 화면에 대한 접근 권한이 없습니다.
          </p>
          <p>
  ...
  if (state.degraded || !state.sellers) {
    return (
      <section aria-labelledby="ecommerce-sellers-heading">
        {heading}
        <div
  ...
  return <SellersScreen sellers={state.sellers} />;
}
