...
/**
 * finance ACCOUNT surface route — `/finance/accounts` (TASK-PC-FE-009 —
 * ADR-MONO-013 Phase 5, the THIRD non-IAM domain federated by the
 * console; **relocated** from `/finance` by TASK-PC-FE-229, which
 * repoints the domain root `/finance` at a new ledger + default-account
 * overview landing — orthodox parity with every other domain's
 * 개요-at-root convention. This is a pure route relocation + label
 * change (`Finance 운영` → `Finance 계좌`, nav testid `nav-finance-ops` →
 * `nav-finance-accounts`) — the `/api/finance/accounts/**` proxy,
 * `features/finance-ops/**`, and the `?accountId=` query param are
 * UNCHANGED). An in-console nav destination, NOT a catalog product
 * re-route — the catalog `finance.baseRoute` now resolves to the § 2.4.7.2
 * overview landing (still data-driven; resolveConsoleRoute leaves
 * non-IAM products on their registry baseRoute).
 *
