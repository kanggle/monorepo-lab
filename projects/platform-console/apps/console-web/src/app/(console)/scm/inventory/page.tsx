...
/**
 * scm 재고 (inventory-visibility) route — split out of the former combined
 ...
 * discipline as `/scm` / `/scm/procurement` (§ 2.4.6): the page resolves
 * scm eligibility from the data-driven registry and passes it into
 * `getScmInventoryState()` so no cross-tenant call is ever fabricated for a
 * non-eligible operator. 401 → whole-session re-login; 403 → inline "not
 * scoped"; 429 → rate-limited notice; 503/timeout → this section degrades
 * only. S5 (§ 2.4.6): the snapshot's REQUIRED `meta.warning` rides through
 * the view-model and is surfaced prominently by the screen (never
 * stripped).
 */
export default async function ScmInventoryPage() {
  ...
  if (registryDegraded) {
    return (
      <section aria-labelledby="scm-heading">
        ...
        >
          scm 재고 정보를 일시적으로 불러올 수 없습니다. 콘솔의 다른
  ...
  const state = await getScmInventoryState(eligible);
    ...
    return (
      <section aria-labelledby="scm-heading">
        ...
        <div
          ...
          className="rounded-md border border-border bg-muted px-4 py-6 text-sm text-muted-foreground"
        >
          scm 재고 화면을 조회할 권한이 없습니다. (테넌트 스코프 / 역할
          확인이 필요합니다.)
        </div>
      </section>
    );
  ...
  if (state.degraded || !state.snapshot || !state.staleness) {
    return (
      <section aria-labelledby="scm-heading">
        <h1 id="scm-heading" className="mb-6 text-2xl font-semibold">
          SCM 재고 가시성
