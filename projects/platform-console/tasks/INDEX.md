...
- `TASK-PC-BE-001-console-bff-skeleton.md` — **REVIEW**. backend-engineer(Sonnet 4.6). **ADR-MONO-017 § 3.3 #2 first post-ACCEPTED execution** impl PR. Landed: `apps/console-bff/` Spring Boot 3.4 / Java 21 Hexagonal skeleton (~35 files) — `ConsoleBffApplication`, domain layer (`DomainTarget` enum / `OutboundCredential` sealed + `CredentialSelectionPort` / `MissingCredentialException` / `LegOutcome` / `DegradePolicy`), application layer (`OperatorOverviewCompositionUseCase` placeholder + `DomainReadPort` outbound port), adapter inbound (`GlobalExceptionHandler`), adapter outbound (`CredentialSelectionAdapter` 5-row switch + `OperatorCredentialContext` `@RequestScope`), infrastructure (`SecurityConfig` OAuth2 RS RS256 + `PublicPaths` + `ObservabilityConfig` 3-metric eager registration + `RestClientConfig` OTel tracing). Tests: domain unit (5-row exhaustive credential selection + fail-closed), application unit (degrade-policy shapes), slice (`ActuatorHealthSliceTest` — 200 no-auth + 401 authenticated chain), IT (`ConsoleBffSmokeIntegrationTest` — `@SpringBootTest` + MockWebServer JWKS + health 200 + prometheus 3-metric + 5-row dispatch dry-run + header reading). `Dockerfile` (eclipse-temurin 21 alpine multi-stage). `docker-compose.yml` updated with `console-bff` service block + Traefik `console-bff.local` internal-only labels (no CORS). `settings.gradle` module registered. CI: `platform-console` positive path-filter + `build-and-test` step + `platform-console-bff-integration-tests` job. Hard invariants enforced: 0 `Idempotency-Key`/`X-Operator-Reason` in production code · 0 fallback path in credential selector · 0 producer spec changes · 0 console-web changes · ADR-MONO-017 D1-D8 byte-unchanged. 분석=Opus 4.7 / 구현=Sonnet 4.6 (shell skeleton).
