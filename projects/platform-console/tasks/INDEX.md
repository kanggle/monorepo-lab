...
- `TASK-PC-FE-010-console-erp-operations-section.md` — (promoted from backlog after the prerequisite gate met) ADR-MONO-013 § D6 **Phase 6** — erp operations console section (read-only: 5 masters × {list, detail} = 10 GETs + `?asOf=<ISO-8601>` point-in-time). **first internal-system-primary non-GAP federation**; fourth confirmation of § 3.3 zero-retrofit (FE-007 wms / FE-008 scm / FE-009 finance / FE-010 erp). spec-first cross-project gate = `TASK-ERP-BE-002` (erp-platform) **MERGED 2026-05-20** (#655 `09d4cb2a` / #656 `083c744b` / #657 `4e626fdc`); FE-007 + FE-008 + FE-009 also merged → `backlog → ready` promote-eligibility verified, now in `ready/`. spec-first **§ 2.4.8** = REUSE of § 2.4.5 (wms) per-domain-credential rule + § 2.4.6 (scm) flat-envelope/read-only + § 2.4.7 (finance) no-fabricated-429 **verbatim** (erp = GAP OIDC access token via `getAccessToken()`, never `getOperatorToken()`; `tenant_id ∈ {erp,*}` from JWT claim, no `X-Tenant-Id`). read surface: 10 v1-live GETs (`/api/erp/masterdata/{departments,employees,job-grades,cost-centers,business-partners}` × {list, detail}) with `?asOf=` point-in-time; 16 mutation endpoints + v2 `approval-service`/`read-model-service`/future `admin-service` excluded. erp flat error envelope (distinct producer parser — `{code, message, details?, timestamp}`, same shape as scm/finance but own parser); **no 429** (erp has none — identical to finance, asserted absent by test). **erp internal-system producer obligations cross-ref (erp analog of scm S5 / finance F5/F7)**: **E2/E3 effective-dating-first-class UX** with `<AsOfPicker>` shared component + `useAsOf()` hook thread-through to every list/detail query (CORE E3 invariant; `?asOf=<past>` MUST yield asOf-instant state, NOT current); E2 active/retired both rendered visually distinct (retired NOT hidden); E1 reference-integrity surfacing (broken/retired cross-references with badge, not silent sanitize); confidential + audit-heavy discipline (no token/PII/business-partner-financials/cost-center-sensitive logging); honest enum surfacing (RETIRED master + SEPARATED employee rendered; unknown → generic label, no throw). UX = **list-driven + asOf-first-class** (inverse of FE-009 finance account-id-driven shape — recorded honestly, not force-fit). § 3 GAP-parity matrix NOT mutated (count == 16). Code (planned): `features/erp-ops/` (api/types/hooks/components — 10 read fns only, NO mutation fn; 5 list views + 5 detail views + `<AsOfPicker>` shared) + `(console)/erp` 5 sub-routes + **GET-only** `api/erp/masterdata/**` proxy (asOf pass-through) + in-console nav. Phase 7 (`console-bff` + cross-domain dashboards) gate ungated on merge (5/5 domains live). 분석=Opus 4.7 / 구현 권장=frontend-engineer Opus 4.7 (FE-007/008/009 verbatim mirror pattern + dispatcher 독립 재검증).
