...
- **depends on**: **ADR-MONO-013 ACCEPTED** (TASK-MONO-108) § D6 Phase 4; `TASK-PC-FE-007` (wms) — establishes the **non-GAP per-domain client + auth-divergence + per-domain-credential** pattern this slice reuses (do not re-derive it). **MERGED** → origin/main (PR #633 squash `81395376`; an empty duplicate squash #634 `9f5499fc` carries no delta). The `console-integration-contract.md` **§ 2.4.5** per-domain-credential rule (GAP=exchanged operator token / wms=GAP OIDC token direct; #569 GAP-domain-scoped) is now on main — § 2.4.6 (this task) **reuses** it for scm (do not redefine).
- **PREREQUISITE SATISFIED (cross-project spec-first — resolved; this is why the task moved `backlog → ready`)**: the scm-side reconciliation = **`TASK-SCM-BE-015`** (`scm-platform`, *platform-console operator read-consumer reconciliation*). It records `platform-console` (ADR-MONO-013 Model B) as a **sanctioned external operator read consumer** of scm's existing read surface — a **(B) document/accept** of the *already-existing* scm gateway capability (`AllowedIssuersValidator` + `TenantClaimValidator` `tenant_id ∈ {scm,*}` + `JwtHeaderEnrichmentFilter` `X-Token-Type=user`) under the governing ADR-MONO-013 (no scm ADR; the "v1 backend-only / human-flow v2-deferred" narrative scopes scm's *own* UI/user-flow client, not authorized API consumers). **Authored + linked here + MERGED** → origin/main: spec PR #635 (`385be18c`) + impl PR #636 (`bfb9ef8a`); scm spec edits (`gateway-public-routes.md` § *platform-console operator read consumer* + `gap-integration.md` + `PROJECT.md`) are on main, additive, scm classification byte-unchanged. The spec-first discipline that gated this task (CLAUDE.md "Specs win over tasks") is **satisfied** — the producer spec now acknowledges the console consumer (mirroring GAP `TASK-BE-296` ⊃ `FE-001`). `backlog → ready` move criteria (INDEX: "cross-project prerequisite tasks identified and linked"; FE-008 AC: "authored + linked here + merged") **all met**.
