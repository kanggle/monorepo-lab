...
- [x] 🟢 **AC-1 — 소유자 결정: 지불하는가.** → **ⓒ 링크** (소유자, 2026-09-22) 갈래를 **비용과 함께** 적어 올린다(내 추천은 이 파일에 적되, 추천은 결정이 아니다):
...
- [~] **AC-2 (ⓐ 선택 시) 계약 먼저** → 🔵 **해당 없음 — ⓐ 를 안 골랐다.** — § 2.4.9.1 의 **producer 표에 행을 먼저 추가**하고(295 가 계약서에 적어 둔 순서), `ok` 카드 `data` 모양 표의 wms 행을 합성 모양으로 고친다. 🔴 **왜 이 레그만 예외인지**를 그 자리에 적는다 — 적지 않으면 다음 레그가 «wms 도 했으니» 를 근거로 삼는다.
- [~] **AC-3 (ⓐ) 구현** → 🔵 **해당 없음 — ⓐ 를 안 골랐다.** — `WmsInventoryReadPort` 에 두 번째 읽기, 어댑터에 `?lowStockOnly=true&size=1`, 유스케이스가 둘을 합성. 🔴 **두 번째 호출의 실패가 카드 전체를 죽이면 안 된다** — 재고 행 수는 살아 있는데 알림만 못 읽은 상태가 표현돼야 한다(빈 값과 «못 물었다» 를 구별).
- [~] **AC-4 (ⓐ) 공유 픽스처 + 양쪽 테스트** → 🔵 **해당 없음 — ⓐ 를 안 골랐다.** — `operator-overview-leg-bodies.json` 의 wms 항목을 합성 모양으로 옮기고, `leg-body-contract.test.tsx`(그린다) 와 `OperatorOverviewLegBodyContractTest`(그대로 간다) **양쪽이 같이** 움직인다. 🔴 한쪽만 고치면 다른 쪽이 빨개져야 하고, **그것이 295 가 만든 장치가 동작한다는 증거**다 — bite 로 확인한다.
- [~] **AC-5 (ⓐ) 샘플 동반 이동** → 🔵 **해당 없음 — ⓐ 를 안 골랐다.** — `dashboards.ts` 의 wms 카드와 `sample-overview-cards-match-lists.test.ts` 의 wms 칸. 🔴 그 칸은 **동일성 비교**(295)이므로 합성 모양이면 «어느 질의의 본문과 같은가» 를 두 질의로 나눠 적어야 한다.
- [x] 🟢 **AC-6 대조군** — 나머지 다섯 레그의 `data` 는 **여전히 producer 본문 그대로**다. 전수 칸(`leg-body-contract`)이 그것을 지킨다.
...
# 🟢 AC-1 — 소유자 결정: **ⓒ 링크** (2026-09-22 UTC · 분석=Opus 5)

소유자가 **ⓒ** 를 골랐다. 근거는 AC-0 이 잰 산술이다 — ⓐ 는 「알림 수 하나」를 위해 **지금
정상인 「재고 행 수」 타일을 잃을 경로**를 만든다(최악 `4.15초` → `8.15초`, 5초 벽 초과 시
카드 전체가 `TIMEOUT` 으로 저하).

## 🔴🔴 그런데 ⓒ 에는 **아무도 값을 매기지 않은 선행**이 있었다

AC-1 의 ⓒ 문구는 *"`/wms/inventory?lowStockOnly=true` 로 가는 링크 … 운영자는 한 클릭으로
도달"* 이다. **구현 직전에 그 목적지를 읽어 보니 성립하지 않았다:**

| 층 | `lowStockOnly` 를 존중하는가 |
|---|---|
| 프록시 라우트 `app/api/wms/inventory/route.ts:25` | 🟢 **예** (`=== 'true'`) |
| 프로듀서 `InventoryDashboardController` + JPQL | 🟢 **예** (AC-0 ② 에서 이미 확인) |
| **화면 `WmsInventoryScreen`** | 🔴 **아니오** — `EMPTY_INV_FILTERS`(`lowStockOnly: false`)로 시작하고 URL 을 **안 읽는다** |

⇒ 링크만 넣었다면 운영자는 **필터 안 된 전체 목록**에 떨어지고, **아무 데서도 오류가 안 난다.**
🔵 세 층 중 둘이 지원하니 「끝까지 된다」로 읽기 쉬운 모양이었다 — 이 저장소가 이름 붙인
«독립 표면 N개의 교집합은 아무도 안 잰다» 그대로다.

## 구현 — 두 쪽을 **같이** 넣었다 (한쪽만은 거짓 약속이다)

1. **목적지** — `wms/inventory/page.tsx` 가 `searchParams` 를 **서버에서** 읽어
   `initialLowStockOnly` 로 넘긴다(이 라우트는 이미 `force-dynamic` 이라 Suspense 경계가 없고,
   시드가 하이드레이션 뒤가 아니라 **첫 페인트에** 도착한다). 판정은 `=== 'true'` — 프록시와
   같은 술어다(느슨하게 하면 `?lowStockOnly=false` 가 참이 된다).
2. **화면** — `initialLowStockOnly` 가 **체크박스와 질의 둘 다** 시드한다.
   🔴 체크박스만 시드하면 «필터를 걸었다고 말하면서 안 건» 상태가 되어 무시하는 것보다 나쁘다.
   🔵 초기값 전용이고 반응형이 아니다 — 운영자가 폼을 고치면 폼이 이긴다(erp-ops 의 `useAsOf`
   와 반대인데, 거기서는 URL 이 프로듀서까지 그대로 실려가므로 URL 이 단일 진실이다).
3. **카드** — `WmsSummary` 에 `operator-overview-card-wms-lowstock-link` 링크를 더했다.
   **「재고 행 수」 타일은 그대로다**(ⓒ 를 고른 이유가 그것을 지키는 것이다).

## 판정 — 테스트

| 칸 | 결과 |
|---|---|
| `DomainCard` «링크로 답한다» — href **verbatim** + 행 수 **생존** | 🟢 |
| `WmsInventoryScreen` «체크박스와 질의를 **둘 다** 시드» | 🟢 |
| `WmsInventoryScreen` **대조군** — 시드 없으면 필터 안 걸리고 **재조회도 안 한다** | 🟢 |
| 전체 `WmsInventoryScreen` + `DomainCard` | 🟢 **48/48** |

🔴 **bite (실측)**: 질의 시드 한 줄(`lowStockOnly: initialLowStockOnly || undefined`)을 지우면
`rc=1` 이고 **정확히 그 칸만** 빨강(16 통과 / 1 실패). ⇒ 선행이 가드로 서 있다.

🔵 **대조군 칸이 필요한 이유**: 없으면 `lowStockOnly: true` 를 **하드코딩**한 화면도 위 칸을
통과한다 — 그리고 `/wms/inventory` 를 평범하게 여는 모든 운영자가 깨진다.

## 계약서 — 「의도적 부재」를 **«결정됨» 으로 승격**했다

§ 2.4.9.1 의 문단이 *"Until that decision is taken"* 으로 **미결**을 표현하고 있었다. 그대로
두면 다음 사람이 같은 질문을 다시 연다(AC-1 ⓑ 가 그 위험을 명시했고 ⓒ 도 같다). 이제:
결정·산술 근거·«다시 열지 마라»·**링크가 정직한 이유**(목적지가 파라미터를 존중한다)를 적었다.

## ⚪ 안 잰 것 (축소하지 않는다)

- **실제 지연** — AC-0 의 표는 **구성된 상한으로 만든 최악 산술**이지 측정이 아니다. 평상시
  p99 는 훨씬 작을 수 있다. 🔴 그래도 ⓒ 의 근거는 «평소에 느리다» 가 아니라 «나쁜 날에
  멀쩡한 타일까지 잃는다» 이므로 이 ⚪ 가 결정을 흔들지 않는다.
- **데모/운영의 저재고 분모**(AC-0 ③) — 여전히 ⚪. 🔵 ⓒ 는 분모가 0이어도 정직하다(링크는
  «0건» 을 보여줄 뿐 «못 물었다» 를 «0» 으로 위장하지 않는다) ⇒ 이 ⚪ 는 더 이상 차단이 아니다.
- **링크의 창 판정** — 실제 데모에서 클릭해 필터된 목록이 뜨는지는 안 봤다. 단위 칸이 두 층을
  각각 물지만, 「브라우저에서 한 클릭」은 다음 창의 한 줄이다.

---

## CORRECTION (2026-09-22 UTC · `TASK-MONO-672` 15차 창 실측)

`done/`(frozen) 이므로 본문을 고치지 않고 덧붙인다.

### 🟢 ⓒ 가 바꾼 동작은 **라이브에서 작동한다** — 도착지 절반 PASS
...
🔵 대조군이 두 층을 한꺼번에 가른다 — 파라미터가 **체크박스를 켰고**(첫 층), **질의도 실제로
달라졌다**(1행 → 0행, 둘째 층). 「체크만 켜지고 목록은 그대로」와 구별된다.

### 🔴 그러나 「한 클릭으로」는 **이 데모에서 영원히 잴 수 없다** — 결함이 아니라 구조다
...
이 티켓이 링크를 단 `operator-overview` 카드는 `/dashboards/overview` **한 곳**에만 그려지고,
그 화면은 `console-bff` 합성이다. `infra/demo/console-vercel.override.yml` 이 **기록된 영구 한계**로
적은 대로 `console-bff` 는 공개 호스트명이 없다(`TASK-MONO-362` 가 Traefik 라우터를 일부러 없앴고,
되돌리는 것은 엣지 노출 금지에 정면으로 걸리는 **아키텍처 결정**이다).

⇒ **데모 배포에서 이 링크는 존재하지만 도달할 수 없다.** 운영(BFF 도달 가능) 또는 로컬
`pnpm console:up` 에서만 «카드 → 클릭 → 필터된 목록» 한 항해가 성립한다.
🔵 설정 파일에서 유도하지 않고 **화면을 열어 마커로 확인**했다.

### 🔵 `AC-0 ③` 의 ⚪ (저재고 분모)가 닫힌다 — **0 이다**

재고 행이 **전체 1건**, 저재고 **0건**. ⇒ 지금 링크를 눌러도 빈 목록이 맞고, 그것은
**결함이 아니라 데이터 상태**다(이 티켓이 미리 «둘을 섞지 마라» 고 적은 그 구별).

### 다음 사람에게

🔴 «링크가 한 클릭으로 닿는가» 를 **다음 데모 창에서 다시 시도하지 마라** — 창의 문제가 아니다.
`TASK-MONO-672` 15차 창 § 항목 10 에 같은 결론이 적혀 있고, 그 항목은 그래서 닫혔다.
