...
- [ ] **AC-0 (착수 시 재측정 — 상속 금지)** — 위 § 실측의 네 값을 **다시 재고** 티켓에 적는다:
...
- [ ] **AC-1** — `package.json` 의 네 줄을 형제와 **문자 그대로 같은 스펙**으로 바꾼다:
...
- [ ] **AC-2** — `pnpm install` 로 락을 재생성하고, **해석된 실제 버전을 티켓에 적는다**
      (`next` · `react` · `react-dom`). AC-0(d) 때문에 이 값은 미리 못 적는다.

- [ ] **AC-3 (게이트 — 각각 독립 statement 로, 파이프 없이)** — 전부 rc=0:
      | 게이트 | 기준 |
      |---|---|
      | `tsc --noEmit` | 0 오류 |
...
- [ ] **AC-4** — `frontend-ui.md` § 5.3 의 버전 표(`console-web | 15.0.3 | ^2.1.4 | …`)를
