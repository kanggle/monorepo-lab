...
Every service declared under `specs/services/<service>/architecture.md` MUST declare exactly one `Service Type` from the catalog below. Implementation must follow the matching `specs/platform/service-types/<type>.md` document.
...
When reading specs for a task, follow `specs/platform/entrypoint.md`. After reading Core platform specs, read the **Service-Type-Specific** spec for the target service's declared type (exactly one file).
