...
**이 디렉토리(`specs/rules/`)가 진실 소스**이지만, 에이전트 런타임 라우팅을 위해 `.claude/config/` 아래에 **짧은 디스패치 카탈로그**가 함께 존재한다:
...
- `specs/rules/` (이 디렉토리) — "X가 무엇을 의미하는가? 어떤 규칙을 반드시 지켜야 하는가?"에 대한 **상세 답**. 구현·리뷰 시 참조.
