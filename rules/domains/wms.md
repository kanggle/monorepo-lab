...
**경계선은 custody(점유)다.** 물건이 이 창고 바닥에 있는 동안 이 도메인이 권위를 가지며, 도크에서 운송 수단에 인계되는 순간까지가 그 범위다. 운송 수단 위의 물건은 **어느 창고에도 없고**, 로케이션·수량 모델로는 표현할 수 없다 — 채워야 할 공백이 아니라 경계 그 자체다. 판정이 갈리는 사례(도크 적재 시점, 창고 간 이동 중 재고)는 이 기준으로 가른다. 근거: [ADR-MONO-052](../../docs/adr/ADR-MONO-052-transport-context-map.md) §D1.
...
- **Transfer** — **같은 창고 내** 로케이션 간 재고를 이동하는 행위. 출발 로케이션 차감 + 도착 로케이션 증가가 하나의 트랜잭션에서 원자적으로 수행. **창고 간 이동은 이 도메인의 Transfer 가 아니다** — 창고 A 출고 + 창고 B 입고 두 구간으로 나뉘며, 그 사이 운송 구간과 이동 중(in-transit) 재고는 **운송 컨텍스트**가 소유한다. 따라서 창고 간에는 원자적 이동이 존재하지 않으며, A 에서 나간 재고가 B 에 도착하기 전까지의 간격은 모델링 부산물이 아니라 실재한다. 이는 "나중에 이 도메인이 확장할 v2"가 아니라 **다른 컨텍스트로의 배치**다. 소유권 배치와 근거: [ADR-MONO-052](../../docs/adr/ADR-MONO-052-transport-context-map.md) §D1·§D3.
...
적치, 피킹, 조정, **같은 창고 내** 이동 등 재고 수량이 변하는 모든 경로는 DB 트랜잭션으로 원자성을 보장한다. 부분 변경(출발지만 차감, 도착지는 미증가) 상태가 발생해서는 안 된다.

이 규칙의 범위는 **한 창고 안**이다. 창고 간 이동은 출고 1건 + 입고 1건의 독립 트랜잭션이며, 그 사이 재고가 어느 창고에도 속하지 않는 간격은 위 "부분 변경"이 아니라 경계 자체다([ADR-MONO-052](../../docs/adr/ADR-MONO-052-transport-context-map.md) §D3). 이 간격을 없애려고 두 창고를 한 트랜잭션으로 묶는 것이 곧 경계 위반이다.
