...
### ⚠️ 이 규칙은 **인덱스 파일 한정**이다 — 저장소 전반의 중복에 반사적으로 적용하지 말 것

`common.md`는 정의상 인덱스이므로 규칙 문장의 사본이 있으면 그건 무조건 제거 대상이다. **그러나 저장소
전반에서 같은 규칙이 여러 곳에 나타나는 것은 그 자체로 병이 아니다.** 정합화에 착수하기 전에 먼저 물어라:
**이 사본들이 서로 갈라졌는가?**

- **갈라졌다** → 중복이 병이다. 정경 하나로 수렴시켜라.
- **안 갈라졌다** → 중복은 **전파**이고, 진짜 결함은 대개 **정경 선언의 부재**다. 처방이 정반대가 된다:
  사본을 지울 게 아니라 정경 홈에 규칙을 **선언**해야 한다. 각 사본은 자기 자리의 구체적 근거를 함께
  담고 있어 자산인 경우가 많다 — 규칙만 남기고 근거를 지우면 다음 사람이 이유를 모른 채 우회한다.
  정경에는 규칙을, 인라인에는 근거를 두고, 인라인에는 정경을 가리키는 한 줄만 얹어라.

**각 규칙에는 그것을 낳은 인시던트 번호를 붙여라.** 근거 없는 규칙은 다음 사람이 "이거 왜 있지" 하며
지운다.

**그리고 어떤 규칙이 저장소에 몇 곳에 있는지 세어라. 1곳뿐이면 그건 사실상 없는 규칙이다** — 그 파일을
열지 않은 사람에게는 존재하지 않기 때문이다. 정경 홈은 **새로 만들지 말고**, `CLAUDE.md`의 Source of
Truth / Layer Rules가 **이미 가리키고 있는** 파일 중에서 골라라. 새 문서를 만들면 그것 자체가 아무도 읽지
않는 또 하나의 자리가 된다. (Worked incident: TASK-MONO-404 — 한 규칙이 14곳에 복사돼 있었으나 전수 대조
결과 갈라지지 않았고, 실제 갭은 `platform/testing-strategy.md`에 그 주제가 **한 줄도 없었다**는 것이었다.)
